...
// ============================================================================
// Strangler shims (plan §4 R7) — the raw-pointer entry points the dispatch tables
// still hold. Each builds a `PlaneCursorMut` over the exact span its kernel
// touches and calls the safe implementation above.
// ============================================================================

/// The reachable span of a `bw` x `bh` intra-prediction block, as a
/// `(slice length, cursor centre)` pair for a slice starting at `pPred - stride - 1`.
///
/// Every kernel in this file reads the row above (`dy == -1`) from `dx == -1` out to
/// `top_reach - 1`, reads the column left (`dx == -1`) for `dy` in `0..bh`, and writes
/// `bw` x `bh`. Anchoring the slice at the top-left neighbour makes the whole
/// neighbourhood addressable with one span, and the arithmetic below is the only
/// place it is computed.
#[inline]
fn shim_span(stride: usize, bw: usize, bh: usize, top_reach: usize) -> (usize, usize) {
    let center = stride + 1;
    let last_write = center + (bh - 1) * stride + bw;      // one past the block
    let last_top_read = 1 + top_reach;                     // one past the top row's reach
    (last_write.max(last_top_read), center)
}
...
///   rows are `kiStride` bytes apart, and the block has **at least one valid row
///   above it and one valid column to its left** — for a decoder picture plane that
///   is guaranteed by `PADDING_LENGTH` (32 luma samples of border on every side,
///   `pic_queue.rs:AllocPicture`), and for a block interior to a macroblock by the
///   macroblock itself.
/// * The bytes `[-kiStride - 1, 3*kiStride + 4)` from `pPred` must be valid to read,
///   and `[0, 3*kiStride + 4)` valid to write. The diagonal modes additionally read
///   the row above out to `pPred[-kiStride + 7]`, four samples past the block's right
///   edge; that is inside the same span.
/// * `kiStride` is positive and large enough for the spans above to lie inside the
///   plane — in practice `WELS_ALIGN(width + 2*PADDING_LENGTH, 32)`.
pub unsafe extern "C" fn WelsI4x4LumaPredV_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 4, 4, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i4x4_luma_pred_v(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI4x4LumaPredH_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 4x4 block inside a pixel surface whose
///   rows are `kiStride` bytes apart, and the block has **at least one valid row
...
pub unsafe extern "C" fn WelsI4x4LumaPredH_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 4, 4, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i4x4_luma_pred_h(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI4x4LumaPredDc_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 4x4 block inside a pixel surface whose
///   rows are `kiStride` bytes apart, and the block has **at least one valid row
...
pub unsafe extern "C" fn WelsI4x4LumaPredDc_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 4, 4, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i4x4_luma_pred_dc(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI4x4LumaPredDcLeft_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 4x4 block inside a pixel surface whose
///   rows are `kiStride` bytes apart, and the block has **at least one valid row
...
pub unsafe extern "C" fn WelsI4x4LumaPredDcLeft_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 4, 4, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i4x4_luma_pred_dc_left(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI4x4LumaPredDcTop_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 4x4 block inside a pixel surface whose
///   rows are `kiStride` bytes apart, and the block has **at least one valid row
...
pub unsafe extern "C" fn WelsI4x4LumaPredDcTop_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 4, 4, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i4x4_luma_pred_dc_top(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI4x4LumaPredDcNA_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 4x4 block inside a pixel surface whose
///   rows are `kiStride` bytes apart, and the block has **at least one valid row
...
pub unsafe extern "C" fn WelsI4x4LumaPredDcNA_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 4, 4, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i4x4_luma_pred_dc_na(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI4x4LumaPredDDL_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 4x4 block inside a pixel surface whose
///   rows are `kiStride` bytes apart, and the block has **at least one valid row
...
pub unsafe extern "C" fn WelsI4x4LumaPredDDL_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 4, 4, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i4x4_luma_pred_ddl(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI4x4LumaPredDDLTop_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 4x4 block inside a pixel surface whose
///   rows are `kiStride` bytes apart, and the block has **at least one valid row
...
pub unsafe extern "C" fn WelsI4x4LumaPredDDLTop_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 4, 4, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i4x4_luma_pred_ddl_top(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI4x4LumaPredDDR_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 4x4 block inside a pixel surface whose
///   rows are `kiStride` bytes apart, and the block has **at least one valid row
...
pub unsafe extern "C" fn WelsI4x4LumaPredDDR_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 4, 4, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i4x4_luma_pred_ddr(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI4x4LumaPredVL_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 4x4 block inside a pixel surface whose
///   rows are `kiStride` bytes apart, and the block has **at least one valid row
...
pub unsafe extern "C" fn WelsI4x4LumaPredVL_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 4, 4, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i4x4_luma_pred_vl(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI4x4LumaPredVLTop_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 4x4 block inside a pixel surface whose
///   rows are `kiStride` bytes apart, and the block has **at least one valid row
...
pub unsafe extern "C" fn WelsI4x4LumaPredVLTop_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 4, 4, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i4x4_luma_pred_vl_top(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI4x4LumaPredVR_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 4x4 block inside a pixel surface whose
///   rows are `kiStride` bytes apart, and the block has **at least one valid row
...
pub unsafe extern "C" fn WelsI4x4LumaPredVR_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 4, 4, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i4x4_luma_pred_vr(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI4x4LumaPredHU_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 4x4 block inside a pixel surface whose
///   rows are `kiStride` bytes apart, and the block has **at least one valid row
...
pub unsafe extern "C" fn WelsI4x4LumaPredHU_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 4, 4, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i4x4_luma_pred_hu(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI4x4LumaPredHD_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 4x4 block inside a pixel surface whose
///   rows are `kiStride` bytes apart, and the block has **at least one valid row
...
pub unsafe extern "C" fn WelsI4x4LumaPredHD_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 4, 4, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i4x4_luma_pred_hd(&mut PlaneCursorMut::new(buf, center, stride));
}

// --- 8x8 luma (High Profile) ------------------------------------------------

/// C++: `WelsI8x8LumaPredV_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 block with **at least one valid row
///   above and one valid column to its left**, per `PADDING_LENGTH` — see the 4x4
///   shims for the argument.
/// * The bytes `[-kiStride - 1, 7*kiStride + 8)` from `pPred` must be valid to read,
///   and `[0, 7*kiStride + 8)` valid to write. `DDL` and `VL` additionally read the
///   row above out to `pPred[-kiStride + 15]`, eight samples past the block's right
///   edge, and the `bTRAvail` forms read `pPred[-kiStride + 8]`; both are inside the
///   same span.
/// * `kiStride` is positive and large enough for the spans above to lie inside the
///   plane — in practice `WELS_ALIGN(width + 2*PADDING_LENGTH, 32)`.
pub unsafe extern "C" fn WelsI8x8LumaPredV_c(
    ...
    // SHIM(phase2) -> i8x8_luma_pred_v
    let stride = kiStride as usize;
    let (len, center) = shim_span(stride, 8, 8, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i8x8_luma_pred_v(&mut PlaneCursorMut::new(buf, center, stride), bTLAvail, bTRAvail);
}

/// C++: `WelsI8x8LumaPredH_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 block with **at least one valid row
///   above and one valid column to its left**, per `PADDING_LENGTH` — see the 4x4
...
pub unsafe extern "C" fn WelsI8x8LumaPredH_c(
    ...
    // SHIM(phase2) -> i8x8_luma_pred_h
    let stride = kiStride as usize;
    let (len, center) = shim_span(stride, 8, 8, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i8x8_luma_pred_h(&mut PlaneCursorMut::new(buf, center, stride), bTLAvail, bTRAvail);
}

/// C++: `WelsI8x8LumaPredDc_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 block with **at least one valid row
///   above and one valid column to its left**, per `PADDING_LENGTH` — see the 4x4
...
pub unsafe extern "C" fn WelsI8x8LumaPredDc_c(
    ...
    // SHIM(phase2) -> i8x8_luma_pred_dc
    let stride = kiStride as usize;
    let (len, center) = shim_span(stride, 8, 8, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i8x8_luma_pred_dc(&mut PlaneCursorMut::new(buf, center, stride), bTLAvail, bTRAvail);
}

/// C++: `WelsI8x8LumaPredDcLeft_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 block with **at least one valid row
///   above and one valid column to its left**, per `PADDING_LENGTH` — see the 4x4
...
pub unsafe extern "C" fn WelsI8x8LumaPredDcLeft_c(
    ...
    // SHIM(phase2) -> i8x8_luma_pred_dc_left
    let stride = kiStride as usize;
    let (len, center) = shim_span(stride, 8, 8, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i8x8_luma_pred_dc_left(&mut PlaneCursorMut::new(buf, center, stride), bTLAvail, bTRAvail);
}

/// C++: `WelsI8x8LumaPredDcTop_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 block with **at least one valid row
///   above and one valid column to its left**, per `PADDING_LENGTH` — see the 4x4
...
pub unsafe extern "C" fn WelsI8x8LumaPredDcTop_c(
    ...
    // SHIM(phase2) -> i8x8_luma_pred_dc_top
    let stride = kiStride as usize;
    let (len, center) = shim_span(stride, 8, 8, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i8x8_luma_pred_dc_top(&mut PlaneCursorMut::new(buf, center, stride), bTLAvail, bTRAvail);
}

/// C++: `WelsI8x8LumaPredDcNA_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 block with **at least one valid row
///   above and one valid column to its left**, per `PADDING_LENGTH` — see the 4x4
...
pub unsafe extern "C" fn WelsI8x8LumaPredDcNA_c(
    ...
    // SHIM(phase2) -> i8x8_luma_pred_dc_na
    let stride = kiStride as usize;
    let (len, center) = shim_span(stride, 8, 8, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i8x8_luma_pred_dc_na(&mut PlaneCursorMut::new(buf, center, stride), bTLAvail, bTRAvail);
}

/// C++: `WelsI8x8LumaPredDDL_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 block with **at least one valid row
///   above and one valid column to its left**, per `PADDING_LENGTH` — see the 4x4
...
pub unsafe extern "C" fn WelsI8x8LumaPredDDL_c(
    ...
    // SHIM(phase2) -> i8x8_luma_pred_ddl
    let stride = kiStride as usize;
    let (len, center) = shim_span(stride, 8, 8, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i8x8_luma_pred_ddl(&mut PlaneCursorMut::new(buf, center, stride), bTLAvail, bTRAvail);
}

/// C++: `WelsI8x8LumaPredDDLTop_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 block with **at least one valid row
///   above and one valid column to its left**, per `PADDING_LENGTH` — see the 4x4
...
pub unsafe extern "C" fn WelsI8x8LumaPredDDLTop_c(
    ...
    // SHIM(phase2) -> i8x8_luma_pred_ddl_top
    let stride = kiStride as usize;
    let (len, center) = shim_span(stride, 8, 8, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i8x8_luma_pred_ddl_top(&mut PlaneCursorMut::new(buf, center, stride), bTLAvail, bTRAvail);
}

/// C++: `WelsI8x8LumaPredDDR_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 block with **at least one valid row
///   above and one valid column to its left**, per `PADDING_LENGTH` — see the 4x4
...
pub unsafe extern "C" fn WelsI8x8LumaPredDDR_c(
    ...
    // SHIM(phase2) -> i8x8_luma_pred_ddr
    let stride = kiStride as usize;
    let (len, center) = shim_span(stride, 8, 8, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i8x8_luma_pred_ddr(&mut PlaneCursorMut::new(buf, center, stride), bTLAvail, bTRAvail);
}

/// C++: `WelsI8x8LumaPredVL_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 block with **at least one valid row
///   above and one valid column to its left**, per `PADDING_LENGTH` — see the 4x4
...
pub unsafe extern "C" fn WelsI8x8LumaPredVL_c(
    ...
    // SHIM(phase2) -> i8x8_luma_pred_vl
    let stride = kiStride as usize;
    let (len, center) = shim_span(stride, 8, 8, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i8x8_luma_pred_vl(&mut PlaneCursorMut::new(buf, center, stride), bTLAvail, bTRAvail);
}

/// C++: `WelsI8x8LumaPredVLTop_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 block with **at least one valid row
///   above and one valid column to its left**, per `PADDING_LENGTH` — see the 4x4
...
pub unsafe extern "C" fn WelsI8x8LumaPredVLTop_c(
    ...
    // SHIM(phase2) -> i8x8_luma_pred_vl_top
    let stride = kiStride as usize;
    let (len, center) = shim_span(stride, 8, 8, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i8x8_luma_pred_vl_top(&mut PlaneCursorMut::new(buf, center, stride), bTLAvail, bTRAvail);
}

/// C++: `WelsI8x8LumaPredVR_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 block with **at least one valid row
///   above and one valid column to its left**, per `PADDING_LENGTH` — see the 4x4
...
pub unsafe extern "C" fn WelsI8x8LumaPredVR_c(
    ...
    // SHIM(phase2) -> i8x8_luma_pred_vr
    let stride = kiStride as usize;
    let (len, center) = shim_span(stride, 8, 8, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i8x8_luma_pred_vr(&mut PlaneCursorMut::new(buf, center, stride), bTLAvail, bTRAvail);
}

/// C++: `WelsI8x8LumaPredHU_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 block with **at least one valid row
///   above and one valid column to its left**, per `PADDING_LENGTH` — see the 4x4
...
pub unsafe extern "C" fn WelsI8x8LumaPredHU_c(
    ...
    // SHIM(phase2) -> i8x8_luma_pred_hu
    let stride = kiStride as usize;
    let (len, center) = shim_span(stride, 8, 8, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i8x8_luma_pred_hu(&mut PlaneCursorMut::new(buf, center, stride), bTLAvail, bTRAvail);
}

/// C++: `WelsI8x8LumaPredHD_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 block with **at least one valid row
///   above and one valid column to its left**, per `PADDING_LENGTH` — see the 4x4
...
pub unsafe extern "C" fn WelsI8x8LumaPredHD_c(
    ...
    // SHIM(phase2) -> i8x8_luma_pred_hd
    let stride = kiStride as usize;
    let (len, center) = shim_span(stride, 8, 8, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i8x8_luma_pred_hd(&mut PlaneCursorMut::new(buf, center, stride), bTLAvail, bTRAvail);
}

// --- 8x8 chroma -------------------------------------------------------------

/// C++: `WelsIChromaPredV_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 chroma block with **at least one
///   valid row above and one valid column to its left** — chroma planes carry
///   `PADDING_LENGTH >> 1` = 16 samples of border.
/// * The bytes `[-kiStride - 1, 7*kiStride + 8)` from `pPred` must be valid to read,
///   and `[0, 7*kiStride + 8)` valid to write.
/// * `kiStride` is positive and large enough for the spans above to lie inside the
///   plane — in practice half the luma stride.
pub unsafe extern "C" fn WelsIChromaPredV_c(pPred: *mut u8, kiStride: i32) {
    // SHIM(phase2) -> chroma_pred_v
    let stride = kiStride as usize;
    let (len, center) = shim_span(stride, 8, 8, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    chroma_pred_v(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsIChromaPredH_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 chroma block with **at least one
///   valid row above and one valid column to its left** — chroma planes carry
...
pub unsafe extern "C" fn WelsIChromaPredH_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 8, 8, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    chroma_pred_h(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsIChromaPredPlane_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 chroma block with **at least one
///   valid row above and one valid column to its left** — chroma planes carry
...
pub unsafe extern "C" fn WelsIChromaPredPlane_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 8, 8, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    chroma_pred_plane(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsIChromaPredDc_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 chroma block with **at least one
///   valid row above and one valid column to its left** — chroma planes carry
...
pub unsafe extern "C" fn WelsIChromaPredDc_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 8, 8, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    chroma_pred_dc(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsIChromaPredDcLeft_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 chroma block with **at least one
///   valid row above and one valid column to its left** — chroma planes carry
...
pub unsafe extern "C" fn WelsIChromaPredDcLeft_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 8, 8, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    chroma_pred_dc_left(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsIChromaPredDcTop_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 chroma block with **at least one
///   valid row above and one valid column to its left** — chroma planes carry
...
pub unsafe extern "C" fn WelsIChromaPredDcTop_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 8, 8, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    chroma_pred_dc_top(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsIChromaPredDcNA_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of an 8x8 chroma block with **at least one
///   valid row above and one valid column to its left** — chroma planes carry
...
pub unsafe extern "C" fn WelsIChromaPredDcNA_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 8, 8, 8);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    chroma_pred_dc_na(&mut PlaneCursorMut::new(buf, center, stride));
}

// --- 16x16 luma -------------------------------------------------------------

/// C++: `WelsI16x16LumaPredV_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 16x16 macroblock with **at least one
///   valid row above and one valid column to its left**, per `PADDING_LENGTH`.
/// * The bytes `[-kiStride - 1, 15*kiStride + 16)` from `pPred` must be valid to
///   read, and `[0, 15*kiStride + 16)` valid to write.
/// * `kiStride` is positive and large enough for the spans above to lie inside the
///   plane — in practice `WELS_ALIGN(width + 2*PADDING_LENGTH, 32)`.
pub unsafe extern "C" fn WelsI16x16LumaPredV_c(pPred: *mut u8, kiStride: i32) {
    // SHIM(phase2) -> i16x16_luma_pred_v
    let stride = kiStride as usize;
    let (len, center) = shim_span(stride, 16, 16, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i16x16_luma_pred_v(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI16x16LumaPredH_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 16x16 macroblock with **at least one
///   valid row above and one valid column to its left**, per `PADDING_LENGTH`.
...
pub unsafe extern "C" fn WelsI16x16LumaPredH_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 16, 16, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i16x16_luma_pred_h(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI16x16LumaPredPlane_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 16x16 macroblock with **at least one
///   valid row above and one valid column to its left**, per `PADDING_LENGTH`.
...
pub unsafe extern "C" fn WelsI16x16LumaPredPlane_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 16, 16, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i16x16_luma_pred_plane(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI16x16LumaPredDc_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 16x16 macroblock with **at least one
///   valid row above and one valid column to its left**, per `PADDING_LENGTH`.
...
pub unsafe extern "C" fn WelsI16x16LumaPredDc_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 16, 16, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i16x16_luma_pred_dc(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI16x16LumaPredDcTop_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 16x16 macroblock with **at least one
///   valid row above and one valid column to its left**, per `PADDING_LENGTH`.
...
pub unsafe extern "C" fn WelsI16x16LumaPredDcTop_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 16, 16, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i16x16_luma_pred_dc_top(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI16x16LumaPredDcLeft_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 16x16 macroblock with **at least one
///   valid row above and one valid column to its left**, per `PADDING_LENGTH`.
...
pub unsafe extern "C" fn WelsI16x16LumaPredDcLeft_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 16, 16, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i16x16_luma_pred_dc_left(&mut PlaneCursorMut::new(buf, center, stride));
}

/// C++: `WelsI16x16LumaPredDcNA_c`, `codec/decoder/core/src/get_intra_predictor.cpp`.
///
/// # Safety
/// * `pPred` points at sample `(0, 0)` of a 16x16 macroblock with **at least one
///   valid row above and one valid column to its left**, per `PADDING_LENGTH`.
...
pub unsafe extern "C" fn WelsI16x16LumaPredDcNA_c(pPred: *mut u8, kiStride: i32) {
    ...
    let (len, center) = shim_span(stride, 16, 16, 16);
    let buf = unsafe { std::slice::from_raw_parts_mut(pPred.sub(center), len) };
    i16x16_luma_pred_dc_na(&mut PlaneCursorMut::new(buf, center, stride));
}

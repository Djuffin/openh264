...
pub fn WelsEncoderEncodeExt(
    ...
    if ENC_RETURN_CORRECTED == pCtx.iEncoderError {
        // **F162, closed at S12.13.** `iSpatialIdx == iSpatialNum` here — the loop
        // above ran to completion — so this addresses the slot *after* the last one
        // the frame wrote. Upstream indexes it anyway (`encoder_ext.cpp:4109-4110`)
        // and the port reproduced that with a raw `add()`.
        //
        // The map is `[SSpatialPicIndex; 4]`, so **only the 4-layer case ran off the
        // end**; at 1, 2 or 3 the read is in bounds and `get` returns the same byte
        // it always did. The fifth slot answers `0` — what an unwritten
        // `SSpatialPicIndex` holds, which is what the in-bounds cases read anyway.

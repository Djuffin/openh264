...
//!
//! `WelsI4x4LumaPredV_c` and its siblings exist three times in this port, with three
//! different signatures and three different destinations:
//!
...
// [`PlaneCursor`] whose span is that kernel's reach and no more. The reach table
// is `REACH_*` below and `ref_span` is the only place it becomes a slice.

/// What one predictor reads of the reconstructed plane, relative to the block's
/// own `(0, 0)` — i.e. relative to `pRef`.
///
...
/// The byte span a [`Reach`] needs at `stride`, as `(len, center)`: the shim hands
/// `from_raw_parts(pRef.sub(center), len)` to a [`PlaneCursor`] anchored at
...
pub fn ref_span(stride: usize, reach: Reach) -> (usize, usize) {
    ...
    // Lowest byte read, and one past the highest, as signed offsets from `pRef`.
    // Seeded from the first read rather than from zero: seeding at zero would drag
    // `pRef` itself into every span, and no predictor in this module reads it.
...
#[inline(always)]
pub fn i4x4_luma_pred_h(pred: &mut [u8; 16], reference: &PlaneCursor<'_>) {
    for y in 0..4 {
...
#[inline(always)]
pub fn i4x4_luma_pred_dc(pred: &mut [u8; 16], reference: &PlaneCursor<'_>) {
    let mut sum: i32 = 4;
    for y in 0..4 {
        sum += reference.at(-1, y) as i32;
    }
    for &v in reference.row(-1, 0, 4) {
        sum += v as i32;
    }
...
#[inline(always)]
pub fn i4x4_luma_pred_dc_left(pred: &mut [u8; 16], reference: &PlaneCursor<'_>) {
    let mut sum: i32 = 2;
    for y in 0..4 {
        sum += reference.at(-1, y) as i32;
    }
...
#[inline(always)]
pub fn i4x4_luma_pred_ddr(pred: &mut [u8; 16], reference: &PlaneCursor<'_>) {
...
#[inline(always)]
pub fn i4x4_luma_pred_vr(pred: &mut [u8; 16], reference: &PlaneCursor<'_>) {
    let lt = reference.at(-1, -1) as i32;
    let l0 = reference.at(-1, 0) as i32;
    let l1 = reference.at(-1, 1) as i32;
    let l2 = reference.at(-1, 2) as i32;
...
#[inline(always)]
pub fn i4x4_luma_pred_hu(pred: &mut [u8; 16], reference: &PlaneCursor<'_>) {
    let l0 = reference.at(-1, 0) as i32;
    let l1 = reference.at(-1, 1) as i32;
    let l2 = reference.at(-1, 2) as i32;
    let l3 = reference.at(-1, 3) as i32;
...
#[inline(always)]
pub fn i4x4_luma_pred_hd(pred: &mut [u8; 16], reference: &PlaneCursor<'_>) {
    let lt = reference.at(-1, -1) as i32;
    let l0 = reference.at(-1, 0) as i32;
    let l1 = reference.at(-1, 1) as i32;
    let l2 = reference.at(-1, 2) as i32;
    let l3 = reference.at(-1, 3) as i32;
    let top = reference.row(-1, 0, 3);
...
#[inline(always)]
pub fn chroma_pred_h(pred: &mut [u8; 64], reference: &PlaneCursor<'_>) {
    for y in 0..8 {
        let v = reference.at(-1, y as isize);
        let row: &mut [u8; 8] = (&mut pred[y * 8..][..8]).try_into().unwrap();
        row.fill(v);
...
#[inline(always)]
pub fn chroma_pred_plane(pred: &mut [u8; 64], reference: &PlaneCursor<'_>) {
...
#[inline(always)]
pub fn chroma_pred_dc(pred: &mut [u8; 64], reference: &PlaneCursor<'_>) {
    let top = reference.row(-1, 0, 8);
    let left = [
        reference.at(-1, 0),
        reference.at(-1, 1),
        reference.at(-1, 2),
...
#[inline(always)]
pub fn chroma_pred_dc_left(pred: &mut [u8; 64], reference: &PlaneCursor<'_>) {
...
#[inline(always)]
pub fn i16x16_luma_pred_plane(pred: &mut [u8; 256], reference: &PlaneCursor<'_>) {
    let mut top_sum: i32 = 0;
    let mut left_sum: i32 = 0;
    for i in 0..8isize {
        top_sum += (i as i32 + 1)
...
#[inline(always)]
pub fn i16x16_luma_pred_dc(pred: &mut [u8; 256], reference: &PlaneCursor<'_>) {
    let mut sum: i32 = 16;
    for &v in reference.row(-1, 0, 16) {
        sum += v as i32;
    }
    for y in 0..16 {
        sum += reference.at(-1, y) as i32;
...
#[inline(always)]
pub fn i16x16_luma_pred_dc_left(pred: &mut [u8; 256], reference: &PlaneCursor<'_>) {
    let mut sum: i32 = 8;
    for y in 0..16 {
        sum += reference.at(-1, y) as i32;
    }
...
// referred to by each:
//
//   **Why the negative reads are legal.** `pRef` points at sample `(0, 0)` of the
//   block in the *reconstructed* plane (`pMbCache->SPicData.pCsMb[i]` plus the
//   block's coordinate offset), whose rows are `kiStride` bytes apart. Reads at
//   `x = -1` and `y = -1` are in-allocation because both codecs allocate picture
//   planes with `PADDING_LENGTH` samples of border on every side (32 luma, 16
//   chroma — `pic_queue.rs:AllocPicture`, `wels_preprocess.rs`), so the padded
//   prefix is part of the same buffer.
//
...
//
//   **The destination.** `pPred` is a *packed* candidate buffer — 16 bytes for
//   I4x4, 64 for chroma, 256 for I16x16, at implicit strides of 4, 8 and 16 — and
//   `kiStride` says nothing about it. Every one of these kernels writes all of it,
//   and none of them writes a byte more.
//
...
/// The packed prediction block as a fixed-size array: `N` is 16, 64 or 256 and
/// the destination stride is `N.sqrt()` by construction, never `kiStride`.
///
/// # Safety
...
/// The `N` samples of the row above the block — the whole reach of every
/// top-only predictor, handed over as an array so the kernel cannot read left.
///
/// # Safety
...
/// A cursor over exactly `reach`, anchored at the block's `(0, 0)`. The one place
/// a reach becomes a slice (R-c), via [`ref_span`].
///
/// # Safety
...
unsafe fn reference<'a>(pRef: *const u8, kiStride: i32, reach: Reach) -> PlaneCursor<'a> {
    ...
    PlaneCursor::new(unsafe { std::slice::from_raw_parts(pRef.sub(center), len) }, center, stride)
}
...
/// # Safety
/// * `pPred` is writable for **16** bytes (packed 4x4, stride 4).
/// * `pRef` has [`REACH_I4X4_LEFT`] readable: the four samples at `x = -1`,
...
/// # Safety
/// * `pPred` is writable for **16** bytes (packed 4x4, stride 4).
/// * `pRef` has [`REACH_I4X4_LEFT`] readable — the four samples at `x = -1`. This
...
/// # Safety
/// * `pPred` is writable for **16** bytes (packed 4x4, stride 4).
/// * `pRef` has the four samples `[-kiStride, -kiStride + 4)` readable — the
...
/// # Safety
/// * `pPred` is writable for **16** bytes (packed 4x4, stride 4).
/// * `pRef` has the **eight** samples `[-kiStride, -kiStride + 8)` readable: four
...
/// # Safety
/// * `pPred` is writable for **16** bytes (packed 4x4, stride 4).
/// * `pRef` has [`REACH_I4X4_VR`] readable: the corner, four samples above and
...
/// # Safety
/// * `pPred` is writable for **64** bytes (packed 8x8, stride 8).
/// * `pRef` has [`REACH_CHROMA_PLANE`] readable: the corner at `(-1, -1)`, eight
///   samples above and eight to the left — the bytes
...
/// * `pRef` has [`REACH_I16X16_PLANE`] readable: the corner at `(-1, -1)`,

...
//! Shared CABAC probability and context-initialization tables.
//!
//! C++: `codec/common/src/common_tables.cpp`, and the CABAC tables carried by
//! `codec/decoder/core/src/decode_slice.cpp` / `codec/encoder/core/src/set_mb_syn_cabac.cpp`.
//!
//! These tables are normative H.264 / AVC data (Tables 9-12 .. 9-24, 9-44 and 9-45) and are
//! therefore necessarily identical on both sides of the codec. Upstream keeps a copy in the
//! encoder and another in the decoder; this module holds the single shared definition, which
//! `crate::encoder::set_mb_syn_cabac` and `crate::decoder::cabac_decoder` re-export under
//! their original paths.
//!
//! The context-index range comments on [`g_kiCabacGlobalContextIdx`] are the union of the
//! annotations that were present on the two former copies — they are the only map from a
//! context index to the syntax element it codes.

/// Total number of standard H.264 CABAC context models.
pub const WELS_CONTEXT_COUNT: usize = 460;

/// Placeholder for context entries that are not applicable to a given slice model.

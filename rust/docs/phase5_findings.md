...
### F43's disposition (session S, T5.S1) — fixed

The five stubs and the second `SFmo` are deleted; `decoder_core.rs` imports the real
bodies by name (`use`, not a glob — the defect *was* name resolution, and an explicit
import is the form that shows which body runs). `pFmo` gained an owner the way W5's
parameter sets did, not W1's: the settlement said "W1's recipe", and the tree
disagrees — `sFmoList` is a `[SFmo; MAX_PPS_COUNT]` the context **already owns**, and
`pCtx->pFmo = &pCtx->sFmoList[iPpsId]` is a raw alias *into* it, which is this phase's
named blocker class. So the field became `fmo_id: Option<i32>` with `fmo_of`/
`active_fmo` resolving per use through `addr_of_mut!` — `pps_of`'s shape, for
`pps_of`'s reason. The layer's own `pFmo` was dead in both trees and deleted.
`FmoParamUpdate` now runs at paramset activation, where the C++ runs it.

**What the gates showed, and what it took to read them.** The two concealed-macro
goldens the settlement expected to move do not exist: there is **one**
`asset_test_concealed!` row (`narrow_16x16_idr_lost`), and it did not move, correctly
— its damage is a lost IDR, every macroblock of every decoded frame *is* correctly
decoded, so `NeedErrorCon` returns false. What moved was the malformed-parity table:
27 rows over 6 streams. The settlement expected those to hold on the grounds that
they "run live against the dylib" and their damage is NAL-level. Both clauses are
wrong about the tree, and the second one is what matters: **the table is a stored
golden of the port's own output**, regenerated by `UPDATE_MALFORMED_GOLDEN=1`, and a
truncation leaves a *partially decoded picture*, which is exactly what `NeedErrorCon`
fires on.

That distinction is the whole finding below.

## F44 — `InitErrorCon` has no production caller, so error concealment is wired up but never switched on

*Found Phase 5 session S (2026-08-14), while checking F43's golden movement against
the C++ decoder rather than against the port's own previous output. **Fixed here**
(T5.S1), because F43's face is "both subsystems go live" and the subsystem is not
live when its dispatch table is null.*

### The defect

`error_concealment.rs` defines `InitErrorCon` and **nothing calls it**. The C++ calls
it twice: `decoder.cpp:665` (`WelsInitDecoder`, after the `eEcActiveIdc` validation)
and `welsDecoderExt.cpp:536` (`SetOption(DECODER_OPTION_ERROR_CON_IDC)`). It does two
things nothing else in either tree does:

* **Clears `bFreezeOutput`.** `WelsDecoderDefaults` sets it `true`; the only other
  site that clears it is the "complete non-ECed IDR frame done" arm of
  `DecodeFrameConstruction`. A stream whose first IDR is missing or damaged therefore
  stayed frozen and emitted **nothing** until a clean IDR arrived.
* **Installs `sCopyFunc.pCopyLumaFunc`/`pCopyChromaFunc`.** `DoErrorConSliceCopy` and
  `DoErrorConSliceMVCopy` guard every copy with `if let Some(f)`, so with the table
  `None` the slice-copy concealment ran to completion and **copied nothing**. A `None`
  guard is not an error; it is a silent no-op, which is why the path looked live.

Measured on `res/BA_MW_D_IDR_LOST.264`: the port emitted **70** frames against the
C++'s **97**, hash `7bdefad8…` against `f31e54a5…`.

### It is not F43, and F43 is not it

Both were live at once and they are separable. On a worktree at the pre-F43 commit
this stream gives the **identical** port hash, and re-stubbing `NeedErrorCon` leaves
it green: F43's stubs never reached it. Conversely F43's own coverage — the FMO asset
and the malformed table's `NeedErrorCon` revert — is untouched by F44. F43 made the
subsystem *reachable*; F44 is why it still did not *work*.

### Why nothing saw it

F43's rule, one turn further on. The instruments could see the subsystem was
unreachable; none could see that reaching it was not enough:

* `find_stub_bodies.py` matches by name against the C++ and finds a real, complete
  `InitErrorCon` body. It is not a stub. Nothing about it is wrong except that the
  call the C++ makes is absent — and **an absent call site has no name to match**.
* The **malformed-parity table pins the port against its own previous output**. It
  reports *change*, and it did — 27 rows moved when F43 landed — but it cannot report
  *wrongness*, because the reference on the other side of the comparison is the port.
* Every conformance row carrying a C++ golden is a **clean** stream, where neither
  decoder conceals and both agree by not running the code. `res/` has held two
  damaged-stream assets since upstream added them in 2014 (`45ef803e`), and the port
  has driven both since Phase 3 — through the malformed gate only.

### The instrument this earns

`rust/tools/ecref` — the C++ decoder's answer for one corpus entry, replicating
`malformed_stream_parity.rs`'s `decode_case` exactly (same annex-B split, same
`ERROR_CON_SLICE_COPY`, same EOS + drain, same plane hashing in emission order). It
reproduces the clean-stream conformance goldens to the digit, which is what says it is
faithful. `compare.sh` runs a whole table through it.

**The general rule**: *a golden that stores the port's own output measures change, not
correctness — and on any path the reference implementation also takes, the reference
is available.* Where a gate could compare against the C++ and compares against
yesterday's self instead, it will hold green through the entire lifetime of a defect.

### Coverage added

`decoder_conformance_test.rs` gains the first three damaged-or-FMO rows in this
project whose goldens are the **C++ decoder's**: `BA_MW_D_P_LOST` (unmoved by either
fix — it is what says the concealment content itself was already right),
`BA_MW_D_IDR_LOST` (F44's row, moved onto the C++'s hash by the fix), and
`fmo_2groups_64x64` (F43's row, built by `rust/tools/make_fmo_asset.py` because **no
stream in `res/` has more than one slice group** — red under the `FmoNextMb` stub as
"No frames decoded").

## F45 — `bInstantDecFlag` is read but never written, so the decoder emits a frame at end of stream that the C++ does not

*Found Phase 5 session S (2026-08-14), immediately after F44's fix: clearing
`bFreezeOutput` unmasked it. **Fixed here** (T5.S1).*

### The defect

The C++ writes `bInstantDecFlag` on exactly one arm and clears it one statement later:

```cpp
} else {                                  // kpSrc == NULL || kiSrcLen <= 0
  pDecContext->bEndOfStreamFlag = true;
  pDecContext->bInstantDecFlag  = true;   // welsDecoderExt.cpp:777
}
...
WelsDecodeBs (pDecContext, kpSrc, kiSrcLen, ppDst, pDstInfo, NULL);
pDecContext->bInstantDecFlag = false;     // :814, "reset no-delay flag"
```

The port declares the field, **reads** it in two places, and never assigns it — so it
is `false` for the decoder's whole life. The reader that matters is
`DecodeFrameConstruction`'s incomplete-frame arm:

```rust
if (*pCtx).iTotalNumMbRec != kiTotalNumMbInCurLayer {
    bFrameCompleteFlag = false;
    if (*pCtx).bInstantDecFlag { return ERR_INFO_MB_NUM_INADEQUATE }   // never taken
}
```

With the flag stuck `false` the early return never fired, the flush call fell through
to the output path, and the port **emitted a frame the C++ does not emit**: one extra
frame at end of stream on every truncated stream, and on a stream cut inside its first
slice, a whole 176x144 frame conjured out of a picture that was never decoded.

### Why it needed F44 to become visible

Both defects suppressed output, from opposite ends. `bFreezeOutput` stuck `true`
(F44) blocked the emission that `bInstantDecFlag` stuck `false` (F45) would have
allowed, so on most streams the two cancelled and the port's frame counts matched the
C++ **for the wrong reason**. Fixing F44 alone made five streams' agreement *worse*
(−55 rows) while fixing the sixth (+73): that spread is what pointed here. Two
compensating defects are one measurement apart from looking like one correct decoder.

### The class, and the sweep it earns

F43, F44 and F45 are three instances of one shape: **the C++ performs a write the port
never performs**. F43 was a call that resolved to a stub, F44 a call site that was
absent, F45 a field assignment that was absent. None is visible to a name-matching
instrument, because in every case the *name* exists in the port and only the *write*
is missing.

W7 takes the mechanical form: **for every field of `SWelsDecoderContext`, does the

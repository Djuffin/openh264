...
**What the layer owns now**: `MbArray<SMB>` — and `ppMbListD` is gone, because `InitMbListD` cut **one** flat block across the layers and stored the cuts twice, so neither field was a carrier — plus `Vec<SliceIdx>` for the slice list, two `Vec<i32>` for the per-slice arrays, `Vec<u16>` for the macroblock map, and `Vec<SSlice>` per bank. **Four malloc/copy/free triples and eleven allocations went with them**, `ExtendLayerBuffer` is three `resize` calls, and `FreeDqLayer` ends in `drop(Box::from_raw(…))` with one member walk left — `sSliceBs.pBs`, which is Phase 7's. **`ReallocateSliceList` is a `resize_with`, and session C's handed-over double free closes with it**: the C++ copied the old slices beside a live original and then freed the *new* list on three error paths, releasing `pBs` allocations the old list still owned; a resize **moves**, so each `pBs` is held once at every point.

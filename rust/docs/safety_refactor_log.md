...
**All six steps landed, in the brief's order, none dropped.** Five of the seven families read **zero** across `src/encoder` + `src/processing`: `SCabacCtx` **25 → 0** (crate-wide), `SWelsMD` **51 → 0**, `SWelsME` **39 → 0**, `SMeRefinePointer` **17 → 0**, `SQuarRefineParams` **1 → 0**. `SMVUnitXY` is **65 → 2**, and both survivors are the F-owned `SPicture.sMvList` sites the boundary list names. **`SMbCache` has zero parameters left** (96 → 0). **`*mut SMB` is 125 → 46**, and that shortfall is the session's main finding rather than its debt.
...
**Rule (c) paid for itself twice.** Fourteen `SMbCache` parameters were caches of `pSlice` and are deleted, not retyped. The sharpest is `WelsWriteMbResidualCabac`, which took `pSlice`, `sMbCacheInfo` **and** `pCabacCtx` — the last two being `&mut (*pSlice)….` — while its body writes `(*pSlice).uiLastMbQp` *between* uses of them. Two of the fourteen (`WelsMdInterMbEnhancelayer`'s `_pMbCache`, `WelsMdInterMb`'s `_pUnused`) were already dead, one of them derived its own from `pSlice` on the line after the parameter it ignored, and `WelsCabacMbRef`'s `pCurMb` was never read at all. The underscore had been carrying that fact; the compiler carries it now.

**What `grep '*mut SMbCache'` still reads, and why it is not a shortfall.** Ten `md::` accessor signatures (`coeff_level`, `dct`, `mem_pred_*`, `buffer_inter_pred_me`, …) plus **one line per converted body**: `let pMbCache: *mut SMbCache = pMbCache;`. The accessors are the root-derivation primitives S28/S29 exist to enforce — each returns a cursor into an inline array carrying the *whole array's* provenance — and the one-line reborrow is what makes them usable from a reference: `pRes`, `pBlock`, `pNonZeroCoeffCount` and the `SPicData` cursors routinely outlive a *later* accessor call, and any access through the `&mut` (a direct `(*p).field`, or re-coercing it for the next accessor) is an access through the Unique tag that pops every raw child taken before it. The reference is spent exactly once, at entry, and everything hangs off that.
...
**The done-test grep, read honestly.** `grep -rn '\*mut SWelsMD\b|\*mut SWelsME\b|\*mut SMVUnitXY\b|\*mut SMeRefinePointer\b|\*mut SCabacCtx\b|\*mut SMB\b|\*mut SMbCache\b'` over `src/encoder src/processing` reads **109 lines**, against 428 at the session's start. They break down as: **46 `*mut SMB` survivors** in the four named groups; **61 `*mut SMbCache`** — ten `md::` accessor signatures, 50 one-line entry reborrows, one doc mention; and **2 `*mut SMVUnitXY`**, both F-owned. **Zero parameters remain in five of the seven families**, and the two that do remain are enumerated rather than counted. No line of the 109 is a parameter this session could have converted soundly.
...
**Metrics, encoder-side** (`src/encoder` + `src/processing`, the ratchet's own patterns), `d7af280d` → close: `raw_ptr` **2331 → 2003 (−328)**, `unsafe_fn` **684 → 679**, `unsafe_block` **283 unmoved**, `mem_zeroed` **21 unmoved**. Crate-wide `raw_ptr` **2924 → 2596 (−328)**, `unsafe_fn` 804 → 799. **Sizes**: `SMeRefinePointer` **48 → 32**, newly pinned in `abi_guard.rs` — a shape record rather than an ABI one, because the struct is stack-local with one builder, which is exactly why it should exist. Everything else unmoved and still pinned: `SWelsME` 96, `SWelsMD` 4000, `SMB` 208, `SMbCache` 5600, `SCabacCtx` 504, `SMVUnitXY` 4.

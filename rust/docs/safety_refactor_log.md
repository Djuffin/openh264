...
## 2026-08-13 — Phase 5, session P (W1 whole, W2's first seam: the two containers whose aliases were removable)

**Commits:** `4f6495dd` (inherited doc tail: D-fid-1, phase5.md as a closed checklist,
the brief), `a3b68334` (T5.P1, W1), `eef8a90b` (T5.P2, W2 seam 1), and this entry.

### The session in one line

**The ordering rule works forwards, and it says which conversions are available**:
`pAccessUnitList` could own because T5.O4 had already given its nodes their own
allocations, and `pDec`/`pECRefPic` could become ids because the pool never owned the
pictures they aliased — while the layer's copy of `pDec` could not, because its 159
readers have no pool to look in. Two W-items' worth of container converted, four
hoists that were legal only against a raw field removed, and **`raw_ptr` did not move
for the second of them**.

### Control battery

Docs-only tail, session O ended `OVERALL: PASS`, `rust/tools/` and the toolchain
unchanged — S27's cheap subset. **OVERALL: PASS**, 474/468/20, ratchet 4420, census
59, matching session O's exit on every figure. Ninth consecutive session whose open
needed no correction.

**No perf measurement** (D-gate-1). The debt is unchanged and W8's.

### 0. The count the brief got wrong, and it changes a face's size

S24, at the open. The brief sized W2 as "236 `.pDec` sites, plus 94 `pRefList` and
209 `pRefPic`". `.pDec` is right (233 at `a3b68334`, and decoder-only by construction
— the field is the decoder's). The other two, re-greped comment-stripped:

| token | tree-wide | `src/decoder/` | `src/encoder/` | the brief said |
|---|---|---|---|---|
| `pRefPic` | 214 | **113** | 101 | 209 |
| `pRefList` | 209 | **82** | 127 | 94 |

**Neither figure states its scope, and the two disagree about what it was.** 209 for
`pRefPic` is the tree; 94 for `pRefList` is nearer the decoder's 82 than the tree's
209. Taken at face value the pair sizes W2 at ~539 sites, of which **228 are in
`src/encoder/`** — code F12/P10 puts outside this phase entirely, and which a
decoder-side `PicId` conversion cannot touch.

This is S24's failure mode one level up: not a stale count, but a count whose *unit*
is right and whose *domain* was never written down. **A count that decides a
conversion's shape carries the directory it was taken in**, and a brief that mixes
two scopes in one sentence is harder to catch than one that is simply out of date —
both numbers look plausible on their own, and only the sum is absurd.

### 1. W1 — the access unit (T5.P1), and the four hoists that were free until it owned

`pAccessUnitList: *mut SAccessUnit` → `access_unit: Option<Box<SAccessUnit>>`, 54
comment-stripped sites over four files. It was available *only* because T5.O4/O7 boxed
the nodes: an owning `Box` at the access-unit level retags the access unit's own bytes,
and the nodes are their own allocations, so `pCtx->pNalCur` and the `&mut BsCursor`
`ParseSliceHeaderSyntaxs` holds into a node are untouched. **The rule two convictions
produced (T5.N1, T5.O7) used forwards for the first time.**

**What the raw field was hiding.** Seven functions hoisted
`let pCurAu = (*pCtx).pAccessUnitList;` and used it across calls that reach the access
unit themselves. Against a raw field that is free — a copied pointer value cannot be
invalidated. Against an owner it is T5.O7's defect one level up. Four of the seven are
real and each re-derives now:

| function | the call in between | what it does to the access unit |
|---|---|---|
| `CheckIntegrityNalUnitsList` | `CheckAvailNalUnitsListContinuity` | derives it, and writes `uiEndPos` through its own borrow |
| `WelsDecodeInitAccessUnitStart` | `WelsDecodeAccessUnitStart` | moves `uiStartPos`, which the hoisted read is *after* |
| `DecodeCurrentAccessUnit` | the whole slice loop | re-derives a node per iteration |
| `CheckAndFinishLastPic` | `ConstructAccessUnit` | decodes |

The other three hold across nothing and keep one borrow — which is the distinction
worth keeping: *derive per use* is not a blanket rule, it is what you do when
something between the derivations can derive again.

`cur_au(pCtx) -> Option<&mut SAccessUnit>` is the only way in, and its autoref is on
the **field**, so the retag covers one word of the context rather than the context
(S29). That is what lets `mark_au_ready` write `bAuReadyFlag` through `pCtx` while the
access-unit borrow is live. Two consumers of it — `au_has_nals` and `mark_au_ready` —
absorb an eight-site guard and a five-site pair; the slice branch's four-site error
tail is `discard_nal_and_close_au`.

**F19, per allocation.** The access unit and its `Vec`: the context's drop glue. Each
node: `TagAccessUnits::drop`. `MemInitNalList`/`MemFreeNalList` are **deleted** — the
allocator is `SAccessUnit::with_nodes`, the deallocator is drop glue, and F39's
rival-allocator shape has no names left to write in. R4's first cascade entry:
`WelsFreeStaticMemory` sets the field to `None`, which is the same program as moving
it into `Drop`, because both of its callers run `drop(Box::from_raw(pCtx))` on the
next line.

**A second rival pair, found by the conversion.** `decoder_core.rs` declared its own
`ForceClearCurrentNal` beside `nalu.rs`'s, **with no caller at all**. F39 was the same
shape with both copies live; this one was invisible to the census (which compares
duplicate *bodies*, and these had diverged in signature) and to every gate (dead code
runs nowhere). Deleted, with `PAccessUnit`, which lost its last referent.

S21: `make_zeroed_shell_valid` writes `None` explicitly. `Option<Box<T>>`'s `None` *is*
all-zero via the null-pointer niche, so the write is redundant today — and it is there
because the alternative is a field whose validity rests on a layout guarantee that
nothing in the tree states.

**Ratchet**: `raw_ptr` 4420 → 4404 (−16), `unsafe_fn` −2 (the deleted allocator pair),
`decoder_context.rs` `unsafe_fn` +3 (the accessor and its two consumers). Probes green
un-ignored, 753s, mid-face — the budget T5.O3 earned and T5.O7/O8 paid for.

### 2. W2 seam 1 — the context stops pointing into the pool (T5.P2)

`pDec: *mut Picture` → `Option<PicId>`; `pECRefPic: [*mut Picture; 16]` →
`[Option<PicId>; 16]`. 74 comment-stripped `.pDec` sites and 11 `pECRefPic` over six
files. These are exactly the two fields session O's closure classified as **pool
aliases**, and they are why `Pool<Box<SPicture>>` has been blocked since T5.N1.

`dec_pic(pCtx)` and `ec_ref_pic(pCtx, i)` **copy a stored slot pointer**; they do not
derive one. That is `TagAccessUnits::nal`'s contract for `PicPool`'s reason — the pool
addresses, it does not own, so the pointer carries `AllocPicture`'s provenance and no
borrow of the pool touches it. **It is also the whole point of the seam**: when W3
flips the slots to `Box<SPicture>`, those two functions are the only sites that become
pool borrows, and everything upstream of them is already in the id world.

`pic_slot(p) -> Option<PicId>` is the one-way door from pointers to ids. Total by
design: a null pointer and a picture outside the pool are both "no slot", which is what
the arms it replaces did.

**One identity comparison stops having a fallback.**
`same_picture((*pCtx).pECRefPic[0], pRef)` is `(*pCtx).pECRefPic[0] == pic_slot(pRef)`
— P3's predicate with a slot on both sides and no address arm to reach. Fourth of the
five identity sites; the deblocking filter (T5.N4) was the third.

**A stored dangling pointer stops being writable.** `WelsFreeDynamicMemory` destroys
the pool without clearing `pDec`. Nothing read it afterwards, so the port held a
dangling pointer it never dereferenced — F38's shape, in the lifecycle instead of in
`api/`, and *unexercised* for the same reason. With a slot handle the same sequence
makes `dec_pic` return null, because `pPicBuff` is null by then. Not a fix of an
observed defect; a defect that can no longer be written.

`PicPool::over(slots)` is `CreatePicBuff`'s tail, factored, because a fixture cannot
otherwise put its own pictures into a pool and
`p3_slice_copy_self_copy_guard_is_by_identity` now needs one. Its two pictures enter
through `addr_of_mut!` — S29's spelling at exactly the fixture class T5.O8 was
convicted on.

**The ratchet is flat across 85 converted sites and that is the honest reading.**
`raw_ptr` 4404 → 4404. Two pointer fields died; four accessor signatures were born.
The metric counts pointer *types written*, never pointer *dereferences* — session K's
observation for the fourth session running, and this is its cleanest instance, because
the structural change (a field that was an alias is now a handle) is exactly the kind
the metric cannot see. What did move: **decoder `raw_ptr` 1283 → 1267**, phase5.md's
own monotone metric, all of it W1's.

Probes and **all five P3 identity tests** green under Miri un-ignored, 756s.

### 3. What W2 seam 1 could not take, and why it is a seam rather than an omission

`DqLayerState::pDec` is the context's pointer copied into the layer at
`InitDqLayerInfo`'s single call site. **159 sites remain** — `decode_slice.rs` 72,
`parse_mb_syn_cavlc.rs` 30, `deblocking.rs` 27, `parse_mb_syn_cabac.rs` 13,
`mv_pred.rs` 13, `decoder_core.rs` 3, `error_concealment.rs` 1 — and every one of them
is in a **layer-scoped** function: `pCurDqLayer` is in scope, `pCtx` is not. A `PicId`
is only a picture if you have the pool, so converting the layer's copy means giving
159 sites a pool, which is session O's §1(b) signature leg (148 functions) arriving
from the other side.

That is not a reason to defer it forever; it is the reason it is a separate commit with
a separate design question, and the question is now specific: **does the layer carry
the pool, or do its functions take it?** The first is a back-pointer of the kind the
phase is deleting. The second is the signature leg.

The rest of W2, sized at `eef8a90b`: `SRefPic`'s three lists (`pRefList` 82,
`pShortRefList` 25, `pLongRefList` 24 — 100 of them in `manage_dec_ref.rs`),
`SPicture::pRefPic` (113 decoder-wide), `ppRefPic` (24), and
`pPreviousDecodedPictureInDpb` (11).
...
| Miri `--lib` | 334 | **334** (941.5s) |
| decode goldens | 57 rows | **57** |
| census allowlist | 59 | **59** |
| census duplicate-body budget | 195 | **192** |
| `raw_ptr` (crate) | 4420 | **4404** |
| decoder `raw_ptr` (phase5.md's metric) | 1283 | **1267** |
| `unsafe_block` | 627 | **627** |
| `unsafe_fn` | 1245 | **1247** |
| `mem_zeroed` | 31 | **31** |
| `SHIM(` | 159 | **159** |
| Miri skips | 2 | 2 |

**Two numbers moved and one of them is the point.** `raw_ptr` −16, *all of it W1's*:
W2 seam 1 converted 85 sites and moved it by zero, because the metric counts pointer
*types written* and the seam trades two pointer fields for four accessor signatures.
That is session K's observation for the fourth session running, and this is its
cleanest instance — the structural change is precisely the kind the instrument cannot
see. The duplicate-body budget is **195 → 192**, its second decrease ever (session M's
198 → 195 was the first): T5.P1's deleted allocator pair and the dead
`ForceClearCurrentNal`. No test was added by either face, which is why Miri and the
test totals are flat across a session that converted 139 sites.

### 5. Perf

**Not measured. D-gate-1.** W8 inherits everything session N and O left, unchanged.
Two things this session did that the exit's ledger row will be measuring through, both
structural: the access unit's guards became `Option` matches rather than null tests,
and `dec_pic` turns what were field reads into pool lookups — a bounds-checked `Vec`
index per use, at 74 sites, some of them per-macroblock. That last one is the first
change in the phase whose *direction* is plausibly negative, and it is written here so
the ledger does not have to guess.

### 6. What went into the rules

* **Nothing new.** The tags that carried the session were S24 (§0's directory-scope
  miss), S29 (the spelling at every derivation, and `cur_au`'s field-precise autoref),
  S25 (the four hoists, enumerated per function rather than per file), S20 (the closure
  that split W2 at the carrier boundary), S16 (the prose floor, eleventh instance, and
  the flat ratchet), S21, S27, R4, R7, F19.
* **S24 gains a scope, exactly as S29 did last session.** Session O's F38 lesson was
  that a module exempt from a lint leaves every instrument that copied the lint's
  scope. This session's is the mirror: **a count carries the directory it was taken
  in**, and a brief that omits it can move half a face's work into another phase's
  code.
* **The ordering rule is now predictive, not just diagnostic.** T5.N1 and T5.O7 each
  reached "a safe container may not lend while raw aliases into it are live" from a
  Miri conviction. This session used it to *choose*: W1 was available because its
  aliases had already died, W2 seam 1 was available because its container never owned,
  and the layer's `pDec` is not because its readers cannot reach the container. No
  probe was needed to make any of the three calls, and all three probe runs were green
  on the first attempt.

### 7. F3 — two hits, one alternation, acquitted (measurements 40–41)

The closing battery drew **one hit per profile**: debug
`mt CiscoVT2people_320x192_12fps t=4 sm=3 n=600 cabac=1 rc=0` (39981 → **0**) and
release `mt CiscoVT2people_160x96_6fps t=4 sm=3 n=600 cabac=0 rc=0` (42538 →
**42616**). Different clips, different `cabac`, one short and one *long*; both inside
S14's signature on every axis. Everything else in the battery was green.

**Step 0 did not apply, and the reason is worth carrying.** A decoder-only diff has
usually meant the hash shortcut — but the shortcut needs the two `rust_enc` binaries
to be byte-identical, and this is production decoder code in the same lib, so base and
head differ even though the sweep compares encoders. Session D's shortcut was earned
by a `#[cfg(test)]`-only diff. "Decoder-only" is not "test-only", and the two should
not be conflated again.

**Step 1**: both configurations **5/5 byte-identical in isolation**. **Step 2** fired
on two hits — twelve whole `mt` presets per side, both debug binaries built once and
swapped inside one loop, 240 configurations per sweep, **2880 per side**:
**base 2 hits, head 3**. Not a difference, and S23b is satisfied.

**The alternation produced the finding's strongest evidence to date, on both sides.**
The base tree — session O's code, predating this session entirely — hit the battery's
own debug configuration twice. And the head binary gave *that same configuration* **0
bytes in the battery and 37837 in alternation round 10**: one binary, one
configuration, two different wrong lengths. Measurement 39 needed fifteen isolation
runs to reach three distinct outcomes; this reached two in an ordinary alternation
without looking. *A deterministic port bug repeats its bytes.*

**Acquitted.** Running total: forty-one measurements, fourteen alternations, twenty
acquittals. Recorded in `phase0_findings.md` at adjudication time per S14 step 4.

### Hand-off: Phase 5, session P′ — W2b, then W3 → W5

The estimate moves by one: **P′ (W2b → W5), P″ (W6 + W7), Q (W8)** — three work
sessions plus the exit, where the re-plan said two. The reason is W2b's and it is
structural, not slippage: `pDec` is **two carriers**, and the second is session O's
148-function signature leg wearing a different hat.

1. **W2b's design question, and it is the whole of W2b's difficulty. Settle it
   before the first edit, in writing, the way session O's §1 closure was settled.**
   `DqLayerState::pDec` is `pCtx->pDec` copied into the layer at `InitDqLayerInfo`'s
   single call site (`decoder_core.rs:3701`). Its **159 remaining readers** —
   `decode_slice.rs` 72, `parse_mb_syn_cavlc.rs` 30, `deblocking.rs` 27,
   `parse_mb_syn_cabac.rs` 13, `mv_pred.rs` 13, `decoder_core.rs` 3,
   `error_concealment.rs` 1 — are **layer-scoped**: `pCurDqLayer` is in scope and
   `pCtx` is not. A `PicId` is only a picture if you hold the pool, so the choice is:
   * the layer carries a pool pointer — a back-pointer of exactly the kind this phase
     is deleting, on the struct 5.2 spent eleven commits emptying of pointers; or
   * the readers take the pool — session O §1(b)'s narrowing sweep, bottom-up and
     S25-gated per function, arriving from the other side.

   Nothing else in W2b is hard. `SRefPic`'s three lists (`pRefList` 82,
   `pShortRefList` 25, `pLongRefList` 24 — 100 of them in `manage_dec_ref.rs`, one
   file), `SPicture::pRefPic` (113), `ppRefPic` (24) and
   `pPreviousDecodedPictureInDpb` (11) are `pic_slot` at the write and `slot()` at the
   read, which is T5.P2's shape repeated. **All counts are decoder-only** — see §0 for
   why that qualifier is now mandatory.

2. **W3 is one seam narrower than it was.** `dec_pic` and `ec_ref_pic` are the only
   two sites in the tree that turn a slot handle into a picture pointer for the
   context, so `Pool<Box<SPicture>>` + `mut_and_rest` changes those two functions and
   whatever W2b leaves. The rest of W3 is unchanged: `pDqLayersList`/`pPicBuff`/
   `pTempDec` owned, the cascade into `Drop` (R4 — `access_unit` is the first entry and
   it is taken), the shell **extended per owned field, never deleted**.

   `pTempDec` is **not** a pool slot and cannot take a `PicId`: it is `AllocPicture`d
   on its own at `decode_slice.rs`'s first B macroblock. It is `Option<Box<SPicture>>`
   directly, and that is W3's, not W2b's. Noted at the field.

3. **Re-grep at every face, and record the directory.** Session P's §0 is the reason:
   the brief's `pRefPic`/`pRefList` sizes were tree-wide and most of both was the
   encoder's, which F12/P10 puts outside the phase. A count that decides a
   conversion's shape carries the directory it was taken in.

4. **Budget a probe run per container.** Session P ran three and all three were green
   first time — the first session since the rule was written where the budget bought
   confidence rather than a finding, which is what a budget is supposed to do.

5. **Findings**: nothing new this session. Inherited unchanged — **F41** and the eight
   `CWelsDecoderImpl` back-pointers and the `src/api/` inventory are **Phase 8's**;
   **F23** Phase 8's; **F36** decoder-threading's; **F3** per S14.

6. **Perf debt for W8**, unchanged and now with one addition to expect: `dec_pic`
   turns 74 former field reads into pool lookups (a bounds-checked `Vec` index each),
   some per-macroblock. It is the first change in the phase whose *direction* is
...
**Both families converted in the recipe the brief ruled, and no encoded byte moved.** `SMB`'s five outward pointers and `SMbCache`'s eleven are inline arrays of the structs that always owned
them (`pEncSad`, the twelfth, aliases a picture and stays for F). Gone with them: five context allocations and their frees, eight per-slice allocations,
`AllocMbCacheAligned`/`FreeMbCache`/`AllocateSliceMBBuffer`, `InitMbInfo`'s wiring half and its `kiMaxMbNum` parameter, the two parity banks, three port-added null guards, and
`deblocking.rs`'s dead `TagMB` duplicate of `SMB` (S18).

**Metrics, encoder-side** (`src/encoder` + `src/processing`, the ratchet's own patterns — `raw_ptr` counts `*mut `/`*const ` occurrences, `unsafe_fn` line-anchored definitions), `2666a83c` →
close: `raw_ptr` **2446 → 2389 (−57)**, `unsafe_fn` **686 → 684**, `unsafe_block` **289 → 288**, `mem_zeroed` 22 unmoved; crate-wide `raw_ptr` **3037 → 2980**. **Two per-file rises, recorded
not hidden** — `md.rs` `unsafe_fn` 17 → 26 and `raw_ptr` 80 → 84: eleven pointer *fields* left `SMbCache` and ten root-deriving accessors arrived, which is the trade the settlement made.
**Sizes, measured and re-pinned in the commit that moved each**: `SMB` **152 → 208**, `sWelsEncCtx` **97952 → 97912** (with all fifteen `assert_ctx_offset!` pins, which sat behind five
deleted pointers), `SMbCache` **576 → 5600**, `SSlice` **1520 → 6544** — the same bytes the C++ malloc'd per slice, in one block instead of eight, with no per-slice allocation left to fail.

**The second probe and the knob it exposed.** `encode_loop_runs_with_cavlc_and_fine_mode_decision_under_the_aliasing_checker` (name read out of the `--lib` output, F17/F18) found **two reds
on its first execution**, both fixed red-before/green-after; coverage measured, not argued — 0/0 calls into the fine partition search and the CAVLC writer from the first probe, 6/18 from
this one. Solo cost **24.96 s** against the first probe's **79.02 s** (**101.60 s** the pair): the brief budgeted ≈80 s, and the two probes are not the same price. It also exposed a hole in
the *byte* gate — every one of the 341 sweep configurations is `LOW_COMPLEXITY` — so both diffharness drivers took an optional complexity argument (default `LOW`: the 341 count and every
existing configuration unchanged), and ten configurations across three clips, both entropy coders and four `iRCMode`s read **BYTE-IDENTICAL** at `MEDIUM` and `HIGH`.

**The span** (7 pairs against `2666a83c`, `perf_baseline.md`): encode median **−0.97%** against a **−2.99 … +0.82%** null band, decode **+0.25%** against **+0.41%** — both inside, worst
single row **+0.49%** against D-perf-4's **+25% median** tripwire. Cumulative encoder deficit ≈ +11…+13% → ≈ **+10…+12%**. Face 1's predicted AoS cost is below this instrument's resolution;
the **SoA fallback is named, not opened** (Phase 9, D-perf-6). **F3, measurements 66 and 67** (S14, at adjudication, `phase0_findings.md`): face 1's release sweep read 340/341 on `mt sm=3
t=2` and the close's debug sweep 340/341 on `t=4` — the signature on every clause both times, and each configuration byte-identical on re-run (6× and 5×). The second **gate** hit triggered
the alternation measurement 66 had written down as owed: **12 `mt` presets a side, base `2666a83c` against head, 2880 configurations, one hit each side, both zero-length — HEAD is not
worse**. Twenty-first alternation, fortieth acquittal.

**The `exit` battery's first run failed one step, and it was this session's own test** (T6.C4): `test_wels_md_i16x16_cost` derived a pointer from `sMemPredMb`, then passed `&mut mb_cache` to
two accessors — a `Unique` retag over all 5600 bytes that pops that tag — and read through it after. Production is unaffected (every call site already holds a raw pointer); fixed by
ordering. Second session running in which `exit` caught a defect the session itself introduced.

**Gates.** `exit` battery **`OVERALL: PASS` — 13 passed / 0 failed / 1 skipped** at `a516ebab`: tests **485 / 479 / 20**, ratchet clean, census **58**, **both sweeps 341/341 in both
profiles**, both benches bit-identical, Miri `--lib` **342 / 0** with **both** encoder probe names read out of the output (F17/F18), differential targets **20 / 7 / 3**. The `--lib` step
reads **1022.46 s** at 342 tests against session B's 1034.61 s at 341 — the new probe's 25 s sits inside the step's own run-to-run spread, which session B measured at 3.3% with isolation
off. A `commit` battery in each code commit, `family` at each face's close, and **three `exit` runs**: the first failed the Miri step on T6.C4's defect, the second failed the ratchet (a
prose `*mut ` in T6.C4's own comment, S16's floor) and the debug sweep (measurement 67), and the third is the one quoted here. Both failures are recorded above, not hidden.
...
**Face 0 found more than it was built to find.** The dynamic-slice probe — `SM_SIZELIMITED_SLICE`, 112x96, a 401-byte constraint, single-threaded **by reading** (`bSliceBsBufferFlag`/`bThreadSlcBufferFlag` both need `iMultipleThreadIdc > 1`) — produced **four reds, three on its first execution**. The largest is **F60**: `FrameBsRealloc` grows `pOut->sNalLen` and the port had dropped the C++'s closing loop that re-aims every `sLayerInfo[..].pNalLengthInByte` at the moved array, so every layer cursor named freed memory. Against the C++ through `compare.sh` — `st`, single-threaded, 320x192, twelve configurations — **three SIGBUS crashes and four byte divergences before, all twelve BYTE-IDENTICAL after**. **No sweep configuration reaches it**: `iMaxSliceNum` opens at `MAX_SLICES_NUM` = 35 and the sweep's two size-limited rows code at most nine slices at its fixed qp, so the path was 341/341-green because nothing drove it. The other three are S29's class — `WelsMarkMMCORefInfoWithBase` holding a `&` to `ppSliceList[0]` across the loop that writes it, both dynamic macroblock loops holding `&mut sSliceEncCtx` across a callee that re-borrows it, and **this session's own first fix spelling**, whose forward walk wrote with the parent's tag and popped the caller's array-wide cursor.

**Non-vacuity and the realloc are measured, not assumed**: **37 / 9 / 3** coded slices against **1 / 1 / 1** at the constraint the sweep runs, and 112x96 is the smallest geometry on the grid that crosses the 35 slices `WelsCodeOnePicPartition` needs before it calls `DynSliceRealloc` — 48x32 codes 3, 96x64 21, 96x96 31, all measured at the same constraint. Solo cost beside the other three (79.02 s and 24.96 s at session C, plus the init probe): the `--lib` step reads **1432.01 s at 344 tests** against session C's **1022.46 s at 342**, and the probe's own in-step window measures **≈ 357 s** — it is the expensive one, and 42 macroblocks against 6 is why.

**S34 is clean here and it is measured, not assumed**: `ppDqLayerList` has **28 occurrences** over `src/` and **zero** `swap`/`rotate`/`retain`/`remove`/`sort`/`drain` anywhere in the tree, so a layer's position is a stable identity and `pRefLayer` may be one; the layer carries its own `iDqIdx` because `WelsSwapDqLayers` must name the *outgoing* layer and holds only its pointer. **What the layer owns now**: `MbArray<SMB>` — `sMbDataP` survives as the owning field rather than as a pointer, and `ppMbListD` is gone entirely, because `InitMbListD` cut **one** flat block across the layers and stored the cuts twice, so neither was a carrier — plus `Vec<SliceIdx>` for the slice list, two `Vec<i32>` for the per-slice arrays, `Vec<u16>` for the macroblock map, and `Vec<SSlice>` per bank. **Four malloc/copy/free triples and eleven allocations went with them**, `ExtendLayerBuffer` is three `resize` calls, and `FreeDqLayer` ends in `drop(Box::from_raw(…))` with one member walk left — `sSliceBs.pBs`, which is Phase 7's. **`ReallocateSliceList` is a `resize_with`, and session C's handed-over double free closes with it**: the C++ copied the old slices beside a live original and then freed the *new* list on three error paths, releasing `pBs` allocations the old list still owned; a resize **moves**, so each `pBs` is held once at every point.

**Every raw pointer still handed out is root-derived, and that is S28 verbatim** — `mb_list_root`/`mb_at`, `slice_bank_root`/`slice_in_bank`, `slice_in_layer`. The encoder's neighbour reads are `pCurMb.offset(-1)` and `.offset(-iMbStride)`, so a pointer taken through a narrowing index has the right address and provenance for the tail alone; `mb_list_root_reaches_the_whole_array_in_both_directions` is the Miri test S28 mandates, walking the array's full reach from the middle both ways and checking that a second derivation does not invalidate the first.

**Two findings recorded rather than fixed.** **F61**: `ReallocateSliceInThread` grows a bank and re-stamps nothing, so under `iMultipleThreadIdc > 1` the slice list is stale until frame-end reordering — **the C++ has the same shape**, checked line by line (`SSlice*& pSliceList` updates the bank pointer in place and nothing else), so it is a shared latent defect and **Phase 7's**. What this session changed about it is real but partial: a stale *position* still resolves inside the live bank, so the memory-safety half of the class is gone and the synchronisation half is what Phase 7 inherits. **S18's item**: `pFeatureSearchPreparation` and five functions behind it are deleted, the list enumerated by strip-and-build rather than by the brief.

**Metrics, encoder-side** (`src/encoder` + `src/processing`, the ratchet's own patterns — `raw_ptr` counts `*mut `/`*const ` occurrences, `unsafe_fn` line-anchored definitions), `085e2e41` → close: `raw_ptr` **2389 → 2331 (−58)**, `unsafe_block` **288 → 283**, `mem_zeroed` **22 → 21**, crate-wide `raw_ptr` **2980 → 2924**. **`unsafe_fn` is unmoved at 684 and that is the trade stated**: six new root-and-resolution accessors arrived, each `unsafe fn` because **its signature names raw pointers** (session Z's clause), against six deleted dead functions. **Sizes measured and re-pinned in the commit that moved each**: `SDqLayer` **512 → 504 → 480 → 496 → 528 → 560 → 576 → 640** — and its `assert_size!` was **restored at the close**, having been dropped rather than re-pinned by T6.D7's edit while its comment carried the number, which is the one place this session's own rule slipped, `SSliceCtx` **32 → 48**, `SSliceBufferInfo` **16 → 32**, `sWelsEncCtx` **97912 → 97904** with thirteen of the fifteen `assert_ctx_offset!` pins moving by 8 — the two that do not are the two that precede the deleted `ppMbListD`.

**The span** (7 pairs against `085e2e41`, `perf_baseline.md`): decode median **−0.08%** against a **−0.11 … +0.11%** null band, encode **+0.00%** against **−3.14 … +2.07%** — both inside, every row of both benches inside the floor, and the encode median *identical* to the null's own. Worst single row **+1.01%** against D-perf-4's **+25% median** tripwire, unbreached by ≈24 points; cumulative encoder deficit ≈ **+10…+12%**, unmoved. **The brief's two named candidates are below this instrument's resolution and neither is claimed** (S33). No day two is owed: no decision rests on the reading.

**F3, measurements 68, 69 and 70** (S14, at adjudication, `phase0_findings.md`), each inside the signature and each re-run 5× byte-identical. The second triggered the alternation S14 owes: **12 `mt` presets a side, base `085e2e41` against head `9d5c0156`, 2880 configurations — base 5 hits, head 1. HEAD is not worse.** Twenty-second alternation, forty-third acquittal. Head's single hit was at `n=1500`, the fourth on record, which is more evidence for session K's correction that `n`'s predominance is a rate artifact rather than a condition.

**Gates.** `family` at every face's close, **both sweeps 341/341 in both profiles at every one of them**. `exit` battery **`OVERALL: PASS` — 13 passed / 0 failed / 1 skipped** at `592801b5`, **on its first run**: tests **487 / 481 / 20**, ratchet clean, census **58**, both sweeps **341/341** in both profiles, both benches bit-identical, Miri `--lib` **344 / 0** with all four encoder probe names read out of the output (F17/F18), differential targets **20 / 7 / 3**. **Per the steward's in-session direction (S30), Miri ran once at the close rather than per face** — face 0's four reds were found before that direction arrived, and nothing the later faces did showed up when the checker finally ran over them.

**Session E inherits, named**: the `*mut SMB` (128 sites) and `*mut SMbCache` parameter families, stopped at face 4's close exactly where §6 authorised and nowhere else; **F61** for Phase 7; `pEncSad`, the plane families and `pCurDqLayer` (271 sites, a *context* field, and the same S21 argument that lets the layer own forbids the context to until 6.6) unchanged for F and G.

## Phase 6, session E — the records become references, and the neighbour rule gets written down (2026-08-19)

**All six steps landed, in the brief's order, none dropped — and then the `exit` battery rejected one of them, correctly.** Five of the seven families read **zero** across `src/encoder` + `src/processing`: `SCabacCtx` **25 → 0** (crate-wide), `SWelsMD` **51 → 0**, `SWelsME` **39 → 0**, `SMeRefinePointer` **17 → 0**, `SQuarRefineParams` **1 → 0**. `SMVUnitXY` is **65 → 2**, both the F-owned `SPicture.sMvList` sites the boundary list names. **`*mut SMB` is 125 → 48**, and **`SMbCache` is 96 → 72** — the second of those is a *retreat*, and the finding behind it is the most useful thing this session produced.

**THE CORRECTION, first, because everything below has to be read through it.** T6.E2c converted 62 `SMbCache` parameters to `&mut SMbCache`. Miri's encode probe rejected it at `WelsEncRecUV`, and the audit that followed found **58 sites** of the same shape. The reason is architectural: **`SMbCache` is an arena, and a `&mut` to it covers all 5600 bytes.** Its consumers hold *several* raw cursors into different parts of it at once — `pCurRS` into `sCoeffLevel`, `pBestPred` into `sMemPredMb`, `pBlock` into `sDct`, `pBufMe` into `sBufferInterPredMe` — and they hold them **across calls**, because the arena is how a macroblock's DCT / quantise / reconstruct steps hand work to each other. Creating a `&mut SMbCache` from the shared parent retags the whole arena and invalidates every one of those cursors, **including the ones the caller is still holding**. The entry-reborrow line E2c introduced protected the *callee's* cursors and did nothing for the caller's — which is the half missed while quoting md.rs's own S28/S29 note. Two shapes make it unfixable rather than repairable: `WelsEncRecUV(pFunc, pCurMb, pMbCache, pRes, iUV)` takes the arena **and** a cursor into it in one call, so whichever argument evaluates second invalidates the first; and `WelsIMbChromaEncode` uses `pCurRS` before *and* after the call, because the sequence is DCT into the buffer, quantise, IDCT back out. **62 parameters reverted, 49 entry reborrows deleted, 101 call sites unwrapped, four slot types restored.** What stands is T6.E2b's fourteen *deleted* parameters — the same problem's other half, and the fix an arena actually wants: the callee derives the arena from the `pSlice` it already has instead of being handed a second path to it. **A reference is the wrong tool for an arena; deleting the second path is the right one**, and md.rs's accessor note says so now.

**The rule the session found, and wrote down.** A macroblock parameter converts to `&mut SMB` only if the function **(1)** does no cursor arithmetic on it and **(2)** does not re-reach the array through another parameter. Both clauses are provenance. Clause 1 is the one the brief already knew for `deblocking.rs` — a `&mut SMB` covers its own 208 bytes, so `pCurMb.offset(-1)` off it is out of bounds for that tag. **Clause 2 is F13's shape** and the brief did not have it: a function holding a `&mut SMB` while re-deriving the array root from `pCurDqLayer` (`mb_list_root`, `mb_at`, `WelsMbToSliceIdc`) re-tags the whole buffer and invalidates the reference it is still holding. `sMbDataP` is touched in exactly two files, so clause 2 is checkable by grep. **The classification was mechanical, not eyeballed**: a script over every `src/encoder/*.rs` collected each function's SMB parameters, marked direct cursor arithmetic, and propagated the mark through call sites to any function forwarding its own parameter into a marked slot; clause 2 went on top by hand. **68 converted, 46 named** in four groups — neighbour-walkers (the CABAC writers' 11, `deblocking.rs`'s 9 over 18 arithmetic sites, `md.rs`'s `FillNeighborCache*`, and the two slot types that carry them), array-owners (`svc_encode_slice` 8, `encoder_ext` 2), slot entry points (`svc_set_mb_syn_cavlc` 3 behind `pfWelsSpatialWriteMbSyn`, whose CABAC arm is neighbour-bound so the slot cannot retype), and Phase 7's one. **The brief's step-4 note about deblocking was the rule for the whole family, and step 1 hit it first**: 11 of the 13 SMB sites in `svc_set_mb_syn_cabac.rs` are neighbour-bound, so step 1's stated goal of zero was not reachable soundly and the honest count is 14 of 24 with the other 10 named.

**Rule (c) paid for itself twice, and it is the part of step 2 that survived.** Fourteen `SMbCache` parameters were caches of `pSlice` and are deleted, not retyped. The sharpest is `WelsWriteMbResidualCabac`, which took `pSlice`, `sMbCacheInfo` **and** `pCabacCtx` — the last two being `&mut (*pSlice)….` — while its body writes `(*pSlice).uiLastMbQp` *between* uses of them. Two of the fourteen (`WelsMdInterMbEnhancelayer`'s `_pMbCache`, `WelsMdInterMb`'s `_pUnused`) were already dead, one of them derived its own from `pSlice` on the line after the parameter it ignored, and `WelsCabacMbRef`'s `pCurMb` was never read at all. The underscore had been carrying that fact; the compiler carries it now.

**Two more findings came out of the same battery, and the third is why the correction stopped rather than iterating.** (i) With `SMbCache` reverted, the probe moved on to `WelsMdInterInit`: a `&mut SMB` forwarded into `pfFillInterNeighborCache`, whose callees walk `pCurMb.offset(-1)`. **E2d's classifier propagated neighbour-boundness through *named* callees and could not see this one, because the call goes through a function-pointer slot.** It was replaced by a type-driven checker — collect every named function and every slot field that still takes `*mut SMB`, then ask whether any converted function forwards a reference into one — which reads zero. (ii) Reverting that callee did **not** fix the call site: `&mut *pCurMb` passed where `*mut SMB` is expected **silently coerces back**, carrying the narrowed 208-byte provenance, and no lint sees it. So every reborrow spelling this session introduced was stripped and **re-derived by the compiler**: strip them all, rebuild, re-insert only where an E0308 says a reference parameter requires one. 37 SMB and 31 SCabacCtx reborrows came back; the stale ones did not, and by construction cannot. That is the class closed rather than the instances. **Verified**: the four encoder probes and three decoder probes run **7 passed / 0 failed** under Miri, 1216 s.

**`SMeRefinePointer` holds no pointer, and both of the brief's leads resolved the other way.** Five `*mut u8` turned out to be four fixed offsets into one `SMbCache` buffer — 0, 640, 1280, 1920, each plus `iStride` — and a fifth the builder never wrote. **`pHalfPixHV` is not dead**: its writers are the four arms of `MeRefineFracPixel`'s half-pixel `match`, each aiming it at whichever of H or V that arm reuses, so it is an *alias slot* and survives as one offset with the aliasing stated instead of implied. **`pQuarPixBest`/`pQuarPixTmp` ping-pong** through `mem::swap` once per candidate, so they are one selector over {1280, 1920} — session C's half-selector recipe, third application. **48 → 32 bytes, measured, newly pinned.** `InitMeRefinePointer` loses its `pMbCache` parameter entirely and its selector reset is now documented as load-bearing. The base comes down as one `pBufMe: *mut u8` derived once in `WelsMdInterMbRefinement`, **not** as a `&mut SMbCache`: there is none in scope in `MeRefineFracPixel`, and adding one would put a second live path into a cache the same call writes through `pMemPredInterMb`.

**SMVUnitXY was mostly casts, and one place where it was not.** 43 of the 65 sites were literally `&mut (*sMe16x8).sMvp as *mut SMVUnitXY` at a call site — rule (b), the cast drops. 19 parameters became `&mut SMVUnitXY`. Two were arrays rather than single elements and took the host instead: `st64_mv`'s cache row is `&mut [SMVUnitXY; 29]` and `SvcMdSCDMbEnc`'s candidate pair is `&[SMVUnitXY; 2]`. **The first of those mattered for a second reason.** Three `UpdateP*MotionInfo` bodies built a raw cursor (`pMvComp.sMotionVectorCache.as_mut_ptr()`) *and* handed the same row to `st64_mv` — two live paths to one array, the shape this session exists to remove. They index the row directly now, which also turns twelve unchecked stores at `kiCacheIdx + {0,1,3,6,7,9,12,15}` into bounds-checked ones against a 29-element array; the debug sweep runs those checks live and reads 353/353, so no covered configuration is near the edge, and a future one that is will panic rather than write past it.

**Step 0a closed F60's coverage hole in the referee, with a red-proof.** Preset `sl` — 12 rows, `st` single-threaded, 320x192, `SM_SIZELIMITED_SLICE`, three (qp, constraint) pairs × rc {−1, 2} × cabac {0, 1} — and **all twelve were measured entering `FrameBsRealloc`** with an `eprintln!` at its head, 1–2 times each over the 18-frame loop. The constraint floor is 401 (`ParamValidationExt` rejects ≤ MAX_MACROBLOCK_SIZE_IN_BYTE); rc 0/1/3 hold the frame budget low enough that the slice count never crosses 35 whatever the qp says, which is why the preset names its rc modes instead of looping RCMODES. **The three constraints must differ from each other, not just the three qps**: under rc=2 the rate controller picks the quantiser and the qp column is inert — the first draft's two C=401 rows were byte-identical, the same encode twice. **Red-proof**: with session D's re-aim loop deleted the preset reads **0/12** (eight byte divergences, two hard crashes at qp=10), restored **12/12**. Sweep totals **341 → 353** in both profiles, wired into `gates.sh`.

**Step 0b fenced the screen-content family** (D-scr-1). The port-added init guard was verified standing at `encoder_ext.rs:818` — it is what makes leaving the family unconverted sound — and 16 items are tagged `// SCREEN_CONTENT(dormant: Phase 10)`, comments only. One correction to the brief's list: `ref_list_mgr_svc.rs:1142` is described there as "the allocation branch" and is not one — it is the branch that gives screen content `bAdaptiveRefPicMarkingModeFlag` straight from `bEnableLongTermReference`. It is still the file's one screen-content-conditional site and it is the site tagged; `RequestScreenBlockFeatureStorage`/`Release…`, the actual allocation pair, live in `svc_motion_estimate.rs` and are deliberately **not** tagged, because the brief's list is the enumeration and its check is that the grep reads exactly it.

**One deviation from the brief's letter, and its reason.** Step 0b says of the screen-content family "convert none of it", and the done-test asks for it to be "byte-for-byte unchanged" apart from the tag. Thirteen of the sixteen tagged items are exactly that. **Three are not**: `WelsMotionEstimateSearchStatic`, `WelsMotionEstimateSearchScrolled` and `WelsDiamondCrossFeatureSearch` each changed **one line** — `pMe: *mut SWelsME` became `pMe: &mut SWelsME` — because they are installed into `PMotionSearchFunc` / `PSearchMethodFunc`, the same dispatch slots the *live* motion-search functions occupy, and a slot has one type. Leaving them raw would have meant leaving the slots raw, and `*mut SWelsME` would not read zero. Their bodies are byte-identical; the diff is one parameter's spelling in each. Recorded here rather than smoothed over: the brief anticipated tagged survivors in this family's counts, but not that the dispatch table would force the opposite.

**Step 5 discharged the SATD debt and parked both families a third time.** `encoder/sample.rs`'s seven kernels had never been measured — the park was a projection off SAD ("SATD is SAD plus a Hadamard butterfly") and the ledger said so as an open debt. Measured: **1.36x–4.05x**, raw against safe, interleaved, `assert_eq!` on every accumulator. **The projection was right in verdict and wrong in shape** — SATD's ratios are *flatter* than SAD's (1.4–1.8x at six of seven shapes against SAD's 1.3–5.7x), because a bigger body absorbs a fixed per-call cost better; 4x4 is the outlier at 4.05x, on the same W = 4 cliff the second verdict named. SAD re-measured on the same run at **1.30–5.68x** against 2026-08-10's 1.41–4.66x. **The ledger's untested slices-and-offsets lead is now tested and it fails**: with `PlaneCursor::new` hoisted out of the candidate loop, one pair of cursors per search and eight candidates rebased off it, the range is 1.42x–6.55x. **And the reason is the finding** — `PlaneCursor::advance` is `Self::new(self.buf, idx(…), self.stride)`, so it re-runs the constructor's two asserts: building the cursor once per search does not remove the per-candidate cost, it moves it from construction to rebasing at the same price. That is why SATD improves under amortisation and SAD does not. **Third park, dated, re-attempt point Phase 9** — and the thing to build there is a *kernel signature* (base slice plus integer offsets, one bounds check per block), not another call-site rearrangement; both attempts so far have re-arranged callers around a cursor whose per-anchor validation is the cost.

**Deletions, all compiler-enumerated.** Fifteen port-added null guards, each read against its C++ counterpart first and none of which has one: `WelsCabacEncodeInit`/`WelsCabacContextInitFromContexts`/`WelsCabacContextInit`'s `pCbCtx` clauses and `WelsCabacEncodePos`'s null-returns-0 (`set_mb_syn_cabac.cpp:94`, `:86`); `WelsMdP16x16`'s five-way and `WelsMdP8x8`'s four-way entry checks (`svc_base_layer_md.cpp` opens both at `&pSlice->sMbCacheInfo`); `WelsInitInterMDStruc`'s `pMd` clause and both macroblock loops' `pWelsMd` clauses; `UpdateP16x16MotionInfo` (`mv_pred.cpp:148`), `WelsInterMbEncode` (`svc_encode_slice.cpp:458`), `UpdateNonZeroCountCache` (`:57`), `WelsIMbChromaEncode` (`:469`), `WelsPMbChromaEncode` (`:493`), `UpdateQpForOverflow` (`:526`), and `PredMv`/`PredSkipMv`/`PredInter16x8Mv`/`PredInter8x16Mv` (`mv_pred.cpp:45`/`:132`/`:114`/`:91`). Sixteen parameters deleted. `Copy`/`Clone` dropped from `SWelsMD` — nothing copied the 4000-byte record by value, and the build is the authority (rule (h)). `MergeSub16Me` drops `unsafe` entirely; so do `WelsCabacEncodePos`, `InitMeRefinePointer`, `st64_mv` and `DeblockingBSMarginalMBAvcbase`.

**`extern "C"` stayed on all thirteen retyped slot types, and rule (e)'s clause is the reason it could.** The rule says a slot whose parameters become references drops `extern "C"` because `improper_ctypes_definitions` fires — but `SWelsMD`, `SMbCache` and `SMB` are all `repr(C)`, so `&mut T` is FFI-safe and the lint never fires. Verified by building `--all-targets` and grepping for the diagnostic. The rule holds for types that are not `repr(C)`; none of this session's are. **`#[no_mangle]` audit**: the only two `extern "C"` exports that gained a reference parameter are `WelsCabacContextInit` and `WelsCabacEncodeInit`, nothing outside the crate calls either, and both keep their attributes.

**Three things this session broke and fixed, recorded rather than hidden.** (1) The first `family` gate after step 2 read `ignored set is 21, must be 20` — a new doc comment used a ```` ```ignore ```` fence, which rustdoc compiles into an *ignored doctest* and libtest counts. Changed to ```` ```text ````. A documentation fence can move a pinned fixture set, and the gate was right to hold it. (2) The ratchet read `encoder/abi_guard.rs raw_ptr: 1 → 2` at step 3's gate — prose again, a raw pointer type spelled inline in the new pin's comment. Reworded. S16's floor, second sighting in three sessions. (3) The call-site fixer that inserts `&mut *` from the compiler's own spans hit two lines it could not fix (`pCurMb as *mut crate::encoder::svc_encode_slice::SMB`) and prefixed reborrows to them **40 deep**, because the error never cleared; the real fix was rule (b), the cast just drops. Caught by grep before the gate.

**The done-test grep, read honestly, after the correction.** `grep -rn '\*mut SWelsMD\b|\*mut SWelsME\b|\*mut SMVUnitXY\b|\*mut SMeRefinePointer\b|\*mut SCabacCtx\b|\*mut SMB\b|\*mut SMbCache\b'` over `src/encoder src/processing` reads **122 lines**, against 428 at the session's start — a 71% cut, where the brief asked for zero. **Five of the seven families are at zero.** The 122 are: **48 `*mut SMB`**, the four named survivor groups; **72 `*mut SMbCache`**, the arena, reverted for the reason above and now documented at its accessor block rather than merely counted; and **2 `*mut SMVUnitXY`**, both F-owned. Not one of the 122 is a parameter this session could have converted soundly, and the `SMbCache` 72 is the number a later session should attack by giving the kernels slices — not by retyping the arena.

**Rule (g) took its default, everywhere.** Not one `pSlice` or `pCurLayer`/`pCurDqLayer` parameter was converted — the rule's condition (the callee must not re-reach the same object through `pCtx` while the borrow lives) is not met on this path, and the aliasing bug it names is the exact one rule (c) was applied fourteen times to remove. The survivor list is the whole family: ~103 `*mut SSlice` and ~78 `*mut SDqLayer`, unchanged, for session G with the context.

**Metrics, encoder-side** (`src/encoder` + `src/processing`, the ratchet's own patterns), `d7af280d` → close: `raw_ptr` **2331 → 2015 (−316)**, `unsafe_fn` **684 → 679**, `unsafe_block` **283 unmoved**, `mem_zeroed` **21 unmoved**. Crate-wide `raw_ptr` **2924 → 2608 (−316)**, `unsafe_fn` 804 → 799. (Before the `SMbCache` correction these read −328 and 2003/2596; the twelve the retreat gave back are the arena's accessor and parameter sites.) **Sizes**: `SMeRefinePointer` **48 → 32**, newly pinned in `abi_guard.rs` — a shape record rather than an ABI one, because the struct is stack-local with one builder, which is exactly why it should exist. Everything else unmoved and still pinned: `SWelsME` 96, `SWelsMD` 4000, `SMB` 208, `SMbCache` 5600, `SCabacCtx` 504, `SMVUnitXY` 4.

**The span, measured twice** (7 pairs against `d7af280d` each time, `perf_baseline.md`) — once at `52814bb5` before the `exit` battery rejected E2c, once at `cee4b86d` after the correction reverted it. **Encode median is +0.00% in both**, and the final maximum is *lower* (+0.61% against +1.52%), min −1.32%, zero rows over 5%, against a fresh 3-pair null of median +0.16% / max +1.52%. The pair is itself a datapoint: a change that retypes ~290 parameters and a change that reverts 62 of them read the same on this instrument. **Decode's final median is +0.33%, just outside its own null band (−0.27…−0.08%), and that is a statement about the instrument rather than the tree** — this session changed no decoder code at all (`git diff -- src/decoder` is empty), so the reading is cross-binary drift by definition, and it is worth keeping as calibration: a 3-pair same-binary null understates the drift between two *different* binaries on rows this size. Cumulative encoder deficit ≈ **+10…+12%**, unmoved; the +25% tripwire unbreached by ~24 points. **S33 applies**: nothing is claimed in either direction, and no swap landed in step 5 so there is no ledger row to open.

**F3, measurements 71, 72 and 73** (S14, at adjudication, `phase0_findings.md`). 71 was `t=2`, the minority arm of the signature, after several `t=4` hits; 5/5 byte-identical on re-run. **72 did not clear on re-run — 4 identical, 1 differ — which is the escalation condition, and it was the second hit of the session**, so the alternation was owed twice over. HEAD against control `4caf198d` in one loop: **0/0 over 120 alternated encodes on an idle machine, then head 3 / control 1 over 800 under load**. Three readings kept: **load is the variable, not the tree** (120 idle encodes drew nothing; the same loop with six CPU burners drew four); **the control hits it too**, on a tree whose sweeps were green an hour earlier; and **3 against 1 is noise** (P ≈ 0.31 under the null with four events). Two of head's three hits were *short* rather than zero-length, the rarer arm of the output clause. 73 was acquitted against 72's alternation without running a second one, the diff touching no threading machinery. Session rate **3/2824 ≈ 1/940**, the quiet end of the documented band. Twenty-third alternation, forty-sixth acquittal.

**Gates.** `family` at every step's close, **both sweeps 353/353 in both profiles** at each. `exit` battery **`OVERALL: PASS` — 13 passed / 0 failed / 1 skipped** at `cee4b86d`: tests **487 / 481 / 20**, ratchet clean, census **58**, both sweeps **353/353** in both profiles, both benches bit-identical, Miri `--lib` **344 / 0** with **all four encoder probe names read out of the output** (F17/F18), differential targets **20 / 7 / 3**. The `--lib` step reads **1413.66 s** at 344 tests, against session D's 1432.01 s at the same count — unmoved, as it should be for a session that added no probe.

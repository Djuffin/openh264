...
**Metrics, encoder-side** (`src/encoder` + `src/processing`, the ratchet's own patterns — `raw_ptr` counts `*mut `/`*const ` occurrences, `unsafe_fn` line-anchored definitions), `085e2e41` → close: `raw_ptr` **2389 → 2331 (−58)**, `unsafe_block` **288 → 283**, `mem_zeroed` **22 → 21**, crate-wide `raw_ptr` **2980 → 2924**. **`unsafe_fn` is unmoved at 684 and that is the trade stated**: six new root-and-resolution accessors arrived, each `unsafe fn` because **its signature names raw pointers** (session Z's clause), against six deleted dead functions. **Sizes measured and re-pinned in the commit that moved each**: `SDqLayer` **512 → 504 → 480 → 496 → 528 → 560 → 576 → 640**, `SSliceCtx` **32 → 48**, `SSliceBufferInfo` **16 → 32**, `sWelsEncCtx` **97912 → 97904** with thirteen of the fifteen `assert_ctx_offset!` pins moving by 8 — the two that do not are the two that precede the deleted `ppMbListD`.

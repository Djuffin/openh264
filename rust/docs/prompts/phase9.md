...
| **B3** ◀ **next** — brief [`phase9_session_b3.md`](phase9_session_b3.md) | plane family, part 1c — what B2 measured and did not convert. The route is proven (T9.B22): resolve through `layer_enc_pic`/`layer_ref_pic`, anchor by macroblock coordinate, re-derive any raw **after** the safe borrow. **Order by F118**: the 20 remaining MC sites caller-by-caller with no table involved (14 in `WelsMdInterMbRefinement`, which binds `pDstLuma`/`pDstCb`/`pDstCr` at the top and holds them across the body — each needs the re-derive treatment), then the constant-index sad/satd sites direct to `sample_sad::<W,H>`, then the 13 runtime-selected `md_cost`/`me_cost` sites with a safe table and the 42 handoff signatures in one commit. The carrier's coordinates (`mb_x`/`mb_y`) are needed only by the six readers that have neither `SMB` nor a slice in scope. **Check S57 before each site**: `WelsMdBackgroundMbEnc` (3 MC) and `SvcMdSCDMbEnc` (3 MC) are not run by any sweep preset. **Two decisions landed for it (2026-08-23): D-dead-1** — delete `WelsMdP4x4`/`P8x4`/`P4x8` under S18 with upstream's `#if 0` quoted (F115; −6 reads, −4 tags); **D-cov-1** — delete the 22 `mc.rs` shims and take `common/mc.rs` to `deny` (F119; **corrected 2026-08-23 while scoping B3**: the harness has no C++ side — the raw-vs-safe equivalences were proven at `46053993` and deleted by the file's own doctrine, and the sole surviving mc test is a span-contract test *of the shims*, which dies with its subject; no rewrite exists to do) | 45 `pEncMb` / 23 `pRefMb`; 20 MC + 37 cost | — |

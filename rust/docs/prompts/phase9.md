...
| **B4** ◀ **next** — brief [`phase9_session_b4.md`](phase9_session_b4.md) | plane family, part 1d — **the referee, then the dark campaign**. Step 1: **D-ref-1** — the `bg` preset (`bEnableBackgroundDetection = true` in both drivers + ~8–16 rows in `sweep.sh`; a failing row is found parity work). Step 2: **D-dead-2** — F122's closure goes. Step 3 **(re-scoped while writing the brief — B3's "unblocks 17" was 8)**: the preset lights only the BGD half — `WelsMdBackgroundMbEnc`'s 5 sites convert (T9.B22's route) and F117's 3 copies light for C; the other 9 dark sites are **screen-content-gated** (`WelsInitSCDPskipFunc` requires `bScreenContent` in its conjunction, `encoder_context.rs:1607`) and are *retagged* `SCREEN_CONTENT(dormant)` — Phase 10's family, mis-filed; reclassification, not progress. The lit sites convert on T9.B22's route (the dark bodies hold the six live `McLuma_c`/`McChroma_c` callers, so `common/mc.rs` can reach `deny` here: D-cov-1's 22 dead shims + the span test + the assert-map go with it, F124's count). The transitional raw triple (T9.B25) retires only after session F's ME threading — `sad_common.rs` deny waits there. | 17 sites + the mc close | — |

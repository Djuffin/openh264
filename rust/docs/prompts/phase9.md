...
| **B4** ◀ **next** | plane family, part 1d — **the referee, then the dark campaign**. Step 1: **D-ref-1** — the `bg` preset (`bEnableBackgroundDetection = true` in both drivers + ~8–16 rows in `sweep.sh`; a failing row is found parity work). Step 2: **D-dead-2** — F122's closure goes. Step 3: the 17 newly-lit sites convert on T9.B22's route (the dark bodies hold the six live `McLuma_c`/`McChroma_c` callers, so `common/mc.rs` can reach `deny` here: D-cov-1's 22 dead shims + the span test + the assert-map go with it, F124's count). The transitional raw triple (T9.B25) retires only after session F's ME threading — `sad_common.rs` deny waits there. | 17 sites + the mc close | — |

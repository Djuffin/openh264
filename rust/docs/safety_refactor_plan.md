...
*The full scope and session plan are the charter, [`prompts/phase9.md`](prompts/phase9.md). In brief: the encoder is already `deny`-everywhere behind 695 `port-raw` + 61 `cursor` tagged allows; Phase 9 retires them by converting the raw cursors (coeff → planes → SMbCache → layer/pic) so the `*mut sWelsEncCtx` conversion (F66, blocked today) becomes legal and the 419 body-only `unsafe fn` fall out, then de-virtualizes the 22 dispatch survivors, finishes `common/`, and does the crate-root deny/`libc`/clippy/Miri-marathon cleanup with D4/D5 to the user. The kernels are already safe — the `unsafe` is shims wrapping them for raw callers, so `common/` retires with the encoder, not independently.*

...
- [ ] **5.1 Picture & DPB** — *closure computed and recorded, no code yet* (session A
      §5 of the log). Nothing embeds the decoder's `SPicture` by value, it has **no**
      `assert_size!` or offset pins, its fourth plane slot is dead (`iPlanes = 3`,
      nothing reads index 3), and `AllocPicture`/`FreePicture` balance on all eight
      allocations (F19's check, run and clean). **One S25 hazard to face first**:
      `SPicture::unref(&mut self)` hands `self as *mut SPicture` to `SetUnRef`, which
      re-borrows it `&mut`.

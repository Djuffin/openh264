...
| **Phase 5b RUNNING** | 2026-08-17, one session, Eugene's direction: *"I want next session to focus on making decoder safe… I see no reason why this can't be finished in one session."* Takes `src/decoder/` from **167 `#[allow(unsafe_code)]` items / 160 `unsafe {` blocks / 42 `unsafe fn`** (re-greped at open, at `6c3e7301`) to **four named FFI items and nothing else**: `api_alias` + `api_alias_mut` (`decoder_context.rs`) and `SPicture::data_ptr` + `data_ptr_ref` (`picture.rs`). Four of the five enumerated families convert here — `PPicture`/F42 (**this supersedes the Phase 8 option-1/2 revisit**), the parse tree, `sMCRefMember`, the zeroed shells — and the api boundary shrinks to the two accessors. Phase 6 waits until this lands. Brief: [`prompts/phase5b.md`](prompts/phase5b.md). |

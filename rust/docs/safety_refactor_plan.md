...
**Sessions.** A (Phase 8 session C continuing): the 21 + 3 rows, the gate and
allowlist, the census, F80's reachability, the F77 instrument — expected tally 155 →
≥179. B: listing strategies + parse-only → 188. C: downsample + denoise + `sp` preset
→ 191, then the census's missing list smallest first. D if needed: residue and the
close. *Exit gate: `gtest --check` green with an allowlist of exactly the Phase 10 rows
...
**Runs after Phase 8b and before Phase 9 (D-prio-1, 2026-08-21)**; nothing in it blocks or is blocked by Phases 7–9
...
```
P0 ──► P1 ──► P2 (kernels) ──► P4 (dispatch) ──► P5 (decoder pivot) ──► P8 (API) ──► P8b (parity) ──► P10 (screen) ──► P9 (safety)

...
| **Next** | **Phase 6 — the encoder structural rewrite, the plan's second pivot** (10–14 sessions), **sessions A and B spent 2026-08-18, sessions C, D and E spent 2026-08-19, sessions F, G, H and I spent 2026-08-20**; **session I spent and the phase did not close on F65 — the decision is made (D-exit-1, §7.4: the fifth category, `port-raw(<phase>)`), and session J executes it (brief: [`prompts/phase6_session_j.md`](prompts/phase6_session_j.md), steward, 2026-08-20): the 117 root-down conversions, the deny sweep with five-way tags over the ~780 survivors, the residue table, and the close on the one unscoped exit battery.** Session I ([`prompts/phase6_session_i.md`](prompts/phase6_session_i.md), `036e18c0`→`8ffdd064`) landed its first two steps in full: `pPSOVector` deleted (the context's one dead field — and **only one of the fifteen pinned offsets moved**, not all fifteen as the brief predicted, S36 again); **`pFuncList` becomes an owned `Box`** with a field-wise 63-member constructor replacing the zeroed one, which takes the encoder's allocator call sites **15 → 12** — the 4 Phase-7 hits and the 8 dormant screen-content ones are all that is left — and closes **F19 structurally**, because the table's drop glue now runs and the hand-written `pParametersetStrategy.take()` is deleted rather than converted; and the table's parameters re-spelled, **`*mut SWelsFuncPtrList` 57 → 28**. **The 28 are structural, not residue**: five function-pointer slot *types* name `*mut SWelsFuncPtrList` in their own signature, so the dispatch table is handed back into its own callees and **22 functions cannot take a reference to it** — retiring them is a Phase-4a-style de-virtualization, handed to Phase 9. **The phase did not close, and the reason is measured rather than a shortfall of effort — F65.** Step 3's deny sweep was run as an experiment and reverted: `#![deny(unsafe_code)]` on all 36 non-MT modules names **894 items** (612 `unsafe fn` declarations, 177 `unsafe` blocks, 92 `unsafe` method impls), and that is a **lower bound**. Of the 709 `unsafe fn` declarations in scope, **272 name `*mut sWelsEncCtx` but only 117 have it as their *only* raw parameter** — so step 2, executed perfectly, clears at most 117 of 612 and leaves ~780 allow items whose raw types (`*mut u8` 177, `SSlice` 105, `SDqLayer` 79, `SMbCache` 66, `SMB` 45) are **every one on this phase's own do-not-touch table, owned by Phases 7 and 9**. §7's condition 2 asks that every survivor be one of four lawful categories; several hundred are none of them, and tagging them would be false. **F65 states the two ways out — a fifth residue category naming an owning phase, or deferring condition 2 past Phases 7–10 — and picks neither: that is the steward's call, and it should be made with the numbers in front of it rather than discovered at a close that cannot happen.** Session I stopped at a whole-closure boundary (step 2 **not begun**, deliberately — a half-done root-down conversion is the exact UB shape rule 6 forbids) and left the phase open rather than weakening a condition to close it (hard rule 9). **The arc this exposes**: encoder-side `raw_ptr` **2669 → 1849 (−31%)** across Phase 6 while `unsafe_fn` went **710 → 709** — the phase has been taking raw pointers out of struct *fields*, and `deny(unsafe_code)` counts *signatures*. **Gates**: `full` with `MIRI_SCOPE=encoder`, 496/490/20, ratchet clean, census 58, both benches bit-identical, **Miri 252/0 with all four encoder probes named**; sweeps 368/369 both profiles across two batteries, **four F3 hits acquitted by measurement 86's alternation** (HEAD 6/48 vs control 3/48, interleaved run-for-run) — which also found F3's **fastest-reproducing configuration on record, ~1 in 11**, and handed Phase 7 the place to open its ablation. **No unscoped `exit` battery was run: that is the phase's handoff gate and the phase is not handing off.** Perf: one span, encode median **+0.00%**, decode **+0.18%**, both inside the null floor; cumulative ≈ **+15…+17%** against D-perf-4's **+25%** tripwire. **Handoffs to Phases 7/8/9/10 were written anyway** (§5 below), since they are verified facts about the tree and do not depend on the phase closing. **Session H landed all five steps** ([`prompts/phase6_session_h.md`](prompts/phase6_session_h.md)): **the members own.** Every row of the brief's table converted — `RequestMemorySvc` calls the allocator **once** (`DynamicSliceBs`, Phase 7's) and the free cascade is **six entries**, none of them this phase's; allocator hits across `src/encoder`+`src/processing` go **45 → 15** — fifteen allocations and fifteen frees became container drops — and `WelsRcFreeMemory`, `RcFreeLayerMemory`, `FreeRefList`, the `AllocCodingParam`/`FreeCodingParam` pair and the dead type `SWelsOut` were deleted rather than converted. **Two arenas, converted two different ways, and the difference is the finding**: `SStrideTables` keeps one owned `Vec` with byte offsets because two spatial layers can name one region, while `SWelsSvcRc` becomes five containers because none of its regions is ever aliased — the C++ allocates one block in both cases, and it is the *aliasing* that decides, not the allocation pattern. **F64's instrument caught a defect the session would otherwise have shipped**: the first `Vec` field made `mem::zeroed::<sWelsEncCtx>()` itself UB (the memset image of a `Vec` is a null `Unique`) and it kept compiling and passing — so the shell is `MaybeUninit` bytes now and the test has a third `OWNED` tier that reports its own shrunk reach. **T6.H7 found a latent leak**: the reference-list free loop read its bound from `iSpatialLayerNum` *at teardown*, so a re-init that lowered the layer count would have leaked every slot past it. **The `pSvcParam` verdict is "the context owns it"** — not the decoder's F41 shape; `WelsInitEncoderExt` takes the block from the context's own allocator and copies the caller's parameters into it, so Phase 8 inherits nothing there. `pFrameBs`'s **nineteen** cursor derivations were enumerated before the conversion and the zero-fill is recorded as its one deviation. **The allocator census reads 15, all attributed** (Phase 7 4, session I 3, Phase 10 8) — the last eight being the census's own finding: unreachable screen-content storage, now tagged `SCREEN_CONTENT(dormant)` rather than left to be re-derived. D-gate-2's `MIRI_SCOPE=encoder` knob exists and is measured — the scoped `exit` step reads **252 / 0 at 600.69 s** against session G's unscoped **349 / 0 at 1411.29 s**, all four encoder probes green by name. `exit` **12/1/1**, the one failure the F3 signature on `mt sm=3 t=2` (short stream, not zero — new shape, recorded), cleared **5/5** on the mandated retry as **measurement 85**; Miri, both benches, the debug sweep and every other step green. **The span breached its null (+1.31% encode median against +0.10%, 2 rows over 5%) and attribution cleared eight of the eleven members** — including the two that looked most expensive: H1's stride accessors run *per 4x4 block* and read +0.00%, H11 replaced the encoder's most-read field at *256 sites* and read −0.01%. **`Option<Box<T>>` accessors are free** (the null-pointer niche makes them an identity function on the loaded word) **and `Vec` accessors are not** (a length load and a branch where there was one pointer load) — *the container's shape decides the cost, not the number of call sites*, which is the opposite of the intuition that aimed the first hypothesis wrong. The cost is in {LTR, rate control, ref lists}, mechanism named (`WelsRcMbInitGom` runs per macroblock and opens with `ctx_rc_at`; `SWelsSvcRc` grew 360 → 440 bytes), **nothing reverted, Phase 9 owns the remedy with a measured target**. Cumulative encoder deficit ≈ **+15…+17%**, D-perf-4's +25% tripwire unbreached by ~24 points. **Session G landed all five steps**: `sWelsEncCtx::new()` exists and `mem::zeroed::<sWelsEncCtx>()` reads 0, proved against the shell it replaces by a two-tier instrument that is the session's finding — **F64**, *a field-wise constructor cannot be proved byte-equal to a memset image, only value-equal, and the difference is exactly the bytes the type does not define* (niche-`Option` payloads and `repr(C)` padding; Miri found it twice). `pCurDqLayer` is `Option<LayerIdx>` behind one accessor at all 152 field accesses, and three of its four writers got *shorter* because they already held the index. The parameter-set aliases are the ids `SDqIdc` already stores, `SLayerInfo`'s two SPS pointers are one `Option<LayerSps>` (the C++ spelled that tag as a null), and **six fields were deleted rather than converted** because neither tree ever read them. `au_set.rs`'s builders take references; `paraset_strategy.rs`'s `*mut *mut` out-parameters handed back the id they already returned. `rc.rs`'s 93 raw pointers are attributed line by line in its own header: all of them H's, I's, 4b's or Phase 10's. Span **+1.07%** encode median, 0 rows over 5%; sizes and all 15 pins re-measured in both profiles. Step 0 paid F63 forward — the decoder's `data_ptr` carried the trap and the S40 test red-proofed it**. Briefs: [`prompts/phase6.md`](prompts/phase6.md), [A](prompts/phase6_session_a.md), [B](prompts/phase6_session_b.md). **Session A built the encoder's Miri probe and it found ten defects on its way in** — F13's last production site, **F57** and **F58** both new, two escaping borrows and four ordering defects. **Session B took the encode probe live**: three settlements first (`SSlice.pSliceBsa`, `SWelsSliceBs.pBsBuffer`, `SWelsNalRaw.pRawData` — each a cache of something the holder already reaches, all three deleted; `WelsEncodeNal` typed), then the walk found **eight more reds**, all fixed, including **F59** (new, [`phase6_findings.md`](phase6_findings.md)): the IDCT-reconstruction shims built `&mut` and `&` over one span because the inter reconstruction is *in place*, and the shims' own doc named that caller as a witness for the opposite claim. The `--lib` Miri step now runs **both** encoder probes green and `--skip encoder_ext` is deleted (**F13 has no open site**). **F52 is closed** (six adjudicated by reading; the sweep skips declarations and self-tests its own finding's shape). **`c_void` 268 → 131** over `src/encoder src/processing src/common`, with every remaining code occurrence in one of three enumerated lines (allocator / C ABI → Phase 8 / MT → Phase 7): the `IWelsVP` vtable dissolved, the typed-at-both-ends casts deleted, and four dead dispatch families with them. The **`SPicture` settlement** is written (three owners, a fourteen-row alias table, S34 measured, two id types, F42's arm answered); 6.1's head is sized at ~184 sites and named for session F. **341/341 both profiles at every face** and `exit` **PASS (13/0/1)** with Miri `--lib` **341/0** carrying both encoder probes — no encoded byte moved across nineteen aliasing fixes. The `exit` battery also caught a **tenth** defect, introduced by this session's own typing (`InitPic` given `*const` where the C++ casts the const away and writes) — recorded, fixed, re-run. **Session C put the scratch inline** ([`prompts/phase6_session_c.md`](prompts/phase6_session_c.md)): `SMB`'s five outward pointers and `SMbCache`'s eleven are inline arrays of the structs that owned them (`pEncSad`, which aliases a picture, is session F's), so five context allocations, eight per-slice allocations, `AllocMbCacheAligned`/`FreeMbCache`/`AllocateSliceMBBuffer`, the two parity banks and `deblocking.rs`'s dead `TagMB` copy of `SMB` are all gone, and `ReallocateSliceList`'s error-path aliasing over copied scratch pointers closes with them. **Why inline and not `MbArray`**: both hosts are `WelsMallocz`'d blocks, so an inline POD array is valid at zero (S21) where a `Vec` field is not until 6.6. Sizes re-pinned from measurement — `SMB` 152 → **208**, `SMbCache` 576 → **5600**, `SSlice` 1520 → **6544**, `sWelsEncCtx` 97952 → **97912** with all fifteen `assert_ctx_offset!` pins. **The second encode probe** (CAVLC + the fine mode-decision family) found **two reds on its first execution**, both fixed, and exposed that the *byte* gate never varied complexity at all: both diffharness drivers gained the knob (default `LOW`, 341 unchanged) and ten configurations read byte-identical at `MEDIUM`/`HIGH`. **The session owed a span and it reads faster** — encode median −0.97% against a −2.99…+0.82% null, cumulative ≈ **+10…+12%**, tripwire unbreached, SoA fallback named not opened. Encoder-side `raw_ptr` 2446 → **2389**, `unsafe_fn` 686 → **684**. `exit` **PASS 13/0/1**, Miri `--lib` **342/0**; the battery's first two runs failed — on this session's own covering test and on a prose `*mut ` in that fix's comment plus **F3 measurement 67**, whose alternation was run (12 `mt` presets a side, one hit each side, HEAD not worse) and acquitted. Starting position re-measured at Phase 5b's exit. **Session D made the layer own** ([`prompts/phase6_session_d.md`](prompts/phase6_session_d.md)), all five faces, and the enabler is one argument used twice: `SDqLayer` is reached only through a pointer in the zeroed context (T3.6's `pOut` precedent), so it became **`Box`-built with a real constructor** and could then own what a `WelsMallocz`'d struct may not — and **the order was forced, not chosen**, because a `Vec` field in a zeroed block is UB at its first drop (S21) and `Option<LayerIdx>` has **no niche to borrow**, so its `None` cannot be inherited from a zero image (F56 from the other side). The layer owns **`MbArray<SMB>`** (with `ppMbListD` deleted — `InitMbListD` cut *one* flat block across the layers and stored the cuts twice, so neither field was a carrier), **`Vec<SliceIdx>`** for its slice list, two **`Vec<i32>`**, **`Vec<u16>`** for the macroblock map and **`Vec<SSlice>`** per bank; four malloc/copy/free triples and eleven allocations went with them, and **`ReallocateSliceList` is a `resize_with` that closes session C's handed-over `sSliceBs.pBs` double free** — a resize *moves*, so each `pBs` is held once at every point. **The dynamic-slice probe found four reds and the largest is byte-visible**: **F60**, `FrameBsRealloc` moving `pOut->sNalLen` and never re-aiming the layers at it — **three SIGBUS crashes and four byte divergences against the C++ before, all twelve configurations BYTE-IDENTICAL after** — on a path **no sweep configuration reaches** (`iMaxSliceNum` opens at `MAX_SLICES_NUM` = 35; the sweep's size-limited rows code at most nine slices). **F61 is recorded, not fixed**: the MT bank growth never re-stamps the slice list and **the C++ has the same shape**, so it is Phase 7's, and the position spelling removes the class's memory-safety half while leaving the synchronisation half. `pFeatureSearchPreparation` and five functions behind it are deleted (S18). Every raw pointer still handed out is **root-derived** (S28) through five accessors, with the Miri test S28 mandates. Encoder-side `raw_ptr` **2389 → 2331**, `unsafe_block` **288 → 283**, **`unsafe_fn` 684 unmoved** (six new root accessors whose signatures name raw pointers, against six deleted dead functions); `SDqLayer` **512 → 640** across six measured steps, `SSliceCtx` **32 → 48**, `SSliceBufferInfo` **16 → 32**, `sWelsEncCtx` **97912 → 97904**. **The span is flat** — 7 pairs, decode **−0.08%**, encode **+0.00%**, both inside their own 7-pair nulls with the encode median identical to the null's, worst row **+1.01%** against the +25% tripwire, cumulative ≈ **+10…+12%** unmoved. **F3 measurements 68–70**, all acquitted, with the alternation S14 owed: base **5 hits** against head **1** over 2880 configurations. `exit` **PASS 13/0/1 on its first run**, Miri `--lib` **344/0** with all four encoder probes; **Miri ran once at the close on the steward's in-session direction (S30)**. Drop-from-the-end taken at the authorised boundary only: the `*mut SMB` (128 sites) and `*mut SMbCache` parameter families go to **session E**. **Session E turned the records into references and wrote down the rule that stops it** ([`prompts/phase6_session_e.md`](prompts/phase6_session_e.md), all six steps, none dropped): `SCabacCtx` **25 → 0** crate-wide, `SWelsMD` **51 → 0** (with `Copy`/`Clone` dropped — nothing copied the 4000-byte record by value), `SWelsME` **39 → 0**, `SMeRefinePointer` **17 → 0**, `SQuarRefineParams` **1 → 0**, `SMVUnitXY` **65 → 2** (both F-owned), and **`SMbCache` 96 → 72** — fourteen deleted outright as caches of `pSlice` (rule (c)); **the retype of the other 62 was reverted, and that retreat is the session's most useful finding**. `SMbCache` is an *arena*: its consumers hold several raw cursors into different parts of it *across calls* (`pCurRS` into `sCoeffLevel`, `pBestPred` into `sMemPredMb`, `pBlock` into `sDct`), and a `&mut` covering all 5600 bytes invalidates every one of them — including the caller's. Miri's encode probe rejected it at `WelsEncRecUV`; an audit found 58 sites; `WelsEncRecUV(pFunc, pCurMb, pMbCache, pRes, iUV)` takes the arena *and* a cursor into it in one call, so no argument ordering works. **A reference is the wrong tool for an arena; deleting the second path is the right one**, which is what the fourteen deletions do. Two further findings came from the same battery: E2d's neighbour classifier propagated through *named* callees and missed `WelsMdInterInit`'s forward into the `pfFillInterNeighborCache` **slot** (replaced by a type-driven checker), and `&mut *pCurMb` passed where `*mut SMB` is expected **silently re-narrows** — so every reborrow spelling was stripped and re-derived from the compiler's own type errors, closing the class rather than the instances. **`SMeRefinePointer` holds no pointer**: five `*mut u8` were four fixed offsets into one `SMbCache` buffer plus an alias slot the builder never wrote, so it is `iStride` + `iHalfPixHV` + a `bQuarPixSwapped` selector replacing the `pQuarPixBest`/`pQuarPixTmp` `mem::swap`, **48 → 32 bytes, newly pinned**. **`*mut SMB` is 125 → 48 and the shortfall is a finding rather than a debt**: a macroblock parameter converts only if the function does no cursor arithmetic on it *and* does not re-reach the array through another parameter (F13's shape), so 68 converted and 48 are named in four groups — neighbour-walkers, array-owners, slot entry points, Phase 7's one. The brief's step-4 note about `deblocking.rs` was the rule for the whole family. **F60's coverage hole is closed in the referee**: preset `sl`, 12 rows, every one measured entering `FrameBsRealloc`, with a **red-proof** (session D's re-aim loop deleted → 0/12, restored → 12/12); sweep totals **341 → 353** in both profiles. Screen content fenced per D-scr-1 (16 items tagged, guard verified, nothing converted). **The SATD debt is discharged and both families take a third park**: SATD's first measurement **1.36–4.05x**, SAD **1.30–5.68x**, and the ledger's slices-and-offsets lead **tested and failing** — `PlaneCursor::advance` re-runs the constructor's asserts, so hoisting cursor construction moves the per-candidate cost instead of removing it; re-attempt named **Phase 9**, and it needs a kernel signature rather than another call-site rearrangement. Span flat: encode median **+0.00%**, decode **−0.24%**, both inside a fresh null. Encoder-side `raw_ptr` **2331 → 2015 (−316)**, `unsafe_fn` **684 → 679**. **F3 measurements 71–74**, with the alternation the second owed: **head 3 / control 1 over 800 encodes under load, 0/0 over 120 idle**.| **Session F took the pictures** ([`prompts/phase6_session_f.md`](prompts/phase6_session_f.md), all five steps, none dropped): **`*mut SPicture` 59 → 0** code sites, `pEncSad`/`pBuffer`/`*mut SMVUnitXY` **0** each, both pools owning (`SRefList::pRef` `Box`-built with a constructor, `CWelsPreProcess::m_pSpatialPicPool` flat across layers), the scaled input an `Option<Box<SPicture>>`, and all fourteen alias rows handles. **The two id types earned themselves twice in places the C++ cannot express**: `SDqLayer::pRefOri` holds a *reconstruction* picture on the camera path and a *source* picture on the screen path — one field, two pools, invisible when both are `SPicture*` — so it is `Option<PicRef>`, an enum over both, with the camera path's writes transcribed as the dead stores they are rather than narrowed away; and `AnalyzePictureComplexity` takes a source picture with a *reconstruction* reference, which the compiler produced as a type error. **F42's encoder arm is one line, and a grep for a type could not have found it**: `AnalyzeSpatialPic` asks picture identity as `pLastPic->pData[0] == pRefPic->pData[0]` — two *plane roots* — so session B's search for `SPicture*` comparisons was looking for the wrong noun. **The coverage check came before the conversion and found F62**: both differential drivers hard-coded `bEnableLongTermReference = false`, so `LTRMarkProcess`, `DeleteInvalidLTR`, `DeleteLTRFromLongList`, `HandleLTRMarkFeedback`, `WelsBuildRefList`'s long arm and **every `pLongRefList` shift** had no byte coverage in any preset in any profile — F60's shape exactly. Preset `ltr` closes it, and needed **two** knobs: without `ENCODER_LTR_MARKING_FEEDBACK` the mark is never confirmed and without `ENCODER_LTR_RECOVERY_REQUEST` `bReceivedT0LostFlag` is never set, so the four feedback values produce four *different* streams; sweeps **353 → 369** in both profiles. F62 itself — `ParamTranscode` transcribing `iLTRRefNum = (bEnableLongTermReference ? pCodingParam.iLTRRefNum : 0)` as `{ 0 } else { 0 }` — is masked by `WelsCheckNumRefSetting` and was **measured both ways** (16/16 identical with the defect in, 16/16 with it out). **`CMemoryAlign` allocates nothing for a picture or the VAA block any more** (four side arrays, three planes, seven VAA result arrays, in three steps), `ExpandReferencingPicture` has no caller and is deleted, and **nothing in the port rebuilds an allocation out of a mid-plane pointer** outside the two differential shims. **S28's mandated test is red-proofed**: `data_ptr` written the narrowing way fails under Miri, written from the allocation root it passes, and **369/369 either way** — no byte gate this project owns can see the difference. **The `exit` battery then failed on the same accessor for a *different* reason — F63, and it is the fourth consecutive session in which that battery caught a defect the session introduced**: the S28-correct spelling `plane.as_mut_slice().as_mut_ptr()` gets provenance right and **aliasing** wrong, because `&mut self.buf` is a `Unique` retag over the whole allocation and pops the pointer the previous call handed out — and the encoder asks twice a frame (`WelsInitCurrentLayer` stamps `pEncData`, `AnalyzePictureComplexity` re-derives the same picture, `WelsMdI16x16`'s SAD reads through the first cursor). `PaddedPlane::root_ptr` reads the address out of the `Vec` header instead, making repeated calls sibling `SharedReadWrite` derivations; three encode probes go from aborting to 3/0. **S28 answers provenance, not aliasing**, the two traps look identical at the call site, and **the decoder's `data_ptr` carries the same spelling and passes only because its callers never re-derive while a cursor lives**. **The size pins split by profile and the split is a decision**: `Option<PicId>` is 4 bytes in release and 8 in debug because `pool::Id` carries its generation counter under `debug_assertions`, and giving `Id` that field in both profiles would cost the *decoder's* per-macroblock `[[Option<PicId>; 16]; 2]` arrays their niche — a measured decoder cost to keep an assertion one line shorter. `SPicture` **136 → 344**, `SMbCache` **5600 → 5584**, `SDqLayer` **640 → 840/768**, `sWelsEncCtx` **97904 → 97888/97792**, `SVAAFrameInfo` **248 → 352**, `SRefList` newly pinned. **The span is the first session-scale encode cost this phase has recorded, and the brief's contingency was aimed at the wrong step**: +4.67% against a +0.00% null, attributed commit by commit — the LTR commit +0.58%, **the id flip +1.45%**, the planes **+0.24%**, VAA and expand +0.05% — so reverting the separable plane step would have fixed nothing, and **T6.F5** took it to **+2.72%** by stamping the reference and reconstruction plane views on the layer once a frame (`pEncData`/`pCsData`'s own idiom, with the handles still the truth). The residue's row pattern is the diagnosis: Mandelbrot +0.22…1.37%, flat mostly-skip content +3.04…6.67% — **a fixed per-macroblock overhead is invisible where a macroblock costs a lot and dominant where it costs nothing** — which names the ~10 surviving resolutions into the four per-macroblock side arrays as **Phase 9's**, where they want slices rather than addresses. Cumulative encoder deficit ≈ **+13…+15%**, tripwire unbreached by ~22 points. Encoder-side `raw_ptr` **2015 → 1888**, `unsafe_fn` 679 → 684, `unsafe_block` 283 → 288 (all five in the S28 test). **F3 measurements 75–78**, all acquitted, and **two of them refined the protocol**: 77 escalated when the retry rule did not clear (head **1/25** against **4/25** on the pre-face tree, with the failure's own account captured — `EncodeFrame failed at 1: 2`, F61's class) and recorded that *a tight re-run loop is not a quiet run*; 78 recorded that **the first retry loop was wrong, not the tree** — `set -- $cfg` in zsh, which does not word-split unquoted expansions.

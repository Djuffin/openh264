...
| **Phase 9 IN PROGRESS — reanalyzed and scoped 2026-08-22; session A's brief written** | The safety endgame, and the reanalysis **resets its size**. The encoder already carries `#![deny(unsafe_code)]` on all 39 files; it compiles only behind **695 `port-raw(Phase 9)` + 61 `cursor`** tagged allows, and Phase 9 retires them by making the code safe — the encoder's equivalent of Phase 5's decoder work. The 695 classified — **corrected by session A (F101): the charter's first table was produced by reading only the first line of each tagged item, so every wrapped signature read as having no raw parameter.** Whole-signature counts (`phase9_census.md`): **147 body-only** (*not 419* — go safe when their callees do), **234 `*mut sWelsEncCtx`** (*not 120*; 111 ctx-only, the F66 conversion, *blocked* until the cursors retire), **123 other** (*not 31*, and the charter's session table owned none of it), **64 `*mut u8` plane**, **55 layer/pic/slice**, **48 SMbCache/SMB**, **19 coeff** (14 after session A), **5 dispatch**. The order is forced (F66): a `&mut ctx` retag invalidates the whole 65-field context, so the raw cursors must go first, then the ctx conversion, then the body-only tail falls out. The kernels are **already safe**; the `unsafe` is shims wrapping them for raw callers — so `common/`'s 9 un-denied files retire *with* the encoder callers, not independently. **Estimate 10–14 sessions** (Phase 6's scale, not Phase 5's 29 — ownership is done, the vocabulary built, the hazards named). **Session A DONE** (`b0eea990`..): the census (`phase9_census.py` → `phase9_census.md`, the family map sessions B–J execute against) and **F101**; `q1c.py` **rebuilt from F66's spec** because it was never committed and is not in the session-J log (**F102**) — it now finds both of the sites Miri confirmed, which are two different shapes, and reads 287 sites in 68 callers; and the coefficient family converted, which turned out to be **5 kernels, not 25** (**F103**) — the other 14 are `SWelsFuncPtrList` slots whose call-throughs pass SMbCache walking cursors, so they move to family 3. Estimate revised **10–14 → 12–17 sessions**, and `other` (123 sites) needs sessions of its own. Inherits from 8b: F91, F92, F93, F100, F84, F86-open (referees green). Charter: [`prompts/phase9.md`](prompts/phase9.md); session A: [`prompts/phase9_session_a.md`](prompts/phase9_session_a.md). Phase 10 (screen content) follows. |

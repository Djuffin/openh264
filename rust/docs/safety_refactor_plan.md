...
| **Next** | **Phase 6 — the encoder structural rewrite, the plan's second pivot** (10–14 sessions), **session A spent 2026-08-18**; **session B is next** — brief written 2026-08-18: [`prompts/phase6_session_b.md`](prompts/phase6_session_b.md). Brief: [`prompts/phase6.md`](prompts/phase6.md), session [A](prompts/phase6_session_a.md). **Session A built the encoder's Miri probe and it found ten defects on its way in** — F13's last production site (a cross-module pair, closed as a 20-derivation family under S29), **F57** and **F58** both new ([`phase6_findings.md`](phase6_findings.md)), two escaping borrows and four ordering defects; all ten fixed, **341/341 both profiles**, no encoded byte moved. The encoder now carries one live Miri probe (initialisation, **+1.95%** of the Miri battery in Miri's own deterministic clock, ≈+20s wall); the encode-path probe is in the tree `#[cfg_attr(miri, ignore)]` and **unblocks with session B's `pBsBuffer` settlement**, which now has a Miri reproduction in front of it. **S32 transfers to the encoder** on picture size (flat to 1.3% across 64x the area; §7.6 amended), and **no decoder probe could be retired** — measured reach says each reaches code no other reaches. Starting position re-measured at Phase 5b's exit: `src/encoder/` **2534 `raw_ptr` / 671 `unsafe fn` over 32 files**, `src/common/` 237/72, `src/api/` 240/48, **0 deny-clean**. |
...
- **S32 — the Miri gate's cost is decoder instantiations, not decoded
  ...
  * **The frame-count axis is owed, not answered.** The encode loop is blocked
    under Miri by the `pBsBuffer` finding (6.4's), so only initialisation could be
    timed. The decoder's own numbers say frames are not free, so assume the
    encoder's per-frame cost is non-zero until 6.4 measures it.

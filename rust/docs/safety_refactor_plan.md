...
* **The gtest tally is the encoder's to-do list.** `tools/abi_harness/gtest_stretch.sh`
  reports **118/199** against the reference's 199/199, and **77 of the 81 failures are
  encoder-side API surface the diffharness cannot express**: `SetEncOptionSize` (21),
  `EncoderOutputTest.CompareOutput` (8), the multi-spatial-layer and simulcast family
  (`SimulcastAVC/SVC`, `Engine_SVC_Switch_*`, `ParameterSetStrategy_*`,
  `DiffSlicingInDlayer`), and ten `GetOption*`/`DecoderVclNal` rows. Of the other four,
  **three are `DecodeParser` (the slot is a stub)** and **one is the documented POC
  tiebreak**. Run it before and after; it is a number that moves.

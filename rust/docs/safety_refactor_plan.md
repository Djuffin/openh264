...
| | |
|---|---|
| **Phase 4b in progress** | 2026-08-11, session A. **§1's seam is closed, both halves**: T4b.1 made the four entropy slots one `EntropyCoder` (`08b7c29d`) and T4b.1b made `SWelsRcFunc`'s nine slots one `RCMode` (`3e583b9a`). `assert_size!(SWelsFuncPtrList)` **1272 → 1184** across the session is the tally of how much dispatch is gone (1280 in C++; 4a's `SSampleDealingFunc` shrink had already taken 8 — the assert's own comment is the running ledger). Remaining: **§2** the two strategy vtables, **§3** 4a's leftovers *(including `transmute`, still 23)*. Brief: [`prompts/phase4b.md`](prompts/phase4b.md) |

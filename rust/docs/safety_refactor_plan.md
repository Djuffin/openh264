...
| **Next** | **Phase 5 — the decoder structural rewrite, the plan's first pivot** (9–12 sessions), **in progress, sessions A–W. The phase stays open: exit condition 5 is discharged at V, conditions 1–3 and 6 remain, and W paid down 21% of condition 1.** **Session W (2026-08-15, 16 commits `b045bd42`…`e6579e3c`): decoder `raw_ptr` 974 → 767, files carrying `#![deny(unsafe_code)]` 6 → 9 (eight allowing nothing), corpus 2690/17 and 2707/0 unmoved, two `exit` batteries `OVERALL: PASS` 13/0/1.** Its lesson is the unit: **the pointer family, not the module** — the layer (`*mut DqLayerState` 115 → 54), `SWelsNeighAvail` (32 → 0), `SRefPic` (11 → 0), the MV pair, the intra-pred array and the nzc record each converted across every module at once, and the mechanism under all of them is that **a callee stops taking a raw pointer by taking what it actually reaches** (three context fields, one `bool`, one 24-byte record) — the view struct at small scale, which is why five families converted before it exists. What remains is three structural moves, each a face rather than a family: the **grid split** that `decode_slice.rs`'s 51 layer pointers stop on (the residual path passes a `&mut` into `grid.scaled_tcoeff` *and* the whole layer in one call — `PoolRest`'s `mut_and_rest` applied to the grid), the plane/dispatch conversion (step 3, `SHIM(` 52 → 1), and the context's view struct. **Exit condition 5 is DISCHARGED (session V).** The day two the escalation table asked for is paid: the D-gate-1 window reads **+3.38% CB / +2.65% decode median** at 7 pairs against session U's **+3.58% / +2.77%** — same sign, same row order, medians 0.12 points apart, every decode row above its own reading's null ceiling, encode flat in both. **The window is real and no longer provisional.** The separation is stated rather than implied: ≈10 hours, *same calendar day*, which is not the different day S2b's clause asks for and is the widest the closing order left; the verdict therefore rests on the two readings agreeing. This is the phase's **third** second-day reading and **the first that confirms** — the two that overturned (N's bisect halves, the niche asymmetry) were measured in pieces at the resolution limit, and the two that survived were measured whole. The niche's own second reading flips Main's sign (−0.72% → +0.33%), so it goes from *directionally consistent, unresolved* to **unresolved**, inside the floor on both days, and stays in the tree. **The stop-line is still breached — cumulative CB ≈ +25.0…+25.5% against ≈+23%, over by ≈2.0–2.5 points** (the +25% median tripwire is not) — and it now has a disposition: **D-perf-6** defers recovery to the Phase 9 perf pass, D-perf-4's own disposition, on Phase 2's precedent. Nothing parks because nothing is attributed; the line is **not** re-baselined to fit the result. **F50 closed, and it was one token.** `au_parser.cpp:947` rejects an unsupported `profile_idc` with `return false;` inside an `int32_t` function whose every other exit is an error code — `false` is 0 is `ERR_NONE`, so the C++ reports **success**, raises no error signal, and sets `bHasNewSps` for an SPS it never stored. The port had transliterated the arm's *intent* instead of its *value*. **Corpus codes 2683/24 → 2707/0**; output unchanged at 2690/17, all 17 the documented `CABA2_SVA_B` tie-break; the 24 golden rows moved **in the code column only** — no frame count, no dimension, no plane hash. The class was checked rather than assumed: all 21 other `return false;` sites under `codec/decoder/core/src/` are in functions that really return `bool`. **W6's done-test is the decoder's, not one file's — measured at the face, and it inverts the decomposition's order.** `decode_slice.rs` calls **49 distinct `unsafe fn`s across 10 other modules**; `#![deny(unsafe_code)]` fires on an `unsafe` block and calling an `unsafe fn` needs one, so the row's S20 closure is the transitive callee set: **16 modules, 439 `unsafe fn`, 977 raw-pointer occurrences** at V's open. The same fact blocks the decomposition's *first* step specifically — a view struct's `&mut` into a context field, held across any of those 49 calls that still take `pCtx`, is F24/F25/F28's retag shape, which the Miri probes exist to catch. S29's objection dies with the last `pCtx` parameter and that is a **decoder-wide** event. So the callees convert first, bottom-up, and the view struct is W6's **last** step; the named list of families with a size each is `phase5_session_w.md` §1. **Deny-clean decoder modules 3 → 6 of 22** (`dec_golomb`, `bit_stream`, `fmo`), and the first two by no conversion at all: a raw duplicate of the safe helper beside it deleted (S18's shape), an accessor moved to the module it belonged in, five vestigial `unsafe` blocks removed under a file-wide `unused_unsafe` that hid them. Decoder `raw_ptr` **974**. **F51** was found by `fmo.rs`'s conversion and fixed with a red-under-revert test: `ResetFmoList` reset the counter and not the list, so `UninitFmoList` had **no caller** where the C++ has one — F44's class, and it bites because `FmoParamSetsChanged`'s first term is the `!bActiveFlag` this reset was supposed to set. **Exit conditions 1–3 are unmet, so `phase6.md` stays deliberately unwritten** for the third session running and the next brief is a Phase 5 one. *Sessions A–U's record follows.* **Prior:** **in progress, sessions A–U. Session U was scoped as the last one and is not: the phase stays open, and the reason is a measured size and a breached stop-line.** **W8 is DONE — the twice-postponed perf adjudication landed whole** (`perf_baseline.md` §Phase 5 exit). Session N's stashed binaries got the day two they were owed two sessions ago: **the whole span confirms** (+0.87% against day one's +1.24%, every row above its own null's ceiling) **and the bisect does not** — Face 1's CB row flips sign, −0.72% → +1.31%, which is S2b's clause exactly, so those halves were never resolvable. **The niche hypothesis lost its evidence in the same run**: the CB-is-half asymmetry that pointed at `Option<PicId>` on B slices does not replicate, and the niche itself reads directionally consistent but unresolved (CB +0.15% — the control behaving, since CB has no B-frames — Main −0.72%, High −0.99%, inside the floor). Its one-pair reading was an artifact its own control convicted, at −6.84% on the stream the mechanism cannot touch. **The D-gate-1 window** — 69 unmeasured commits, N-tail through U — reads **+2.77% decode median / +3.58% CB at 7 pairs**, +2.63%/+3.87% at 3, every decode row above the 7-pair null's ceiling and encode flat: **real, and read twice**. **The stop-line is BREACHED: cumulative CB ≈ +25.2…+25.8% against ≈+23%**, over by ≈2.2–2.8 points; D-perf-4's +25% *median* tripwire is not breached and both are stated. The bisect of the window does not attribute it (the CB rows sum, the medians do not — session K's law), so **nothing can be parked, because nothing is attributed**. The escalation table is written with six options and measured sizes, and its recommendation is **option 2: day-two the window first**, because it is free, it is the only reading the verdict rests on, and this phase has twice had a second day overturn a first. Every reading is provisional; all six binaries are stashed so day two needs no build. **The 7-pair null is 2.56 points wide where the 3-pair one is 0.22** — session K's law arriving from the other side, and the reason every verdict here is judged against the 7-pair band. **Face 0 landed whole and closed the blindness session T named**: all **2707** corpus rows now have a C++ referee where 389 had none, via `ecref --stdin`, a harness corpus dump and a `compare_all.sh` that *checks* the dump instead of trusting it. It found three causes on its first run — **F48** (a buffer with no start code answered `dsErrorFree`: the C++'s `DetectStartCodePrefix` returns a *verdict* as well as an offset, and the transliteration inherited only the result the call site read) and **F49** (`IS_SPS_NAL` carried the subset-SPS, which `wels_common_defs.h:146` excludes and `:145`'s `IS_PARAM_SETS_NALS` includes — one caller on each side, the same gate) are **fixed**; **F50** is scoped with a reproduction (24 rows: the port's `ParseSps` returns `ERR_INFO_INVALID_PARAM` where the C++'s returns `ERR_NONE` on the same 8 bytes). Corpus: **output 2690/17, codes 2683/24**, and all 17 output rows are the documented `CABA2_SVA_B` tie-break — the 5 newly-refereed ones settled by per-frame multiset, one adjacent pair swapped. **The per-commit gate now compiles every target** (T5.U1), proved red under T5.T5's revert. **W7 is three items in**: the F40-class sweep is crate-wide and clean (0 suspects over 81 files) as an instrument that was itself corrected after it filed F40 into the wrong bucket, and **three decoder modules are deny-clean where the phase had none**. **W6 did not start, and U's re-derivation of its size is why**: the settlement said 80 `(*pCtx)` derefs over 44 functions touching 24 fields; the file holds **202 raw-pointer types over 55 `unsafe fn`**, and the lint fires on all of them — the view struct addresses 44 of 202. The settlement's own census was wrong when written (86 derefs over 30 fields at `a158183c`, not 80 over 24). The design does not move; the *face* does, and W6 is not one session. **Exit conditions 1–3 are unmet, so `phase6.md` stays deliberately unwritten** and the next brief is a Phase 5 one. *Sessions A–T's record follows.* **Prior:** **in progress, sessions A–T**. **Session T is the parity closure D-par-1 asked for, and it closed it.** **F46 is fixed** — the `DECODING_STATE` the API returns now matches the C++ on **every truncation row of every corpus stream: 2318 agree / 0 differ** (measured with `ecref`; 1142/1176 at the session's open). Its cause was not the one the brief hypothesised, and falsifying that took one command per side: both decoders recover **zero** macroblocks from the reproduction, and the port had already computed `0x24` — the C++'s answer — one line before it returned `0x4`. **`DECODING_STATE` is a bit set that the C header declares as an `enum`**, and a Rust `enum` cannot hold `dsBitstreamError | dsDataErrorConcealed`, so the port collapsed the accumulator to its first set bit; it is a `#[repr(transparent)]` newtype now (T5.T1). With the accumulator surviving the return, **what it contained became measurable for the first time, and five more sites were missing** (T5.T2): `UpdateAccessUnit`'s mosaic-avoidance block — so **`dsRefLost` had one producer against the C++'s four** — the clear of `bParamSetsLostFlag` (the port had taken the **non-`LONG_TERM_REF` side of the `#ifdef`**), subclause 8.2.5.2's frame-gap check, the `iPrevFrameNum` writer on the ordinary path, and the **four reserved zero bytes** written before every `ParseNalHeader`, which are both a zero-length NAL's header byte and F4/P6's guard bytes — without them a truncated SPS parsed successfully off the *previous* NAL's data. The last 22 rows were **two `if` arms in the wrong order** (T5.T3): the C++ tolerates exactly one `ParseVui` failure and propagates the rest, the port did the opposite, so it accepted a truncated SPS the C++ rejects. **The lesson worth carrying**: two of the five sites are *read* as well as reported (`dsRefLost`, at `mv_pred.cpp:1161` and `rec_mb.cpp:258`), so each moved plane hashes when added alone, and the two that repair them were found by the **output-column guard** rather than by reading — *a partial state machine reads as a permanent alarm*. **Face 1 is confirmed, not fixed**: `CABA2_SVA_B`'s 12 rows are the reordering tie-break `test_scalinglist_jm` documents, settled by hashing every emitted frame *individually* on both sides — identical multisets, one adjacent pair swapped, exactly one `uiDecodingTimeStamp` tie per row — and the annotation is **generated into the golden's header** so regeneration cannot drop it. **The corpus statement (face 2)**: thirteen tables, 2318 truncation rows, **codes 2318/0**, **output 2306/12** with those 12 being face 1's confirmed rows; conformance 60/60 asset and 16/16 e2e, goldens unmoved. **What it does not cover, stated rather than inferred**: the corpus's other **389 rows (14%)** — header corruption, emulation-prevention edges, synthetic tails, raw feeds — build their bytes inside the Rust harness, so `ecref` cannot replay them and they remain pinned against the port's own previous output, which is exactly the blindness F43–F46 lived behind; teaching `ecref` to take a byte blob is the instrument item in the hand-off. **Decoder `raw_ptr` 980, unmoved: T closed no W-item, by direction** (D-par-1 defers W6/W7) **and the progress metric does not measure parity work** — said plainly, per phase5.md's contract. New instrument: `examples/portref.rs`, `ecref`'s port-side counterpart, per-frame hashes included. *Session S's record follows.* **Session S did not close the phase, and the reason is a change of priority, not a blocker** (Eugene, 2026-08-14: *"if InitErrorCon is called in C++ code, it needs to be called in Rust code as well. we aim for test parity first, safety refactoring comes later"*). W6 and W7 **are** the safety refactoring — the per-slice view struct and `#![deny(unsafe_code)]` per module — so they stand deferred, and with them exit conditions 1–3, which is why the phase stays open. What S did instead is the parity work, and it found that **the port's error-concealment and FMO subsystems had never run at all**: **F43** (five stubs in `decoder_core.rs` shadowing the real `error_concealment.rs`/`fmo.rs` bodies), **F44** (`InitErrorCon` with no caller, so `bFreezeOutput` was never cleared and `sCopyFunc` was `None` — slice-copy concealment ran and *copied nothing*), and **F45** (`bInstantDecFlag` read but never written, so the port emitted a frame the C++ does not). F44 and F45 were **compensating**, which is why frame counts matched the C++ for the wrong reason and fixing either alone reads as a regression. Measured against the C++ decoder with the new `tools/ecref`, truncation rows agreeing went **641/541 → 1182/0** across six streams; ten of eleven corpus streams are now exact on every row. **F47**: the error-concealment probe fired on its first run and found real UB on the *ordinary* CAVLC path — six functions each taking their own `&mut` derivation of one bit-reader cursor, nesting three deep, both callers using theirs after the inner retag popped it — invisible for five phases because the two existing Miri probes decode a one-macroblock stream and a CABAC stream, so **no CAVLC residual was ever executed under the checker**. Fixing it made a second instance (`DecodeMbCavlcPcm`) a hard error, found by an all-I_PCM FMO asset built for an unrelated purpose. **F46 open**: with output now matching everywhere, the `DECODING_STATE` returns do not — ~45% of truncation rows, the port under-reporting — scoped to three causes and a 41-byte reproduction. **The instrument lesson, which is the phase's**: the malformed-parity table compares the port against *its own* previous output, so it can see change and never wrongness; every one of F43–F46 lived its whole life behind a green gate. Three tools keep it from recurring — `ecref` (the C++'s answer, replicating the harness exactly), `find_unwritten_fields.py` (F45's class made mechanical), `make_fmo_asset.py` (because **no stream in `res/` had more than one slice group**). Decoder `raw_ptr` **980**. Miri 4 probes, 4 green. *Session R's record follows.* **W3, W4 and W5 are CLOSED (session R)** — three W-items in one session, and the order was forced by the flip that preceded them: `pCurDqLayer` was a *cache* of `pDqLayersList`, and a stored derivation through an owning `Box` is invalidated by the next derivation, so the cache died first (85 sites, 48 functions, 8 files, threaded from three bracket tops) before the list could own. With `TagFmo`'s map and the parse-only descriptor behind it, **`WelsMallocz|WelsFree` in `src/decoder/` reads 0** and both allocator wrappers are gone. W4 deleted the packed-word idiom — `LD*`/`ST*` and the two byte-pointer block helpers, **219 uses become assignments of the values they were moving** — which closed **F35's second half** (no site asks 4-byte alignment of an align-2 `Vec` any more) and killed T5.N5's `debug_assert!`, because **F42 had disproved it**. W5 made the parameter sets ids (`SpsRef = (id, subset)`, since the C picks between two SPS buffers with the extension flag), resolved by five `addr_of_mut!` lookups that take **no borrow at all** — F41's question answered by construction rather than by reading 197 sites. W6 has two legs in (the NZC cache is `&mut [u8; 48]`; F31's zeroing is one operation) and **its done-test is blocked by a design question, not by conversion work**: `decode_slice.rs` holds 80 `(*pCtx)` dereferences over 44 functions taking a raw context pointer, `deny(unsafe_code)` forbids every one, and the reference form was removed deliberately at T5.G1 (S29). **No decoder module is deny-clean today.** New finding **F43**: `decoder_core.rs` declares stubs that *shadow* the real error-concealment and FMO implementations, so neither subsystem runs — and no instrument can see it, because name-matching tools resolve to the real body and the census compares text. Decoder `raw_ptr` **980** (1237 at session R's open). Five probes, five green (Miri `--lib` 336/0), run on detached worktrees so the main tree kept moving. *Session Q's record follows.* **W3's flip landed (session Q, `a8eaa3e5`)** and with it
...
| **Ratchet** | *(Phase 5 session U, 2026-08-15)* `unsafe_fn` **1245**, `raw_ptr` **4117**, `unsafe_block` **618**, `mem_zeroed` **30**, `SHIM(` **160**; decoder-only `raw_ptr` **980** (phase5.md's monotone exit metric, 1283 at the re-plan). **Unchanged again, and again that is the correct outcome rather than a stall to explain away**: U's code work was two one-line parity fixes, three `deny` attributes and instruments, none of which write a pointer type. What U *did* move is not on this row — 389 corpus rows gained a referee, two findings closed, and the phase's perf position is finally measured. **The ratchet also caught U's own comment**: a note added beside a new `#![deny(unsafe_code)]` contained the words `unsafe impl`, the metric is `grep -Eo 'unsafe impl'`, and three files whose code had not changed went red — S16's floor, arriving from the direction of a comment written to celebrate it. *(Prior: session T — identical figures; session R — `unsafe_fn` 1248, `raw_ptr` 4117, decoder-only 980; session P″ — 1257 / 4368 / 1229.)* |
| **Gates** | *(Phase 5 session T, 2026-08-14)* **482 debug / 476 release / 20 ignored**, Miri `--lib` **337/0** plus the three `exit`-level differential targets (**20 / 7 / 3**, all 0 failed), census **59**, decode conformance **60/60** (**58 asset rows** — 55 `asset_test!` + 3 `asset_test_concealed!`; the exit conditions' "goldens 57" is stale and is counted here rather than repeated) with **zero golden hash literals changed all session**, both benches bit-identical, sweeps **341/341 in both profiles**, ratchet clean. Close was **`OVERALL: PASS` at `exit`, every step green** — 12 passed / 0 failed / 1 skipped, the skip being the fuzz corpus that has never existed. **F3 drew zero hits.** **The session's own gate is parity**: against the C++ via `ecref`, the malformed corpus's 2318 truncation rows read **codes 2318 agree / 0 differ** (1142/1176 at the open) and **output 2306 agree / 12 differ**, the 12 being `CABA2_SVA_B`'s confirmed reordering tie-break. The corpus's **389 non-truncation rows have no C++ referee** — `ecref` replays a prefix length, not a byte blob — and that gap is handed to session U. **One `FAIL` is worth keeping rather than burying**: the first `exit` run failed on `decode_1080p_bench` **not compiling**, because T5.T1's public type change reached `benches/` and `cargo test` does not build benches — so the per-commit gate (D-gate-1: tests + ratchet + census) is structurally blind to it. Fixed at T5.T5; the rule earned is *`cargo build --all-targets` at the face whenever a public type changes*. *(Prior: session R — 476/470/20, Miri 336, `OVERALL: PASS` at `full`.)*
| **Standing rules** | **§7.6** below, now S1–S29. Phase 3+ briefs cite it rather than copying rules forward. **S20** (a struct conversion's decomposable unit is the signature-reachability closure, not one struct) and **S21** (the construction audit, and a duplicate-family finding must inventory every function per copy) were hoisted out of Phase 3 and are what Phases 5 and 6 are made of; **S22** (a repaired instrument has a backlog — run everything it covers at every level, the session it is repaired) is F17 → F18's lesson; **S23** (a cached table becomes a derived value only if the source cannot change behind the cache — a property of the *update paths*, read one field at a time) and **S23b** (an alternation that returns 0/0 has not run yet — alternate at the level the hits occur) are 4b session A's; **S24** (a count that decides a conversion's *shape* comes from a grep over the definitions, never from a doc or a brief) is 4b session B's, earned by two wrong premises in one brief; **S25** (a raw-to-borrow conversion surfaces pre-existing aliasing — the re-entrancy audit is part of the conversion, enumerated with the S20 closure) is session B's second, hoisted from its log §4, and Phases 5/6 inherit it by name; **S26** (a brief is an execution document — facts, order, gates, non-goals; cite rules by tag, no history retelling, no provenance narration) is Eugene's direction at Phase 5's open. **Section revised 2026-08-11** (Eugene's direction): R-letter tags moved to the preamble map, **S14 refreshed to the measured protocol** (briefs now cite it instead of restating F3), **S17 generalized** — its specific check is automated in `gates.sh` — and S3–S5/S10–S11 tagged **[dormant: kernel work]** until 6.3's re-attempt. Instrument: `rust/tools/perfpair.py`, which implements S1/S2/S17. **S14 gained one step-2 shortcut at Phase 5 session D**: before alternating, hash the sweep binary against the base's — if they match, the alternation is vacuous by construction and the hash is the acquittal (F3, measurements 27–28). Step 2 is unchanged for any session whose commits reach `rust_enc`. |
| **Open findings** | **The open set, each with an owner (Phase 5 session U's exit pass):** **F3** → **Phase 7** (the encoder MT race; protocol S14, rate ≈1/307 under sustained presets). **F23**, **F38-class**, **F41**, and the **`src/api/` instrument inventory** → **Phase 8** (the API rewire; S22's clause — `api/` is exempt from `deny(unsafe_code)` and that exemption silently propagated into every other instrument's scope, so all of them run over `api/` before anything is rewired). **F36** → **decoder threading, or deletion** (the port's MT slice loop is a partial translation; dead behind `GetThreadCount() == 0`, and 5.6's straggler sweep may close it by deleting the arm). **F37** → **Phase 8** (`DestroyPicBuff` omits the reordering-buffer reset, so an uninit/re-init cycle indexes a rebuilt pool with old indices; no gate reaches it — every decode gate creates, decodes and destroys once). **F50** → **Phase 6 or a parity session** (new at U: the port's `ParseSps` returns `ERR_INFO_INVALID_PARAM` where the C++'s returns `ERR_NONE` on the same 8 bytes; 24 corpus rows, one cause, reproduction in the finding). **F4, F6, F7** (S6-parity class) and **F8–F12, F14** plus **F13's two remaining sites** → per file, owners as recorded. **The `CABA2_SVA_B` tie-break annotation stands** — 17 rows, expected-divergent, generated into the golden header so regeneration cannot lose it; it is a recorded position, not a defect. **Closed at U: F48, F49.** *Prior wording follows.* **Open:** [`phase0_findings.md`](phase0_findings.md) **F1** (closed history, kept as the debug/release worked example), **F3** — the encoder MT race: **41 measurements, 14 alternations, 20 acquittals**; signature `mt sm=3 t∈{2,4}`, any wrong length, either profile, **either bench clip** — and that is the whole of it. Three clauses have fallen, each on a hit matching everything but the clause: `n=600` at session K, `cabac`/`rc` at session M, **the stream itself at session N**. All were rate artifacts of how much slice-list churn a configuration does, not conditions. Rate **≈1/307** under sustained back-to-back presets, ≈1/800 under a battery's interleaved sweeps; protocol is **S14**, which is current. [`phase1_findings.md`](phase1_findings.md) **F4, F6, F7** (S6-parity class, owners per file). [`phase2_findings.md`](phase2_findings.md) **F8–F12, F14**, plus **F13's two remaining sites** (`encoder_ext` — Phase 6; F12's pool — Phase 7). [`phase5_findings.md`](phase5_findings.md) **F37** (new at session N, owner 5.5 or Phase 8: `DestroyPicBuff` omits the `ResetReorderingPictureBuffers` call the C++ opens with, and the port's only call to it is at decoder *creation* — so an uninit/re-init cycle leaves `sPictInfoList` holding POCs and `iPicBuffIdx` values naming slots of a pool that has been freed and rebuilt, and the emit path indexes the new pool with the old index. Not a stale pointer — the port stores an index and T5.N1's `slot_at` bounds-checks it — a wrong-picture bug. No gate reaches it: every decode gate creates, decodes and destroys once), **F23** (owner Phase 8: the 19 `&mut self` vtable methods, borrow eight bytes wide, thunk writes at 0x20 — every Rust-side consumer hits it), **F36** (dormant, **widened at session L**: the port's MT slice loop is a *partial translation* — it is missing the `pSliceIdc` write, the `pNzc` copy into the picture, the `SetNonZeroCount` call, the per-MB deblocking call and the border-padding block, and the C++'s `pThreadCtx` re-point of the flag array has no port counterpart at all; owner is whoever ports decoder threading, before `GetThreadCount` can exceed 1 — 5.6's straggler sweep meets the dead arm and may close it by deletion). **Resolved and recorded in their files, not retold here: F2, F5, F13 (two of four sites), F15–F21, **F22** (closed at Phase 5 session M, T5.M4 — eight duplicate function pairs became one each, resolved per function against the C++'s own home and guard, zero bytes moved), F24–F35, F42–F45, F47, and **F46** (closed at Phase 5 session T, T5.T1–T5.T3 — one type, five unported error-signal sites and one inverted `if`, taking the malformed corpus's truncation rows from 1142/1176 to **2318 agree / 0 differ** against the C++ with no pixel moved).** The findings files are the histories; this row is the open set |

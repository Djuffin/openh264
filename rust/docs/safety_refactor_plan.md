...
| **Phase 8b in progress — session A DONE (`925ac828`..`aeb011db`), session B's brief written** | *Correctness and parity before safety (D-prio-1; Phase 10 now after Phase 9).* **The tally is a ratchet**: `gtest_stretch.sh --check` is gate 6c of `exit`, red-proved three ways, with a named allowlist (`tools/abi_harness/gtest_known_failures.txt`) — **165/199 at A's close, 34 rows, every row owned** (8b.C 17+1, 8b.B 8, Phase 10 7, D-poc-1 1). The number went 155 → **179** (T8b.A4) → **165** (T8b.A5): the S48 refusal of multi-layer/denoise took 14 rows from passing-for-the-wrong-reason to failing-at-init, and both readings are true — the exit gate stays at 7 + 1 rows, so C's downsample port returns all 17 at once. **Landed**: the decoder's option arms whole (the port had 4 get + 6 set of upstream's 16 + 9) with the two never-written feeders and the per-call reset block `welsDecoderExt.cpp:784–811` that had no counterpart — 21 rows, referee `ecref --options` + `decoder_options_parity_test.rs` (474 golden lines, measured red); `ForceCodingIDR` was a **stub** (3 rows, and behind it **F86**: the un-reset state overran `pShortRefList` and aborted the process — reachable only once `GetOption` stopped returning garbage, D-prio-1's thesis measured); S48 refusals for denoise and downsample; **the census** (`port_census.py`: 1351 C++ definitions, 1064 present, 287 missing → 153 dead / 92 renamed / **42 missing, all owned**: 8b.B 9, 8b.C 12, Phase 10 19, F80 2) and `find_stub_bodies.py` widened (the `plus` dirs, an empty-body rule that had been discarding empty bodies before any rule saw them — F85); **F80 is reachable** (asset made with the reference encoder, `tests/data/f80/`) and **worse than expected — F87**: the port answers `dsOutOfMemory` where the reference decodes all 48 frames; the F77 instrument (`MbGrid` `#[track_caller]`). **F88**: one row red once in seven runs on the Rust link, never on the C++ — a real concealment divergence under `test/api`'s time seed, deliberately not allowlisted; the gate gets a pinned seed (S49). Close: fast set only (D-gate-3) — commit gate PASS 523/518/20, `--check` rc 0, exports 7/7, harness 12/0, corpus 2902/17 · 2919/0 unchanged, one targeted `ltr` pair byte-identical. **B next** ([`prompts/phase8b_session_b.md`](prompts/phase8b_session_b.md)): the seed pin, parse-only (3 missing pieces under a stub thunk; 2 rows now), the listing strategies (6), F88 to a seed and a cause. Then **C**: downsample + denoise + the `sp` preset (17 rows), F80/F87 with `safe::Pool` growth. Charter: [`prompts/phase8b.md`](prompts/phase8b.md). |
...
**Sessions.** **A — done** (`925ac828`..`aeb011db`): the 21 + 3 rows, the gate and
allowlist, the census, F80's reachability (F87), the F77 instrument; 155 → 179 → 165
with the S48 refusal (both true; see §0). **B**: the seed pin (S49), parse-only's three
missing pieces under the stub thunk (2 rows now, `ParseOnly_General` after C), the
three listing strategies + `WriteSavcParaset_Listing` + the validation adjustments
(6), F88 to a seed and a cause → 173. **C**: downsample + denoise + the `sp` preset
(17 rows), F80/F87 as grow/shrink on `safe::Pool`, the census's 12, then the residue.
D if needed: the close. *Exit gate: `gtest --check` green with an allowlist of exactly the Phase 10 rows

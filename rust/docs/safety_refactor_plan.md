...
| **Phase 9 IN PROGRESS — reanalyzed and scoped 2026-08-22; session A's brief written** | The safety endgame, and the reanalysis **resets its size**. The encoder already carries `#![deny(unsafe_code)]` on all 39 files; it compiles only behind **695 `port-raw(Phase 9)` + 61 `cursor`** tagged allows, and Phase 9 retires them by making the code safe — the encoder's equivalent of Phase 5's decoder work. The 695 classified: **419 body-only `unsafe fn`** (go safe when their callees do, not converted directly), **120 `*mut sWelsEncCtx` params** (the F66 conversion, *blocked* until the cursors retire), **51 `*mut u8` plane**, **33 layer/pic/slice**, **31 other**, **25 coeff**, **15 SMbCache/SMB**. The order is forced (F66): a `&mut ctx` retag invalidates the whole 65-field context, so the raw cursors must go first, then the ctx conversion, then the body-only tail falls out. The kernels are **already safe**; the `unsafe` is shims wrapping them for raw callers — so `common/`'s 9 un-denied files retire *with* the encoder callers, not independently. **Estimate 10–14 sessions** (Phase 6's scale, not Phase 5's 29 — ownership is done, the vocabulary built, the hazards named). **Session A**: the census (`phase9_census.py` → the family map sessions B–J execute against), the `q1c.py` aliasing detector wired as family 6's precondition, and the **coefficient-only kernels** as the proof family (kernels already safe; callers pass local/inline `[i16; N]` arrays through a pointless raw round-trip — the cleanest possible first conversion). Inherits from 8b: F91, F92, F93, F100, F84, F86-open (referees green). Charter: [`prompts/phase9.md`](prompts/phase9.md); session A: [`prompts/phase9_session_a.md`](prompts/phase9_session_a.md). Phase 10 (screen content) follows. |

...
| | |
|---|---|
| **Phase 4b in progress** | 2026-08-11, sessions A-B. **§1 and §2 are closed and §3 is started at its highest-value item.** Session A: T4b.1 (`08b7c29d`) and T4b.1b (`3e583b9a`) made thirteen `Option<fn>` slots two enums. Session B: T4b.2a (`d6c78c1b`) and T4b.2b (`be67a754`) deleted both strategy vtables — 27 vtable entries, 5 static instances, 38 thunks — and T4b.3a (`33b1f0f3`) deleted the intra-pred constraint family. `assert_size!(SWelsFuncPtrList)` **1272 → 1184** and then stayed there, because `Option<Box<_>>` is pointer-sized: **the ratchet is this phase's instrument, not the size assert** — across the two sessions `raw_ptr` 5001 → **4834**, `unsafe_fn` 1286 → **1259**, and **`transmute` 23 → 5**, its first movement since Phase 0. Remaining: **§3 items 3-5** and the **phase exit**. Brief: [`prompts/phase4b.md`](prompts/phase4b.md) |
...
| **Next** | **Phase 4b session C — §3 items 3-5, then the phase exit, which is not compressible.** The exit is S18's straggler sweep, the full 3-pair perf protocol over the phase (entry `6e15c907`), bookkeeping, and **S19: write `prompts/phase5.md`**. If items 3-5 run long they stop at a seam boundary and the exit gets its own session. Order per D-seq-1: 4b → 5 → 6 → 7 → 8 → 9 |
...
| **Ratchet** | *(Phase 4b session B, 2026-08-11)* `unsafe_fn` **1259**, `raw_ptr` **4834**, `unsafe_block` **613**, `SHIM(` **157** (of which **`SHIM(phase3)` is 2**, both enumerated and both owned by Phase 6), `no_mangle` 24, `mem_zeroed` **32**, **`transmute` 5 — and this is the number to read this phase**. It had not moved since Phase 0; session B found that **two of the old 23 were prose** (so the real figure was 21 calls) and that all 19 in `decode_slice.rs` were one dispatch family, not the data puns the brief predicted. The crate now contains **two** `transmute` calls, both at `decoder_core.rs:893-894`, both owned by §3 item 4. Session B's deltas: `raw_ptr` **-164**, `unsafe_fn` **-39**, `unsafe_block` -3, `mem_zeroed` +4 (three prose, one *real* — T4b.2b discharged S21 with a **test** instead of a sentence, which is the better trade). *(Session A read: `unsafe_fn` 1298, `raw_ptr` 4998, `mem_zeroed` 28. Phase 3 exit read: 1286 / 5001 / 26.)* Baseline regenerated three times per S16 this session. Run `bash rust/tools/unsafe_ratchet.sh check`; never trust a remembered number |
| **Gates** | *(Phase 4b session B, 2026-08-11)* **435 debug / 429 release / 20 ignored** (+4/+4 across session B: six new tests less two merged). T3.0's **2316 malformed-stream golden rows over 12 files** are part of the battery and **none is `WITHHELD`** since T3.3 — it is the phase's permanent instrument and goes on gating Phases 5 and 8. Sweeps 341/341 both profiles. Miri **295 `--lib` tests**, same three skips (F12 thread pool, F13's `manage_dec_ref` and `encoder_ext`), plus the widened `exit`-level run over the differential integration tests. F3 per S14 — **nineteen measurements, seven alternations, seven acquittals**. **Session B's contribution is a zero**: four full batteries, eight 341-configuration sweeps, both profiles, **no hit** — ~2728 configurations against a measured ~1-in-800 rate, so P(0) ≈ 3%. It sharpens S23b rather than contradicting it: session A's hits came from sweeps run *back to back*, and a `gates.sh` battery interleaves its sweeps with builds and benches. **The load is part of the signature; escalations must run sweeps with nothing between them.** The session-start battery has now been clean two sessions running. **F17 is fixed and proven**: `gates.sh` takes every verdict from `${PIPESTATUS[0]}` plus log corroboration |
| **Standing rules** | **§7.6** below, now S1–S25. Phase 3+ briefs cite it rather than copying rules forward. **S20** (a struct conversion's decomposable unit is the signature-reachability closure, not one struct) and **S21** (the construction audit, and a duplicate-family finding must inventory every function per copy) were hoisted out of Phase 3 and are what Phases 5 and 6 are made of; **S22** (a repaired instrument has a backlog — run everything it covers at every level, the session it is repaired) is F17 → F18's lesson; **S23** (a cached table becomes a derived value only if the source cannot change behind the cache — a property of the *update paths*, read one field at a time) and **S23b** (an alternation that returns 0/0 has not run yet — alternate at the level the hits occur) are 4b session A's; **S24** (a count that decides a conversion's *shape* comes from a grep over the definitions, never from a doc or a brief) is 4b session B's, earned by two wrong premises in one brief; **S25** (a raw-to-borrow conversion surfaces pre-existing aliasing — the re-entrancy audit is part of the conversion, enumerated with the S20 closure) is session B's second, hoisted from its log §4, and Phases 5/6 inherit it by name. Instrument: `rust/tools/perfpair.py`, which implements S1/S2/S17 |
| **Open findings** | [`phase0_findings.md`](phase0_findings.md) F1, F3 (**F2 RESOLVED at T3.4** — one writer, zero bytes of output moved, and the finding's own inventory turned out to be one divergence short; F3 now has **nineteen** measurements and **seven alternations, seven acquittals**; measurements 16-18 are Phase 4b session A's — the ones that finally measured its *rate*, ~1 in 800 configurations under load, by alternating whole sweeps instead of single configurations — and the nineteenth is session B's **zero across eight sweeps** (P(0) ≈ 3% at the measured rate), which sharpened the signature: the load must be *sustained*, back-to-back sweeps, not a battery's interleaved ones), [`phase1_findings.md`](phase1_findings.md) F4, F6, F7 (**F5 RESOLVED at T3.4**, by deletion, exactly where its "who fixes it" line predicted), [`phase2_findings.md`](phase2_findings.md) F8–F14 (**F13's third site CLOSED at T3.4** — `InitBits` deleted rather than amended; three sites remain), [`phase3_findings.md`](phase3_findings.md) F15–F18, [`phase4b_findings.md`](phase4b_findings.md) **F19–F20, both FIXED at T4b.2a**: F19 is a leak of the parameter-set strategy on every encoder teardown (C++ deletes it at `encoder_ext.cpp:1995`; the port did not) that **no gate in this project could catch** — found by the ownership audit an `Option<Box<_>>` conversion forces, not by a test; F20 is three stale comments claiming one ported paraset strategy where there are two, which had propagated into a session brief and nearly into a conversion. **All four of Phase 3's own findings are closed**: **F15 FIXED at T3.3**, **F16 resolved at T3.1b with its second instance and the whole stored-extent class closed at T3.3** (nothing stores an extent any more, so it is unrepresentable rather than repaired), **F17 fixed and proven red** at session C's start (`eae61b94`), and **F18** — an address-comparison test that had been failing the Miri gate since *before* F17 made that gate capable of failing — found and fixed at the exit. **F18's lesson: a repaired gate has a backlog, and it surfaces only at the level and moment the repaired gate first runs**; the `exit` level runs once per phase. **F3's session-start advice, revised at 4b session B: a start-of-session hit is a tendency, not a rule — two sessions running have opened clean. Apply S14 to whatever arrives, and escalate at sweep level with nothing between the sweeps (S23b).** |
...
- [ ] **T4b.3 items 3-5** — `sBlockFunc`, the expand fn-pointer re-wraps (**which own
      the last two `transmute` calls**, `decoder_core.rs:893-894`), and the ~55-member
      CPU-dispatch filler, last and non-displacing.
- [ ] **Phase 4b exit** — whole and uncompressed per the brief's §4: S18 straggler
      sweep, the full 3-pair perf protocol over the phase (entry `6e15c907`),
      bookkeeping, and **S19: `prompts/phase5.md`**.

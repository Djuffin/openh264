...
| **Phase 7 COMPLETE** | 2026-08-21, sessions A–C (three sessions). **The encoder forks and joins with `std::thread::scope` over one documented seam, and nothing the phase was given to remove is left in `src/encoder`.** 3 files / 3,143 lines at open → **1 file / 1,741**; `unsafe impl` **11 → 1** (the seam, `send-seam(Phase 9)`); the `static mut` pool singleton, the `usize` spawn laundering, both C++-list ports and the whole task hierarchy **gone**; live allocator call sites in `src/encoder` **15 → 0** and `pMemAlign` deleted with them; mutexes **4 C++ sites → 1** (F69's); Miri `--lib` skips **1 → 0**; `deny(unsafe_code)` exemptions at a declaration **2 → 0** (**37 of 37** modules); `port-raw(Phase 7)` tags **89 → 0**; encoder+processing `raw_ptr` **1849 → 1773**, `unsafe_block` **313 → 283**. Sweeps **369/369 both profiles on every commit of the phase**. **F3 is CLOSED — and not by what the charter expected.** The charter fixed the ablation in advance as a test of the deleted pool machinery; session B ran **three arms with one variable each** and got before 25/3000 → **arm A (pool deleted, lock still missing) 20/2000, z = 0.61, p ≈ 0.54 — no change** → **arm B (one restored mutex) 0/2000, P ≈ 2e-9**. F3's cause is **F69**, a `WelsMutexLock` the raw translation dropped from around `AddSliceBoundary` + `++iSliceNumInFrame`. The machinery was worth deleting for five other reasons and **was not the defect**; a single after-arm on a tree carrying both changes would have credited it anyway. **One variable per arm is why the answer is usable**, and that belongs in every future ablation's brief. **The method that found F69 ran at phase scale in session C** as the dropped-sync census: all **16** C++ synchronisation sites in the encoder's MT paths classified against their port counterparts (10 guard deleted machinery, 6 guard shared state and every one has a named counterpart), plus the inverse list of every shared write the workers make, with Miri on the MT probe as that half's instrument. **No dropped statement remains in either direction**; the one surviving shared C++ race (`pGomCost`) is byte-neutral because it has **zero readers on either side**. **Session C also**: completed **F72** under **D-mt-2** (a default-on feature must not ship degenerate — one call at the C++'s own site, both fences retired, recorded as the project's **second expected-divergent class** after `CABA2_SVA_B`, structural coverage only since two *C++* runs differ); closed **F71** by reading its residue as a *scheduling* rather than an *ownership* question — the shared `bIdrFlag` write is loop-invariant across the fork, so hoisting it to the calling thread is byte-neutral by construction and the race is gone rather than serialised; closed **F61** by construction (the slice list has one reader and **no worker among its 21 call sites**); left **F36** fenced as `DECODER_MT(incomplete: F36)` with the list of absent reference statements at the site. **The thread ceiling was the pool's**: session B measured the port flat past two threads, and on `thread::scope` 1080p Mandelbrot `sm=1 n=4` goes **1.65x → 2.47x** at four threads — and where the port still stops the **reference stops with it**, so the charter's conditional pool rebuild is definitively **not wanted**. Span `b_close`→`c_close`, 7 pairs: encode 84 rows, **median −0.46%**, 0 over +5%. Cumulative encode ≈ **+15…+17%** vs the **+25%** tripwire. **Findings**: F3, F12, F61, F69, F70, F71, F72 closed; F68, F74, F75 fixed; F36 fenced; **F67 ruled (D-mt-1)**; **F73 open → Phase 9**. **Ten of the thirteen were found by reading the reference or by Miri and none by a byte-level gate** — the sweep proved 369/369 throughout and discovered nothing, which is what it is for. **Four are one shape** (F17, F68, F74, F75) and the phase states it as a rule: *an instrument's empty case is a case, and it is the one nobody tests* — three of the four were in the measuring apparatus, and **F75 disabled the unscoped Miri gate at the exact moment the project earned zero skips**. Briefs: [`prompts/phase7.md`](prompts/phase7.md) and its three session briefs |
| **Phase 8 COMPLETE** | 2026-08-21, sessions A–C (three sessions, the estimate). **The crate is a drop-in: it ships as `rlib + cdylib + staticlib`, the cdylib exports exactly upstream's seven symbols, and a C++ driver compiled against `codec/api/wels` `dlopen`s it and reproduces every in-process hash.** `src/api/` 2,788 lines at open → **4,690** (the growth is 406 lines of layout pins, 19 `# Safety` contracts and P13's guard, not code); `#![deny(unsafe_code)]` on **2 of 2** api modules with **44 allow items, 44 tags** (39 `C-ABI`, 5 `C-ABI(test)`) covering 131 spans — S22's seven-phase exemption is gone and the plan's "module-wide allow" is superseded by D-exit-1's regime. **Exports 7/7** with a gate (`tools/abi_exports.sh`), red-proved by exit code. **51 of the 53 named typedefs in the three public headers pinned** — 51 sizes, 51 alignments, 140 field offsets, every number from `tools/abi_sizes.c`'s committed dump, which is compiled as C *and* C++ and diffed. **The external-ABI harness** (`tools/abi_harness/`) runs four parts through `dlopen` in both profiles: conformance **58/58** bit-identical to the in-process goldens, encode loopback **14/14** byte-identical to `rust_enc`, the version pair and capability block against `codec_ver.h`/`welsDecoderExt.cpp`, and `Error_I_P.264` returning an error code with the process alive — `TALLY 10/0`, wired into `gates.sh exit` beside the export gate. **The gtest stretch is a number**: upstream's `test/api`, same objects, **cxx 199/199, rust 118/199 -> 155/199**: a first reading of that number tallied the failures by *fixture name* and called 77 of 81 "encoder-side", which was wrong — the fixtures are encode->decode round trips and the assertion that fires is usually the decoder's. Re-tallying by assertion location found **F82** (T8.C8) and the number moved the same day. Decoder output **50 of 51** throughout (the one failure is the documented POC tiebreak), `DecodeParser` 3 by construction, **44 left**. **Findings**: F23, F37, F41, F76, F78, F79 closed in A/B; **F77 FIXED** (and its diagnosis corrected — see below); **F81 new/fixed**; **F80 new, open → Phase 9**. **Session C (2026-08-21, `eb939c34..`, nine commits; `exit` unscoped, both new gates in it)**: **F77 was not an off-by-one.** `WelsDecodeSlice`'s bound is ported faithfully and the index that panicked was in range for the layer — `res/Error_I_P.264` **changes resolution three times** (352x288 → 640x480 → 352x288) and **this port had never handled a resolution change at all**: `SyncPictureResolutionExt` had only `WelsRequestMem`'s first-allocation case while `InitialDqLayersContext` re-sized the layer unconditionally, so the layer went to 1200 macroblocks with the pictures still at 396. The three fields the C's size test reads (`iImgWidthInPixel`, `iImgHeightInPixel`, `bHaveGotMemory`) were declared, reset, and never set. Fixed by porting `decoder.cpp:518–540` whole and refereed against `libopenh264.dylib`: five frames, five hashes, seventeen codes, seventeen buffer statuses, **all identical**; the asset joins the corpus (**+212 rows → 2902/17 output, 2919/0 codes**) and regenerating the goldens **rewrote no existing table**. **A correct index into a stale allocation looks exactly like an off-by-one**, and the port has no instrument that tells them apart — the cause fell out in one step from a `#[track_caller]` bounds assert plus printing both structures' dimensions. Owner of that instrument: Phase 9. **P13**: all 24 `extern "C"` entry points run inside `catch_unwind`, mapping a caught panic to the slot's failure code and a `WELS_LOG_ERROR` line; the profile verdict was read (`panic = "unwind"` in every profile this project builds) rather than assumed. Session B's throwaway reset-arm probe is a standing test — **63 × 4 × 2 = 504 whole-stream decodes**, union pinned at `0x36` both ways. **Three of the brief's premises did not survive a re-grep** and each was worth more than its step: the version functions already had `#[no_mangle]` (the export count was **24**, not 5 — fourteen `WelsSampleSad*_c` and three `WelsCabac*` were escaping, every one a name `libopenh264` also exports); the boundary types are **51 from three headers**, not 27 from two; and F77's cause was elsewhere. **F81** — `SDeliveryStatus` was **1 byte against the ABI's 12** — was found by the new pins on their first build, and could never have been found by any test that only talks to itself: the one field the option arm reads is at offset 0 in both. **D-api-1 executed**, and it found a second thing: `CWelsDecoder::CWelsDecoder()` calls `SetTraceLevel (WELS_LOG_ERROR)` (`welsDecoderExt.cpp:164`) and the encoder does not, so the port's decoder was a level more talkative than the reference. **Session B's stated reason for the divergence did not survive measurement** — the predicted flood is **4 lines in the whole release suite**, **0** from the corpus and **0** from the conformance. Five covering tests, three measured red, capturing stderr by re-executing the test binary. **The internal `SParserBsInfo` is `ParseOnlyBsBuffers`** (the boundary struct keeps its name, so not an ABI change; census **58 → 57**). Span 7 pairs + a 3-pair null, both benches: decode median **+0.04%** (null −0.12%), encode **+0.00%** (null −0.04%), **zero rows over 5%**. **The two sessions before it, as they were written at their own closes:** **Session B (2026-08-21, `51a0956a..e0f6c03d`, ten commits plus a three-commit close; **`exit` unscoped `OVERALL: PASS` 13 passed / 0 failed / 1 skipped** — 502/496/20 tests, sweeps 369/369 both profiles with no F3 hit, both benches bit-identical, Miri `--lib` **347/0** unscoped plus the differential targets 20/7/3) closed its whole brief and three findings — one inherited, two of its own.** **F76 is CLOSED** across three commits with six covering tests, each measured red against the tree that precedes its fix, and the corpus (**2707/0 codes**) and conformance (**60/60**) unmoved at every one: the two `DecoderConfigParam` statements and both `eEcActiveIdc` clamps (T8.B1), the live re-initialisation rebuild (T8.B2), and `welsDecoderExt.cpp:815–905` whole (T8.B3) — the two reset arms, the key-frame-loss notification, the trace throttle, `dsDataErrorConcealed` and the four EC statistics, `dDecTime`, `OutputStatisticsLog`, and the error-free tail's own frame count. **The range clamp could not be written where the reference writes it**: `decoder.cpp:654` clamps *after* the memcpy because in C the field is an `int`, and in this port `*pParam` is undefined for exactly the inputs the clamp exists to handle — *it assumed the property the clamp was there to establish* — so the block is sanitised as bytes at the boundary. `DECODER_OPTION_ERROR_CON_IDC` and `DECODER_OPTION_GET_STATISTICS` are wired at `GetOption`, which is what made any of it observable. **Two arms are ported but uncovered and that is measured**: `0x4000`/`0x40` appear in **zero of the 2707 golden rows**, and a sweep of all 62 `res/*.264` × 4 concealment modes × both bitstream declarations returns a state union of `0x36`. **F78 (new, closed in place)**: the encoder carried a **second, entirely dead C-ABI surface** — `G_ISVCENCODER_VTBL`, nine `ext_*` thunks duplicating the nine `encoder_*_c` bodies, and two factories upstream does not declare — reached through a `vptr` slot written once and never read; deleted (`raw_ptr −30`, `unsafe_fn −11`). Rule 6 found it: the charter's inventory counted `*_c` **in `src/api`**, and this set is neither. **F79 (new, closed at T8.B6)**: **the trace callback never fired, on either codec.** `WelsLog` was a stub in *two* places, the encoder had **zero** `WelsLog` call sites against the reference's 87, and the decoder's seventeen had no destination — `WelsDecoderDefaults`' log parameter was ignored and `CWelsDecoderImpl` had no trace object at all. `common/wels_trace.rs` is now the one declaration of `SLogContext`, `WelsLog`, `welsCodecTrace` and the six levels (census **60 → 58**), `m_pCodecInstance` is deleted as having no counterpart in `welsCodecTrace.h`, and **the reference's back-pointer could not be transliterated** — it would store a pointer into a `Box` field of a struct every entry point reaches through `&mut`, F38's class, on the logging path where nothing would observe it — so `SLogContext` carries the settings and the six option arms re-stamp the context's copy (24 → 32 bytes; the encoder context's offsets re-measured, as at T6.C1/T6.D5/T7.B4/T7.C6). **A stated divergence**: upstream's default sink is `welsStderrTrace` at `WELS_LOG_WARNING`; this port defaults to `None`, because the corpus alone would emit a line per damaged access unit across 2707 rows. **Ownership**: `m_pEncContext: Option<Box<sWelsEncCtx>>` with `WelsUninitEncoderExt` taking the context **by value** (its trailing `*ppCtx = NULL` is the type's now), `m_pWelsTrace: Box<_>`, `CWelsDecoderImpl::pCtx: Option<Box<_>>`; the two S42 roots are one expression each and take the **slot**, not `&mut self`, so a derivation cannot retag a sibling field. **19 thunks, 19 `# Safety` contracts**, each naming the window, and three naming what cannot be translated and why. **The safe cores are carved**: `Decoder` and `Encoder`, newtypes not re-exports, with the blob calls surviving as `unsafe fn …_raw`. **`Send` verdict: neither, and the inventory is measured** — 14 `E0277`s, 3 for `Decoder` and 11 for `Encoder`, every one a raw pointer in the `port-raw(Phase 9)` tree; recorded by an inherent-method probe that can report *both* answers, because a test that only compiles when the answer is yes is not a verdict. **The `c_void` line: 26 code occurrences, all C-ABI, each tagged**, plus 21 prose; the mutex handle is typed, six dead imports deleted. Crate ratchet `raw_ptr` **2277 → 2225**, `unsafe_block` **417 → 416**, `unsafe_fn` **809 → 802**; `wels_encoder_ext.rs` alone **136 → 85 / 14 → 8 / 27 → 16**. **New finding F77, OPEN, owner Phase 9**: `res/Error_I_P.264` **aborts the process** — a bitstream-derived macroblock index one past the end in `mb_grid.rs`, panicking across an `extern "C"` thunk; measured pre-existing at `51a0956a`, in no gate, and deliberately not fixed here because a behaviour change on an ungated path is what this session's rule 4 forbids taking in passing. `src/api/` — the C ABI boundary and the vtable object model. **Session A (2026-08-21, `82e1e54f..01926eb6`, eight commits) closed its whole brief and three of the phase's open findings.** **The api inventory (S22) is done and permanent**: all five instruments reach `src/api` — two by name, two by a *written and verified* whole-`src` scope claim in their own docstrings — and the first run found **six duplicate declarations of boundary types**, two of them *divergent*. Four unified (`SDecoderStatistics`, `SSysMEMBuffer` + a dead `SUsrData` whose `Default` disagreed about `iFormat`, `SVideoProperty`, and `VIDEO_BITSTREAM_TYPE` whose `#[default]` named a different variant on each side); two enumerated with owners — `SParserBsInfo` is **two entities under one name** (ABI raw pointers vs the decoder's owned `Vec`s), a rename, so **D5/Phase 9**, and `WelsTraceCallback` is **8.B**'s with the trace pair. **F23 is CLOSED on both codecs**: the conveniences are **19**, not 12, and all nineteen are associated functions taking `this` as a raw pointer — option (a), chosen because every caller holds an interface pointer and option (b) would have made all of them cast to an implementation type the public header does not name. **122 call sites** re-spelled, not 55: 109 in `tests`/`benches`/`examples` (`examples/portref.rs` was in no earlier tally, and five are dotted receivers), 6 in the probe, and **7 in `rust/tools/diffharness/rust_enc/main.rs` — a separate cargo project no source grep reaches and only the sweep builds**. The covering test was measured red under Miri (`SharedReadWrite retag at [0x0..0x8]` against a write at `0x20`) and is green. **F41 is CLOSED**: `CWelsDecoderImpl::param` had **no counterpart in the reference at all** — the C++ context owns its own block (`welsDecoderExt.cpp:426` allocates, `DecoderConfigParam` copies) — so the invented member is deleted, the context owns `pParam`, `SetOption` writes it where `welsDecoderExt.cpp:535` writes it, and `DecoderConfigParam` exists by name. Covering test: concealment switched off *after* `Initialize` must reach the decoder; red under a scratch-copy revert. **F37 was already fixed at T5.O1** (the brief's premise did not survive a re-grep — the reset is at `DestroyPicBuff`'s head where `decoder.cpp:260` has it); what was missing was the public-API probe, and `test_decoder_reinit_does_not_inherit_reordering_slots` now drives Initialize → decode → Uninitialize → Initialize → decode with pictures still buffered, red under a revert of T5.O1. **The twelve api-owned fields are the context's**: **10 `addr_of_mut!` stamps → 0**, **69 `api_alias`/`api_alias_mut` call sites → 0** and both accessors deleted, `m_pPicBuff` deleted as provably null (its only C++ writers are the threaded frame loop's), and the C's `pCtx ? pCtx->pPicBuff : m_pPicBuff` re-expressed as the one bit it carries (`bUsePool`). **`src/common/memory_align.rs` is DELETED** with `pMemAlign` and `CWelsDecoderImpl::align` — the crate no longer allocates through the C's allocator anywhere. `src/decoder/` is down to **two** `#[allow(unsafe_code)]` items, both `picture.rs`'s S28-mandated Miri instruments for `data_ptr`; no production item remains. Crate ratchet: `raw_ptr` **2334 → 2277**, `unsafe_block` **440 → 417**, `unsafe_fn` **814 → 809**; `api/codec_api.rs` rises by nineteen pointer receivers, which is what F23's fix *is*, regenerated three times per §7.2 P2. **Span: no measurable movement** — decode median −0.34% (null −0.18%), encode +0.00% (null band −1.25%…+1.27%), 7 pairs, both benches. **New finding F76** (owner **8.B**): `eVideoType` is write-only in the port, and the whole `welsDecoderExt.cpp:813–900` error-reporting block it belongs to — `ResetDecoder`, the key-frame-loss notification, the trace throttle, the four EC statistics counters — has no counterpart in `decoder_decode_frame2_c`; plus three `DecoderConfigParam` statements and the fact that a second `Initialize` on a live decoder rebuilds the context in the reference and does not here. **B next**: the encoder boundary — `m_pEncContext` (83 sites), the trace pair and `m_pCodecInstance` (3), the **19** thunks (not 18), the 8 `*mut sWelsEncCtx` boundary lines, 136 raw tokens in `wels_encoder_ext.rs`, and **41** `c_void` occurrences across `src/encoder` (12 in `wels_encoder_ext.rs`; the charter's 27 does not reproduce). Then C: the cdylib, the 7 exports, the external-ABI harness, the phase close. Briefs: [`prompts/phase8.md`](prompts/phase8.md) and its three session briefs. |
| **Phase 8b CLOSED (2026-08-22, `1b921d7f`)** | *Correctness and parity before safety (D-prio-1; Phase 10 after Phase 9).* **Exit: `gtest_stretch.sh --check` 191/199 at `SEED=20260822` with an allowlist of exactly 8 — the 7 Phase 10 screen-content rows and D-poc-1** — and 191/199 on all ten seeds with one distinct failure set. `gates.sh exit` **OVERALL PASS** unscoped: sweeps **535/535 in both profiles** (369 -> 535, `ps` and `dl` added and each measured green first), both benches bit-identical, Miri `--lib` whole library 375/0 plus the three differential tests, exports 7/7, harness 14/0, corpus 2902/17 and 2919/0. Span vs Phase 8's close: decode **+0.87%**, encode **+0.96%**, zero rows over +5%, against D-perf-4's +25% tripwire; a second span isolates session C at **+0.11% / +0.26%**, inside the null, so the phase's decode movement is A/B's (candidate: F76's per-call block, named not measured). **A** (155->179->165): the decoder option arms, `ForceCodingIDR` (F86), the census and its instruments, F80 reachable (F87), S48's two refusals. **B** (164->173; A's 165 was an unseeded sample, F89): the seed pin (S49), `DecodeParser` whole with four missing pieces, all five parameter-set strategies, `ecref --ec`/`--trace`/`ecref_rs`. **C** (173->**191**): `METHOD_DENOISE` and `METHOD_DOWNSAMPLE` ported (`processing/denoise.rs`, `processing/downsample.rs`, safe over slices) — seventeen rows in one commit; the `dl` preset; **F80/F87 closed** (`safe::Pool` grows and shrinks now, generation contract tested); **F96 closed** — `isNewFrame` was missing the `iThreadCount > 1` guard, so `InitRefPicList` was skipped on continuing frames — and **F88 closed with it**, 0/10 seeds including its own; F93 re-measured to four parse-only rows; F95 corrected; the census to zero open 8b rows. **The session's pivot was a measurement, not a reading**: the brief's headline `_c`-vs-NEON risk does not exist (40 kernels, all bit-identical) and the real trap was the aarch64 dispatch *table*, which rebinds general-ratio luma to the accurate downsampler — plus `m_bNoSampleBuffer`, which selects the opposite arm from the one it reads like, making 4:1 a cascade and not the quarter kernel (**F97, F98**, `tools/vp_kernel_probe/`). Findings **F97-F100**. Charter: [`prompts/phase8b.md`](prompts/phase8b.md); the close is in the log. **Next: Phase 9** (safety endgame), which inherits F91, F92, F93, F100, F84 and F86's open half, with every referee it was promised built and green. |
| **Phase 9 COMPLETE (2026-08-27, sessions A–J — twenty sessions)** | **THE ENCODER RUNS UNDER A `&mut` ROOT AND THE FORK RUNS UNDER MIRI: the first complete multi-threaded encodes under the aliasing checker in this project's history (T9.E8), the context flip at 152 of 155 single-threaded bodies (H), and every live UB the phase created caught by its own gates (F114 session D, F148.3 E2, F171 H) — never by a user, never in a bitstream.** **Scorecard, one instrument at both ends** (`unsafe_ratchet.sh report`, live; the end numbers are post-F178, which stopped counting comment text): `raw_ptr` **5390 → 1180**, `unsafe_fn` **1372 → 593**, `unsafe_block` **507 → 266**, `transmute` **23 → 0**, `shim` **42 → 0**, `mem_zeroed` **26 → 3**, `unsafe_impl` **11 → 2**. Of the phase's own span, `raw_ptr` fell **1587 → 1181** on the fixed counter (−157 of that is F178's prose correction, stated so it is not claimed as conversion). **Tags after D-exit-3** (`phase9_census.py`, and `phase9_disposition.md` for the per-item map): `fork-shared(S63)` **204** — the S63 theorem's lane, its soundness argument written once at the category's definition and refereed by the two fork/join Miri probes — `instrument(test)` 56, `C-ABI` 48 + `C-ABI(test)` 7, `SCREEN_CONTENT(dormant)` 33, `lawful-single` 3, `recon-seam` 2, `send-seam` 1, and **306 `port-raw`/`cursor` handed on as an owned queue**. **Firsts worth recording**: the fork entered under a `&mut` root (H, step 0a); `cargo clippy --all-targets` completed for the first time ever (J, F203); the crate root denies `unsafe_code` and found two files that had escaped the discipline for eight phases; the shipped cdylib has **no dependencies**. **The exit's honest gaps**: the Miri half of the exit battery was stopped by direction and F207 enumerates what that leaves unverified; the perf chain needs a fresh `perfpair` anchor before D-perf-6 can be costed (F206). **Exit conditions**: `prompts/phase9.md` §7, ticked item by item — 1 met as amended by D-exit-3, 2/3/5/6 met, 4 partly. **Decisions**: D-exit-3, D-fid-5, D-fid-6, D4 deferred post-queue, D5 declined, D6 not-now, D-perf-6 re-deferred. **Findings F199–F207.** *(Session history below.)* | The safety endgame: the encoder is `deny`-everywhere behind **521 `port-raw(Phase 9)` + 36 `cursor`** tagged allows — 557, `phase9_census.py`'s number, against 567 at G's close (695 at the charter, 687 at session D's start, 699 at B2's close, 659 at E2's close, 601 at F's; B3's D-dead-1 took the three `cursor` tags of the sub-8x8 trio), retired by making the code safe (the encoder's Phase-5). **Session A** built the census (`phase9_census.py` — and corrected the charter's family table, which was a first-line artefact: body-only is 147 not 419, F101), rebuilt the F66 detector (`q1c.py`, which had never existed — F102) with both hazard shapes, and converted the 5 directly-called coefficient kernels (the other 14 are table slots whose call-throughs pass SMbCache cursors — F103). **Session B** built the plane family's *caller* census (`phase9_plane_callers.py`: 109 sites — 64 safe-now, 32 blocked on the reconstruction write, 13 coefficient-gated, 0 unclassified) and **stopped the conversions it was briefed for because the census cancelled them (F104)**: every cost-table site pairs a plane operand with an SMbCache buffer, and a table has one type, so `pfSampleSad`/`Satd`/`4Sad`, `mc` and `dct` flip only after *both* the plane roots and family 3 — `sad_common.rs`/`mc.rs`/`sample.rs` reach `deny` there, not in the plane sessions. What did land: `expand_pic.rs`'s dead shims deleted (F106), 20 dead `mc.rs` kernels found (F105), `calc_psnr` over plane cursors (F109 — the first handle conversion was wrong: `pDecPic` is reassigned by `UpdateRefList` between capture and use, and **no gate covers the PSNR path**, both diffharness drivers set `bPsnr=false`), `cpu_core`/`expand_pic`/`wels_common_defs` under `deny`; ratchet −10/−12/−10/−6, tags unchanged (the deletions were untagged `common/` sites). **F107 — the reconstruction write, measured**: of four MT configurations only `SM_FIXEDSLCNUM_SLICE` with RC on is row-aligned; a worker reads only what it wrote; and *no* `&mut [u8]` can express the disjointness because a macroblock's contiguous span is the full width of its rows — `&mut SPicture` per worker (F73), per-MB `PlaneCursorMut`, and `split_at_mut` bands all fail. **F108**: deblocking runs *inside* the fork under MT (the brief said after the join). Rules S52/S53 and an S47 clause hoisted. **Order, re-read from the callers**: plane roots unblock nothing alone → **family 3 (SMbCache/SMB + the 14 coefficient slots) next (session D)** → the tables flip one commit each → the reconstruction consumers under D-mt-3 (session C). **Session D landed the arena whole, in ten commits, byte-identical at every one** (535/535 both profiles, five times). It fixed the detector's 21 held-cursor hazards *first*, with the pointers still raw: ten were modelling artefacts (the "retagging callee" was one of the arena's own accessors, which became field projections) and the eleven real ones were removed by **narrowing the callee to the field it touches** (`&mut SMVComponentUnit`, `&[i8; 48]`) so no whole-arena retag happens — not by restructuring a caller. Then it aimed the detector at the tree it was gating and found **two blind spots (F111)**: `addr_of_mut!` — the spelling S28/S29 *mandate* — was not a derivation to it, and a root minted from an owned field was invisible; 22 more `SMbCache` sites and **20 context sites**, so family 6's precondition is **308 sites / 71 callers**, not 287/68. Those 22 fixed, the ten raw accessors deleted (six spelled at the site, four `const fn` half-selectors), and **all 41 `*mut SMbCache` parameters are `&mut SMbCache`** (`grep ': \*mut SMbCache' src/encoder` = 0; `q1c.py --type SMbCache --kind ref` = 0 of 37 bodies). The 32 *roots* stay `addr_of_mut!((*pSlice).sMbCacheInfo)` by design (**F112**: a `&mut` root to the owned field is popped by any nested callee re-deriving it — T6.C2's Miri verdict generalised; each of the 40 call sites reborrows `&mut *pMbCache` for exactly the call; the roots convert with `*mut SSlice`, which the detector prices at 68/22). **F103's coefficient family is closed, 14 of 14** slots over exact arrays (**F113**: one raw type served two spans — `[i16; 16]` and `[i16; 64]` — and the odd lengths 49/241 are the kernels' reach; a "shared with the decoder" claim was a directory name, one grep disproved it). 6 of 42 `*mut SMB` converted; the other 31 read `pCurMb.offset(-1)` and belong to the layer. **F110 — the brief's `WelsEncRecUV` claim was false**: the two callers pass different bases (`+0` intra, `+256` inter — `svc_encode_slice.cpp:475`/`:499`); the parameter survives as a `usize` index (S54, the steward's miss). Ratchet raw_ptr 2249→2087, unsafe_fn 812→760, unsafe_block 445→398, shim 111→91; no rebaseline. Rules **S54/S55**, clauses on **S37** (the arena takes `&mut` at every parameter once its hazards are zero — make them zero first) and **S52** (a slot's union may be two types). **The double gate now has one gate left**: `pfSampleSad`/`Satd`/`4Sad`, `mc` and the encoder's `pfDct*`/`pfIDct*` wait on the plane roots alone → **session B2 next** (source/reference roots + the table flips), then **C** (the reconstruction consumers under D-mt-3, two MT Miri probes), then **E** (layer: D's 32 roots and 31 SMB parameters convert there, one step each). **D closed at `0bfc7687` (15 commits) after T9.D11 — F114**: the first encoder-scoped Miri run over the finished session found **two UB the byte gates had passed** (535/535 twice, both benches bit-identical, 1089 tests): (a) a raw derived by `addr_of_mut!` from the now-`&mut` parent is popped by the safe whole-field borrow a flipped slot takes (`blk4x4_mut(&mut (*pMbCache).sCoeffLevel, k)` — four bodies, one report); (b) the `&SMB` arguments T9.D9 introduced are *protected* for the call while a nested callee re-borrows the same macroblock mutably. Both fixed by removing pointers (indexed DC writes, `fill(0)`, an `i32` parameter), not by restoring them. Close: `MIRI_SCOPE=encoder gates.sh session` **PASS** — 535/535 both profiles, 548/541, **Miri 274/0** — D-gate-4's first green run, and F112's roots argument is now Miri-tested rather than reasoned. Ratchet raw_ptr −164. Rules **S56** and an **S29 clause**; `q1c.py` models neither shape yet (C: a raw popped by a safe borrow; D: a protected reference argument) — B2's detector step. **Session B2 (2026-08-23, three commits, closed at `7917aa0b`)** repaired both instruments and converted the first picture operands. **F116**: `phase9_plane_callers.py` had drifted 40 sites into `?` and 13 coeff into 0 after session D's accessor rename, and **exited 0** — the brief scoped against those numbers; the regenerated tables are byte-identical to session B's, so the document had been right for two sessions and only the tool had rotted (**S58**: put the loudness in the exit code). `q1c.py` gained **F114's shapes C and D**, calibrated against `0bfc7687^` to report F114a's four bodies and F114b's two *exactly* — three corrections were needed to get there, each of which had made the scan silently wrong (the body index is the opening brace not the `fn` head, so shape D missed `AddSliceBoundary` itself; "takes `&mut T`" is not "mints `&mut T`"; the argument must match the parameter *position*). `SDqLayer` gains `pEncPic`/`pSrcPool`/`layer_enc_pic` — the ten context-free consumers' route to the spatial pool — and `WelsMdPSkipEnc`'s three motion compensations take cursors. **Three of the brief's structural claims were wrong. F117**: trap (a)'s copy runs *previous → current* (`PCopyFunc` is `(pDst,..,pSrc,..)`), so the destination is the very picture `pEncMb` reads (probe: `pEncPic == GetCurrentOrigFrame`, 18/18 frames) — harder than scoped, and **unreachable by any gate** because the harness sets `bEnableBackgroundDetection = false`, whence **S57**. **F118**: steps 2 and 4 cannot be sequential — `PlaneCursor` has no `as_ptr` and `root_ptr` is `&mut self`, so operand and slot move in one commit; but MC needs no slot and the cost tables are provably constant after `WelsInitSampleSadFunc`, so constant-index sites may call `sample_sad::<W,H>` directly, leaving only 13 runtime-selected sites forced together. **F119**: the 22 "dead" `mc.rs` shims have a caller — `tests/kernels_differential_phase2.rs` drives 13 of them — which the brief's greps scoped out. **F115** (`WelsMdP4x4`/`P8x4`/`P4x8`, dead in the port and `#if 0` upstream) recorded undone: the user's call. Close: `MIRI_SCOPE=encoder gates.sh session` **PASS**, 535/535 both profiles. Tags unchanged at 638 (one accessor added, one deleted). **Session B3 (2026-08-23, eight commits `d4e930d4`..`15b20607`, every one byte-identical)** ran the campaign B2 measured. **D-dead-1 executed** (the sub-8x8 trio; −3 tags, all `cursor`). **F118 answered by carrying both shapes**: the three cost slots are safe (`fn(&PlaneCursor, &PlaneCursor) -> i32`) *beside* a transitional raw triple filled by the same `WelsInitSampleSadFunc`, so the campaign has a compiling middle and each commit converts one function; two ABI pins re-pinned with the reason and retiring with the triple. Converted: `WelsMdPSkipEnc`+`AcceptPskip`, `WelsMdInterMbRefinement` (twelve chroma MCs + three SADs), `MeRefineFracPixel`/`MeRefineQuarPixel` — **`md.rs` leaves the plane census** — and the four intra cost sites against `SPicData`'s new `iMbX`/`iMbY` carrier coordinates. **The session's real product is subtraction**: of 25 remaining `safe-now` plane sites **17 are dark**, 7 wait on the ME struct's three raw fields and 1 is a preprocessing SAD, so what is left is mostly a *referee*, not work. **F120** the census had never counted `md.rs`'s ten MC sites (30, not 20 — a missing entry point is silent where an unclassified operand is loud, S58's clause); **F121** `CalUVSadCost`+`JudgeStaticSkip`/`JudgeScrollSkip` are dark (probe 0 vs a calibration probe at 300–2136), so step 1 item 3 is refused under S57 **and B2's two `planes()` retags are live as code, unreachable as behaviour** (S57's clause); **F122** three of `WelsMdInterMbRefinement`'s seven arms are unreachable — D-dead-1 took the producers and left the consumers (S18's clause: deletion is a closure question); **F123** `q1c.py`'s shape C matched `&` as well as `&mut` and over-reported, narrowed and re-calibrated against `0bfc7687^` (S55's second edge); **F124** the brief's D-cov-1 correction is right about the C++ and wrong about the count — two mc tests survive, and `mc.rs` cannot reach `deny` while the six dark `McLuma_c`/`McChroma_c` sites stand. **`q1c.py` also caught a real defect no byte gate could**: `&mut SMbCache` beside a raw derived from that arena is shape B at all seven `MeRefineFracPixel` sites; fixed by passing a `usize` offset (S54). Seven planted faults calibrate the seven new conversion shapes. Close `MIRI_SCOPE=encoder gates.sh session` PASS, 535/535 both profiles. Ratchet raw_ptr −181; `SPicData` source/reference reads 68 → 50. **C and E next.** **Session B4 (closed at `6233e202`, six byte-identical commits, session gate 583/583 both profiles + Miri 273/0)**: the `bg` preset is the background-detection paths' first byte referee ever (`bEnableBackgroundDetection` was pinned false in both drivers since the harness existed, while `FillDefault` ships it on) — 48 rows, calibrated by entry probes (72–80 analyzer calls, 1159–5771 MD entries, 0/0 on the dark control) and by teeth (a planted sample failed 32 of 48 rows; **the 16 that passed it at +128 are F126** — on the Static clip the family executes 4524 times per row and is byte-inert, so the MD referee is 32 rows and S59 is the rule). Parity: **48/48 both profiles, unplanted, first run** — the never-compared path was right. `WelsMdBackgroundMbEnc`'s five sites converted on the T9.B22 route; the SCD eight reclassified `SCREEN_CONTENT(dormant)` (F125/F128 — the queue shrinks by 8 with no conversion behind it, and reports must not double-count); D-dead-2 −280 lines with `SWelsMD` re-pinned 4000→928 (F129: the closure was two items larger than F122 listed); D-cov-1 closed — 26 shims, three tests, three dead typedefs go, `SMcFunc` holds the safe kernels, **`common/mc.rs` denies with three dormant allows** (F124's corrected count plus `shim_wh`). Ratchet raw_ptr −245 / shim −46 / unsafe_fn −81; tags 688 = 631 + 57. The sweep is **583 rows** per profile now (F127: `gates.sh` had said 505 and the prose 535 — both wrong). Inheritance: C gets F117 lit and refereed on all three clips **and its constraint measured** — the source pool cannot become a shared `&SrcPicPool` across the loop while that write is raw; the conversion moves the write, not borrows around it. F gets the 7 ME sites, the transitional triple, `sad_common.rs`'s deny. Phase 10's dormant set is 19. **Session C (closed at `bde85857`, ten byte-identical commits; close gate 583/583 both profiles, Miri encoder scope 277/0 in 1012 s — +34 s on B4, of which the four seam probes are 1.84 s measured standalone)**: opened by running the ignored fork/join probe under Miri *before designing anything* (now S60), and the probe's verdict reshaped it — the first race was on the **pool** (`pic_mut` retags from two threads), not a pixel. **F131**: the seam had to be a *picture* view — three planes **and four per-macroblock side arrays** (`sMvList`/`pRefMbQp`/`pMbSkipSad`/`uiRefMbType`), where a `&mut Vec<T>` is a whole-array retag however narrow the write; ten of the eighteen `layer_dec_pic*` lines reached those arrays, nine tenths of the work the brief filed under "list in the report". **F132, the headline**: the fork contends over **six** shared-state families — recon picture (C, fixed), `SStrideTables` and `sSliceEncCtx` read-only-taken-`&mut` (4/6's, fixed here), `pGomCost` (X's, made atomic here — **F133: a real data race in upstream**, write-only in both codebases), `&mut SMB` vs deblocking's cross-slice read (E's, open), `pOverallMbMap` rewritten in-fork (open) — so **F107's acceptance was mis-assigned**: probe-passing is a whole-fork property, and C met what a family session can meet (the seam's own two threaded probes, calibrated both ways). `encoder/rec_view.rs`: one `UnsafeCell` accessor + one `unsafe impl Sync`, both `recon-seam`; placement `SDqLayer.pRecView`; `layer_dec_pic`/`_mut` and `SPicData.pDecMb` **gone** (18 → 0 `&mut` routes in-frame; **F134**: `pDecMb` *was* `pCsMb` under a second name, proved by a family-long `debug_assert` and deleted, `SMbCache` 5600→5568). `WelsRecPskip` converted through `copy_block_to_view` (planted fault fails 150/210 st rows); the mid-row probe landed with its boundary asserted from the bitstream; **F135**: 3 of its 6 census rows belong to a callerless twin. Tags 686 = 629 + 57 with `recon-seam` +2 (a lawful category, not a conversion); blocked 32 → 29. Pending the user: F133's deletion, F135's deletion, and the acceptance re-scope (steward's ruling: `pOverallMbMap` atomics → C2 step 0; the SMB/deblocking family and the two encoder fork probes' un-ignore → **session E's acceptance**). **Session C2 (nine commits `e1cb5ad9`..`f1e1e0f8`, every one byte-identical — 583/583 both profiles six times — but no green session Miri: F140)**: every in-scope site converted — blocked census **29 → 3** (the dormant SCD trio; F135's twin deleted on the ruling, 3 rows as a deletion; **F137**: the census had never known `pfIDctI16x16Dc`, so a sixth `WelsEncRecI16x16Y` site was invisible), the intra-pred family safe **tables and all** (three arrays split by span — F113's shape — flipped rather than bypassed because the mode index is runtime), `common/intra_pred_common.rs` **and** `encoder/get_intra_predictor.rs` at `deny` with zero allows, deblocking's encoder half on the seam (`deblocking.rs` 21 → 13 allows, every survivor on a raw-parameter function — **E's exit criterion now**), **F136**: fork round 6 was *two* fields (`iSliceNumInFrame` beside the map — the whole-struct retag at `svc_encode_slice.rs:1982` reaches both; caught by the addendum's expected-next-verdict check, now an S60 clause), **F139**: the session left three *write-only dispatch tables* (`pfIDct*` x3, 8 of 10 deblocking slots) beside `pGomCost` — converting consumers to direct calls silently demotes slots and nothing notices; the phase exit gains a per-slot READ grep. Ratchet raw_ptr −110 / unsafe_fn −44 / unsafe_block −58 / shim −28 (and 19 sites moved at ratchet-zero — the census is the seam's instrument, the ratchet is the floor); tags **580 + 57**. Eight planted faults, one per shape, 16–210 rows each — including the honest correction that `WelsMdBackgroundMbEnc`'s copy-referee is **16** rows, not F126's 32, because fault *placement* moves the covered-row count. **Aliasing evidence at session scope is missing since `bdf425fd`** — one clean Miri run after F140's ruling is a precondition for building on C/C2. **Session E (closed at `a6291dbd`, ten commits, every gate green)**: the acceptance met — `fork_join_encodes_a_multi_slice_frame` in 3356 s and the mid-row probe in 3449 s, zero reports, ignores deleted (T9.E8). The gate was rebuilt twice under the user's own rulings (D-gate-6: 15-minute cap, met by the parallel sharded Miri lane, close at **517 s**; D-gate-7: per-probe invocations, the CABAC grid probe ignored under Miri with its axes covered more deeply by the size-limited and fork probes). **Round 5 closed by a different mechanism than briefed — F142**: the briefed cross-thread `debug_assert_eq!(record, map)` *is* the race it interrogates, and its value equality is legitimately false cross-partition mid-frame; what is provable is **outcome equality** — race-free asserts at every `uiSliceIdc` *stamp* (a planted +1 aborted 210/210 rows and both fork probes at mb 0) plus a mod-N residue argument — and T9.E4 then pointed deblocking's eight guards at the atomic map, so **no foreign `SMB` record is read in the fork at all** (which also structurally closes the uiMbType/QP/MV hedge: MT validation rewrites `idc=0` to 2). **F143**: F132's "complete" six-round list was a *frontier* — rounds 7–9 were masked behind round 5 (a per-slice dispatch-table stamp hoisted per F71, four `as_mut_ptr` autoref mints re-spelled on `addr_of!`, one array-autoref mint). Step 3 landed whole: `q1c --type SSlice` **68 → 0** with the pointers still raw (three callee groups narrowed, fourteen callers given per-use-cluster derivation windows, the dispatch crossing placed by hand); session ratchet raw_ptr −111 / unsafe_fn −44 / unsafe_block −58 / shim −28, one reasoned rebaseline. **F144 → the steward's recalibration ruling**: eleven of the layer family's fourteen "hazards" are the detector counting a held *bool* as a held cursor — the narrowing lands as E2's step 0, F123's precedent (re-fired against the calibration tree). E2 inherits the 122-parameter flip with its precondition met and live referees. **Session E2 (closed 2026-08-24, nine commits `e818f36f`..close, every family gate 583/583 both profiles)**: step 0's narrowing calibrated (S60 expected-vs-actual on the layer family: 3 shape-B expected, 3 actual, exact); **the slice flip landed whole** — all NINE slice-carrying typedefs (the brief said six, F145) and 82 bodies root-down in six stages, zero `*mut SSlice` parameters left (the three fork-worker mints stay), the 41 arena roots now plain field borrows, `q1c --kind ref` all four shapes 0 over 80 bodies; **the layer campaign split under its own edge-2 rule (F146)** — a fork-reachability closure keeps 43 definitions raw (the in-fork surface + accessors, G–H's) and flips the 30 single-threaded, after 3 shape-B hoists raw-first plus 4 more found by hand where shape B is blind at accessor-minted roots (F148.2); **F147** names the popped-sibling-argument class (five S54 narrowings); **the close's single Miri run caught the one read the static discipline missed** (F148.3: a foreign READ under a fresh `&mut SDqLayer` protector — shape D models mints, not reads), fixed by reading through the parameter, both probes re-run green, session gate re-run PASS (282/0, S61 553 s vs 517 s = 1.07), the MIRI_FULL fork pair green. Ratchet raw_ptr −142, all else flat; tags 659 unchanged (flips retype; the harvest retires). Steps 3–4 dropped at the frontier per the brief's order with split reports delivered (deblocking's 13 allows: 6 SMB-grid, 2 in-fork layer, 1 ctx, 1 func-list, 2 tables, 1 test; `sRefPicView` 22 mentions; `WelsGetFirstMbOfSlice` dead, S18). F145–F148. **Session E2 (closed at `c480b12f`, ten commits; ratchet raw_ptr −142 in-session, then rebaselined)**: step 0's expected-vs-actual *exact*; the slice flip root-down in six stages on session J's model, detector 0 in all four shapes at every stage, shape D's transient 3→6→8 frontier adjudicated and collapsing to 0 as predicted; only the three fork-worker mints remain raw — the permanent shape. **F146, the structural finding: the brief's step 2 contradicted its own edge-2 rule** — the layer is the one object all N workers share, 42 of 71 param bodies fork-reachable, and no hazard count makes N simultaneous protectors on one allocation lawful; the sound campaign is the **fork-reachability split** (S63), 30 ST bodies flipped, 43 in-fork kept raw with their two end states (interior mutability in `rec_view`'s shape, or lawful raw) enumerated for G–H. **The close vindicated the once-at-end directive**: the first gate run FAILED at `WelsUpdateSliceHeaderSyntax` reading `(*current_layer(pCtx)).iMaxSliceNum` under its own fresh `&mut SDqLayer` protector — a foreign READ pops a protected Unique, and **shape D models only mints** (F148); fixed by T9.D11's read-through-the-parameter move, confirmed on exactly the two failed probes, then OVERALL PASS 282/0, S61 ratio **1.07**, and the `MIRI_FULL` fork pair green at 3294.86/3343.20 s — the flip is Miri-cost-neutral on the fork. F145 (nine typedefs, not six — the wrap artefact in a brief citing F101 against itself), F147 (four same-call hazards at accessor-minted roots, by hand), F148 (the detector's three gaps; the read-blindness is the close-catch's class). Process: two merged stages from edits-while-a-gate-ran, self-reported, re-gated quiescent — §7.5's rule now. **Session F (closed at `9977ac10`, seven code commits `487a2cf0`..`29951b85`; every claim re-measured by the steward at HEAD)**: **the dispatch family is closed — `: *mut SWelsFuncPtrList` 24 → 0**, and the crate's only remaining raw-table spellings are `ctx_func_list`'s own return and read (`encoder_context.rs:710/713` — G–H's accessor; its mints are all shared reads in-fork and retire with the ctx family). The five self-referential typedefs de-virtualized on Phase 4a's move: the search family takes `&SMeFuncs` (a new reach-set group inside the table, `svc_motion_estimate.rs:415`, embedded at `wels_func_ptr_def.rs:387`) + `&SSampleDealingFunc` + the two planes as `&PaddedPlane`; **`SWelsME` lost `pEncMb`/`pRefMb`/`pColoRefMb`** on B3's coordinate identity re-verified at this tree (583/583 rows, ~3.09M search entries/exits asserted, zero violations; 96 → 64, `SWelsMD` 928 → 640, the three identities in the tombstone comment). **The transitional triple retired whole**: `SSampleDealingFunc` back to 176 as T9.B25's pin promised, the 14 raw SAD shims + 7 raw SATD bodies deleted, and `common/sad_common.rs` **and** `encoder/sample.rs` at `#![deny(unsafe_code)]` with zero allows — the fifteenth caller the brief missed (F151, the scene-change block walk) keeps a private relocated `sad_8x8_raw` under the ownership rule, the session's one ratchet rebaseline. Deblocking: `pfDeblockingBSCalc`/`pfSetNZCZero` went direct on F118 (slots, installers, typedefs deleted together); `pfDeblockingFilterSlice` stays as the one genuinely two-valued slot, de-virtualized (layer raw in-fork, S63); `DeblockingFilterFrameAvcbase(&mut SDqLayer)` with F108's ST-only claim verified at `encoder_ext.rs:3861`. F139's write-only slots deleted per S18 plus two the brief never listed (`pfSampleSadHor8`, `SetMeMethod` — zero callers each); **`pGomCost` untouched — F133 is still unruled**. Planted faults per conversion shape, all reverted: 210/210, 30/210 (the chroma skip-tiebreaker's honest referee count, S59), 124/210 st rows. Close: session gate PASS first run — Miri 282/0, lane wall **528 s vs E2's 553, S61 ratio 0.95** (baseline updated); censuses regenerated (601 tags = 559 port-raw + 42 cursor, none dispatch; plane census 27 sites / 6 safe-now, all Phase 10's or F117's — the ME rows gone); ratchet raw_ptr 1753 → 1635 (−118, steward-verified live), unsafe_fn −32, unsafe_block −25. Four brief defects, F149–F152 — all four steward-owned classes, three hoisted as clauses (S24/S54/S58). **Session E3 (closed at `3cf113a7`, ten commits `0186d622`..`0c9de7eb` + the docs close; steward-verified at HEAD)**: **the grid is closed at zero raw record spellings** — `grep -rn '\*mut SMB\|\*const SMB' src tests benches` → 0 (the brief's 34 was a lower bound, F154: three `*const` parameters, `GetRefMb`'s raw return, the dead `PMb` alias) — on **`MbWindow<'a,T>`** (`safe/mb_grid.rs:377`): a borrowed range of the grid + stride + raster index, designed against the callers, whose **bounds are the fork-disjointness mechanism** — each worker mints over exactly the records it owns, a cross-slice record read panics with coordinates instead of racing, ST callers mint the whole grid. `mb_at`/`mb_list_root`/`ctx_pic_ref_mut` retired; nine CABAC syntax fns + the eight arithmetic-engine primitives went fully safe (vestigial unsafe). **The harvest**: `sDecPicView` was write-only since T9.C2 (F156 — four mentions, all writes; F139's write-only class one container over) and `sRefPicView`'s 23 resolved into one stamp block + two guards + the dormant pointer; two `&self` root mints (F71's argument transplanted) removed the struct's one reason to exist — **both fields, the struct, its constructor and stamp deleted**; the dormant SC pointer lives behind `layer_ref_feature_storage` (Phase 10's). Planted faults taught F155: the first (the QP average) failed **zero** rows — QP is GOM-row-granular, a fault proves the *field* it perturbs — re-planted at the BS-calc record: st 180/210, dl 76/76; the harvest's guard inversion st 180/210. `deblocking.rs` 11 → **5** allows (2 S63 raw-layer drivers, 1 null slot fn, 1 ctx `PerformDeblockingFilter`, 1 test — the four non-test are G's). **G's ground pre-paid**: `q1c.py` shape E (the foreign-read class) calibrated both directions (exactly one site at `020dfb0a^` line-for-line with Miri's verdict; A–D unchanged at `0bfc7687^`); `phase9_forksplit.py` + `phase9_ctx_forksplit.md` split the ctx family **111 in-fork / 157 ST over 268 bodies** — F157: the split is decided by the **static function-item array arm** (`g_pWelsSliceCoding`), which no `.pf` census can see (27 without it; the tool ships that as `--no-slots` calibration). Close: gate PASS first run, 583/583 both profiles, Miri 291/0, lane **536 s vs 528 — S61 ratio 1.02**; tags (crate-wide command both ends) port-raw 574 → 545, cursor 42 → 40 (census-official 569 = 530 + 39); ratchet **live-at-both-ends** `raw_ptr` 1635 → **1586 (−49)**, `unsafe_fn` 655 → 625 (−30), `unsafe_block` 287 → 282 (−5) — the report's −63/−32/−6 differenced the baseline *file*, which session F had left at its rebaseline value (1650); one instrument at both ends, §7.1's clause. F153–F158; F149+F154+F155+F156 hoisted together as **S64**. **Session G (closed at `4c9b230e`, eleven commits `571ddf2d`..`4c9b230e`; steps 0–2 complete, steps 3–6 chartered to H; steward-verified at HEAD)**: **D-dead-3 executed** — `pGomCost` at zero non-comment mentions, `assert_size!(SWelsSvcRc, 416)`, eight cross-references not the brief's seven (the `rc_gom_cost` gravestone was the eighth). **The worker's second commit fixed the steward's stale-driver guard (F159 → S66)**: `583cd21a` was aimed at the cargo crate *directory* — `[ -x dir ]` is always true and `find -newer` against the directory's mtime matched 83 sources forever — so every sweep since it landed exited 2 without probing a byte, and the guard's own "fired truthfully" was a false positive read as a confirmation; a detector that fires on everything passes a positive-only calibration perfectly. The fix is calibrated **six arms in both directions**; F159 notes `q1c`/`forksplit` still owe negative calibrations. **The instruments reproduced exactly** (111/157/268; 266/69/82) and the shipped join (`phase9_ctx_join.py`) showed **135 of 266 hazards model a retag S63 forbids** — including all 55 against `ctx_param`, the brief's centrepiece, worth zero live sites (F161 → S65). **Live hazards 131 → 7** in six commits: `pSpatialIndexMap` retired (61 sites, one binding), the two LTR list-editors narrowed onto `&mut SRefList` (11 sites; two bodies leave the family — ST 157 → **155**, bodies 268 → **266**), derivation order sorted by the fork column (T9.G4 — S65's corollary: the exempt half is a resource), `ctx_frame_bs_at` loses its constant position (18 sites), the shape-B hoists, the four holding `pLtr` bindings made raw. The 7 survivors are analysed false positives (4 `pParamD` into the coding-param `Box`, 3 hoisted-layer) — **F163: q1c models the retag by type, the hazard is by allocation** (accessors launder provenance out of containers, F71), and its shape-B remedy produces shape A on pointer hoists, so "detector to zero" is not a writable exit criterion; the per-allocation classifier is the H/exit upgrade. **F162**: the port reproduces upstream's one-past-the-end read at the encode error tail (`encoder_ext.cpp:4109-4110`, twice) — kept byte-identical, commented, hazard-free; diverging is a ruling, pending. **F164**: F67 re-derived — still twelve `!Sync` reasons but a *different* twelve (`CMemoryAlign` out as predicted, `SrcPicPool` in; a stable total is not a stable list), and **only 4 of 12 are G's** — the send-seam cannot retire in this phase's ctx split; it is a **phase-exit item** contingent on X, Phase 8's, Phase 10's, trace and preprocess; the seam comment corrected in place, the exit condition's amendment pending the user. **F160**: D-dead-3's ground reaches two orphans (`SharedMbArray::capture`, dead from birth; F138's write-only `pfIDct*` trio whose stay-alone note cited the now-reversed F133) — left untouched: a second ruling, not an inference. **The close**: session gate PASS 583/583 both profiles, lane **521 s vs 536 — S61 ratio 0.97**; the `MIRI_FULL` fork pair launched per D-gate-7 and **stopped by the user at 56 min, both probes healthy — recorded incomplete, not green** (G's report called E8's numbers serial — F168 later proved them parallel per-probe walls all along, exactly as E8's log and D-gate-6's block had said; the steward inherited G's claim into H's brief over two standing records, S58's re-derive corollary unapplied, and the hour H spent running the pair serially is that error's cost); the un-refereed residue is narrow and named (T9.G7's raw `pLtr` + three hoists in `slice_multi_threading.rs`), and **H runs the pair first, serially**. Ratchet live-at-both-ends: `raw_ptr` −1, `unsafe_fn` −2, `unsafe_block` −3 (steward-verified 1586→1585 / 625→623 / 282→279; step 2 is deliberately signature-preserving; the worker self-caught two baseline-relative commit messages — §7.1's clause, second instance). Tags 569 → **567** (528 + 39). Family: **266 bodies / 111 in-fork / 155 ST**. F159–F164; **S65/S66 worker-minted**. **H fires** (D-scope-1 anticipated it): the serial pair, then step 3 re-planned around the accessor-coexistence crux — `ctx_ltr_at`/`ctx_sps` are themselves ST bodies, and two live `&mut`-ctx projections are a borrow error regardless of hazards; that end state decides how the 155 stage. **Session X2, steward verification (verified at HEAD `6b27662d`, eight commits `3873450b`..`6b27662d`)**: **D-fid-3 executed** — `GetRefMb` clamped, **the gtest suite tallies for the first time since E3 (191/199, allowlist still exactly 8)**, and the 9-s Simulcast smoke joined the session battery, calibrated both ways; **D-dead-6 executed** (pin 416 → 392). Ratchet live 1369 → **1356 (−13)**, unsafe_fn −3, unsafe_block −1 — modest by design: the session's value was two rulings executed, a dead gate revived, and nine defects found. S61 lane **480 s vs 507 — 0.95**; battery **977 s, over D-gate-6's 900-s cap** (F185, ruled below). **The brief was wrong four times, three from documents quoted rather than re-checked — the steward's**: F179 (step 3's premise false — `SetRefMbType` was ported 806 commits before HEAD and session H edited it; the claim traveled comment → F177 → brief, and one `grep -n 'fn SetRefMbType'` refutes it → **S68's absence clause**); F181 (the brief's S54-delete on au_set's `_pLogCtx` pair would have **destroyed evidence** — upstream logs through them eight times; the port had dropped the `WelsLog` calls, now restored → **S54's both-trees clause**); F180/F182 (two enumerations short: `WelsUnloadNal` has nine callers not five — a whole file ungrepped; `TraceParamInfo` four C++ sites not three, the port's missing `:796` restored). **F100 landed behind the new instrument**: `diffharness/log_referee.sh` — the port went from 1 of 19 reference messages to **20 of 35 character-identical**, and the referee earned its cost five ways: it found **F184** on its first run (the C-ABI level divergence — `WELS_LOG_*` is a bit mask upstream, consecutive integers here; INFO is 4 upstream and 3 in the port, so the port's DEBUG is bit-identical to the reference's INFO; five duplication sites, one a **green test asserting the divergence**), found 16 sign-reinterpretation defects in its own F100 port (C's `%d` on unsigned prints the signed value), found a missing SetOption line, and **found a hole in itself** — its pointer normalization matched any `0x<hex>` and swallowed `<w>x<h>`, so `80x49` would have been certified by the instrument built to catch it → **S69**. **F183**: `pCurPath` is the third write-only field (three touches in each tree, no reader; also `SWelsSvcCodingParam`'s only raw member — F67's `*mut i8` row, so deleting it is **ten → nine**, the number step 3 wanted; ruled below as D-dead-7). **Step 1 honest at ~50 remaining**: ~8 permanently raw (five C-ABI trace sites, F166, the two seam tags), **~36 blocked on H2's read surface** (`LoadPrevious`'s four-simultaneous-`&mut`-projections shape is S63's forbidden retag exactly — folded into H2's scope per the worker's recommendation), ~6 **documented refusals** (F187 — every refusal note was within twenty lines of the site; the worker's own estimate ran 43% optimistic by trusting a count table over those notes). Process: gate-per-group with the end state gated; one self-caught §7.5 slip; one corrected commit message — three costumes of one error, **believing a document over the thing it describes**, which is S68's whole family. **Session H2 (sections A–D; verified in two passes, the close at `adf7dbce` and section D at HEAD `266a8bbf`)**: **the seam the session was scoped for needed no new seam item** — the in-fork read surface resolved as a shared borrow with **zero** UnsafeCell/Sync additions (D-mt-3's count stays 2), though F191 records the honest shape: the "read routes" were four different problems wearing one label. Step 0's three rulings executed — D-fid-4 in **six** places not five (F188: the sixth was inside the acceptance instrument itself, and F189: the acceptance had to be repaired before it could accept); D-dead-7 with the ABI pin's intentional field-count divergence noted. **S67's audit: 22 blessed, 1 converted** — the per-site table is in the log. **F167 is closed as UB, not argued-sound (F192)**: the owed Miri run refused the stored-copy read in one line — *a reference argument is strongly protected for the duration of the call; disjointness buys nothing* — and the real path measurably cannot be driven (SCREEN_CONTENT is rejected at `RequestMemorySvc`), so the unit drive was the only referee available. Closed by **deleting the storage**: `m_pEncCtx` is gone (zero non-comment mentions), all four readers take the context as a parameter like their ten siblings, and H's interim re-stamp went with it — the record now says why the interim was insufficient (re-stamping made the stored raw a *child* of the borrow: survives the retag, not the protector — S56's new clause). **X2's ~36 unblocked by one function**: the charter called the `LoadPrevious` shape "S63's forbidden retag exactly" and that was the steward's error — S63 forbids N *workers'* concurrent entry retags, not one thread's N disjoint projections; a split-borrow function returning the three paraset arrays fixed it (S63's clarifying clause), and turned an argument into an assertion (three `copy_nonoverlapping` writes of `MAX_SPS_COUNT` entries had been guarded only by `is_null()`). **`ctx_ltr_at` attempted, measured, reverted (F196)**: converted, the compiler reports **84 errors across 75 distinct sites — 83 borrow conflicts, zero mechanical type errors** — so the raw return is load-bearing at 75 sites, not T9.G7's documented 4 (a comment records the conflicts someone tripped over; the borrow checker enumerates the ones that exist). Reverted under the session's own pre-set rule: two of the four bodies are live camera-path reference-list management in F3's neighbourhood; its home is ruled below. **F193**: two of the five "slice-returning" APIs can never return a slice (the reason is `codec_app_def.h`'s shapes). **F195, load-bearing for J**: the F67 probe counts distinct *types*, not fields — it cannot see a deletion whose type survives elsewhere, so the twelve→ten→nine series four sessions quoted was mismeasured; **J re-derives the send-seam contingency table by field**. F190 (two documents state stale numbers about the instruments they describe), F194 (the ratchet's per-file rule makes test-only unsafe compete with library unsafe for headroom). **The close**: `gates.sh session` PASS — 583/583 both profiles, Miri **292/0**, gtest smoke 4/4, battery **551 s** against the amended 1200-s cap; S61 lane 551 vs 480 = **1.15** (machine load, F170's caveat noted); the **fork pair green at 1.008 / 1.001 vs H — the read surface is Miri-cost-neutral on the fork**. Six code commits, all gated. F188–F196. **Session H3 (closed at `27ffad18`, seven commits `8182edab`..`27ffad18`; steward-verified at HEAD)**: **the phase's last conversion landed whole** — `ctx_ltr_at` is a **safe `pub fn` returning `&mut SLTRState`** (`encoder_context.rs:889`), `ctx_ltr` is deleted outright (zero mentions; its one remaining caller reads `pLtr.first()` directly), and **no raw spelling survives in the family**. Step 0's reproduction produced the session's first finding before any edit (**F197**): F196's recorded breakdown (49/28/5/1 over 75 sites) was exact but its stated total of 84 never summed — the actual census is **83**, the tree had not moved, and the finding was internally inconsistent when written. The classification held to the digit: hoist / reorder / re-derive-after-call plus two callee narrowings, **zero split borrows and zero genuinely-interleaved sites** — the charter's "resolved by split-borrows" prediction was the steward's, and the structural reason it was wrong is worth keeping: the in-fork accessors still take `*mut`, so every conflict was the `&mut`→`*mut` coercion of `pCtx`, dissolved by hoisting the accessor call. Each staged commit was verified by locally re-applying the flip: the error count fell **83 → 71 → 55 → 22 → 9 → 0**, each step exactly the table's share; the revert rule never fired; F3's protocol stood armed with nothing to adjudicate. **F198, the planted fault**: LTR index off by one → `ltr` **16/16 FAIL** (14 byte-different, 2 collapsed to ~20% size), `mt` 0/120 by preset design (mt runs LTR-off; the family's 583 include the ltr sixteen); first fault moved the output, no escalation needed. **The close**: Miri 291/0, lane **506 s vs 551 — S61 0.92**; ratchet live 1345 → **1338** raw_ptr, 595 → 593 unsafe_fn; census **540 = 507 + 33** (cursor down two, the retired pair); forksplit at both ends confirms zero fork-reachable bodies — the pair lawfully not owed. One correction with standing value: **"`gates.sh commit` (~2.5 min)" has been wrong for some time — 15–19 minutes every run on this machine** — the figure propagated brief-to-brief unre-measured (S68's class, in the steward's own boilerplate); J's brief carries the real number and F170's wall-clock work owns the why. **Hand-off to J: nothing new** — the exit's ledger shrinks by F196's item and two counts. F197–F198. **Session X, steward verification (the worker's list entry above stands; verified at HEAD `445aeda7`, ten commits `83072e53`..`445aeda7`)**: ratchet live at both ends **1434 → 1369 raw_ptr (−65)**, unsafe_fn −12, unsafe_block −11, shim −5 — quoted correctly by the worker, the first session since E3 to do so (F178 notes 4 of the 65 are prose the ratchet miscounts); S61 lane **507 s vs 543 — ratio 0.93**; census **543 = 507 + 36**; Miri 291/0. **F67's probe: twelve → ten**, both fallen reasons `SVAAFrameInfo`'s — but the remaining half of F164's four is *blocked, not deferred*: the `*mut u32` rests on `SetRefMbType`, a function the port never had (F177), and the `*mut u8` is eight plane views into pool pictures, not owned buffers, so the brief's `Vec`-with-index prescription does not apply to them. Remaining owners: H2 ×4, X2 ×2, Phase 10 ×1. **F177's class is new**: three buffers the port never allocates with four unguarded null dereferences, all dark behind `bEnableAdaptiveQuant = false` — a permanently-null field with live readers looks identical to a dead field under every instrument this phase owns; only the *reference tree's writer* distinguishes them (S64's new clause). Two fixed matching `encoder_ext.cpp:1721/:1724`; the third is the missing port, filed. **F172 closes F86 by measurement**: F86's own text misattributed its abort (the real site was `PrefetchNextBuffer`'s read, `cpp:343`, not the insert shift; checking out the commit F86 names and reading the line it quotes settles it — S68, three instances now with F153 and F160), the brief compounded it with a third site and two citations no tool run produces, and the resolution needed no ruling: `uiShortRefCount` tops out at **2** against a threshold of 6 across all 583 rows, the exposed reads guarded where upstream's invariant is implicit. **F173, the important one**: `gtest_stretch.sh --check` has been **aborting since E3** — `GetRefMb`'s checked index (`svc_mode_decision.rs:1477`) panics-then-aborts on `SimulcastAVC_SPS_PPS_LISTING` because upstream reads the same index unchecked under an implicit 2:1 invariant that non-2:1 simulcast breaks; the row is not allowlisted, **the exit gate is red and nothing below exit runs it** — three sessions closed green at family with this in the tree. F162's shape live; ruled by the user (D-fid-3, below). **F174**: `pGomComplexity` is a *second* dead GOM sibling (allocated, nulled, memset, never read in either tree; its accessor had zero production callers, retired S18) — the brief's "three live arrays / ten uses" was wrong twice (the ten were `pTemporalOverRc`'s), and the field's deletion is ruled below (D-dead-6). **F175 → S59's clause**: the scene-change fault sequence (plane root +1 → **0/583**; verdict forced → **71/583**, Process entered in 101 configs) proves the path covered and the fault under-powered — a doubly-thresholded verdict needs a verdict-level fault, and a 0-row reading is informative only beside a non-zero maximal one. **F176**: the vaacalc read grep was six callers short on F119's own family — nine sites, six in `kernels_differential_phase2.rs`; caught by the worker, corrected in the same step. **The MT adjudication**: two of `nal_encap.rs`'s three tags are seam-shaped (both fns fork-reachable via the walker's `--why`) → H2's; the third was a test → `C-ABI(test)`; and `WelsUnloadNal` is **not** a C-ABI boundary at all (no export attribute, five Rust callers) — the brief filed one site twice, corrected. **F178 → J with F170**: the ratchet counts `*mut` and `no_mangle` in *prose* — two gate failures on documentation this session; a conversion is currently penalised for explaining itself; the fix is small but is a ratchet change and rebaselines with its reason. **Deferred to X2 per the drop order**: step 5 whole (F100 + the log referee) and step 4 minus the MT adjudication. **Session H, steward verification (the worker's own list entry above stands; verified at HEAD `e470d855`, nineteen commits `5846c35b`..`e470d855`)**: forksplit reproduces **114/111/3**; the 3 are `ParasetStrategy` (F166: an unbound-lifetime accessor whose flip turns ten sound-by-allocation sites into E0499 — keeping it raw is what unblocks them; rustc-model-verified before the finding was written, S60) and the two dead bodies pending **D-dead-5** (`WelsRcDropFrameUpdate` — dead in both trees, its one C++ mention is its own log line; `WelsMdInterFinePartitionVaaOnScreen` — **installed by C++'s SCREEN_CONTENT arm**, `encoder_ext.cpp:2711`, so deletion would force Phase 10 to re-port it). **Step 0a paid both ways**: serial on G's exact tree 3194/3260 s GREEN — G's close retroactively whole — and parallel on the flipped tree 3412/3493 s, pair wall 58 min, GREEN: **the fork entered under a `&mut` root for the first time in the project's history**, and running both settled F168 (the serial hour was the steward's inherited error). **The close's first Miri run FAILED with real UB the flip created (F171 → S67, worker-minted)**: `UpdateStatistics` — a body *outside* the parameter family, ctx via a local raw from `ctx_ptr` — held `pStatistics` across `ctx_ltr(&mut *pCtx)`'s whole-context retag; the join read **0 LIVE for six consecutive stages** while the UB was being created, and nine byte-gated stages passed 583/583 with it live — no byte gate can see a read that returns the right value through an invalidated tag. Fixed (T9.H14), re-run **291/0**; D-gate-4's lane has now caught this class twice (F114, F171); the standing audit list is the **24** out-of-family retags (`grep -rn '&mut \*pCtx\|&mut \*\*ppCtx\|&mut \*pEncCtx' src/encoder | wc -l`, verified 24 at HEAD). **F169's arc**: `as *mut _` fails the ratchet at +92 raw_ptr per body; `from_mut` is net-zero; the amendment wins both — implicit coercion, no boundary spelling at all. **F165 (S64's fifth instance, the steward's)**: the accessor family is 27 bodies / 17 in-fork / 10 ST, not one file's 21/14/7; `ctx_sps`/`ctx_pps` chain through in-fork arrays and cannot dissolve into field paths. **F167**: the cached `m_pEncCtx` struct-field copy was sound pre-flip only because nothing minted `&mut` ctx; the flip ended that argument — the live half fixed, the dormant screen-content half **argued, not run: its Miri verification is owed**. **F170 → J**: wall-clock sweep timing reported a suspended laptop as a 125x regression (5887 s vs 44.8 s re-run); `s61_report` shares the bug — today's 1.3x tripwire can be tripped by an idle machine; CPU-beside-real is J's fix, not a mid-campaign instrument change. Three worker instrument failures self-reported (the `is_null` regex's 51-vs-10, reverted whole and redone from the compiler's list; `flip.py`'s four silently skipped `_pCtx` bodies; the harvest detector's 22-cleared → 37 errors → 10 real) — S64/S66 in their own tooling, so the rules held where the tools did not. **Close numbers verified live**: lane **543 s vs 521 — S61 1.04**; census **557 = 521 + 36** (tool-counted; the hand grep's 536+37 counts comment lines, not items — the tool is the authority); ratchet live-at-both-ends `raw_ptr` 1585 → **1434 (−151)**, `unsafe_fn` 623 → 613 (−10) — the report's 1587/−153 was baseline-relative, **§7.1's third instance**. **Not done, honestly named**: step 3 (the in-fork read surface — D-mt-3's count still 2, none added) never started though the drop order protected it; step 4 partial (10 bodies de-unsafed; 5 ST accessors need slice-returning APIs on `pLtr`/`pFrameBs`/`pDqIdcMap`/`SStrideTables`); `ctx_func_list`'s 106, the 21 MT tags, the layer's 42 wait with the seam (S63). **Steward cleanup after the close**: H's two docs-close commits swept 33 `target-fixed/` cargo artifacts into the repo (a 689KB binary and two 43MB dep-graphs, pushed before caught); untracked + gitignored at `d1ade9d9`, history left alone — hashes are load-bearing. Charter: [`prompts/phase9.md`](prompts/phase9.md). |
| **OPERATIVE PLAN (2026-08-27, D-plan-1): [safe_conversion_execution_plan.md](safe_conversion_execution_plan.md)** | The final push to safe Rust: `#![forbid(unsafe_code)]` everywhere outside the C-ABI island plus two audited MT `unsafe impl` lines. Supersedes D-exit-3's tagged-lawful end state — the fork-shared 204 and the 306-item queue become the work; four large sessions (S1–S4, slack 6), checkpoints individually gated, adoption amendments in its §10. Phase 10 follows, landing screen content on safe storage. |
| **SAFE-CONVERSION PLAN — CODE COMPLETE, EXIT BATTERY RUN WITH TWO NAMED HOLES (2026-09-01, S1–S13, thirteen sessions)** | **The conversion is done and the deferred verification found two real defects that no byte gate could see.** **Scorecard, project start -> here** (`unsafe_ratchet.sh`, live): `raw_ptr` **5390 -> 393**, `unsafe_fn` **1372 -> 52**, `unsafe_block` **507 -> 122**, `transmute` **23 -> 0**, `shim` **42 -> 0**, `mem_zeroed` **26 -> 1**, `unsafe_impl` **11 -> 1**. `common/` and `processing/` reach **absolute zero** on all three primary metrics (17/24/46 and 14/1/12 -> 0). Outside `src/api/`: 95 raw-pointer occurrences remain — 31 in `unsafe extern "C" fn` **type aliases**, 17 in test instruments, 47 in production code (repr(C) ABI fields, the instrumented cursor accessors, `WelsMoveMemory_c`'s C-ABI params, `rec_view`'s `SharedCells` base). **The enumerated floor: 21 allows in three categories** — 17 `instrument(test)`, 2 `recon-seam`, 2 `C-ABI` — across 7 files; **69 of 86** files sealed `#![forbid(unsafe_code)]` (F311 corrects the recorded 70). **S13's two fixes**: F312 (Miri caught the ABI pointer's tag popped by a later `&mut`; S11.47's, invisible for 53 checkpoints under D-gate-9) and F313 (upstream `test/api` red since S11.27; the pre-fork carve miscounted an empty partition). **The honest gaps**: the fork/join pair and the size-limited full drive have **no verdict on HEAD** — F314, the Miri cost tripwire at 3x after 147 commits unrun — so **D1's `unsafe impl` proof obligation is unverified since S4**. |
...
| **Standing rules** | **§7.6** below, now S1–S69. Phase 3+ briefs cite it rather than copying rules forward. **S20** (a struct conversion's decomposable unit is the signature-reachability closure, not one struct) and **S21** (the construction audit, and a duplicate-family finding must inventory every function per copy) were hoisted out of Phase 3 and are what Phases 5 and 6 are made of; **S22** (a repaired instrument has a backlog — run everything it covers at every level, the session it is repaired) is F17 → F18's lesson; **S23** (a cached table becomes a derived value only if the source cannot change behind the cache — a property of the *update paths*, read one field at a time) and **S23b** (an alternation that returns 0/0 has not run yet — alternate at the level the hits occur) are 4b session A's; **S24** (a count that decides a conversion's *shape* comes from a grep over the definitions, never from a doc or a brief) is 4b session B's, earned by two wrong premises in one brief; **S25** (a raw-to-borrow conversion surfaces pre-existing aliasing — the re-entrancy audit is part of the conversion, enumerated with the S20 closure) is session B's second, hoisted from its log §4, and Phases 5/6 inherit it by name; **S26** (a brief is an execution document — facts, order, gates, non-goals; cite rules by tag, no history retelling, no provenance narration) is Eugene's direction at Phase 5's open. **Section revised 2026-08-11** (Eugene's direction): R-letter tags moved to the preamble map, **S14 refreshed to the measured protocol** (briefs now cite it instead of restating F3), **S17 generalized** — its specific check is automated in `gates.sh` — and S3–S5/S10–S11 tagged **[dormant: kernel work]** until 6.3's re-attempt. Instrument: `rust/tools/perfpair.py`, which implements S1/S2/S17. **S14 gained one step-2 shortcut at Phase 5 session D**: before alternating, hash the sweep binary against the base's — if they match, the alternation is vacuous by construction and the hash is the acquittal (F3, measurements 27–28). Step 2 is unchanged for any session whose commits reach `rust_enc`. |
| **Open findings** | **The open set at Phase 5's close (session AC's exit pass), each with an owner.** **F3 is CLOSED at Phase 7 session B (2026-08-20) — cause F69**: the raw translation dropped the `WelsMutexLock(&mutexSliceNumUpdate)` that brackets `AddSliceBoundary` + `++pSliceCtx->iSliceNumInFrame` (`svc_encode_slice.cpp:1776–1791`); a lost increment makes `iEncodeSliceNum != iSliceNumInFrame` in `ReOrderSliceInLayer`, which returns `ENC_RETURN_UNEXPECTED` and emits an empty frame — the signature exactly, 18 of the before-arm's 25 hits. **Closed by a three-arm ablation** (S45): before 25/3000 ≈1/120 under saturating load; pool machinery deleted, lock still missing 20/2000 (z = 0.61, p ≈ 0.54 — **the pool was never the cause**); lock restored **0/2000** (P ≈ 2×10⁻⁹). 92 measurements in all; the acquittal protocol (S14) is retired — a wrong-length sweep output is a defect from here. The finding was reached by S44: the field was on a delete list as dead, and reading the reference to confirm found the dropped bracket. *(History: signature `mt`, `sm=3`, `t∈{2,4}`, any wrong length; rate ≈1/120 pooled, density-sensitive; the older ≈1/307 was the battery-interleaved reading.)* AC added a protocol clause worth having: **step 0's hash shortcut is profile-dependent** — `rust_enc` never calls the decoder and release eliminates it, so a decoder-only change can hash identically in release where it does not in debug. Hash the profile the hit occurred in. **And do not read the clause as a rule** (session D, measured): a decoder-only window that *deletes types and a struct field* moved the release binary too — base `6b1be2a4…` vs head `50d1e52d…`, one clean build each at one path — so the clause says the shortcut *can* apply in release, never that it does. Hash it; do not predict it. **F23 is CLOSED at Phase 8 session A (2026-08-21), on both codecs** — the 19 conveniences are associated functions taking `this: *mut _`, 116 callers re-spelled, the covering test red under Miri before and green after. **F41 is CLOSED (8.A)** — the context owns its `pParam`; `CWelsDecoderImpl::param` had no counterpart in the reference and is deleted. **The twelve api-owned decoder-context fields are CLOSED (8.A)** — owned by the context, stamps 0, `api_alias` deleted, the decoder's production allow items 0. **The `src/api/` instrument inventory is DONE (8.A)** — five instruments reach `src/api`; four duplicate boundary types fixed, two enumerated (`SParserBsInfo`'s two entities → D5/Phase 9; `WelsTraceCallback`'s duplicate → 8.B). **F38-class back-pointers in `api/`** (`m_pCodecInstance`, `SLogContext::pCodecInstance`) and **F76** (the decoder's `DecodeFrame2` error-reporting block absent from the port — `ResetDecoder` on the reset arms, the key-frame-loss flag, the trace throttle, four EC statistics, three `DecoderConfigParam` statements, the live re-init rebuild; none of it moves a corpus byte or code, which is why it survived) → **8.B** — **and 8.B closed them**: **F76 CLOSED** (the arms ported whole with covering tests red before; `dsOutOfMemory`/`dsRefListNullPtrs` ported but measured unreachable from any of the 62 assets × 4 EC modes), **F78 CLOSED** (the encoder's dead second C-ABI surface deleted), **F79 CLOSED** (`WelsLog` was a stub in two places with zero encoder call sites against the reference's 87; one `common/wels_trace.rs` now delivers, three tests red before; the default-sink divergence ruled by **D-api-1**). **F77 OPEN → 8.C** (`res/Error_I_P.264` aborts the process — a bitstream-derived macroblock index one past the end panicking across a thunk; P13's line, reassigned from Phase 9 by D-api-1's paragraph). **The parse tree's raw slots and the zeroed shells are CLOSED at Phase 5b** (`6fe88832`, `1abc379a`) — the slots own and both shells are field-wise constructors. **F56 is CLOSED** (opened session C, **ruled by the steward 2026-08-18 and fixed at `318c9f87`, session D**): F54's niche artifact on two fields F54 did not reach — `SWelsDecoderContext::active_sps` and every fresh NAL node read back as `Some(SpsRef { id: 0 })` where the C memsets a null. `None` is the faithful value at all three sites, and the ruling was made rather than defaulted because the referee suite is what makes a behaviour question answerable: the fix landed **only with the corpus (2690/17 + 2707/0 over 2707 rows) and conformance (60/60) beside it, both unmoved**, the byte comparison reading **573,574 of 573,576** with the two differing offsets attributed by `offset_of!`, and three covering tests each measured red under its own revert. **The arm it unblocks is still unreached — 0 hits across all three stream suites, instrumented** — which is the finding's point: *no gate this project owns can see this class*, so the instrument is the byte comparison against the zero image and the decision is a ruling. Phase 6 inherits that as a method, not as an open item. **`PPicture`'s option-1/2 revisit and `sMCRefMember` are CLOSED at Phase 5b** (`7a4ad7b5`), and the revisit closed by *neither* option: the F42 arm was answered by **identity** rather than by an address — `PicRefs::resolve` for the readers, a same-picture kernel family for motion compensation — so no parity divergence and no interior mutability. `phase5_findings.md` §"CLOSED at Phase 5b" carries the argument; it is the shape the encoder's pool should reach for at 6.1/6.2. **The parse tree stays open with a measured lead**: Phase 5b built the conversion whole (owned slots, both stored aliases as indices, the NAL union as a struct) and reverted it at the gate — 11 of 60 conformance assets, every one a B-slice stream, frame counts correct; the patch is preserved and two hypotheses are eliminated. **F52 is CLOSED at Phase 6 session B** (2026-08-18): the six encoder-side shadowing-stub candidates were adjudicated by opening both lines each — four are trait-method *declarations* the sweep had scored as empty bodies (`Uninit`, `InitFrame`, `ExecuteTasks`, `OnTaskStop`), `WelsRcPostFrameSkipping` is a faithful `return false` (`ratectl.cpp:1015`) beside its `RCMode` dispatcher, `push_back` is two methods on two types; `find_shadowing_stubs.py` now skips declarations (22 → 18 candidate names, measured) and carries a `--self-test` that keeps F52's own stub shape printing. `phase6_findings.md`. **F60 is CLOSED in the session that found it** (Phase 6 session D): `FrameBsRealloc` moved `pOut->sNalLen` and never re-aimed the layers at it — three SIGBUS crashes and four byte divergences against the C++ before, all twelve configurations byte-identical after, on a path no sweep configuration reaches. **F61 is CLOSED BY REDESIGN at Phase 7 session C** (2026-08-21), verified by construction: the layer's slice list has exactly one reader (`slice_in_layer`) and **no worker is among its 21 call sites** — every one runs on the calling thread, before the fork or after the join; a worker resolves slices by bank position (`slice_in_bank(layer, slot, coded)`) and never through the list; and the bank being grown belongs to the worker growing it, by static partition since T7.B2. The window is real and **unobservable**, which is stronger than "short". **The C++ has the same shape** and the divergence note is unchanged — byte-neutral on both sides, for the same reason. Session D's position spelling had already removed the memory-safety half. **F57**, **F58** (Phase 6 session A) and **F59** (session B) are all **CLOSED in the session that found them** — the encoder Miri probe's three new findings, each fixed with red-before and green-after observed (`phase6_findings.md`). **F13 is CLOSED**: session A fixed its last production site as a 20-derivation family, and session B measured the backlog behind its Miri skip (exactly two tests, both green under the step's own flags) and **deleted the skip** — the `--lib` step now skips `wels_thread_pool` (F12, Phase 7) and nothing else. **F36 is SETTLED at Phase 7 session C — left, and fenced.** The port's MT slice loop is a partial translation (101 lines against the reference's 162, missing the `pSliceIdc` write, the `pNzc` copy, `SetNonZeroCount`, the per-MB deblocking call and the border padding, with `decoder_core.cpp:2595`'s `pMbCorrectlyDecodedFlag` re-point absent entirely) and it is unreachable — `GetThreadCount` returns 0. Deleting it would change the shape of a live decoder function and delete the one thing a porter needs, which is that list; the body is safe Rust and blocks no deny. It is greppable now: **`DECODER_MT(incomplete: F36)`** at the function and at the branch. Must be finished *before* `GetThreadCount` returns anything above 1. **F37 is CLOSED** — it was fixed at T5.O1 (the reset sits at `DestroyPicBuff`'s head) and this row was stale since; Phase 8 session A added the public-API re-init probe that proves it, red under a revert. **F73 is OPEN and owned by Phase 9**: the encoder's workers reach the reconstruction picture through `&mut` (`layer_dec_pic_mut` → `SRefList::pic_mut` → `&mut SPicture`, and `SPicture::planes`' `&mut self`), so two workers race on the *accessor's own retag* while the macroblock rows they then write are disjoint. Measured: **32 `planes()` sites, 68 `_mut` picture-accessor calls** across seven modules — a family, not a site, and the same object F67 and F66 already name. The MT Miri probe's `#[cfg_attr(miri, ignore)]` cites it and retires with it. **F4, F6, F7** (S6-parity) and **F8–F12, F14** → per file, owners as recorded. **The `CABA2_SVA_B` annotation stands** — 17 rows, expected-divergent, generated into the golden header so regeneration cannot lose it; a recorded position, not a defect. **It has a sibling since Phase 7 session C**: the **load-balancing path** (`bUseLoadBalancing` with `uiSliceMode == SM_FIXEDSLCNUM_SLICE` and `iMultipleThreadIdc >= uiSliceNum`) is the project's **second expected-divergent class**, and it cannot ever be anything else — frame N+1's slice boundaries are computed from frame N's measured per-slice encode *times*, so two runs of the **C++ reference alone** return different byte counts (measured at T7.B0: 22834/22660, 144438/144340, 233477/233251). Both diffharness drivers pin the flag off (`cxx_enc.cpp:119`) and every byte-asserting probe pins it off; its coverage is **structural only** — `load_balancing_completes_frames_with_sane_slice_counts`, which asserts frame counts, non-emptiness and `vcl_nals == 4` and never a byte. Completed under **D-mt-2** at T7.C1, because a default-on feature must not ship degenerate. **F72.** **Closed in Phase 5**: F15–F22, F24–F35, F39, F40, F42–F51, F53 — and F54's rule is folded into S21. *The findings files are the histories; this row is the open set.* **F80 OPEN (Phase 8 session C) → Phase 8b**: `IncreasePicBuff`/`DecreasePicBuff` never ported, `WelsRequestMem`'s third arm absent — measure reachability first (make the asset with the reference encoder if `res/` has none). **F81 CLOSED (8.C4)**: `SDeliveryStatus` 1 byte vs the ABI's 12, fixed toward the header, found by the pins built from the C++'s own `sizeof` dump. **F82 CLOSED (8.C8)**: `DecodeFrameNoDelay` had no referee and neither half was ported — the second `DecodeFrame2` call and the unguarded null-input arm; 37 gtest rows in one commit, `ecref --nodelay` and a five-asset parity test are its referees now; the rule is S47. **F84 (8b.A) → Phase 9**: two threaded-decoder functions transliterated with zero callers (`decode_slice.rs:5444`, `deblocking.rs:2295`) — S18 deletions, with the 18 `MBPad*`/`PadMB*` kernels they alone reach. **F85 (8b.A) → S22's clause**: the stub finder collapses same-named methods (the decoder's `GetOption` hid behind the encoder's). **F86 (8b.A)**: the `ForceCodingIDR` stub's reproduction is fixed; **its open half → Phase 9** — the unchecked `pShortRefList[iRefIdx + 1]` write (`ref_list_mgr_svc.cpp:387–391`, panic at `ref_list_mgr_svc.rs:684`). **F87 (8b.A) → 8b.C**: `dsOutOfMemory` on a reference-count change where the reference decodes cleanly (F80 made concrete). **F88 (8b.A) → 8b.B**: one gtest row intermittently red on the Rust link only; chase with a pinned seed. **Phase 8b session B (F89–F96):** F89 (a seventh listing row hidden by the clock — S49's case), F90 (`DecodeParser` copy-out overwrote the caller's input timestamp; its `memset` sized an int array in bytes), F91→Phase 9 (upstream writes into the caller's `const` bitstream in parse-only SVC; the port does not), F92→Phase 9 (two unbounded writes + an RBSP/EBSP length bug in the subset-SPS rewrite; the port bounds them, reachability unproven), F93→8b.C-after-F96 (parse-only on a damaged stream, six rows disagree — the messenger for F96), F94 (the SPS_PPS_LISTING structure export read one PPS and copied 57; the port copies the array; reachability needs `--reinit-at`), **F95→phase close** (`find_stub_bodies.py` cannot count through `abi_guard!`, so its signal on all 20 C-ABI slots is zero — a mechanical fix), **F96→8b.C** (`ERROR_CON_DISABLE` unrefereed; `Error_I_P.264` diverges in `InitRefPicList`, narrowed, two candidates eliminated), **F88→8b.C** (concealment under `SLICE_COPY`, reproduces at `--seeds=5..5`). **Phase 8b session C (2026-08-22) closed F80/F87** (`WelsRequestMem`'s third arm — `safe::Pool` now grows and shrinks under a tested generation contract), **F96** (`isNewFrame` lacked the reference's `iThreadCount > 1` guard, so `InitRefPicList` was skipped on continuing frames; the earlier `InitRefPicList`-internal narrowing was wrong — found by two `eprintln`s, not a third reading) **and F88 with it** (0/10 seeds, seed 5 included; the concealment output the row asserts follows from the same missing guard). **F97/F98/F99 resolved the downsampler question** and it was not the one the brief feared: the `_c` and NEON kernels are byte-identical (40/40), the divergence is the aarch64 dispatch *table* (general-ratio luma rebinds to the accurate kernel) and an inverted-sense flag (`m_bNoSampleBuffer`, false on success) that makes 4:1 a half-average cascade — S50. **F93→Phase 9** (its remaining four rows are parse-only's own; the ordinary decode path agrees once F96 is fixed). **F95 fixed** (`find_stub_bodies.py` now counts through `abi_guard!`), which surfaced **F100→Phase 9** (`LogStatistics`/`TraceParamInfo` are empty bodies — pure `WelsLog`, no byte a gate can see). **Phase 9 sessions A/B (F101–F109):** F101 (the charter's family table was a first-line artefact — S24 clause), F102 (`q1c.py` never existed; rebuilt with both hazard shapes), F103 (coefficient family is 5 + 14 slots → family 3; S51), **F104** (the plane family's tables are double-gated on SMbCache — S52; re-ordered the phase), **F105 → session C** (20 dead `mc.rs` kernels, S18 deletions), F106 CLOSED (dead expand shims deleted), **F107 → D-mt-3 / session C** (the reconstruction write cannot be `&mut [u8]`; design decided below), **F108 → session C** (deblocking is in-fork under MT), **F109 → S53 + an S47 clause** (a handle re-read at the point of use named a different picture; the PSNR path has no referee — `bPsnr=false` in both diffharness drivers). **Phase 9 session D (F110–F113):** **F110 → S54** (the brief's `WelsEncRecUV` derivation was false — two callers, two bases; the cursor parameter survives as an index), **F111 → S55** (`q1c.py` was blind to `addr_of_mut!` and to owned-field roots — 22 + 20 hidden sites; family 6's precondition is 308/71), **F112 → S37 clause, owner session E** (the arena's 32 roots stay raw until `*mut SSlice` converts), **F113 → S52 clause, CLOSED** with the 14-of-14 slot flip (one raw slot type served two spans). **F114 → S56 + an S29 clause, CLOSED by T9.D11** (two UB the conversion *introduced* and no byte gate could see — a raw under a converted parent popped by a safe borrow, and a reference argument's protector; D-gate-4 is the instrument; the detector's shapes C/D landed in T9.B20, calibrated against `0bfc7687^`). **Phase 9 session B2 (F115–F119):** **F115 → D-dead-1, delete (2026-08-23)** (`WelsMdP4x4`/`P8x4`/`P4x8` are dead in the port and `#if 0` upstream), **F116 → S58** (the plane census drifted 40 sites into `?` and exited 0 while the document it feeds stayed right — a reading aid sessions scope against is a gate; loudness lives in the exit code), **F117 → S57 + an S50 clause, owner session C** (the background-detection copy writes the *current* source picture — the brief read the direction backwards against a dst-first typedef — and no gate runs the path: `bEnableBackgroundDetection=false` in the driver; left raw and tagged), **F118 → an S20 clause** (a `PlaneCursor` cannot feed a raw slot, so operand and slot convert together; constant-after-init slots decouple by direct call — only the 13 runtime-selected cost sites force the atomic flip), **F119 → D-cov-1 + an S18 clause (2026-08-23)** (the 22 "dead" `mc.rs` shims are `tests/kernels_differential_phase2.rs`'s raw entry points — the dead-grep excluded `tests/`; the harness is rewritten to drive the safe kernels, then the shims go). **Phase 9 session B3 (F120–F124):** **F120 → an S58 clause, CLOSED** (a missing entry point in the census's shim table is silent where an unclassified operand is loud — `md.rs`'s ten MC sites were invisible and three documents quoted three different wrong numbers), **F121 → an S57 clause, owner Phase 10 / session C's referee** (`CalUVSadCost` and its two callers are dark, measured against a calibration probe; **B2's two `ctx_pic_ref_mut(..).planes()` retags are inside them** — live as code, unreachable as behaviour), **F122 → an S18 clause, the user's call** (three of `WelsMdInterMbRefinement`'s seven arms, three `UpdateP*MotionInfo`, three `SWelsMD` members and two entropy writers' sub-type arms are unreachable for D-dead-1's own reason — a deletion is a closure question), **F123 → an S55 clause, CLOSED** (`q1c.py`'s shape C over-reported shared borrows; narrowed and re-calibrated against `0bfc7687^`), **F124 → D-cov-1 re-scoped, owner session C** (the harness has no C++ side, as corrected — but **two** mc tests survive, the second being Phase 4a's dispatch assert-map, and `common/mc.rs` reaches `deny` only when the six dark `McLuma_c`/`McChroma_c` call sites get a referee; the 22 dead shims and the two tests can go before that). **Phase 9 session B4 (F125–F129):** **F125/F128** (the dark 17 split 8 BGD / 9 screen-content — `bScreenContent` sits in `WelsInitSCDPskipFunc`'s conjunction; the nine reclassified to Phase 10, the two censuses disagreeing about them on purpose), **F126 → S59** (the Static clip executes the MD family 4524 times per row and swallows a +128 planted fault — coverage is teeth, not entries; the MD referee is 32 of 48 rows), F127 CLOSED (the sweep row count was wrong in two places — 505 in `gates.sh`, 535 in prose, 583 measured with `bg`), F124 CLOSED in B4 (**three** mc tests, not two — all gone with the 26 shims; `mc.rs` denies with three dormant allows), **F129** (D-dead-2's closure was two items larger than F122 listed, and the compiler cannot see the class). **Phase 9 session C (F131–F135):** **F131** (the seam is a picture view — planes *and* side arrays; the brief's plane-only shape could not have moved the probe), **F132 → S60 + the acceptance re-scope** (six fork families, not one; two open, owned by E and the slice structures), **F133 → the user** (`pGomCost` is a real upstream data race on a write-only accumulator — atomic now, deletion pending), **F134 CLOSED** (`pDecMb` was `pCsMb` renamed; deleted on a calibrated family-long assert), **F135 CLOSED** (the twin deleted in T9.C2a on the user's ruling). **Phase 9 session C2 (F136–F140):** **F136 → an S60 clause** (round 6 was two fields — the fork contends with the *struct*; the expected-next-verdict check caught it), **F137 CLOSED** (the census learns `pfIDctI16x16Dc`), F138 (session-internal), **F139 → the phase-exit checklist** (write-only dispatch tables: a per-slot READ grep, not an install grep — `pfIDct*` x3, 8/10 deblocking slots, `pGomCost`), **F140 → S61 + D-gate-5/6/7 (2026-08-24), CLOSED by T9.E1/E2/E9** (the Miri step never completed: 2.6x uniform, the seam's bounds-checked interpretation; the probes were *three*, not two; the gate is now a parallel sharded lane at 517 s). **Phase 9 session E (F141–F144):** **F142 → S62** (the briefed assert was the race it interrogates; prove at the stamp, and prove *outcome* equality), **F143 → an S60 clause** (an iterated instrument's list is a frontier, not an inventory — rounds 7–9 were masked behind 5), **F144 → E2 step 0, CLOSED** (the `&&` narrowing, recalibrated; expected-vs-actual exact), F141 (session-internal). **Phase 9 session E2 (F145–F148):** **F145** (nine slice-carrying typedefs, not six — the wrap artefact again), **F146 → S63** (fork-reachability splits a family: the shared object's in-fork half has only seam or lawful raw as end states), **F147** (four same-call hazards at accessor-minted roots, found by hand where shape B is blind), **F148 → G–H's step 0, the steward's ruling** (the detector learns *reads* before G–H scopes with it; the accessor-mint blindness and typedef blast radius recorded beside it). **Phase 9 session F (F149–F152), all four the steward's brief classes:** **F149 → S24's brief clause** (the phantom "16 uses outside md.rs" corresponded to no grep the tree can produce — the raw accessors had zero callers from birth; a workload count without its command beside it is presumed wrong), **F150** (the install loop installs the one live search into every slot; the screen-content variants have no installer — corrects F125's attribution — and the runtime-indexed dispatch is double-locked; `SetMeMethod` deleted, S18), **F151 → S54's whole-tree clause** (the fifteenth caller: a deletion's read grep spans tests, benches and foreign families; resolved by F139's ownership rule — the kernel relocates to its last caller's family with the ratchet rebaselined and the reason stated), **F152 → S58's visibility clause** (the census keys on spellings: T9.E7's interior-pointer respelling made the one live raw cost read invisible — F137's third instance — and a relocated kernel takes its census row with it; a respelling edits the census in the same commit, and a row that leaves by relocation is named in the log so shrink is never read as conversion). **Phase 9 session E3 (F153–F158):** **F153** (F84 was stale twice — T7.C7's fence stands and the pad kernels were never ported; a brief inheriting a finding inherits the finding's *date*), **F154 → S64** (the grid's 34 was a lower bound: `*const` parameters, a raw return, a type alias — enumerate the type, not one pointer spelling), **F155 → S64** (a planted fault proves coverage of the field it perturbs, not of the site — the QP average is GOM-row-inert; perturb what varies per site), **F156 → S64** (grep readers per field before re-routing a cached copy — `sDecPicView` was write-only from birth of its readers' seam), **F157** (a `static` array of fn items is a dispatch surface with no owner to key on — the fork-split walker's decisive arm, worth 84 bodies, shipped with `--no-slots` calibration), **F158** (the closing raw inventory: ctx 111/157 over 268 bodies + the in-fork layer are G's and move together; the VAA flag walks, residual cursors and MVD tables are X's, all three `SharedMbArray`-shaped; the dormant pointer is Phase 10's behind one named accessor). **Phase 9 session G (F159–F164):** **F159 → S66** (the steward's stale-driver guard aimed at a directory — fired on everything, and passed its positive-only calibration perfectly; calibrate on a known NEGATIVE; `q1c`/`forksplit` still owe negative calibrations), **F160 → D-dead-4, with the steward's correction** (the `pfIDct*` half was a stale gravestone comment — session F had already deleted the trio, and the finding inherited the comment's date, F153's class one level down; what executes in H's step 0 is `SharedMbArray::capture` plus the comment's correction), **F161 → S65** (135 of 266 hazards model a retag S63 forbids — the join of the two instruments is the work list, shipped as `phase9_ctx_join.py`), **F162 → D-fid-2** (kept byte-identical by ruling; revisited at J beside D4/D5), **F163 → S65** (the retag is by allocation, not type; "detector to zero" is not an exit criterion; the per-allocation classifier is the H/exit upgrade), **F164 → D-exit-2** (the send-seam is a phase-exit item by ruling; exit condition 1 amended). **Phase 9 session H (F165–F171):** **F165** (the accessor inventory bounded by one file — 27 bodies / 10 ST, not 21/7; S64's fifth instance, the steward's), **F166** (the ST column is a candidate list, not the flip list: `ParasetStrategy` stays raw permanently — flipping it is E0499 at ten sound-by-allocation sites; keeping it raw is what lets them flip), **F167** (the cached `m_pEncCtx` struct-field copy — sound pre-flip only because nothing minted `&mut` ctx; the dormant half's Miri verification owed), **F168** (E8's pair numbers were parallel all along — G's claim contradicted two standing records and the steward inherited it; settled by running both: 108 min serial / 58 min parallel), **F169+amendment** (`as *mut _` fails the ratchet +92/body; `from_mut` net-zero; implicit coercion wins — no spelling at all), **F170 → J** (wall-clock timing reports machine sleep as compute; `s61_report` shares the bug; CPU beside real), **F171 → S67, worker-minted** (real UB in a body outside the parameter family — the join read 0 LIVE for six stages while it existed; D-gate-4's second F114-class catch; the 24 out-of-family retags are the standing audit list). **Phase 9 session X (F172–F178):** **F172 → S68** (F86 misattributed its own abort and two briefs inherited the citation — check out the commit a finding names and read the line it quotes; resolved by measurement, `uiShortRefCount` max 2 vs threshold 6), **F173 → D-fid-3** (the gtest gate aborting since E3 — a Phase 9 safety conversion turned upstream's silent OOB read into an abort on non-2:1 simulcast; the exit gate was red for three sessions and nothing below exit runs it), **F174 → D-dead-6** (`pGomComplexity`, the second dead GOM sibling; accessor already S18-retired), **F175 → S59's escalation clause** (a doubly-thresholded verdict needs a verdict-level fault; report the pair), **F176** (the read grep six short on F119's own family — self-caught), **F177 → S64's writer clause** (a permanently-null field with live readers is invisible to every instrument; grep the reference's writer — two allocations restored matching upstream, `SetRefMbType`'s port filed), **F178 → J beside F170** (the ratchet counts prose; documentation currently fails gates). **Phase 9 session X2 (F179–F187):** **F179 → S68's absence clause** (`SetRefMbType` was ported 806 commits before the brief said 'never ported' — a claim of absence is the cheapest to verify and the easiest to write unverified), **F180/F182** (nine callers not five, four sites not three — S64 again, a file ungrepped), **F181 → S54's both-trees clause** (the 'unused' parameters carried eight dropped reference WelsLog calls — restored; deleting would have destroyed the evidence), **F183 → D-dead-7** (`pCurPath`, the third write-only field, F67's `*mut i8` row — ten → nine), **F184 → D-fid-4** (the C-ABI log-level divergence, a green test asserting it; the referee's level check is the acceptance), **F185 → D-gate-6 amended** (the battery at 977 s vs the 900-s cap — the Miri lane's parallelism taxes the native lane), **F186 → S69** (the referee's normalization was wider than the nondeterminism it hid), **F187** (six documented refusals within twenty lines of their sites — the estimate that ignored them ran 43% optimistic; the blocked ~36 fold into H2). **Phase 9 session H2 (F188–F196):** **F188/F189** (D-fid-4's sites were six, the sixth inside the acceptance instrument, which had to be repaired before it could accept — S66's family), **F190** (two docs state stale numbers about the instruments they describe — S68), **F191** (the in-fork "read routes" were four problems wearing one label; the largest cannot take a shared projection), **F192 → S56's clause** (F167's stored copy was UB — a reference argument is strongly protected for the call's duration, disjointness buys nothing; a re-stamped child tag survives the retag but not the protector; closed by deleting the storage), **F193** (two of the five APIs can never return slices — `codec_app_def.h`'s shapes), **F194** (the ratchet's per-file headroom decides where a probe may live — J's instrument list), **F195 → J** (the F67 probe counts types, not fields — the send-seam contingency table is re-derived by field at the exit), **F196 → ruled below** (`ctx_ltr_at` load-bearing at 75 sites, 83 borrow conflicts zero mechanical — T9.G7's 4 was a trip log; reverted under a pre-set rule). **Phase 9 session H3 (F197–F198):** **F197** (F196's own total never summed — 83, not 84; the breakdown was exact; a finding can be internally inconsistent at birth, and step-0 reproduction is what catches it), **F198** (the LTR planted fault: ltr 16/16 FAIL on the first try, mt 0/120 by preset design — a zero that is *explained* is a calibration, not a gap). Also recorded: the charter's split-borrow prediction was the steward's error (zero were needed — the in-fork accessors' `*mut` parameters meant every conflict was a coercion, dissolved by hoisting), and the briefs' "~2.5 min" commit-gate figure is 15–19 min in reality — J's brief carries the correction. **Phase 9 session J (F199–F207):** **F199** (F93 was one missing statement — `iTotalNumMbRec` never zeroed at fresh-picture prefetch; seven phases of gates never saw it because the ordinary path is identical), **F200** (the census tool's exact tag match silently dropped 16 annotated tags — the header everyone quoted undercounted; true count 551), **F201 → D-exit-3 executed** (278 retags into named lawful categories, a 306-item owned queue with a per-item map in `phase9_disposition.md`), **F202** (both instrument fixes calibrated both ways; `shim` and `transmute` close at zero — both had been comments recalling deleted code), **F203** (the crate-root `deny` found two files that escaped the per-file discipline for eight phases — 13 untagged decoder shims and the trace callback), **F204** (the log referee closes at 33/35 with the level sets agreeing; the two survivors unportable by construction), **F205** (the `!Sync` table by field, as F195 demanded: 7 of 65 fields, five owners), **F206** (the +59% perf reading is an instrument boundary — the C++ reference now dispatches NEON where the baseline chain is Rust-vs-Rust; not filed as a regression), **F207** (the exit battery's Miri half stopped by the user's direction — what that leaves unverified is enumerated, not glossed; the successor plan's exit battery covers it if adopted). |
...
### Phase 8b — Correctness and parity *(3–4 sessions; D-prio-1, 2026-08-21)*

*Inserted after Phase 8's close, under the direction that correctness and feature
parity come before safety and performance. It keeps Phase 9's number free because
690 in-code tags (`port-raw(Phase 9)`, `send-seam(Phase 9)`) name that phase.*

**What it is for.** Phase 8's gtest stretch measured parity from outside for the
first time — upstream's `test/api`, 199 tests, against the Rust cdylib: 118 at
T8.C5b, **155 after F82**. F82 taught the phase's method: an entry point with no
referee had neither half ported, and the failures tally by *assertion site*, not by
test name. The 44 that remain are seven features:

| family | gtest rows | assertion site | port site | reference |
|---|---|---|---|---|
| decoder `GetOption`/`SetOption` arms | **21** | `decode_api_test.cpp:45/81/132/204/348/404` ×3, `decoder_ec_test.cpp:701/789`, `encode_options_test.cpp:1990` | `decoder_get_opt_c` 4 arms, `decoder_set_opt_c` 6 arms (`api/codec_api.rs:~3334/~3258`); the rest fall to `_ => {}` and return success without writing | `welsDecoderExt.cpp:584–695` (17 get arms), `:479–584` (10 set arms), the per-call resets in `DecodeFrame2WithCtx` `:735–780` |
| LTR + `ForceIntraFrame` | **3** | `ltr_test.cpp:39` | `wels_encoder_ext.rs:1940` | `welsEncoderExt.cpp:491–520` + `ForceCodingIDR` |
| parameter-set listing strategies | **6** | `encode_options_test.cpp:556/656/705/772/844/911` | `CreateParametersetStrategy` → `None` for `SPS_LISTING`, `SPS_LISTING_AND_PPS_INCREASING`, `SPS_PPS_LISTING` (`paraset_strategy.rs:755` pins the refusal) | `paraset_strategy.cpp:404–713` |
| parse-only (`DecodeParser`) | **3** | `decode_api_test.cpp:923/1001/1048` | `decoder_decode_parser_c` stub (`codec_api.rs:~3459`); 12 + 7 parse-only sites already in `decoder_core.rs`/`decode_slice.rs`, `ParseOnlyBsBuffers` at `decoder_context.rs:646` | `welsDecoderExt.cpp:1180–1262`, `ParseAccessUnit` `:1301–1348`, parse-only branches in `decoder_core.cpp` (8), `au_parser.cpp` (4), `decode_slice.cpp` (5), `pic_queue.cpp`, `parse_mb_syn_cabac.cpp` |
| downsample + denoise | **3** | `HashFunctions.h:31` (`EncoderOutputTest` rows 4 denoise, 5 two layers, 7 four layers) | `METHOD_DOWNSAMPLE`/`METHOD_DENOISE` untranslated (`processing/mod.rs:29–39`; `wels_preprocess.rs:1583–1591`, `:1642–1647`); **`DownsamplePadding`'s `RET_NOTSUPPORTED` is dropped at both callers (`:1240`, `:1327`)** — silent | `codec/processing/src/denoise/*.cpp` 250 lines, `downsample/*.cpp` 594 lines |
| screen content | **7** | `BaseEncoderTest.cpp:92` ×5 (rows 8–12, all `SCREEN_CONTENT_REAL_TIME`), `encode_options_test.cpp:2195`, `encoder_test.cpp:323` | the D-scr-1 guard | **Phase 10's**; allowlisted with that owner |
| POC tiebreak | **1** | `HashFunctions.h:20` (`DecoderOutputTest/39`, `test_scalinglist_jm.264`) | `decoder_conformance_test.rs:238`, deliberate | **D-poc-1**: keep (JVT-correct) or match upstream — **decided 2026-08-22: keep** — the row is permanent, with the reason |

Beyond the gtest rows: **F80** (no asset known to drive it; measure, making the asset
with the reference encoder if `res/` has none), **the F77 instrument** (`MbGrid`
accessors `#[track_caller]` with both dimensions in the message), and **the
port-completeness census** — every C++ function definition under `codec/` checked for
a port counterpart by name, the missing ones classified by reading (dead in the
reference / renamed / missing) — because F80, the listing strategies and the two
plugins were all found by accident, and the census is what turns that class into a
list.

**Instruments first, in this order.** (1) The gtest tally becomes a gate: an allowlist
of named known failures (test · owner · reason), `--check` failing on any failure
outside it **and on any listed row that passes** (stale rows), wired into `exit`; a
`--filter` passthrough for per-family runs. (2) A diffharness `dl` preset
(`iSpatialLayerNum` 2/4 × denoise on/off × rc modes) lands *with* the downsample +
denoise port and must be red before it. (3) `tools/port_census.py`. (4) Every decoder
entry point has a referee by the close — `DecodeParser` gets one when parse-only
lands, as `DecodeFrameNoDelay` got `ecref --nodelay`.

**Sessions.** **A — done** (`925ac828`..`aeb011db`): 155→179→165, the option arms,
`ForceCodingIDR`, the census, F80/F87, the F77 instrument. **B — done**
(`b2c5cd7e`..`65042ac9`): the seed pin (S49, F89), `DecodeParser` whole (four missing
pieces; S47 discharged), all five listing strategies, F88 to seed 5, F96. 164→173 at
the pinned seed. **C**: downsample + denoise + the `dl` preset (17 rows) — **measure
`_c` vs the reference's NEON downsampler before anything** — F80/F87 as grow/shrink on
`safe::Pool`, the census's 12, F96, F88, F93. D if needed: the close. *Exit gate: `gtest --check` green with an allowlist of exactly the Phase 10 rows
and D-poc-1's; the census's missing list empty or every entry owned by name; `sp` in
the sweep; `exit` battery PASS unscoped; **sweeps, Miri, benches and the one perf span run
only here (D-gate-3)** — fast gates between.* Charter: [`prompts/phase8b.md`](prompts/phase8b.md).

### Phase 9 — Safety endgame *(reanalyzed 2026-08-22 then refined by session A's census to 12–17 sessions; the old "2–3" predated F66/F73 and the cursor coupling, and the first "10–14" undercounted — F101)*

*The full scope and session plan are the charter, [`prompts/phase9.md`](prompts/phase9.md). In brief: the encoder is already `deny`-everywhere behind 695 `port-raw` + 61 `cursor` tagged allows at the charter (**638 + 61 after session D**); Phase 9 retires them by converting the raw cursors (coeff → SMbCache → planes, roots first and then the shared tables that F104 double-gates → layer/pic) so the `*mut sWelsEncCtx` conversion (F66, blocked today) becomes legal and the **147** body-only `unsafe fn` (F101 — not the 419 first counted) fall out, then de-virtualizes the 22 dispatch survivors, finishes `common/`, and does the crate-root deny/`libc`/clippy/Miri-marathon cleanup with D4/D5 to the user. The kernels are already safe — the `unsafe` is shims wrapping them for raw callers, so `common/` retires with the encoder, not independently.*
...
**Inherited from Phase 8 (written session C, 2026-08-21; every count re-grepped
against the tree at the phase close, not carried from a brief).**

* **`#![deny(unsafe_code)]` at crate root: 67 of 80 modules are already there** —
  **61** deny and **6** forbid — and **`src/api/` is among them since T8.C7**, so
  §2.2.8's "single scoped allow on `src/api/`" is **superseded**: the api modules deny
  like everything else, with 44 individually tagged allow items. The **13** modules
  still without a declaration are `lib.rs`, `api/mod.rs`, `safe/prng.rs`,
  `common/mod.rs`, and nine of `common/` (`sad_common`, `mc`, `copy_mb`, `expand_pic`,
...
* **F66 / S42 — the context-parameter split.** Session J *made* the ~109-function
  ...
  invalidates the whole context at every offset, so the conversion cannot land until
  the context is split. **93 context-derived-pointer sites in 28 callers** is the
  measured surface. This is the item the `cursor` tag and the `send-seam` both wait on.
* **F73 — the picture-accessor `&mut` family, 32 + 68 sites**, open since Phase 7
  session C, with the MT Miri probe's `#[cfg_attr(miri, ignore)]` citing it. It retires
  with F66's split, and the probe's ignore retires with it.
* **`Send`: neither boundary object is, and the inventory is measured** — **14
  `E0277`s, 3 for `Decoder` and 11 for `Encoder`**, every one a raw pointer in the
  `port-raw(Phase 9)` tree, recorded by a probe in `api/codec_api.rs`'s `send_verdict`
  module that can report *both* answers. The day the last `port-raw` tag goes, that
  test's assertions flip and it says so.
* **F80 (new, Phase 8 session C)** — `IncreasePicBuff` / `DecreasePicBuff` have never
  been ported, so `WelsRequestMem`'s third arm (same resolution, changed ref-list size)
  is absent. **First measure whether any asset drives it**; it may be as unreachable as
  `dsOutOfMemory` turned out to be, and if it is, the site should say so.
* **The F77 instrument.** A correct index into a stale allocation is indistinguishable
  from an off-by-one in this port's panic messages, and that cost session B a wrong
  diagnosis and session C a detour. `MbGrid`'s accessors want `#[track_caller]` with an
  explicit `assert!(idx < len)` permanently — it names the call site instead of
  `mb_grid.rs:277` — and the grid/picture dimension pair wants to be in the message.
* **The gtest tally is the encoder's to-do list.** `tools/abi_harness/gtest_stretch.sh`
  reports **155/199** against the reference's 199/199. **Tally the failures by where
  the assertion lives, not by test name** — the fixtures are encode->decode round
  trips, and reading them by name is what hid F82 for a day. By assertion file the 44
  are: `decode_api_test.cpp` **21** (decoder `GetOption` values — `DECODER_OPTION_
  FRAME_NUM` returns 9 where upstream returns -1, and the `Tid`/`IDR`/`IsRefPic`/`LTR`
  family with it), `encode_options_test.cpp` **8**, `BaseEncoderTest.cpp` **5**
  (`InitializeExt` rejecting parameter sets the reference accepts), `HashFunctions.h`
  **4** (three `EncoderOutputTest` hashes plus the documented POC tiebreak),
  `ltr_test.cpp` **3**, `decoder_ec_test.cpp` **2**, `encoder_test.cpp` **1** (one of
  which is screen content, Phase 10's guard). Plus `DecodeParser`'s **3**, which are
  the stub. Run it before and after; it is a number that moves, and it moved 37 in one
  commit.
* **D4 (workspace split) — revisit with better information than the plan had.** The
  original argument was that a split turns `deny`+`allow` into a true
  `#![forbid(unsafe_code)]` core crate. Phase 8 makes the boundary crate's contents
  precise: the seven exports, 19 thunks, 44 allow items, 51 layout pins, and the
  harness that tests it from outside. **The split is now a mechanical extraction**
  rather than a design question — and `tools/abi_exports.sh` already asserts what the
  boundary crate must export.
* **D5 (naming) — one question smaller.** T8.C7 renamed the decoder-internal
  `SParserBsInfo` to `ParseOnlyBsBuffers` because it was an *ambiguity* (two entities
  under one name) rather than a style choice; the boundary struct keeps its name, which
  is why it was not an ABI change. What D5 inherits is only the Wels-style question:
  whether an owned decoder type should carry the `S` prefix at all.
* **Perf: D-perf-6's parked recovery, cumulative encode ≈ +15…+17%** against the
  **+25%** median tripwire, unmoved by Phases 7 and 8 (session C's span: decode
  **+0.04%**, encode **+0.00%**, zero rows over 5%). The parked families are Phase 6's
  list above. Phase 8 added a `catch_unwind` window to every decode and encode entry
  and **it did not register on either bench** — that is measured, not assumed, and it
  means P13's guard is not a recovery target.
* **A perf blind spot named at Phase 8b's close (session C):** the downsample and
  denoise kernels have **no bench row** — `decode_1080p_bench` is a decoder and
  `c_vs_rust_bench` encodes one spatial layer with denoise off, so neither reaches
  them. Their per-frame cost is unmeasured by construction — the same gap that hid
  the *features* for eight phases (D-prio-1). The correctness referee exists (the `dl`
  sweep preset, 76 configs); the perf pass should add a `dl` bench row so the multi-
  layer path is measured, not assumed. Detail in [`perf_baseline.md`](perf_baseline.md).
* **The absent instrument is still absent.** `gates.sh` says `SKIP  fuzz corpus
  replay` on every run because the fuzz crate (Phase 0 T7) was never built. F77 is the
  second finding in this project a fuzzer would have found first (§0's tally). Phase 9
  is the last phase that can build it before the ABI is frozen by consumers.
...
**Runs after Phase 9 (D-prio-1 as amended 2026-08-22; it was sequenced before 9 for one day)**; nothing in it blocks or is blocked by Phases 7–9
...
**Decision D-mt-2 (2026-08-20, steward, ruling F72): the load-balancing path is completed, not fenced.** The port has the consumer of the feedback loop (`DynamicAdjustSlicing`) and not the producer (`CalcSliceComplexRatio` is never called), and the feature is **on by default** via `GetDefaultParams` — so a fence leaves every default-configured drop-in user with degenerate balancing silently. Completion is one call at the C++'s own site. The feature can never be byte-gated (two runs of the C++ reference alone differ — frame N+1's boundaries read frame N's slice times), so it becomes the project's **second expected-divergent class** beside `CABA2_SVA_B`: recorded in the referee's annotations, structural coverage only (one probe configuration — no crash, sane slice counts, Miri where cheap), both diffharness drivers keeping `bUseLoadBalancing = false`. Session C executes.

**Decision D-api-1 (2026-08-21, steward, ruling F79's stated divergence): the default trace sink matches upstream.** `welsCodecTrace()` upstream installs `welsStderrTrace` at `WELS_LOG_WARNING`; Phase 8 session B's port defaulted to `None` and recorded the divergence. A drop-in that is silent where the reference speaks is a divergence a C consumer can observe, and consumers who want silence install a callback exactly as they do against the reference. Phase 8 session C implements the stderr writer as the default and covers it. **And F77 is session C's, not Phase 9's**: a decoder panic that aborts a C host process on one malformed stream (`res/Error_I_P.264`, a CAVLC I-slice index one past the end) is a boundary defect — the asset joins the corpus, the index reaches the C++'s error code (P13's first line), and every thunk catches unwinds and maps them to the entry's failure code (P13's guard, profile-aware).

**Decision D-prio-1 (2026-08-21, the user, on reading Phase 8 session C's report): correctness and feature parity come before safety and performance — "let's get things working first".** Consequences, recorded by the steward: a **Phase 8b** (correctness and parity) runs now; **Phase 10** (screen content, D-scr-1 — a feature gap like the others, with 7 gtest rows of its own) follows it; **Phase 9** (the hygiene/safety endgame: the 689 `port-raw(Phase 9)` items, the F66/S42 context split, F73, the `Send` verdict, the crate-root deny, D4/D5, and D-perf-6's recovery at its close) runs after both and keeps its number because the in-code tags name it. Every reference to "Phase 9's perf pass" written before this date still means Phase 9. What the ordering does not change: D-gate-1 (no mid-phase perf work), D-gate-2's phase-exit Miri, D-exit-1's tag regime for any new raw signature, and the rule that parity is proved against the reference's own answers (S47/S48). **Amended 2026-08-22 (steward, on the user's question whether to swap 8b and 9): Phase 10 runs after Phase 9, not before — its FME half is encoder-internal code that would otherwise be written raw and converted again, and Phase 9 cannot verify a conversion of the dormant search half until Phase 10 makes it reachable; 8b stays ahead of 9 because its work lands in already-safe code (decoder, processing) or adds a dozen tagged signatures at most, and because it builds the referees — the gtest gate, the census, the `dl` preset, `ecref --options` — that Phase 9's ~700 encoder conversions need in place first.**

**Decision D-mt-3 (2026-08-22, steward, ruling F107 — the reconstruction-write design; confirmed by the user 2026-08-23, option A kept): the multi-threaded reconstruction plane is written through one shared interior-mutable view, option (A).** F107 measured the three facts the decision rests on: only one of four MT slice modes is row-aligned (so `split_at_mut` bands cannot be the mechanism), a worker reads only the macroblocks it wrote (so the byte sets are disjoint), and yet **no `&mut [u8]` can express that disjointness** because a macroblock's contiguous span is the full width of its rows — `&mut SPicture` per worker (F73), a per-macroblock `PlaneCursorMut`, and row bands all fail the same way. The ruling: session C builds one seam type over each reconstruction plane before the fork, from `&mut PaddedPlane` (so the plane is exclusively borrowed for the fork's scope, which is what retires F73), holding the buffer in an `UnsafeCell` under **one** tagged `unsafe impl Sync`; workers share `&SharedPlane` and write through `&self` (`set`, `write_row`), reads keep working. This is D-mt-1's shape a second time — one named seam, safe consumers, a Miri **data-race** probe that now checks a real overlapping access rather than a retag — and it earns a lawful `unsafe-cat` category (`recon-seam`) alongside `send-seam`. The copy / IDCT-add / deblock kernels that write the plane gain a write-through-`&self` flavour (`row_mut` cannot exist on a shared view). **Acceptance:** the MT Miri probe loses its `#[cfg_attr(miri, ignore)]` **and** a second probe on a mid-row slice boundary (`SM_RASTER_SLICE` uneven, or `SM_SIZELIMITED_SLICE` threads=2) passes — the existing probe drives the one row-aligned mode, i.e. the easy case. **Not taken: option (C)** — keeping the raw seam and declaring the reconstruction write a lawful raw boundary. It is cheaper and it is the one lever that shrinks the endgame, but it reduces the definition of done (§2.3: every module outside `src/api/` unsafe-free) by admitting raw pointers into the hot encoder path permanently, against the project's drop-in-*safe* premise; option (A) keeps the goal and is consistent with D-mt-1. If the smaller scope is wanted, this is the place to say so.

> **Amended by session C (F131, F132), 2026-08-23 — two corrections, both measured.**
>
> **(a) The seam is a *picture* view, not a plane view.** The ruling above scopes it as
> "one seam type over each reconstruction plane". Of the 14 in-frame `layer_dec_pic*`
> call sites only **4** reach a plane; the other 10 reach four *per-macroblock side
> arrays* of `SPicture` — `sMvList`, `pRefMbQp`, `pMbSkipSad`, `uiRefMbType` — where a
> `&mut Vec<T>` is a `Unique` retag over the whole array however narrow the write, i.e.
> F107 §3's argument verbatim, one storage class over. A plane-only view cannot carry
> them, and the *first* thing Miri reports on the ignored probe is a race on `SRefList`
> itself (`SRefList::pic_mut(&mut self)`, two workers), not on any pixel. `RecPicView`
> therefore carries all seven storages behind one `UnsafeCell`-crossing accessor and one
> `unsafe impl Sync`, both `recon-seam`. **`recon-seam` is a lawful category from
> `85a525d3`.**
>
> **(b) The acceptance was assigned to the wrong session.** "The MT Miri probe passes" is
> a property of the *whole fork*, not of the reconstruction write. Measured by fixing
> what Miri names and re-running, six times: the fork shares **six** families — the
> reconstruction picture, `Option<Box<SStrideTables>>`, `SWelsSvcRc::pGomCost`,
> `SDqLayer::sSliceEncCtx`, `SMB` (deblocking's cross-slice neighbour read against the
> encoder's whole-record `&mut`), and `SSliceCtx::pOverallMbMap` (rewritten in-fork by
> `AddSliceBoundary`). Session C fixed the first four; the last two are **session E's**
> and the slice structures' respectively, and two of the six (`pGomCost`, `pOverallMbMap`)
> are genuine races in upstream OpenH264 rather than port-introduced over-claims — see
> **F133** for the first, which races on storage that nothing in either codebase reads.
> The acceptance that session C *can* meet, and did: the seam's own two data-race probes
> pass under Miri with the overlap calibration run both ways, and the mid-row probe exists,
> passes as a behaviour test, asserts its boundary from the bitstream, and names what
> stops it. The encoder probes' `#[cfg_attr(miri, ignore)]` retires with families 4-6.

**D-gate-3 (the user, 2026-08-22): Phase 8b gating — the sweeps and Miri run once, at the phase close; everything between is fast.** Per commit: `gates.sh commit` (~2–3 min: build all targets, `cargo test` debug + release — which already carries conformance 60/60 and the in-process malformed corpus — ratchet, duplicate census) plus the in-tree parity test the commit adds. Per session close: `gates.sh commit` + `tools/abi_harness/gtest_stretch.sh --check` (~40 s once linked) + `abi_exports.sh` + `abi_harness/run.sh` (~70 s) + `tools/ecref/compare_all.sh` when the session touched the decoder — **no sweeps, no Miri, no benches, no span**. A family that could move bytes on a sweep configuration (LTR, slicing, rc) runs *one* targeted diffharness configuration (a single `rust_enc` vs `cxx_enc` pair, seconds), never the preset. At the phase close: `gates.sh exit` unscoped — sweeps 369/369 in both profiles, both benches, Miri `--lib` whole library (~30 min) and the differential tests — plus the one 7-pair span with its null. Same reason as D-gate-1: the sweeps take the better part of an hour across both profiles and Miri ≈30 min, and a parity fix has a referee of its own (its in-tree parity test, its gtest row) that is faster and more direct than either; a divergence found at the close is bisected across the phase's commits, which stay small. What does not move: byte-exactness is never traded, the ratchet is per commit, and `SKIP_SWEEP=1` is named in the log whenever it is used.

**D-gate-4 (the user, 2026-08-22, on session D's F114): Phase 9 sessions run the encoder-scoped Miri `--lib` step at every session close.** The trigger: session D's ten commits passed `gates.sh family` five times — 535/535 in both profiles, every byte in place — and an encoder-scoped Miri run afterwards found a hazard the session had *introduced*: the flipped residual slots take `&mut [i16]` of the whole `sCoeffLevel` field, so every slot call retags the field and any raw cursor held across it is dead (F114, T9.D11 is the fix). That is S37's sentence — *the byte gates read clean over a tree carrying the class; only Miri sees it* — and Phase 9 is the phase made of exactly such changes, so a per-phase Miri (D-gate-2's shape, written for Phase 6's encoder sessions) is the wrong cadence here. The instrument: `gates.sh` gains a **`session`** level — `family` + Miri `--lib`, **no benches** (D-gate-1 keeps perf at the phase close; the benches' bit-identity half is already the sweeps') — run as `MIRI_SCOPE=encoder bash rust/tools/gates.sh session` (≈2.5 min family + Miri **894 s** measured at `0bfc7687` — 274 tests, the decoder's 101 skipped, the four encoder probes most of it; the steward's first "≈3 min" was Phase 6's number and wrong by 5x). A session that touches decoder code drops the scope. The unscoped step, the differential Miri and the benches stay at the phase exit (`gates.sh exit`), unchanged. **The verdict names the commit it ran at**: code landing afterwards re-runs the gate in full (B3 caught its own close one commit stale and re-ran at `16a18859`); after a docs-only commit the cheap corroboration B4 used — the ratchet and one sweep re-run at HEAD, with the full verdict cited at its own commit — is sufficient, said in those words. Not a reversal of D-gate-3: that was Phase 8b's parity gating, where the code changed was already safe and Miri could find nothing a sweep would not; here the code changes *are* aliasing changes.

**D-dead-1 (the user, 2026-08-23, ruling F115 by its recommendation): the sub-8x8 mode-decision trio is deleted.** `WelsMdP4x4`/`WelsMdP8x4`/`WelsMdP4x8` — no caller in the port, `#if 0 //Disable for sub8x8 modes for now` upstream (`svc_mode_decision.cpp:635–655`) — go under S18 with the upstream guard quoted as the reason (~210 raw lines, 4 tags, 6 `SPicData` reads off B3's plate). If sub-8x8 partitions ever return they return as a feature port with a referee, Phase 10's pattern.

**D-cov-1 (the user, 2026-08-23, ruling F119 by its recommendation): the phase-2 kernel differential harness is rewritten to drive the safe kernels, then the 22 `mc.rs` shims are deleted.** The parity evidence survives on the side that is actually live (the decoder's MC calls `mc_luma`/`mc_chroma` directly); the shims — 13 of which existed only as the harness's raw entry points — then delete cleanly and `common/mc.rs` can reach `#![deny(unsafe_code)]`. Both land in session B3. **Steward's correction (2026-08-23, scoping B3): the premise was wrong and the execution is cheaper than ruled.** `tests/kernels_differential_phase2.rs` has **no C++ linkage**; its own header says raw-vs-safe equivalences are deliberately short-lived and the mc set was proven at `46053993` and deleted then. The sole surviving mc test (`mc_shims_stay_inside_the_spans_they_declare`) checks the *shims'* span contracts and dies with its subject. Corrected execution: delete the shims and the span test together, doctrine and `46053993` quoted; no harness rewrite exists to do. The question put to the user repeated F119's unread claim — S24's shape, the steward's fourth miss; the user's intent (keep parity coverage) is preserved trivially because that coverage never lived there.

**D-ref-1 (the user, 2026-08-23, on the measured costs — ~1 s on a ~40 s sweep): the diffharness gains a `bg` preset.** `bEnableBackgroundDetection = true` in both drivers plus one preset in `sweep.sh` (~8–16 rows, `ltr`'s shape), so the background-detection paths — 17 of the plane family's 25 remaining convertible sites, plus F117's copy — get a byte referee for the first time. A failing row there is found parity work, not new risk; the row count rebaselines from 535 with the reason recorded. First step of the next session (B4).

**D-dead-2 (the user, 2026-08-23, ruling F122 by its recommendation): D-dead-1 extends to its closure.** The consumers the producer deletion orphaned go — `WelsMdInterMbRefinement`'s three sub-8x8 arms (converted in T9.B28 with no gate behind them), `UpdateP4x4MotionInfo`/`UpdateP8x4MotionInfo`/`UpdateP4x8MotionInfo`, `SWelsMD`'s `sMe4x4`/`sMe8x4`/`sMe4x8`, and the `SUB_MB_TYPE_*` arms of both entropy writers — upstream's own `#if 0` makes every one unreachable there too. Sweep-neutral. Executes S18's F122 clause: the straggler sweep runs *with* a deletion, not at the phase exit.

**D-gate-5 (the user, 2026-08-24, ruling F140 by its recommendation): the two full-encode Miri probes shrink under `cfg!(miri)`, on the house `scale()` pattern.** `encode_loop_runs_over_a_macroblock_grid_…` and `…size_limited_dynamic_slices_…` drive a smaller resolution/frame count under Miri only — the load-bearing assertions stay (the mid-row boundary asserted from the bitstream, a macroblock-count floor), full size still runs on every plain `cargo test`, and the phase exit runs whatever the exit battery has always run. Target: the session Miri step back under ~20 min. Rationale: both historical session-scope catches (F114's two UB, C's six fork families) fired within the first frames — the detection is about shapes, not volume — and E/G–H, the riskiest aliasing work left, keep their per-session net. Execution is the next session's step 0, together with S61's wall-time tripwire in `gates.sh`, and **one clean re-scoped Miri run is the precondition for building on C/C2** (the encoder set is unverified since `bdf425fd`).

**D-gate-6 (the user, 2026-08-24, mid-session E): the whole session gate is capped at 15 minutes.** D-gate-5's shrink alone could not meet reality — F140's "two full-encode probes" were **three** (the CAVLC/fine-MD probe, 216.5 s, unnamed by F140), and the target was unreachable under the probes' own realloc floor. Met by T9.E2's re-architecture: the Miri step runs as a **parallel background lane** beside the native battery — one compile, then concurrent per-probe shards, encoder-scoped by construction, small drives at session scope — with `MIRI_FULL=1` restoring the full drives at the `full`/`exit` tiers. Validated at 542 s; session E's close ran **517 s (8:37)**. That first validated run also paid C/C2's aliasing-evidence debt. The fork probes' standing cost is measured and recorded in the D-gate-6 block: **3356 s / 3449 s, ~57 min as a parallel pair** — the explicit full-drive fork-probe run E2's exit requires.

**D-gate-7 (the user, 2026-08-24, at session E's close): every encoder Miri probe runs as its own invocation with its own verdict line, and the CABAC/low-complexity grid probe is `#[cfg_attr(miri, ignore)]`.** Its axes are covered more deeply elsewhere — the size-limited probe drives the CABAC writers plus their stash/restore arm, and the fork probes the threading — and it keeps its full native drive on every plain `cargo test`. Landed as T9.E9.

**D-poc-1 (the user, 2026-08-22, on the steward's recommendation): the port keeps the JVT-correct POC tiebreak.** `test_scalinglist_jm.264` keeps two pictures buffered at end of stream; upstream's flush gives them the same `uiDecodingTimeStamp` and `ReleaseBufferedReadyPictureNoReorder` falls back to slot order, emitting POC 8 before POC 6; the port breaks the tie by POC (`decoder_conformance_test.rs:238`), the direction the JVT gold confirms for `CABA2_SVA_B`, which upstream also fails. The cost accepted: one permanent allowlist row (`DecoderOutputTest.CompareOutput/39`) and a conformance hash that differs from libopenh264's on streams whose buffered pictures share a timestamp at flush. Not a precedent for matching the reference's other defects case by case — a divergence is still bought with a measurement and a finding (S48's spirit), and this one has both.


**D-scope-1 (the user, 2026-08-25, after session F's verification): sessions are cut large, with strict drop-from-the-end; the Phase 9 board re-cut from five-plus sessions to four planned.** The re-cut: **E3** takes the grid and the harvest *plus* F84's S18 deletions and G's ground-laying (the read-shape detector, F148; the S63 ctx fork-split census) as droppable tail steps — measurement and tooling only, zero ctx conversions; **X1–X2 collapse to one X** absorbing F86-open (its own file's conversion), F100 (trace is X's family; the log-capture referee is new work), and the already-assigned F117/F151; **G–H becomes G** with its step 0 pre-paid by E3 and **H demoted to a spillover name**, as X2 is; **session I dissolves** — F84 to E3, F86/F100 to X, the au_parser cluster F91/F92/F93 to J's step 0, where the exit battery that proves them runs anyway. Planned phase total: 15 sessions (11 done + E3, X, G, J), worst case 17 if both spillovers fire. The guard rails that make large cuts safe are already rules: drop-from-the-end with the frontier named (S60/F143), stage-boundary stopping points (E2's model), and the standing gating directive (byte gates between commits, Miri once at the close). **F133 must be ruled before G's brief is written** — it is the one decision a large G would otherwise carry as dead weight.


**D-dead-3 (the user, 2026-08-25): `pGomCost` is deleted.** The write-only GOM cost accumulator — F133's upstream data race, F139's write-only measurement, `Vec<AtomicI32>` since T9.C5 — goes whole: the field, its allocation, the per-frame zeroing and the per-MB accumulate, with the read grep quoted for both trees in the commit (upstream's five sites are declare/alloc/null/zero/accumulate — `ratectl.cpp:79/90/669/1273`, `rc.h:191` — and its three sibling GOM arrays are the live mechanism with ten uses). The seven comment cross-references update and `assert_size!(SWelsSvcRc, 440)` re-pins with the reason. Byte-neutral by construction; executed as session G's step 0. **This ends F133** — the port keeps no artifact of the race, and the fork's per-MB loop drops its one pointless atomic.


**D-dead-4 (the user, 2026-08-25): D-dead-3's ground extends to F160's orphans — with the steward's correction to their number.** The user ruled "delete both"; verification found item 2 already executed: the `pfIDct*` trio was deleted by session F's step 0 (the installer's own note, `decode_mb_aux.rs:482`-region, carries the read greps), and the gravestone comment twenty lines above it (`decode_mb_aux.rs:~460`) still claimed the slots existed — G's worker read that comment and F160's item 2 inherited its date (F153's class, one level down: a *finding* inheriting a *comment's* date). Execution, session H step 0: delete `SharedMbArray::capture` (`rec_view.rs:348`, zero callers verified) and correct the stale gravestone. 

**D-fid-2 (the user, 2026-08-25): the port keeps upstream's one-past-the-end read at `WelsEncoderEncodeExt`'s error tail (F162), byte-identical.** Upstream does it twice (`encoder_ext.cpp:4109-4110`); the port's spelling is a raw derivation living inside one statement, commented, hazard-free. An index would panic where the pointer reads, and a panic is not byte-identical. Revisit at session J beside D4/D5, where the port's deliberate divergences are decided together.
...
**D-fid-5 (the user, 2026-08-27, mid-session J, F91): the port never mutates the caller's input buffer.** Upstream's parse-only slice-extension arm rewrites the NAL type byte in the application's own memory before copying it out; the port diverges — it performs the rewrite on its own copy — and the divergence is blessed with the golden referee showing outputs identical. A library scribbling on caller-owned memory is upstream's defect, not a behavior to preserve.

**D-fid-6 (the user, 2026-08-27, mid-session J, F92): the subset-SPS rewrite's three behaviours are blessed — the bounded writer, the refusal, and the escaped length** — and the previously unreachable escape arm now has a synthetic referee (no `res/` asset reaches it; one test now does). Recorded with F92's table in the session log.


**D-plan-1 (the user, 2026-08-27): the safe-conversion execution plan is adopted as the operative plan** ([safe_conversion_execution_plan.md](safe_conversion_execution_plan.md)), with the steward's four adoption amendments folded in as its §10 and the session map amended larger at the user's direction — **four stage-aligned sessions (S1–S4), slack to six**, checkpoints unchanged as individually gated commits, ordered drop-from-the-end. This supersedes D-exit-3's permanence reading: `fork-shared(S63)`'s 204 items and the 306-item owned queue become S1–S2's work; the two D-mt seam impls stay as the plan's D1 (rec_view's `Sync`, the job handle's `Send`); Phase 10 runs after, landing screen content on safe storage. F207's deferred Miri battery folds into the plan's E3 so it runs once. The Phase 9 corpus — rules S1–S69, findings F1–F207, the tag discipline, the F3 protocol, the brief style — remains binding throughout.


**D-gate-7 (the user, 2026-08-28, before S5): the hour-scale fork-pair probes are struck from the per-session regime — small Miri runs of unit tests are the ceiling.** The `fork_join_encodes` pair costs ~59 minutes as a parallel pair (F225's first tripwired measurement) and does not fit the user's time budget. MT-seam verification is therefore the session lane's Miri shards plus **second-scale purpose-built probes in F226's pattern** — drop the encoder, hand-build the racing shape; F226 bought an ~8-hour test's verdict for 1.24 s — one per seam moved, named under `encoder::` so the lane keeps running them. The pair remains an exit-battery instrument (E3), not a session gate. Priced consequence, stated plainly: F223's ladder still holds — a worker-vs-worker race in a shape no targeted probe models goes unseen until E3. This also subsumes S4's two hand-off debts: the stage-C pair verdict is waived to E3, and E2a's sweep verdict is covered by S5's first family-level run without a dedicated duty.


**D-scope-5 (the user, 2026-08-28, at S5's close, from F229/F233): the screen-content feature family is descoped from the safe-conversion plan — neither converted nor deleted.** `SScreenBlockFeatureStorage` is structurally unreachable (its allocator was deleted at T7.C6; `PerformFMEPreprocess` has zero call sites; the storage field is never assigned), so converting its fields would invent an ownership shape — `pFeatureOfBlockPointer` borrows a buffer with no owner in this tree — on a path no instrument can referee, for a measured yield of two allows and zero sealed files (F229). Deletion is rejected on F233's fact: `SCREEN_CONTENT_REAL_TIME` is public API and the search tree is API-reachable — reachable enough to have contained a live null dereference, fixed by type at S5.C6d — so deletion would silently downgrade real callers to camera-content search with no instrument able to see it. The family stays raw and tagged `SCREEN_CONTENT(dormant)`, its sites enumerated into the census allowlist at E2, and the plan's end state (§1 items 1, 2 and 6) carries the carve-out explicitly. The conversion belongs to a successor screen-content effort that **nothing currently schedules**: the old plan called that effort "Phase 10", but the operative plan has stages, not phases (the user's correction, recorded so the label cannot mislead again) — D-plan-1's "Phase 10 runs after" sentence is to be read as "a future effort, not scheduled here".

**D-scope-5 REVERSED (the user, 2026-08-28, the same day): the screen-content family CONVERTS.** The descope stood for hours; the user changed their mind and the family is in scope for S6, user-directed. The ownership question that stopped S5 is answered by the steward's recommendation, recorded here so no session stops on it again: **full ownership** — `SScreenBlockFeatureStorage` owns all five buffers (`feature_of_block: Vec<u16>`, `times_of_feature_value: Vec<u32>`, the location arena as `Vec<u16>`, and the two per-feature pointer tables — `pLocationOfFeature` and the roving `pFeatureValuePointerList` — become **index tables** into that arena, `Vec<usize>` base + cursor). F229's objection ("the shape names an owner that does not exist") dissolves under ownership: the storage *is* the owner, and `PerformFMEPreprocess` — zero call sites, signature free — takes the feature buffer by value and moves it in, preserving upstream's "stores the caller's buffer" semantics without aliasing. F229's second objection (no instrument can referee a mistake) is answered by making the conversion **create the referee**: owned storage is constructible in a unit test, so the checkpoint ships the family's first-ever test — hand-build a small storage, run `CalculateFeatureOfBlock`, assert the arena invariants (histogram sums to the block count; every cursor ends exactly `2 × times[feature]` past its base; every written location lands inside the arena). The dormant discipline still binds: logic line-for-line, reachable behavior unchanged, the sweep proves nothing here. The three dispatch typedefs (`pfCalculateBlockFeatureOfFrame`, `pfInitializeHashforFeature`, `pfFillQpelLocationByFeatureValue`) and their `_c` bodies convert with the fields.

**D-exit-4 (the user, 2026-08-31, at the remaining-work scoping): the test-instrument allows retire by deny-plus-enumeration, in place.** The ~39 `instrument(test)` allows (Miri probes and provenance tests, ~17 files) cannot become safe without unmaking what they verify — the provenance probes dereference raw pointers because raw-pointer semantics are their subject, and they must stay in `--lib` or the Miri lane stops running them. At the final seal those files take `#![deny(unsafe_code)]` instead of `forbid`, with every test-instrument allow enumerated in the pinned census under its tag. Product code in those files can grow no unsafe; the census pins the exact set; the probes keep refereeing. Consolidating probes into dedicated modules was considered and declined — probes stay beside the code they watch. The plan's end state (§1 items 1 and 6) is amended accordingly, and the tracking number's honest floor is the enumerated instrument set, not zero.

**D-scope-6 (the user, 2026-08-31): the 23 `SCREEN_CONTENT(dormant)` casts convert to safe form.** The `SVAAFrameInfoExt` downcast arm is structurally unreachable (nothing in the port allocates an `Ext` — F177), so the downcast converts to a safe shape (an enum or an unreachable-guarded accessor), line-for-line, keeping working scaffolding for any future screen-content effort. Deletion was declined (dark code has no instrument watching a deletion) and leaving them enumerated was declined (the floor should not carry a dormant arm's unsafe forever). Dark-code discipline binds: the sweep proves nothing here; the compiler and review are the referees.

**D-gate-8 (the user, 2026-08-31): benches remain E3-only.** Reaffirmed at scoping with S10's 7% decoder catch known — the first bench in thirty checkpoints found a real regression the byte sweep could never see, and the user still keeps the full bench debt at the exit battery rather than paying a per-session close. Recorded so no session re-litigates it: the bisection range for anything E3 finds is everything since S5, priced knowingly, mitigated by per-checkpoint commits.

**D-gate-9 (the user, mid-session S11, given twice): only small probes, no full Miri — the per-checkpoint Miri lane is struck for the remainder of the conversion work.** S70's rule (reference conversions gate at `session`) is superseded for S11: per-checkpoint verification is the differential sweep (847/847 configs, both profiles, byte referee), `cargo test --lib`, the span scanner, the cascade to convergence, and the ratchet — with small targeted Miri probes where a seam moved. The honest consequence, recorded so E3 is priced correctly: the aliasing class only Miri sees was watched across 53 gated checkpoints by the span scanner and targeted probes alone, so E3's full Miri run carries that verification debt on top of the fork pair (F273: the pair is 73% of the lane's budget).

**D-gate-8 amended (the user, at S11's close): benchmarks are postponed "till we're done with the refactoring" — that is, to E3.** The debt stands at **81 checkpoints**. The postponement's own condition is met at S11.52: the conversion queue (`port-raw(Phase 9)`) is empty, so the next session IS the exit battery and the benches run there or nowhere.

**D-scope-6 amended (recorded at the steward's S11 verification): 22 of the 23 screen-content casts DISSOLVED rather than converted** — S11.3 made the downcast unrepresentable (the storage type cannot hold an `SVAAFrameInfoExt`), which is a strictly stronger outcome than the ruled conversion: not a safe cast, but no cast to make. One cast remains in `svc_mode_decision.rs`, rolled to S12; the ruling's intent (no dormant-arm unsafe at the floor) still binds it, and "Phase 10's lane" — the label the session report used for it — remains retired vocabulary: nothing schedules such an effort, and the cast retires under this ruling, not by deferral.
...
MIRI_SCOPE=encoder bash rust/tools/gates.sh session   # family + Miri --lib (encoder scope), NO benches — the Phase 9 session close (D-gate-4, ~18 min: Miri is 894 s of it)
...
**Two sessions in one working tree (2026-08-22, Phase 9 session D's close).** The steward
and a worker were live in the same checkout at once: the steward's in-place rewrite of
`gates.sh` (a truncating write, not an atomic replace) landed while the worker's
`gates.sh full` was executing — bash reads a script lazily by offset, so that run could
have executed garbage past the edit point, and the worker rightly killed it rather than
trust its verdict — and the worker's next commit (`0bfc7687`) swept the steward's
uncommitted tool and docs edits in with its own. Rules, from that: while a worker is live
in the tree the steward edits **docs only** and commits at once; a tool another session
may be executing is changed only by atomic replace (write a temp file and `mv` it over —
`sed -i` does this, a Python `write_text` does not), and preferably not until the worker's
gate has reported; `git status` before every commit, and never `git add` another
session's files. A shared script's running instance keeps its inode; an in-place truncate
is the one edit that reaches it.

**One gate battery at a time (Phase 9 session C's close).** Two `gates.sh` invocations
ran concurrently and both `tee`d into the same `.gates/miri_lib.log` — the file the
Miri verdict greps its totals from — which would have interleaved two runs' totals
into one log. The session killed both and re-ran the step alone against the committed
tree. Rule: before starting a battery, check for and kill a stale one; a verdict is
trusted only if its log had one writer.

**No tree edits while a gate runs (Phase 9 session E2, self-reported).** Twice a
stage's edits began while the previous stage's gate was still running, voiding that
verdict and forcing merged commits (each merged tree was re-gated quiescent, so every
landed verdict is honest — but one-level-per-commit bisectability was weakened at both
...
- **S18 — the straggler sweep.** Before closing a phase, list every definition the
  ...
  **Phase 9 session B2's clause (F119): the "no caller" grep sweeps the whole tree —
  `tests/` included — and a differential harness is a caller.** 22 `mc.rs` shims read
  as dead from `src/`-scoped greps; `tests/kernels_differential_phase2.rs` drives 13
  of them against the C++ reference. Deleting raw code a referee exercises trades
  away coverage, which is a recorded decision (the user's, here), never a sweep.
  **Phase 9 session B3's clause (F122): a dead-code deletion is a *closure* question,
  not a name question.** D-dead-1 asked "does anything call `WelsMdP4x4`/`P8x4`/`P4x8`"
  and the answer was no. The question that would have found the rest is **what becomes
  unreachable when they go** — the consumers of values only they produced. Three arms
  of `WelsMdInterMbRefinement`, three `UpdateP*MotionInfo` functions, three `SWelsMD`
  members and two entropy writers' sub-type arms are dead for the deleted functions'
  own reason, and a later session converted three of those arms with no gate behind
  them. Run the straggler sweep *with* the deletion, not at the phase exit.
...
- **S20 — the decomposable unit of a struct-field conversion is the
  ...
  **Phase 9 session B2's clause (F118): the closure includes the *slot* a converted
  operand feeds, and a plan's step ordering must have a compiling intermediate
  state.** `PlaneCursor` has no raw escape (`root_ptr` is `&mut self` — F73's retag),
  so "convert the readers, then flip the tables" has no state that compiles: for any
  call site the operand and the slot move together. What decouples them is direct
  call, not sequencing — a slot **constant after init** (one writer, `_uiCpuFlag`
  unused) may be bypassed per site byte-identically, so only runtime-selected sites
  (13 of the 37 cost sites, via `md_cost`/`me_cost`) force the atomic flip. A brief's
  phases are checked for a compiling middle before they are written down.
...
- **S22 — a repaired instrument has a backlog, and it surfaces only at the level
  ...
  **Phase 8b session A's clause (F85):** an instrument's *key* is part of the
  instrument as much as its scope list. `find_stub_bodies.py` keyed the C++ map by
  bare function name and kept the longest body, so `CWelsDecoder::GetOption` (53
  statements) was invisible behind `CWelsH264SVCEncoder::GetOption` (230) and a
  clean row said nothing about the decoder's; and its empty-body rule never fired
  because `len(body) > len(...)` discarded empty bodies before any rule saw them —
  `GetVclNalTemporalId {}` vanished from the very tool built to find it. Before
  trusting a tool's clean row, read what it keys on and what it discards.
...
- **S24 — a count that decides a conversion's shape comes from a grep over the
  ...
  **Phase 9 session A's clause (F101), and it was the steward's own miss:** a grep that
  reads only the **first line** of each item misses everything a wrapped signature
  carries. The port wraps parameter lists past ~100 columns, so `pub unsafe fn WelsDctMb(`
  with five raw parameters on the lines below read as having none, and the Phase 9
  charter's family table filed 272 raw-carrying sites as "body-only, converts for free"
  — 419 where the real figure is 147. Proven an artefact by crippling the census tool to
  one line, which reproduced the wrong table almost exactly. Count over the whole item,
  and when a tool reports suspiciously round good news, feed it the known-bad case first.
  **F149's brief clause (Phase 9 session F — the steward's own): a workload count in a
  brief is itself a grep; quote the command beside the number or the number is presumed
  wrong.** The F brief's "16 uses outside md.rs" for `md_cost_raw`/`me_cost_raw` matched
  no grep the tree can produce — the accessors had zero callers from birth (the safe
  pair has 3) — and it was the one count in that brief without its command printed
  beside it. The house style's "counts with commands" is this rule's brief-side half,
  not a formatting preference: the command is what lets the worker's re-run falsify the
  number.
...
- **S29 — inside a raw-reached struct, derive field pointers with `addr_of_mut!`,
  ...
  **Phase 9 session D's clause (F114a): the spelling's boundary moves with the parent's
  type.** `addr_of_mut!((*p).field)` reuses the parent's `SharedReadWrite` item only
  while `p` is raw; once the parameter is `&mut T` (T9.D7) the same spelling is "created
  by a SharedReadWrite retag" of its own, above the parent's `Unique`, and a sibling
  *safe* borrow of the field — `blk4x4_mut(&mut (*pMbCache).sCoeffLevel, k)` at a flipped
  slot call, which borrows the whole 384-coefficient field however the helper indexes —
  pops it. T9.D7 removed the precondition, T9.D8 added the sibling, and Miri named the
  raw (`:650`), the `Unique` (`:684`) and the use (`:360`, inside `WelsIDctT4Rec_c`,
  reached through the real encode loop); four bodies had the shape, one was reported.
  **When a parameter converts to a reference, every `addr_of_mut!` beneath it changes
  meaning — re-audit them** ("what did each derivation mean while the parent was raw, and
  does it still?"); within a body reach a field one way — derive the raw *at* the call
  that still needs one, never hold it across a safe borrow of the same field. The grep
  for the shape: a bound `addr_of_mut!((*root).field)` held across a `&(mut)
  (*root).field` and used after. The fix removes pointers (indexed writes, `fill(0)`).
...
- **S37 — an arena cannot take a `&mut`: type the parameter from the aliasing
  ...
  **Phase 9 session D's clause (T9.D7, F110/F112): the arena takes `&mut` at every
  *parameter* once S37's behavioural test is false at every call site — and the order
  is to make it false first.** D fixed the 43 held-cursor sites with the pointers
  still raw (one derivation per use; callees narrowed to the field they touch; a cursor
  parameter kept as a `usize` index where it carried caller state — F110), watched the
  detector read 0 in both kinds, and only then flipped all 41 signatures in one commit
  that drew a single compile error. The *roots* do not convert with the parameters: a
  `&mut` to the owned field is popped by any nested callee re-deriving that field
  (T6.C2's Miri verdict — these bodies nest four deep), so the 32 roots stay
  `addr_of_mut!` and each call site reborrows `&mut *pMbCache` for exactly the call.
  They convert with their owner (`*mut SSlice`), where it is one step, not thirty-two.
...
- **S44 — a field that looks dead in the port and is live in the reference is
  a dropped statement, and no gate this project owns can see one** (Phase 7
  session B, F69). `mutexSliceNumUpdate` was on the delete list as dead; reading
  the C++ to confirm found the `WelsMutexLock` bracket the raw translation had
  dropped around `AddSliceBoundary` + `++iSliceNumInFrame` — **F3's cause**,
  invisible for ninety-two measurements to every byte gate because a race
  is not a divergence until it fires. Three of that session's four findings
  came from the same question. Rule: a deletion list is read against the C++
  *before* anything is deleted — port-dead + reference-live is not a
  candidate, it is a finding.

- **S45 — an ablation separates one variable per arm; when a second candidate
  appears mid-ablation, add an arm rather than attribute by wish** (Phase 7
  session B). The charter's single after-arm would have read 0 hits after the
  pool deletion *and* the lock restoration together and credited the pool.
  Run as three arms — before 25/3000, pool deleted 20/2000 (p≈0.54: the pool
  acquitted), lock restored 0/2000 (P≈2×10⁻⁹) — the verdict landed on the
  variable that earned it.

- **S46 — an instrument's empty case is a case, and it is the one nobody
  tests** (Phase 7 session C, F74/F75; earlier sightings F17, F68). In one
  session: `perfpair.py`'s encode parser matched no multi-slice row and
  printed an **empty table**, so every span since `BENCH_SLICE_MODE` landed had
  measured the decoder only; and the unscoped Miri gate died on `set -u` +
  bash 3.2 **the moment the skip list emptied** — the gate's own success
  condition disabled it, on the phase's exit battery. Rule: every instrument's
  empty / zero / success-state branch gets a red-proof the way its failure
  branch does — an empty table is a failure, not a result; an empty skip list
  must run; a knob that changes nothing must be shown to reach the code (F68).
  **Phase 8 session B's clause**: a compile-time assertion that cannot report
  the negative is not an instrument — `fn assert_send<T: Send>()` states a
  verdict only when the answer is yes; the `Send` verdict on the safe cores was
  measured with a macro that reports both answers, and the answer was no, with
  the 14 `E0277`s as the inventory.
- **S47 — every entry point gets its own referee, and failures tally by
  assertion site** (Phase 8 session C, F82). `src/api/` was inside every
  instrument's scope for the whole phase and one of its ten decoder slots had
  neither half ported: every gate drives `DecodeFrame2`, and the one test that
  called `DecodeFrameNoDelay` followed it with an explicit `DecodeFrame2(NULL, 0)`
  — by hand, the statement the reference makes internally — so it compensated
  for the defect instead of detecting it. Rule: the instrument inventory is kept
  **per public entry point** (slot, export), each naming the gate that drives it
  against the reference; a slot with no row is unreferenced however well its
  module is scanned; a test that works around a behaviour is evidence of a
  defect, not coverage. And a failing external suite is tallied by **where the
  assertion fires**, never by fixture name — `EncodeDecodeTestAPIBase/...` is
  an encode→decode round trip, and reading 81 failures by name hid 30 decoder
  assertions for a day (S24's "count before claiming", aimed at test names).
  **Phase 9 session B's clause (F109):** an entry point's *configuration* is part of the
  inventory. The PSNR block of `WelsEncoderEncodeExt` has **no referee** — both
  diffharness drivers set `bPsnrY/U/V = false` (`cxx_enc.cpp:148`, `rust_enc/main.rs:133`)
  and no test asks for PSNR — so the 535-configuration sweep was blind to the whole
  block, and a wrong handle conversion there would have passed every gate. The per-
  entry-point inventory names each slot's *options* too; a parameter nothing drives is
  an unreferenced path. The fix is a knob and a targeted pair, not a rule alone.
- **S48 — an unported path refuses at the API, and a test pins the refusal**
  (Phase 8b charter, 2026-08-21; the sites are Phase 6's). S18 forbids inventing
  a stub plugin; it did not say what the caller does with the refusal.
  `DownsamplePadding` returns `RET_NOTSUPPORTED` and both callers drop it
  (`wels_preprocess.rs:1240`, `:1327`), so an encode with two spatial layers
  reports success and ships un-downsampled layers; `BilateralDenoising` is an
  empty body behind `bEnableDenoise`. A refusal the caller ignores is a stub by
  another name, and "off in every gate configuration" describes the gates, not
  the consumers. Rule: every unported branch either ports, or returns an error
  that reaches the entry point **and** has a test that asks for the feature and
  asserts the error code — the way `paraset_strategy.rs:755` pins its `None`.
  The gtest allowlist (Phase 8b) is the same rule at the suite level: a known
  failure is named with its owner, never absorbed into a tally.
  **Session A's clause (F86):** a stub that *returns success* is the same class as a
  dropped refusal. `ForceCodingIDR` null-checked and returned 0, so `ForceIntraFrame(true)`
  reported success and did nothing — and the state it failed to reset overran an
  unchecked bound and aborted the process the moment `GetOption` stopped returning
  garbage (one parity fix made the next defect reachable). `find_stub_bodies.py`'s
  empty-body rule exists for this; a stub that survives it is filed, not tolerated.
- **S49 — a time-seeded external suite is a finder, not a gate; the gate pins the
  seed** (Phase 8b session A, F88). Upstream's `test/api` seeds `rand()` from
  `time(NULL)`, so every `--check` run drove a different stream set, and one row was
  red once in seven runs on the Rust link while the C++ link passed all seven — a real
  divergence in the port's concealment path, and a gate that is red one run in seven
  is a gate nobody can act on. Rule: a gate over a randomized suite pins the seed (one
  constant, documented) so that red means regression; the randomness is kept as a
  separate `--seeds=A..B` sweep whose job is to *find* seeds, and a finding from it is
  filed with the seed that reproduces. Never allowlist a row that passes most runs —
  the stale-row check would go red on the passing runs, which is the lie the check
  exists to catch.
- **S50 — port the dispatch arm that runs, verified against a diffed model of the
  real dispatch, not the source that reads clearest; a kernel identical across
  implementations can still be reached differently** (Phase 8b session C, F97/F98).
  The brief feared the `_c` and NEON downsamplers would differ; a probe
  (`tools/vp_kernel_probe/kernel_parity.cpp`, linking the reference and calling each
  `_c` kernel against its NEON sibling) proved them byte-identical, 40/40. The real
  divergence was one line over, twice: the aarch64 *dispatch table* rebinds
  general-ratio luma to the accurate kernel where the scalar table binds the fast one
  (Fast != Accurate, 3366/26624 px), and `m_bNoSampleBuffer` — allocation success,
  **false**, so its name inverts its sense — selects the multi-pass arm as normal,
  making 4:1 a cascade of half-averages and the quarter/one-third kernels dead code.
  A port written from the source that reads clearest would be right on every dyadic
  ratio and wrong on every other. Rule: when the reference dispatches (a function
  table selected by CPU or config), build a small model of the arm that *actually*
  runs on the target and diff it against the reference over the real inputs
  (`dispatch_model.cpp`, 19 ratios x 3 planes, 19/19) — that model, not the prettiest
  source path, is what the port is written from; and read a boolean's *definition*
  before trusting its name.
  **Phase 9 session B2's clause (F117, the steward's third miss): read the callee's
  *parameter order* before claiming a data-flow direction.** The B2 brief read
  `copy16(pCurY.., pRefY..)` as cur→ref with `PCopyFunc`'s dst-first signature
  already quoted elsewhere in the same scoping pass; the copy runs previous→current,
  which put the write on the picture being read and inverted the trap's difficulty.
  A name pair (`pCur`/`pRef`) suggests a direction; only the typedef states one.
- **S51 — `pure` is a property of a signature; convertibility is a property of the
  call sites; grep the callers before scoping a family** (Phase 9 session A, F103). The
  coefficient family read as 19 single-family (`*mut i16`-only) kernels, so the brief
  scoped it as a clean first cut; 14 of the 19 turned out to be `SWelsFuncPtrList` slots
  whose 59 call-throughs pass **SMbCache walking cursors** (`pRes.add(64)` per quadrant),
  so they cannot convert until the MB-metadata family does — only the 5 with a direct
  caller passing an owned `[i16; N]` were convertible now. A family census that reads
  signatures alone over-promises; the census tool (`phase9_census.py`) now reports
  `pure` (signature single-family) and `total` (workload once earlier families land)
  separately, and a family's session is scoped by its callers, not its signatures.
- **S52 — a shared dispatch slot converts atomically, so its gate is the union of
  every caller's operand families; read the callers of the *table*, not of the
  kernel** (Phase 9 session B, F104). Every one of the 37 cost-table call sites was
  safe on its plane operand, and 25 of them pair it with an `SMbCache` prediction
  buffer; a table has one type, so `pfSampleSad`/`pfSampleSatd`/`pfSample4Sad` cannot
  flip until *both* the plane roots and the MB-metadata family are safe — twelve
  source-x-reference-only sites cannot go first because the slot type is shared. The
  same double gate held for `mc` (23) and `dct` (13, all coefficient-gated). Rule: a
  family whose kernels are reached through a table is scoped by the *table's* callers
  (`phase9_plane_callers.py` is the instrument), and a shim under a table lives until
  the last family among all its callers' operands is safe. Corollary: "convert the
  roots and the shims fall out" is false wherever a slot is shared.
  **Phase 9 session D's clause (F113): the union may not be *one* type.**
  `PQuantizationFunc` served `pfQuantization4x4` (one block) and `pfQuantizationFour4x4`
  (four) under one `*mut i16`; the safe slot is two typedefs over `[i16; 16]` and
  `[i16; 64]`, and the odd spans (49 for the 2x2 Hadamard's four DC reads, 241 for the
  block-15 DC) are the kernels' *reach*, already pinned by the T7 differential
  allocations and now in the signatures. A raw slot type that serves two lengths is a
  type that was not saying what it meant — when a slot flips, count its spans, not only
  its callers.
- **S53 — snapshot the *name* where the raw was snapshotted; a handle re-read at the
  point of use may name a different object** (Phase 9 session B, F109). Replacing a
  raw plane root with "carry the handle, resolve at the read" is right only if the
  field the handle comes from is unchanged between the old capture and the new read.
  `WelsCalcPsnr` captured `pDecPic`'s planes at the top of the layer body; the obvious
  conversion re-read `(*pCtx).pDecPic` at the PSNR block — after `UpdateRefList` had
  already advanced it to the *next* frame's slot (`PrefetchNextBuffer`). The fix keeps
  the snapshot and changes only what it holds: `Option<RecPicId>` captured at the same
  line. S37 said not to hold a *borrow* across the calls; this is the value half of the
  same rule. Before each handle conversion ask: is the source field stable across the
  span? (`SPicData` is re-stamped per macroblock from layer fields that are stable per
  frame; `(*pCtx).pDecPic` is not stable per layer body.)
- **S54 — "delete the path, keep the information": a parameter is redundant only if
  every caller passes the same function of the arguments that remain; read every caller
  before a brief or a commit derives it in the callee** (Phase 9 session D, F110 — and it
  was the steward's own miss). Session D's brief quoted `WelsPMbChromaEncode` in full and
  generalised its `pCurRS = coeff_level + 256` to the callee: "`pRes` is `coeff_level +
  256 + (iUV - 1) * 64`, a pure function of `iUV`; delete it." `WelsIMbChromaEncode`
  passes base `+ 0` (`svc_encode_slice.cpp:475` against `:499`), so the derivation would
  have moved every intra macroblock's chroma residual by 512 bytes — and the two paths are
  independent enough that a P-only sweep would not see it. S37's "delete the second
  path" is about the *pointer*; the information it carried — which half of
  `sCoeffLevel` this call works in — is caller state and survives as a `usize` index,
  which no retag can invalidate. S51 made convertibility a call-site property; this is its
  value half: a parameter's meaning is the union of its call sites, and one quoted caller
  is a sample, not a proof. Two of the same family's parameters *were* redundant
  (`WelsMdInterInit`'s and `AcceptPskip`'s arena) and looked identical from the callee's
  side — the difference is visible only from the callers.
  **F151's whole-tree clause (session F): the caller enumeration behind a deletion spans
  the whole tree — tests, benches, and other families included (F119 paid this once;
  F151's fifteenth caller re-paid it).** `WelsSampleSad8x8_c`'s read grep found the
  scene-change detector's block walk in the family the brief itself excluded, colliding
  "sad_common denies" with "preprocess is not this session". The resolution is F139's
  ownership rule, now standing: a kernel whose last caller lives in a foreign family
  relocates privately to that family, takes its census row and ratchet lines with it
  (the rebaseline states the reason), and the denying file still denies on schedule.
  **F181's both-trees clause (session X2): read every caller in *both* trees before
  deleting a parameter — unused-in-port can be the port's gap, not the parameter's
  deadness.** au_set's `_pLogCtx` pair was unused here because eight upstream `WelsLog`
  calls through them had been dropped in translation; the brief's delete instruction
  would have destroyed the evidence. The underscore was the tell: "unused by name"
  should prompt the reference grep, never the deletion.
- **S55 — calibrate a detector on a known positive in the spelling the house mandates
  before believing its zero; an instrument aimed at a new target is re-fired, not
  re-read** (Phase 9 session D, F111). `q1c.py` was rebuilt from F66's written
  specification (T9.A2) and gated session B's scoping and session D's; aimed at
  `SMbCache` and run against the tree it was about to gate, it had two false-negative
  classes — its derivation pattern did not match `addr_of_mut!`, the spelling S28/S29
  *require* for a cursor into an inline array, and a root could not be an owned struct
  field, which is how all 32 `SMbCache` roots are minted — so it read **0 with nine live
  sites in it**, and 22 + 20 sites were hidden (family 6's precondition: 308/71, not
  287/68). S46 makes "nothing scanned" loud; this makes "scanned but blind" visible:
  before a detector's zero decides anything, feed it one site known to be hazardous in the
  code's own spelling and see it fire, and when it is re-aimed (`--type`), re-check the
  spellings its patterns were written against. Standing instrument: every Phase 9 family
  aims `q1c.py --type <its root type>` at itself before scoping (charter §6), and the
  tool's docstring carries its three remaining limits (bare accessor calls, calls through
  dispatch slots, the single conversion it models).
  **Phase 9 session B2's clause: calibrate the *conversion* against the byte gate the
  same way.** A conversion's 535/535 is evidence only if the gate visits the site: B2
  proved T9.B22's coverage by planting a one-sample offset in the converted `mc_luma`
  call and watching the 320x192 stream differ (char 9223), then reverting. The
  complement of S57 — S57 leaves dark sites raw; this proves a lit site is lit.
  **Phase 9 session B3's clause (F123): the rule has a second edge — an instrument that
  over-reports is as useless as one that reads zero, and a change to a detector in
  *either* direction is re-fired against the tree it was calibrated on.** `q1c.py`'s
  shape C matched `&` as well as `&mut`, so a *shared* reborrow beside a raw into the
  same field read as F114a — which it is not: a shared reborrow is a read through the
  parent and a read access does not remove `SharedReadWrite` items above the tag it
  reaches through. A scanner that fires on a sound spelling trains the session to argue
  with it, and the next true hit gets the same argument. Narrowing it was allowed to
  land only because the narrowed scanner still reports F114a's four bodies at
  `0bfc7687^` with the Miri-reported one line-for-line.
- **S56 — a reference argument is a protector for the whole call; a raw parameter never
  was. Before `*mut T` → `&T` or `&mut T`, walk the callee's callees for any access to the
  same object through another path — "it only reads" describes the body, the protector
  describes the call** (Phase 9 session D, F114b). T9.D9 converted `AddSliceBoundary` and
  `DynSlcJudgeSliceBoundaryStepBack` to `&SMB` because they only read the macroblock —
  true, and beside the point: a reference passed as an argument is *strongly protected*
  for the duration of the call, nothing may invalidate it even transiently, and
  `AddSliceBoundary` calls `UpdateMbNeighbourInfoForNextSlice`, which walks the macroblock
  list taking `&mut` to each element, the protected one included. The conversion
  *created* the UB — the C++ has neither a protector nor a conflict, and `*mut SMB` had
  neither. F66's rule is about a `&mut` parameter retagging on entry; this is its
  counterpart: a *shared* reference retags harmlessly and still protects. The fix that is
  right removes the pointer: both bodies read one field, so the parameter is the `i32`
  they read. Instrument: D-gate-4's session-close Miri, and — since T9.B20 — `q1c.py`
  itself: **shape D** is this rule and **shape C** is F114a's, both implemented and both
  calibrated against `0bfc7687^`, the tree that carried the two live defects. Shape C
  reports exactly the four bodies F114a named, the Miri-reported one line-for-line; shape
  D reports exactly `AddSliceBoundary` and `DynSlcJudgeSliceBoundaryStepBack` and nothing
  else. Run `q1c.py --type T --kind ref` before *and* after converting a family.

  **F192's clause (session H2): protection is about the tag being used, not how it was
  derived — and disjointness buys nothing while the call is on the stack.** A stored raw
  re-stamped under a live borrow becomes that borrow's *child*: it survives the retag
  and still dies at the protector, because removing a strongly-protected Unique is
  refused outright, whatever bytes the read touches and whatever the caller does
  afterwards. F167's copy was argued sound on exactly the disjointness escape; Miri's
  one-line refusal closed the argument and the storage both.
- **S57 — before converting a site, ask which gate runs it; "no gate runs this" is an
  argument for leaving it raw, not for converting it carefully** (Phase 9 session B2,
  F117). `VaaBackgroundMbDataUpdate` writes the *current* source picture — the one
  `SPicData.pEncMb` reads — from inside the macroblock loop, which is the single hardest
  aliasing shape in the plane family. It is also unreachable in every sweep preset in
  both profiles, because the diffharness driver sets `bEnableBackgroundDetection = false`
  (`rust_enc/main.rs:126`), so neither the 535 sweep rows nor the encoder-scoped Miri step
  touches it. A conversion there would be a byte-identical result that means nothing:
  the gates would pass on a path they never entered. The phase's value comes from
  retiring raw sites *whose behaviour something checks*; a dark site converted is a tag
  moved, not a risk removed. Measure reachability the cheap way — a probe that prints
  once and is reverted — and record the answer beside the site so the next session does
  not re-derive it.
  **Phase 9 session B3's clause (F121): a hazard in dark code is not a hazard, and the
  probe needs its own calibration.** Session B2's brief raised two
  `ctx_pic_ref_mut(..).planes()` calls as "F73's retag already in the tree" and asked
  B3 to repair them; they sit inside `JudgeStaticSkip`/`JudgeScrollSkip`, which no
  sweep preset enters, so they are live as *code* and unreachable as *behaviour* — and
  repairing them is the same trade this rule forbids for conversions. The question
  "which gate runs this" belongs to hazard triage as much as to conversion scoping.
  Two mechanics, both learned the hard way in one afternoon: the probe must print
  through a channel the harness preserves (`compare.sh` sends the driver's stderr to a
  log file, so a first run read zero for *everything*), and it must run beside a
  **calibration probe in a body known to execute** — the dark bodies read 0 against
  `WelsMdI16x16`'s 300-2136 in the same runs, and without the second number the first
  proves nothing (S46).

- **S58 — a reading aid that sessions scope against is a gate; make its unknown column
  fail** (Phase 9 session B2, F116). `phase9_plane_callers.py` classified operands by
  spelling and exited 0 whatever it found. Session D renamed ten accessors, 40 call sites
  fell into `?`, the coefficient column went 13 → 0, and the tool reported it as a clean
  run that a later brief then quoted as fact. Nothing was broken and nothing was
  detectable. The generalisation of S46 (an empty scan must be loud) and S55 (calibrate on
  a known positive) is that **loudness has to live in the exit code**, because a printed
  warning above a well-formed table is not read. Any tool whose output scopes a session
  gets: a non-zero exit on unclassified input, an explicit allowlist with a reason per
  entry, and a calibration that shows it failing — knock out one classifier arm and
  confirm the count moves. Corollary worth its own line: when the tool and the document
  it generates disagree, **re-derive before believing either**. Here the document had
  been right for two sessions and only the instrument had rotted.
  **F152's visibility clause (session F): loudness covers only what the instrument can
  see.** The exit-1-on-unclassified tooth cannot fire on a site that no longer matches
  any pattern: T9.E7's interior-pointer respelling moved the encoder's one live raw cost
  read out of the census's slot-mention grammar and the row vanished silently — F137's
  third instance, which is why `CheckChromaCost`, session F's largest live conversion,
  appeared nowhere in its brief. Two duties follow: a commit that respells a fact the
  census keys on edits the census in the same commit; and a row that leaves the table by
  relocation rather than conversion is named in the close's log, so the table's shrink
  is never read as progress it isn't.
- **S59 — coverage is byte-sensitivity, not execution: a row that swallows a planted
  fault referees nothing at that site, whatever the entry counter says** (Phase 9
  session B4, F126). The `bg` preset's Static clip enters `WelsMdBackgroundMbEnc`
  4524 times per row and passed a planted fault escalated from +1 to **+128**: on
  that content every background macroblock codes as `MB_TYPE_BACKGROUND` P_SKIP with
  no residual and the decision inputs come from source-domain VAA planes, so the
  prediction bytes never reach the stream. The MD family's referee is therefore 32
  of the preset's 48 rows, corrected everywhere the number appears. Two duties: the
  S55 planted-fault calibration is evaluated **per site and per row** — what a
  conversion may cite is the covered-row count, not the row count; and an inert row
  is not waste when another site in it has teeth (the same Static rows fail a fault
  planted in `VaaBackgroundMbDataUpdate`'s copy, which is why they stay). Entry
  probes prove a path *runs* (S47's inventory); only a fault that fails proves a
  gate *watches* it.
  **F175's escalation clause (session X): when the observable is a threshold over a
  count of thresholded quantities, per-site variation is not enough — the fault must
  move the verdict.** The scene-change detector swallowed a plane-root +1 at 0/583 while
  its path ran in 101 configs; the forced verdict failed 71. A 0-row reading on a
  thresholded site is informative only beside a non-zero maximal fault: escalate before
  concluding, and report both numbers.
- **S61 — the gate's own cost is a gated quantity: the battery records its wall-times
  and a close compares them against the previous close's** (Phase 9 session C2, F140).
  The seam's bounds-checked writes made Miri's interpreted encode probes 2.6x slower —
  uniform, no hot spot, the honest price of `at`/`set` over raw stores — and the
  regression passed eight commits unremarked because `gates.sh` measures byte-identity,
  test counts, the ratchet and the census, and nothing measures the instrument itself.
  It surfaced as a 1.5-hour gate a person happened to notice. The charter was already
  quoting the series (894 / 978 / 1012 s) without checking it as one. Duty: each close
  states the Miri wall-time beside the previous close's, and a ratio past ~1.3x is a
  finding to file before it is a nuisance to route around; the two wrong hypotheses
  (cursor construction, the atomics) are recorded with the experiments that killed
  them so nobody repeats them.
- **S62 — an instrument placed in a racy context is itself a racy access: prove an
  invariant at the race-free write site, not the contended read site — and the
  equality worth proving is *outcome* equality, not value equality** (Phase 9 session
  E, F142). The E brief prescribed `debug_assert_eq!(record, map)` at every deblocking
  read to license pointing the guards at the atomic map. Doubly wrong: the assert
  *reads a foreign `SMB` record cross-thread*, which is precisely the race being
  interrogated — the instrument would have been round 5's UB in a test harness — and
  the value equality it asserts is legitimately false cross-partition mid-frame, so
  the prescribed fallback would have fired on a false alarm. What was provable:
  asserts at every `uiSliceIdc` *stamp* (each stamp is race-free by construction),
  calibrated by a planted +1 that aborted every row and both fork probes, plus a
  residue argument that the guards' *outcomes* agree even where the values differ.
  Before planting any cross-checking instrument, ask what the instrument itself
  reads, and from which thread.
- **S63 — fork-reachability splits a parameter family before any flip is planned:
  the shared object's in-fork half cannot take `&mut` by any amount of hazard work,
  and its end states are interior mutability (the seam's shape) or lawful raw**
  (Phase 9 session E2, F146). The E2 brief ordered "then `*mut SDqLayer` →
  `&mut SDqLayer` the same staged way" three edges below its own rule that nothing
  takes `&mut` on the layer inside the fork — the layer is the one object all N
  workers share, and 42 of its 71 parameter bodies are fork-reachable. Zero detector
  hazards on such a body is necessary and *irrelevant*: the blocker is not a held
  cursor but N simultaneous protectors on one allocation. The sound campaign: split
  the bodies by fork-reachability first (the reachability walker is the instrument),
  flip the single-threaded half, enumerate the in-fork half with its two end states
  for its owner. Applies verbatim to `sWelsEncCtx` before G–H plans anything: the ctx
  is shared by every worker too, so its family splits the same way, and the split —
  not the hazard count — is the first number G–H's brief must carry.
  **H2's clarifying clause (F-section D, the steward's error corrected): S63 forbids N
  workers' *concurrent* entry retags of one allocation — it says nothing about one
  thread's N disjoint projections.** `(&mut s.a, &mut s.b, &mut s.c)` is ordinary Rust;
  what the compiler refuses is N separate accessor *calls* each borrowing the whole
  object, and the remedy is a split-borrow function, not a raw pointer. X2's ~36
  "blocked" sites fell to one such function — and the brief that called the shape "S63's
  forbidden retag exactly" was quoting the rule against itself.
- **S64 — a grep bounds a spelling, not a relationship: enumerate a family from the type
  outward, and aim the referee at the field that varies** (Phase 9 sessions F and E3;
  F149, F154, F155, F156 — four instances of one class in two sessions). A correct count
  of accessor mentions that were not callers (F149); a correct `*mut` count that missed
  the `*const`, return-position and type-alias spellings of the same relationship
  (F154); a correctly placed planted fault that perturbed a field too slow-varying to
  referee the site (F155, the GOM-granular QP); a 'pair' of fields re-routed together
  when one had zero readers (F156). S24's clause makes a number falsifiable — the
  command beside it; this rule is about the number being *sufficient*: before scoping a
  family, enumerate every spelling of the type's reach (`*mut T`, `*const T`, `-> *mut
  T`, `type X = *mut T`, cached copies — F101's whole-item law applied to types, and a
  brief that quotes one spelling is quoting a lower bound and says so); before a fault
  ...
  **F177's writer clause (session X): grep the reference tree's writer before believing
  a field inert.** A permanently-null field with live readers — three of them in X, with
  four unguarded dereferences behind a disabled flag — looks identical to a dead field
  under the census, the ratchet, and every byte gate; the one cheap check that
  distinguishes them is whether the *other* tree allocates it. F156's readers-per-field
  is this clause's twin.
...
- **S67 — a hazard detector's domain is the set of bodies that can *reach* the
  object, not the set that takes it as a parameter** (Phase 9 session H, F171).
  `q1c.py` and `phase9_forksplit.py` both scope to "bodies with a `*mut T`
  parameter", which is right for S63's fork-split question — fork-reachability
  really is a property of a parameter family — and the hazard question was allowed
  to inherit that domain without anyone checking that it fits. It does not. H's
  flip created genuine UB in `UpdateStatistics`, a method on
  `CWelsH264SVCEncoder` that holds the context in a **local** raw from `ctx_ptr`
  and has no context parameter at all: a `&mut` cursor into `sEncoderStatistics`
  popped by `ctx_ltr(&mut *pCtx)` retagging the whole allocation. The join read
  **0 LIVE for six consecutive stages** while it was being created, and nine
  stages passed `gates.sh family` at 583/583 in both profiles with it live —
  no byte gate can see a read that returns the right value through an invalidated
  tag. F167 is the same hole from the other side (a *stored* copy in a struct
  field, predicted and not yet run). Before a campaign converts a shared object to
  `&mut`, enumerate every route to it — parameters, locals derived from an
  owner-accessor, struct fields, and `self` on the objects that own it — and treat
  a detector's parameter-shaped domain as a lower bound that says so (S64's
  clause, applied to the detector rather than to the family). The standing guard
  in the meantime is D-gate-4's Miri lane at every session close, which is what
  caught this one and F114 before it.
- **S68 — a citation decays: before executing on a finding's attribution, check out the
  commit it names and read the line it quotes** (Phase 9 sessions E3/H/X; F153, F160,
  F172 — three instances of one class). F84's zero-caller claim was true when filed and
  falsified by T7.C7's later fence (F153); F138's gravestone claimed slots session F had
  already deleted, and a finding written beside it inherited the comment's state (F160);
  F86 misattributed its own abort to a site guarded since the raw translation, and two
  successive briefs copied the citation — a citation copied twice reads as twice-
  confirmed (F172). The check costs one `git checkout` and one read, and it is the only
  check that distinguishes a finding's *evidence* from its *age*. Corollary for the
  steward: a brief that quotes a finding quotes its date and says so when the finding is
  more than a session old.
  **F179's absence clause (session X2): a claim that something is absent from this tree
  — "never ported", "no callers", "no reader" — is the cheapest claim to verify and the
  easiest to write without verifying.** One `grep -n 'fn SetRefMbType'` refuted an
  entire step's premise: the function was ported 806 commits before the brief, and the
  absence claim had traveled comment → finding → brief without anyone running the grep.
  Before asserting absence, run it and quote it.
- **S69 — a normalization is a hole cut in your own referee: cut it no wider than the
  nondeterminism it hides, anchor it to the field it excuses, and prove the referee
  still fails beside it** (Phase 9 session X2, F186). The log referee normalized
  pointers as any `0x<hex>` and thereby swallowed `<w>x<h>` dimensions on both sides —
  `SpatialId = 0,80x48` became `80xPTR`, and a port printing `80x49` would have been
  certified by the instrument built to catch it. Anchor each normalization to the field
...
- **S60 — run the acceptance instrument before designing against it; its current
  ...
  "build the seam, then un-ignore the fork/join probe — those probes passing is the
  acceptance." The session ran the ignored probe under Miri *first*, before writing a
  line, and the verdict was not a pixel race but a pool retag — then five more
  families, one per re-run, three of them other sessions' property and two of them
  genuine races (one in upstream, F133). A plane-shaped seam built to the brief would
  have been correct and moved the probe zero instructions. Two duties: the instrument
  that will judge the work runs at session start, and what it names *now* is the real
  work list (the brief's map is a hypothesis about that list); and the steward scopes
  acceptance to what the session owns — a probe spanning six families is the *phase's*
  acceptance, delegated per family, with each family's own narrower probe as the
  session-level check (C's four seam probes are that shape).
  **Session E's clause (F143): an iterated instrument's list is a *frontier*, not an
  inventory, until the instrument passes.** F132's six rounds read as "the fork's
  contention, enumerated"; rounds 7–9 were sitting behind round 5, invisible until
  its fix let the probe run further. Write "frontier" in the finding and the brief
  until the run is green end-to-end; only the passing run turns the list into an
  inventory.
  **Session C2's clause (F136): after a partial fix, state the instrument's expected
  next verdict, not just "re-run it".** The addendum's check was "the mid-row probe
  must now name the SMB family, not SSliceCtx" — it named `SSliceCtx` again, 28 bytes
  further in (`iSliceNumInFrame`), because the whole-struct retag reaches every field
  and round 6 was two fields, not one. "Convert these 18 sites" would have been
  reported done and green; the expected verdict is what made the gap loud.
  **Phase 9 session B3's clause (F120): failing on an unclassified operand does not
  cover a *missing entry point*, which is silent by construction.**
  `phase9_plane_callers.py` knew two of `common/mc.rs`'s entry points and not the four
  the fractional refinement calls directly, so ten call sites were invisible — no
  operand to leave unclassified, no `?` column, exit 0, and a brief, a charter row and
  the tool all quoting different numbers with none of them right. The entry-point
  table is as load-bearing as the classifiers; the check that finds a hole in it is a
  grep for the kernel's name over `src/` compared against the census's own row count,
  run when the family is scoped. **Corollary, and it is what makes a census mean
  something across a campaign: a converted site must *leave* the census**, not be
  re-spelled inside it — when a family grows a transitional second spelling, the tool
  tracks the raw one only.
...
```
P0 ──► P1 ──► P2 (kernels) ──► P4 (dispatch) ──► P5 (decoder pivot) ──► P8 (API) ──► P8b (parity) ──► P9 (safety) ──► P10 (screen)

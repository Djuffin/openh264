...
| **Ratchet** | *(Phase 5 session R, 2026-08-14)* `unsafe_fn` **1248**, `raw_ptr` **4117**, `unsafe_block` **618**, `mem_zeroed` **30**, `SHIM(` **160**; decoder-only `raw_ptr` **980** (phase5.md's monotone exit metric, 1283 at the re-plan, 1237 at this session's open). **−257 in one session, and S16 read from both ends of it**: T5.R1 pushed it *up* 37 by writing `*mut DqLayerState` in 35 signatures where a context field spelled it once, and T5.R5 pulled it down **174 in one commit** because the packed-word idiom spelled a pointer type at every one of its 219 uses. Neither number is the face's verdict — the probe is. *(Prior: Phase 5 session P″ — `unsafe_fn` 1257, `raw_ptr` 4368, decoder-only 1229.)*
| **Gates** | *(Phase 5 session R, 2026-08-14)* **476 debug / 470 release / 20 ignored**, Miri **336**, census **59**, decode goldens **57 none moved**, both benches bit-identical, sweeps **341/341 in both profiles**. Close was **OVERALL: PASS** at `full`. **F3 drew zero hits**, as at Q's and P″'s closes and for the reason S14 records: 682 configurations against a measured ≈1/800 rate makes zero the likeliest single outcome. Five in-session Miri probes, all 336/0, each on a detached worktree at the seam's commit — the trick that let the probe budget be spent without freezing the tree.

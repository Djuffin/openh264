...
| **Ratchet** | *(Phase 5 session P″, 2026-08-14)* `unsafe_fn` **1257**, `raw_ptr` **4368**, `unsafe_block` **623**, `mem_zeroed` **31**, `SHIM(` **159**; decoder-only `raw_ptr` **1229** (phase5.md's monotone exit metric, 1283 at the re-plan). **−16 across the session, and one of its three faces moved it by exactly nothing** — the hoist converted 44 sites for `raw_ptr` ±0, which is S16's law from the side session P′ did not see: the instrument counts pointer *types written*, and threading `pDec: PPicture` parameters writes an alias (free) while deleting accessor *calls* (never counted). A W-item can be a third done with the metric flat. Regenerated twice, both for accessors this session added to `decoder_context.rs` — `pic_pool_mut`/`pic_pool_ptr` (+2) and `pic_refs`/`ref_id` (+2). — *(Phase 5 session P′, 2026-08-14)* `unsafe_fn` **1253**, `raw_ptr` **4382**, `unsafe_block` **626**, `mem_zeroed` **31**, `SHIM(` **159**; decoder-only `raw_ptr` **1245** (phase5.md's monotone exit metric, 1283 at the re-plan). **−22 across the session, and it went *backwards* for one of the three faces**, which is why S16 says read the shape: T5.P′1 converted 160 sites for −2, T5.P′2 converted 142 sites for **+4**, T5.P′3 converted 204 sites for −24. The +4 is honest — five pointer fields and two pointer-array locals died (−7) against nine pointer *parameters* born (+9), eight of them the `pCtx` the DPB helpers need to resolve a slot handle — and a face that deletes five fields each holding up to 34 slot addresses can still cost the instrument four, because it counts pointer types *written*. T5.P′1's 67 new `pDec: PPicture` parameters were free for the mirror reason: `PPicture` is an alias, and an alias writes no pointer type. `unsafe_fn` +6 (five pool accessors and `insert_ref`); `unsafe_block` −1. Census allowlist **59**, duplicate-body budget **192**, both unmoved. S16's prose floor, **twelfth** instance: four of T5.P′2's new doc comments named the pointer type they exist to delete, and only the per-file table showed it — the total read −20 and hid the +4. — *(Phase 5 session P, 2026-08-13)* `unsafe_fn` **1247**, `raw_ptr` **4404**, `unsafe_block` **627**, `mem_zeroed` **31**, `SHIM(` **159**; decoder-only `raw_ptr` **1267** (phase5.md's monotone exit metric, 1283 at the re-plan). **`raw_ptr` −16, and all of it is W1's**: T5.P1's owned access unit. T5.P2 converted **85 sites** — `pCtx->pDec` and `pECRefPic` from raw pointers into pool slots to `PicId`s — and moved the metric by **zero**, because two pointer fields died and four accessor signatures were born. That is S16's shape in its cleanest form: the instrument counts pointer *types written*, and the change (a field that was an alias is now a handle) is exactly the kind it cannot see. `unsafe_fn` +2 net (−2 for T5.P1's deleted allocator pair, +4 for the new accessors). Census **duplicate-body budget 195 → 192**, its second decrease ever. S16's prose floor, **eleventh** instance and the first caught by re-reading the ratchet rather than the diff: `pic_slot`'s doc comment named the pointer type the function exists to delete. — *(Phase 5 session N, 2026-08-13)* `unsafe_fn` **1247**, `raw_ptr` **4436**, `unsafe_block` **622**, `mem_zeroed` **31**, `SHIM(` **159**. **`raw_ptr` −24**, and per file: `deblocking.rs` −13 (30 `pRefPics` dereferences, the `c_void` erasure in four signatures, five mirror writes), `pic_queue.rs` −13 (`ppPic` and the four scans that walked it), `decoder_context.rs` −4 (two field declarations), `error_concealment.rs` −1, `picture.rs` **+2** (`same_picture`'s two parameters), `mv_pred.rs` flat. **S16's prose floor, ninth collection, and this one was the session's own**: the first count read −17 with four files flagged as increases, and seven of those occurrences were pointer types written in comments the same commits had added — `[*mut u8; 3]` in the note explaining that `pCsData` is gone, `[*mut *mut Picture; LIST_A]` in the note explaining that `pRefPics` is gone. Rewording took the total to −24. The sharp form of the rule is that **a comment mourning a deleted pointer type reinstates it in the metric**, and the file where a deletion just happened is the likeliest place to write one. **`unsafe_fn` +6** is S16's trade running the other way from session M's −8: bodies moved out of unsafe free functions into six named unsafe methods, nothing double-counted, the raw work now behind contracts instead of inline in four scans. `unsafe_block` −3, and **no S28 test owed for a third session** — the pool stores pointers it did not derive. — *(Phase 5 session M, 2026-08-13)* `unsafe_fn` **1241**, `raw_ptr` **4460**, `unsafe_block` **625**, `mem_zeroed` **31**, `SHIM(` **159**. Two readings matter. **`raw_ptr` −47** is T5.M2's 45 scratch-cache parameters (`parse_mb_syn_cabac.rs` −27, `parse_mb_syn_cavlc.rs` −10, `mv_pred.rs` −10) plus T5.M3's deleted `pBitStringAux` field (−2) against `bit_stream.rs` **+2** for the accessor that replaces it; **T5.M1's rename is invisible in the per-file table, which is exactly its evidence** — a rename writes no pointer type and deletes none. **`unsafe_fn` −8 is the first fall of the phase and the shape S16 predicts**: it stays ~flat while raw bodies are strangled and drops hard when they are *deleted*, and F22's eight duplicate function definitions (T5.M4) are the first such deletions here. `unsafe_block` unchanged for a **fourth** session, with no S28 full-reach test owed for a second — stack arrays are reachable whole by a borrow, and T5.M3's one derivation replaces a field that was already raw. — *(Phase 5 session L, 2026-08-12)* `unsafe_fn` **1248**, `raw_ptr` **4507**, `unsafe_block` **625**, `mem_zeroed` **31**, `SHIM(` **159**. Session L's **−33** is the last seven grid families, and **−22 of it is `decoder_core.rs`** — seven field declarations, eight allocations and seven free blocks — while `mv_pred.rs` is **flat at 274 across 25 converted sites** and `error_concealment.rs` flat across four deleted base-pointer bindings. Same lesson as session K's, twice the sample. **`unsafe_block` did not move**: no production `unsafe {}` for a third session and, for the first time in the flip, **no S28 full-reach test was owed** — none of the seven families needed a raw bridge. The prose floor was collected an eighth time here (a doc comment naming the pointer type its own flip had deleted), caught by the per-file map and invisible in the total. — *(Phase 5 session K, 2026-08-12)* `unsafe_fn` **1248**, `raw_ptr` **4540**, `unsafe_block` **625**, `mem_zeroed` **31**, `SHIM(` **159**. Session K's −10 `raw_ptr` is **entirely `decoder_core.rs` (−9) and `deblocking.rs` (−1)**, and the four other files it converted are flat across 32 sites — these conversions delete pointer *dereferences* while the metric counts pointer *types written*, which is the per-file map earning its keep again. Both `unsafe_block` increases are S28 full-reach tests; no production `unsafe {}` in two sessions. *(Session J read 4550 / 623.)* — *(Phase 5 session I, 2026-08-12)* `unsafe_fn` **1248**, `raw_ptr` **4570**, `unsafe_block` **622**, `mem_zeroed` **31**, `SHIM(` **159** — **session I is flat on every metric and every per-file row**, which is what accessor reshaping looks like: it converts safe indexing into safe borrows and deletes no pointer. Read the per-file map, not the totals (S16): the check is to regenerate the baseline into a copy and diff its `files` map, because `check` only reports *increases* and a real decrease cancelling a prose rise looks identical to nothing happening. *(Session H read `unsafe_fn` 1248 / `raw_ptr` 4570 / `unsafe_block` 622 after −34 from conversion rather than deletion.)* — *(Phase 5 session C, 2026-08-11)* `unsafe_fn` **1248**, `raw_ptr` **4595**, `unsafe_block` **613** *(session B read 1249 / 4597 / 616; session A read 1250 / 4597 / 618, and its −218 `raw_ptr` came from deleting duplicate declarations rather than converting anything)*, `SHIM(` **158** (of which **`SHIM(phase3)` is 2**, both enumerated and both owned by Phase 6; 154 are `SHIM(phase2)` and **2 are `SHIM(phase5)`** — `cabac_decoder.rs`'s, which dies in 5.2, and `SPicture::data_ptr`, which dies as 5.2–5.6 convert the kernels *except* at `decoder_core.rs:1087`, where the public output contract hands the pointers to the API consumer and they outlive the call), `no_mangle` 24, `mem_zeroed` **32**, **`transmute` 4 — and this number has a floor: all four matches are prose and the crate contains zero calls.** Do not chase it. The metric never covered the whole population either: T4b.3c found `as *mut _ as *mut _` doing a transmute's job, and a double cast does not match the grep — for the rest of that family use `find_dup_types.sh`, not this metric. *(Phase 4b exit read: `unsafe_fn` 1250, `raw_ptr` 4815, `unsafe_block` 613.)* **A second census now covers what the ratchet cannot**: `rust/tools/census.sh` (duplicate declarations, double-cast laundering, duplicate function bodies), allowlisted in `census_allowlist.txt`, run at `commit` level. Baseline regenerated per S16 at every seam. Run `bash rust/tools/unsafe_ratchet.sh check`; never trust a remembered number |
| **Gates** | *(Phase 5 session P″, 2026-08-14)* **474 debug / 468 release / 20 ignored**, Miri **334**, census **59**, decode goldens **57 none moved**, both benches bit-identical, sweeps **341/341 in both profiles**, and the once-per-phase `exit` level ran clean on all four Miri targets (334 / 20 / 7 / 3 — S22's backlog check). Close was **OVERALL: PASS**, the first unqualified one since Phase 3's exit. **F3 drew zero hits**, which is a *sample* and not a signal (S14 step 4): the measured battery rate is ≈1/800 configurations against 682 swept, so zero is the likeliest single outcome and says nothing about the tree. Running total unchanged at **42 measurements, 14 alternations, 21 acquittals**. **Every figure is unchanged across a session that converted 130 sites**, because no face needed a new test — two of the three were pure motion and the third swapped a lifecycle on a path with one caller. **The probe budget ran three and all were green** (944s, 943s, 943s); session P′'s conviction came from a semantic change mid-face and this session made none, which is the budget working rather than idling. — *(Phase 5 session P′, 2026-08-14)* **474 debug / 468 release / 20 ignored**, Miri **334**, census **59**, decode goldens **57 none moved**, both benches bit-identical, sweeps **341/341 debug, 340/341 release**. **Every figure but the ratchet is unchanged across a session that converted 506 sites**, because no face needed a new test. **The probe budget fired for the first time since it was written**: three mid-session Miri runs (754s, 766s, 768s) and the second went **red** — `AddShortTermToList` took `let pic = &mut *pPic` and then read `pPic` raw to name its slot, popping the borrow it was about to use. S29's boundary from the other side (T5.O8's lesson): the invalidator is a raw read under a live `&mut`, and only *ordering* fixes it. No byte gate can see that class; the alternative to the budget was finding it in a close battery three commits later. F3 drew **one** hit (measurement 42, release, `mt Static_152_100 t=4 sm=3 n=600 cabac=0 rc=0`, 29375 → **0**). **Step 0 was checked rather than assumed** — the two release `rust_enc` binaries hash differently (`4a730f44…` base, `db175dc1…` head), so the shortcut cannot acquit, which is session P's clause applied rather than restated. **Step 1 reproduced inside its own five runs**: run 4 gave a Rust stream shorter than the C++ one, and 21 of 22 isolation runs were byte-identical at 29375 — one binary, one configuration, **three distinct outcomes**, S14 step 1's discriminator for the third measurement running. One hit, so no alternation. Acquitted; running total **42 measurements, 14 alternations, 21 acquittals**. `Static_152_100` is the third of three sweep clips to hit, which removes the last trace of a per-clip clause. Session close was **OVERALL: FAIL(1)**, that one step, adjudicated under S14 and acquitted here — S27's "accepted" predicate for the next open. — *(Phase 5 session P, 2026-08-13)* **474 debug / 468 release / 20 ignored**, Miri **334** (941.5s, both probes), census **59**, decode goldens **57 none moved**, both benches bit-identical, sweeps **340/341 in both profiles**. **Every figure but the ratchet is unchanged across a session that converted 139 sites**, because neither face needed a new test — and the two faces ran **three mid-session probe runs, all green first time**, which is the budget T5.O3 earned and session O's three-round close battery paid for. F3 drew **two** hits, one per profile and one battery — measurements 40 and 41, both `mt sm=3 t=4 n=600` with a wrong length (zero and long), different clips, different `cabac`. Both **5/5 byte-identical in isolation**. Two hits in one battery fires S14 step 2, and **step 0's hash shortcut did not apply** for the first time in a decoder-only session: the diff is production decoder code in the same lib, so base and head build different `rust_enc` binaries even though the sweep compares encoders. **Step 1: both 5/5 byte-identical in isolation. Step 2's alternation acquits** — twelve whole `mt` presets per side, 2880 configurations each, **base 2 hits / head 3**. And it produced the finding's strongest evidence yet, on the tree under suspicion: the head binary gave *one* configuration **0 bytes in the battery and 37837 in the alternation**, while the base binary (session O's code) hit the same configuration twice. One binary, one configuration, two wrong lengths — S14 step 1's discriminator met without going looking. Measurements 40–41; running total **41 measurements, 14 alternations, 20 acquittals**. Session close was **OVERALL: FAIL(2)**, both steps F3, adjudicated under S14 and acquitted here — S27's "accepted" predicate. — *(Phase 5 session N, 2026-08-13)* **470 debug / 464 release / 20 ignored** (+2, the two new tests), Miri **330**, census **59** and duplicate-body budget **195** — both unmoved — decode goldens **57 none moved**, sweeps 341/341 both profiles on the final battery, both benches bit-identical. **Miri earned its keep on a test rather than on production code**: T5.N2's new identity test took a `*const SPicture` from `&loose`, wrote through `loose`, then read through the raw pointer — the write invalidated the shared tag. `addr_of!` and writes-first, which is S29's spelling for S29's reason; no byte gate could see it and none did. F3 drew **two** hits, one per battery, both release, both 5/5 byte-identical in isolation, both single so neither alternated, **both acquitted** (measurements 37 and 38 — ≈1/682 over 1364 swept configurations, consistent with the ≈1/800 battery rate). Measurement 37 takes the **stream** out of the signature, leaving `mt` + `sm=3` + `t∈{2,4}` + any wrong length as everything that has survived thirty-eight measurements. Session close was **OVERALL: FAIL(1)**, that one step, adjudicated under S14 and acquitted in the log entry — S27's "accepted" predicate. Its first isolation attempt reported a false reproduction, because `compare.sh` `cd`s to the repo root and a relative `<yuv>` path resolves elsewhere while the stale output file answers `stat`. — *(Phase 5 session M, 2026-08-13)* **468 debug / 462 release / 20 ignored**, Miri **328** (~908s, both probes), census **59**, decode goldens **57 none moved**, sweeps 341/341 both profiles on the final battery, both benches bit-identical. **Two census numbers moved and both are subtractions**: the allowlist is 60 → **59** (T5.M1's rename left one `SDqLayer` declaration, and the instrument only reports names declared more than once — so the line was *removed*, not re-keyed to `x1` as the brief said), and the duplicate-body budget is 198 → **195**, its first decrease since session A widened the instrument. **That second number is the session's best lesson about instruments**: F22's unification deleted **eight** duplicate function pairs and the count moved by three, because `find_stub_bodies.py --dups` can only see pairs whose bodies stayed *identical* — the five that had diverged, the ones the finding was about, were invisible to it by construction. A duplicate-body count is a floor on duplication, never a measure of it. Miri and the test totals are unchanged across four faces, because none of them needed a new test. F3 drew **one** hit (measurement 36, release, zero-length output), 5/5 byte-identical in isolation, one hit so no alternation, acquitted — and it took `cabac` and `rc` out of the signature, leaving `320x192 t=4 sm=3 n=600` as the susceptible configuration. — *(Phase 5 session L, 2026-08-12)* **468 debug / 462 release / 20 ignored**, Miri **328** (~908s, both probes green), census **60**, decode goldens **57 none moved**, sweeps 341/341 both profiles on the first battery and **340/341 debug** on the second, both benches bit-identical. **Every figure but the ratchet is unchanged across a session that flipped seven families** — no test was added, because no family needed one. F3 drew **one** hit (measurement 35) and its isolation re-run **reproduced it twice in ten**, the second reproduction in the finding's history and the same configuration and wrong length as measurement 29's; one hit, so no alternation (S14 step 1), acquitted. — *(Phase 5 session K, 2026-08-12)* **468 debug / 462 release / 20 ignored**, Miri **328** (~910s, both probes), census **60**, decode goldens **57 none moved**, sweeps 341/341 both profiles, both benches bit-identical. **F3 drew four hits — two per battery, one battery per profile — and both alternations acquitted** (control 8 vs head 9 over 2880 configurations per side, measurement 34); the rate under sustained back-to-back presets measured **≈1/307**, and **`n=600` fell out of the signature as a condition** on the first `n=1500` hit in 34 measurements. — *(Phase 5 session I, 2026-08-12)* **463 debug / 457 release / 20 ignored**, Miri **324** (~524s), census **60**, decode goldens **56 rows none moved**, sweeps 341/341 both profiles, both benches bit-identical — every figure unchanged across the session, which retrofits accessors and adds no test. **The Miri gate's reach has a limit worth naming here** (F34): `decode_slice_loop_runs_under_the_aliasing_checker` decodes `narrow_16x16.264`, **one macroblock per frame**, so no neighbour is ever available and no neighbour-dependent path has been under the checker for the whole phase. F34 is a real UB site that this gate executed and passed for exactly that reason. A second probe stream with a real macroblock grid is worth more than the next family flip. — *(Phase 5 session D, 2026-08-11)* **449 debug / 443 release / 20 ignored** (+1 at T5.D2: `decode_slice_loop_runs_under_the_aliasing_checker`, the only unit test that decodes a real stream). Miri **309** — unchanged, because that test ships `#[cfg_attr(miri, ignore)]` until **F23/F24** close, and the attribute is labelled a debt at the site. **The gap it exposed is the headline**: `gates.sh` runs Miri over `--lib` and, at `exit`, over `tests/*differential*.rs` — kernel, plane and bits level, all calling internal functions — so **no Miri-covered path had ever entered the decoder's public API or its slice decode**, and the first one that did found two UB defects in two runs. Ratchet regenerated per S16: `raw_ptr` 4595 → **4600**, `unsafe_block` 613 → **614**, both in `decode_slice.rs` and both the new test. F3 **measurements 27–28**, one hit per profile, both signature, both 5/5 byte-identical in isolation — and S14 step 2's alternation was **replaced by a hash**: the `rust_enc` binary is byte-identical with the test present and stashed, since `#[cfg(test)]` code is absent from a dependency build, so base and head are the same binary and an alternation could not produce information. *(Session C's reading was 448/442/20 and Miri 309.)* — *(Phase 5 session C, 2026-08-11)* **448 debug / 442 release / 20 ignored** (+5 at T5.C3: two on `PaddedPlane::empty`, two on `SPicture`'s plane accessors, one on `AllocPicture`'s parse-only arm). Miri **309 `--lib`** (+5, the same tests) — **and one of them is the session's whole point**: `data_ptr`'s first draft narrowed provenance to `[origin..]` and was UB at the first read into the picture's padding, while 448/442 tests, all 56 golden rows, both benches and 341/341 sweeps agreed with it. Nine gates passed a soundness defect on the mainline decode path; Miri failed on the first run. S15 is not a formality. *(Session B's reading was 443/437/20 and Miri 304.)* **Decode goldens 53 → 56 rows**, additive, none moved: three narrow-frame assets at T5.B1, of which only `narrow_16x16_idr_lost` covers F21 — the other two are green under a revert of its fix, because the divergent copy's one call site is a concealment path. *(Session A's reading was 440/434/20 and Miri 300.)* **New gate at `commit` level: the duplicate census** (`census.sh`, 61 allowlisted entries + three budgets), proven red on both a planted duplicate declaration and a planted double cast before being trusted. Prior reading was — unchanged across session C, which deleted a test *arm* rather than a test (`nonzero_count_duplicates_agree` lost the decoder copy it guarded, because T4b.3c deleted that copy). T3.0's **2316 malformed-stream golden rows over 12 files** are part of the battery and **none is `WITHHELD`** since T3.3 — it goes on gating Phases 5 and 8. Sweeps 341/341 both profiles. **Two Miri skips, not three** — F13's `manage_dec_ref` site was closed at Phase 5 T5.B2 (six `as_ptr()`/`as_mut_ptr()` list shifts, not the one the finding named, plus three test-side instances the skip had been hiding); F12's thread pool (Phase 7) and F13's `encoder_ext` (Phase 6) remain. **The phase-exit level ran and its widened Miri is clean on all four targets** (295 / 20 / 7 / 3) — F18's backlog class, checked at the one level that can see it. F3 per S14 — **twenty-five measurements, ten alternations, ten acquittals**; session A's is the first **even** split (base 4 / head 4 over 12 `mt` sweeps per side, 1440 configurations each), on a session whose every commit is decoder-only while the sweep compares encoders. **Session C's contribution is the first alternation that is not 0/0**: 12 `mt` sweeps per side, 1440 configurations each, base 2 hits / head 3, and the head side hit one configuration twice with two *different* wrong lengths — the finding's own race criterion met on the tree under suspicion, which is a stronger acquittal than a rate comparison. It also narrows the signature to **`n=600`, not `sm=3` generally** (all nine of the session's hits). **F17 is fixed and proven**: `gates.sh` takes every verdict from `${PIPESTATUS[0]}` plus log corroboration |

...
| **Phase 7 in progress** | Sessions **A** (2026-08-20) and **B** (2026-08-20, `9a392cc9..`), **B handing steps 6-7 to C at a green boundary**. **A's result was a finding, F67**: the fork/join's target shape needs a third premise nobody wrote down — `shared` must cross the spawn, which is a property of the *type* — and `sWelsEncCtx` is `!Sync` for **12** distinct reasons, so the context split is a **precondition** of the fork/join and the two-session plan had it backwards. The **105 tagged items the phase was handed are, item for item, the slice-encode tree's raw signatures**, with zero in the three seam files: the inventory and the blocker are the same object. Resolved by **D-mt-1** (§7.4) taking F67's option 2. **Session B built it and then found what the phase was actually for.** The seam: **one** hand-written `Send` on `SliceJobHandle` (`slice_multi_threading.rs`), tagged `send-seam(Phase 9)`, three-part argument — index-based disjointness now *static*, order-based assembly, and **concurrency capped at the allocated buffer count** (F67's unwritten bound, now `SSliceThreading::uiThreadBsBufferNum` and `debug_assert!`ed). `std::thread::scope` replaces the pool for **every** slice mode including `SM_SIZELIMITED`, whose claiming order turned out never to have been a queue — partition is a static modulo, the slice indices a static progression, and `ReOrderSliceInLayer` reads the stamp, so a static partition reproduces it exactly and a counter or `mpsc` would have *added* nondeterminism. Then **2,115 lines deleted** in two files: `unsafe_impl` **12 → 2**, `raw_ptr` −76, `unsafe_fn` −22, `unsafe_block` −44; `TaskPtr`, `CWelsTaskThread`, both C++-list ports, the mutable process-wide singleton, the `self as usize` spawn laundering, the whole task hierarchy, the claiming mutex, `mutexEncoderError`, `pTaskManage`, and `SSliceThreadPrivateData` (**zero readers**, allocated every frame-setup since the raw translation). **F12 closes by deleting what it named.** **F3 IS CLOSED, and not by any of that.** Session B ran the ablation as **three arms with one variable each** rather than the charter's single after-arm: before 25/3000 (1/120) → **arm A, whole pool machinery deleted, 20/2000 (1/100), z = 0.61, p ≈ 0.54 — no change** → **arm B, one restored mutex, 0/2000, P ≈ 2e-9**. **The hypothesis the charter fixed in advance was false.** F3's cause is **F69**: `68c4f6a5 "Raw translation"` dropped the `WelsMutexLock(&mutexSliceNumUpdate)` that brackets `AddSliceBoundary` and `++iSliceNumInFrame` in `svc_encode_slice.cpp:1776-1791`; `pSliceCtx` is the *layer's*, shared by every worker, and a lost increment makes `ReOrderSliceInLayer` answer `ENC_RETURN_UNEXPECTED` — **an empty frame**, F3's shape in 18 of 25 before-arm hits. It was found on this session's *delete list*, by asking which fields were dead and reading the reference to check. **Three more findings came the same way**: **F68** (fixed — the encoder bench never set `uiSliceMode`, so every `[4t]` row on both sides ran single-threaded; the first honest numbers show **the port scaling 1→2 threads and then stopping** where C++ keeps going); **F70** (fixed — `InitSliceSettings` read a slice-argument through a tag its own callee popped, on the arm every one of 369 multi-slice sweep rows takes); **F71** (partly fixed — S28's root-accessor idiom is a `Unique` retag over the container, so sixteen accessors raced under any concurrency; the residue is a shared write the C++ makes too and needs F67's context split); **F72** (open, fenced — the load-balancing path is **half-translated**: `CalcSliceComplexRatio`, the producer of the only input `DynamicAdjustSlicing` consumes, is never called, so the balance is *degenerate rather than absent*, and it is **on by default** through `GetDefaultParams`). Determinism held throughout: **369/369 both profiles** on every step gate. **Remaining for C**: step 6 (the last four allocator sites, `pMemAlign`, `memory_align.rs`, F61, F36), step 7 (module denies, the 105 re-tags, the span, the unscoped exit battery, the phase close). |

...
| **Ratchet** | *(Phase 5 session X, 2026-08-16)* `unsafe_fn` **1156**, `raw_ptr` **3593**, `unsafe_block` **531**, `mem_zeroed` **30**, `SHIM(` **115**; decoder-only `raw_ptr` **456** (phase5.md's monotone exit metric, 1283 at the re-plan, 765 at X's open). **The largest single-session movement the instrument has recorded**: −499 `raw_ptr`, −59 `unsafe_fn`, −57 `unsafe_block`, −45 `SHIM(`, and **not one per-file increase**. Two of those numbers are deletions rather than conversions and the distinction is the session's, not the metric's: 82 of `mv_pred`'s casts were two helpers with no callers since T5.R5, and 46 of the `SHIM(` fall are Phase-2 strangler wrappers whose kernels the dispatch tables now name directly. **The ratchet also caught two of X's own comments** (S16, for the fifth session running) — a note explaining the shim deletion contained the token `SHIM(`, and a new test added an `unsafe` block to a file whose budget was full; both reworded rather than re-baselined. *(Prior: sessions U and T — `unsafe_fn` 1245, `raw_ptr` 4117, decoder-only 980; session R — 1248 / 4117 / 980; session P″ — 1257 / 4368 / 1229.)* |
| **Gates** | *(Phase 5 session X, 2026-08-16)* **484 debug / 478 release / 20 ignored**, census **59**, decode conformance **60/60** (58 asset rows) with **zero golden hash literals changed all session**, both benches bit-identical, ratchet clean, malformed corpus **2690/17 output, 2707/0 codes** end-to-end against the C++ — unmoved from W's close across two behaviour fixes. Close was **`OVERALL: PASS` at `exit`, every step green** — 13 passed / 0 failed / 1 skipped, Miri `--lib` **339/0** plus the three differential targets (**20 / 7 / 3**), sweeps **341/341 in both profiles** with **F3 drawing zero hits on the final run**. **The `exit` battery earned its keep and that is the row's point**: its first run at `574bd987` failed Miri `--lib` on **undefined behaviour this session had created** (F53 — a raw alias into the layer, sound while its parent was a pointer and UB the moment the parent became a borrow, with no line of it edited). No cheaper gate could see it: the sweeps are the encoder's, the goldens and the corpus are byte comparisons, and the read returns the correct byte on every real target. **The second phase-5 session running to be told this by Miri rather than by review** (T5.W11 was the first), which is why F53 states the rule as a work-list item rather than as a caution. *(Prior: session W — 483/477/20, Miri 338/0 + 20/7/3; session T — 482/476/20, Miri 337.)* |

...
| **Next** | **Phase 7 — the multi-threading seam** (the MT files, the thread pool, and the slice-bank ownership). Phase 6 hands it: the two undenied modules `slice_multi_threading.rs` and `wels_task_management.rs`, exempted at their `pub mod` line in `encoder/mod.rs` (an inner attribute on a module file reaches every file below it, so the exemption is spelled at the declaration and both allows retire with the phase); **89 `port-raw(Phase 7)` items plus 16 `MT` items**; **F61**, the slice-bank growth that never re-stamps the layer's slice list; the **4 live allocator call sites** of the 12 that remain (`DynamicSliceBs` and `sSliceBs.pBs`, malloc and free each — the other 8 are the dormant screen-content ones); `pMemAlign` and with it `common/memory_align.rs`; **F12** and the thread pool, still the only `--skip` in the Miri `--lib` gate; and **F3's ablation**, which Phase 6 leaves at its fastest-reproducing configuration on record — `mt CiscoVT2people_320x192_12fps sm=3 n=600 t=4 cabac=1`, **~1 in 11** under load against the finding's recorded ~1/800. **Open the ablation there.** No brief written yet |

...
Known verification caveats (from prior sessions, they still apply): a Rust decoder error silently drops frames from the conformance hash — **always compare frame counts, not just hashes**; JVT/ffmpeg golds can't judge B-slice streams where upstream C++ itself diverges (the 13 `#[ignore]`s in e2e are permanent fixtures, their set must not change); all C++-vs-Rust perf numbers are scalar-vs-scalar because the C++ dylib never dispatches NEON.

Two sharper caveats about the *encoder's* evidence (measured 2026-08-07 at `eb463dbd`):

1. **All encoder parity evidence is debug-build.** `diffharness/build.sh` runs plain `cargo build`; a `--release` build segfaults on a null `(*pCurMb).pNonZeroCount` in `encoder::deblocking::WelsNonZeroCount_c` (`deblocking.rs:873`, reached via `pfSetNZCZero` from `DeblockingBSCalcEnc`) while the debug build encodes the same input for 200 frames. That is latent UB surfacing under optimization — a live instance of exactly the T5 derived-pointer-wiring class this plan exists to eliminate, and proof that "the tests pass" and "the code is sound" are different claims here. Phase 0 must root-cause and fix it *first*: you cannot run byte-exactness gates on top of UB, because UB has no defined behavior to preserve.
2. **All encoder parity evidence is explicit-param.** `cxx_enc.cpp` pins ~40 fields (frame-skip, scene-change, background detection, `bFixRCOverShoot` all off). The ordinary `GetDefaultParams` + `InitializeExt` path diverges from the C++ partway into a sequence (frame 17 on the synthetic 320×240 pattern; RC-involved but not RC-alone). This is a **pre-existing correctness bug, out of scope for the refactor**: gates pin to the harness-drivable configs, the bug is tracked separately, and no refactor commit may silently change behavior on that path either.

Also: commit `eb463dbd` deleted the port's handoff/status docs and the diffharness from git; the harness currently exists only as untracked files under `rust/tools/`. Phase 0 re-commits it — the plan's gates depend on it.
...
1. **Fix the release-build segfault** (§1.4): root-cause the null `pNonZeroCount` in `encoder/deblocking.rs:873` — almost certainly a T5 wiring gap (an `SMB` whose derived pointers were never (re)wired on some path, masked by debug-build layout/codegen). This is the one behavior *fix* permitted before the refactor starts, because every subsequent gate is meaningless in a profile that crashes. Landing it also gives the plan its first regression test: the diffharness gains a `--release` variant and both profiles must agree byte-for-byte from here on.
2. Re-commit the diffharness (`rust/tools/`, currently untracked after `eb463dbd`) and recover the deleted handoff/status docs into `rust/docs/` (`git show eb463dbd^:rust/docs/...`) — they hold measured facts (knobs the C++ itself rejects, non-byte-verifiable comparisons, the zsh word-splitting trap in sweeps) the gate battery depends on.
3. Record baselines: conformance hashes + frame counts, diffharness sweep outputs **in both build profiles**, bench numbers (3-run medians), unsafe-ratchet counts (§7.1) checked in as `rust/tools/unsafe_baseline.json`. Add a `GetDefaultParams` mode to the diffharness and record its (currently diverging) behavior as a pinned known-failure baseline so refactor commits can prove they didn't move it.
...
4. Diffharness: at minimum the gate configuration; full sweep (slice modes × RC × threads) at phase exits.
...
|---|---|---|---|
| More latent UB like the release-deblocking segfault surfaces mid-refactor (an optimizer or layout change flips behavior that was never defined) | Medium | High | Phase 0 fixes the known instance and adds the dual-profile gate; any later debug/release disagreement halts the phase until root-caused — it is always a pre-existing bug being exposed, and fixing it is in scope |

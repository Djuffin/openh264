...
| **Ratchet** | *(Phase 4b exit, 2026-08-11)* `unsafe_fn` **1250**, `raw_ptr` **4815**, `unsafe_block` **613**, `SHIM(` **157** (of which **`SHIM(phase3)` is 2**, both enumerated and both owned by Phase 6; 154 are `SHIM(phase2)` and 1 is `SHIM(phase5)`), `no_mangle` 24, `mem_zeroed` **32**, **`transmute` 4 — and this number has a floor: all four matches are prose and the crate contains zero calls.** Do not chase it. The metric never covered the whole population either: T4b.3c found `as *mut _ as *mut _` doing a transmute's job, and a double cast does not match the grep — for the rest of that family use `find_dup_types.sh`, not this metric. Session C's deltas: `raw_ptr` **-19**, `unsafe_fn` **-9**, `transmute` 5 → 4. *(Phase 4b entry read: `unsafe_fn` 1286, `raw_ptr` 5001, `transmute` 23.)* Baseline regenerated per S16 at every seam. Run `bash rust/tools/unsafe_ratchet.sh check`; never trust a remembered number |
| **Gates** | *(Phase 4b exit, 2026-08-11)* **435 debug / 429 release / 20 ignored** — unchanged across session C, which deleted a test *arm* rather than a test (`nonzero_count_duplicates_agree` lost the decoder copy it guarded, because T4b.3c deleted that copy). T3.0's **2316 malformed-stream golden rows over 12 files** are part of the battery and **none is `WITHHELD`** since T3.3 — it goes on gating Phases 5 and 8. Sweeps 341/341 both profiles. Miri **295 `--lib` tests**, same three skips (F12 thread pool, F13's `manage_dec_ref` and `encoder_ext`). **The phase-exit level ran and its widened Miri is clean on all four targets** (295 / 20 / 7 / 3) — F18's backlog class, checked at the one level that can see it. F3 per S14 — **twenty measurements, eight alternations, eight acquittals**. **Session C's contribution is the first alternation that is not 0/0**: 12 `mt` sweeps per side, 1440 configurations each, base 2 hits / head 3, and the head side hit one configuration twice with two *different* wrong lengths — the finding's own race criterion met on the tree under suspicion, which is a stronger acquittal than a rate comparison. It also narrows the signature to **`n=600`, not `sm=3` generally** (all nine of the session's hits). **F17 is fixed and proven**: `gates.sh` takes every verdict from `${PIPESTATUS[0]}` plus log corroboration |
...
**Phase 4b is complete.** Phases 5-9 not started; Phase 5 is next per D-seq-1.

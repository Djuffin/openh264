...
| **Next** | **Phase 5 — the decoder structural rewrite, the plan's first pivot** (9–12 sessions), **in progress, sessions A–I**. 5.1 is closed but for `PicPool`/identity (deferred, accepted 2026-08-11); **5.2 is half done and the second half has Eugene's direction — **probe first, then flip** (D-perf-5's return, §7.4: a multi-MB probe stream, then the hot eleven hottest-first under D-perf-4 at 7 pairs with day-two confirmation, stop-line ~+23% CB)** — `MbGrid` exists, is proven under Miri and is owned by `SDqLayer`, which since T5.H3 can no longer be constructed by zeroing, and **11 of its 22 per-macroblock array families have flipped** (T5.H4–T5.H14). The aliasing probe has been an un-ignored gate since T5.G1 and stayed green through all of it. **D-perf-5's measurement is in and it is negative** (session I): the per-MB window retrofit landed across all eleven flipped families, turns 76 bounds checks per macroblock into 38, and recovers **nothing** — decode median **+0.14%** at 7 pairs against a bar of −0.66%. Disassembly and profile say the functions involved are 1–3% of decode time, so the recovery was never there. **Session I stopped at D-perf-5 §3.4's instruction; whether the hot eleven flip is Eugene's call on those numbers.** Two new facts go with it: the flip's own span re-measured at **half** what session H recorded (+0.65% decode / +1.18% CB from the same two stashed binaries, 7 pairs, a different day), and this null result does **not** transfer to the hot eleven, which are indexed *inside* the record in inner loops where the element re-type is the mechanism instead. Brief written at 4b's exit per S19: [`prompts/phase5.md`](prompts/phase5.md). Per-session scope is **the S20 closure, not the file**. The three rules it inherits by name are **S24** (re-grep every shape-deciding count), **S25** (Phase 5 *is* raw-to-borrow conversion, so the re-entrancy audit is planned work) and **F19's class** (a `Box::into_raw` with no live `from_raw` is a leak no gate here can see — 5.5 runs that check decoder-side). Order per D-seq-1: 4b → 5 → 6 → 7 → 8 → 9 |
| **Governing decisions** | **D-perf-4** (§7.4 v3): swap-and-ledger by default, +25% median cumulative tripwire, no optimization boxes, byte-exactness never traded. **D-perf-5** (2026-08-12): recovery-first at 5.2's midpoint — **answered at session I, negatively**; the retrofit recovers nothing; **Eugene's direction on the return: probe first, then flip** (§7.4). **D-seq-1**: 4a ran before Phase 3 — done. **D-perf-3's fallback is now spent, on the decode side only** (see the finding below) |
...
| **Open findings** | [`phase0_findings.md`](phase0_findings.md) F1, F3 (**F2 RESOLVED at T3.4** — one writer, zero bytes of output moved, and the finding's own inventory turned out to be one divergence short; F3 now has **twenty-five** measurements and **ten alternations, ten acquittals**, and session C's is the first that produced hits on both sides — it also narrowed the signature to `sm=3` **`n=600`**; measurements 16-18 are Phase 4b session A's — the ones that finally measured its *rate*, ~1 in 800 configurations under load, by alternating whole sweeps instead of single configurations — and the nineteenth is session B's **zero across eight sweeps** (P(0) ≈ 3% at the measured rate), which sharpened the signature: the load must be *sustained*, back-to-back sweeps, not a battery's interleaved ones), [`phase1_findings.md`](phase1_findings.md) F4, F6, F7 (**F5 RESOLVED at T3.4**, by deletion, exactly where its "who fixes it" line predicted), [`phase2_findings.md`](phase2_findings.md) F8–F14 (**F13's third site CLOSED at T3.4** — `InitBits` deleted rather than amended — and **its first, `manage_dec_ref`, CLOSED at T5.B2**, where it turned out to be six sites and to have three test-side instances behind its Miri skip; **two remain**, `encoder_ext` for Phase 6 and F12's pool for Phase 7), [`phase3_findings.md`](phase3_findings.md) F15–F18, [`phase4b_findings.md`](phase4b_findings.md) **F19–F21, all FIXED in-phase**, [`phase5_findings.md`](phase5_findings.md) **F22 (reachability ANSWERED at T5.D1), F23 (OPEN, Phase 8's), F24–F34 all FIXED in-phase** — **F34 is the one to read before the next flip**: T5.H8's flip turned a `bool*` out-parameter into `&mut bool`, and `ParseTransformSize8x8FlagCabac` reads *its own array* at the neighbour addresses while holding that strongly-protected borrow, which `Vec`'s whole-buffer `Index` retag removes. Fixed at T5.I2, found by T5.I1's window analysis rather than by any gate, and **proved on a standalone reproduction under Miri rather than argued**. Its lesson is about the instrument: the aliasing probe *executed this function and passed it*, because `narrow_16x16.264` is one macroblock per frame so neither neighbour read runs. **S22 aimed at a stream instead of a scope list** — the probe's coverage is its asset, not its target list — **F32 and F33 are the pair that made 5.2's flip mechanical**, both found by inventorying the 24 per-MB arrays *before* converting them rather than by any gate: F32, two arrays whose declared pointee disagreed with their allocation by 8x and 16x, and F33, two arrays with no reader in either tree (one of which has 14 live writes and would be reported as used by every reference-counting instrument — an unread write hides from exactly the tools that find unused fields) — F24/F25 at T5.E1, F26 at T5.F2 (the allocator stopped laundering provenance, which un-blinded *both* codecs' heaps), and F27–F30 at T5.F3, which is S22's backlog arriving on schedule: the repaired instrument found four defects in six round trips, three of them new. **F26's write-up carries a correction worth reading before trusting any single-site diagnosis**: the site it was found at fails with a *tracked* tag too, so it was never the allocator's defect — that is F27 — and the one-run experiment F26's entry demanded returned a third answer where the finding had enumerated two. **F22, owner 5.3** — `parse_mb_syn_cabac.rs` re-translated six `mv_pred` functions and three of them lost a `pDec` null guard, F21's class again, found by widening `find_stub_bodies.py` to `src/decoder`, which it had never scanned in three phases of running. Session D settled its open question and corrected its arithmetic: **`pCurDqLayer->pDec` cannot be null on the CABAC parse path** — one writer crate-wide, one call site, dominated by a prefetch-or-return, and identically so in the C++ — so the guard is dead code in both trees and this is a divergence, not a latent crash. And the C++ side of its "28 vs 0" had **never been greped** (S24): `mv_pred.cpp` has **20** null tests, not 28, because it mostly spells them as a ternary that a search for `NULL` misses, and `parse_mb_syn_cabac.cpp` has **0** of its own with 28 unguarded `pDec` dereferences — so the CABAC copy is *faithful* and the divergence is **three** functions, with `Update8x8RefIdx` running the other way (C++ unguarded, `mv_pred.rs` the copy that added a guard). 5.3 unifies per function, not per module. **F23, owner Phase 8** — every one of the **19 `&mut self` methods** on `ISVCDecoder`/`ISVCEncoder` is UB: the structs are one pointer wide, the borrow covers eight bytes, and the vtable thunk casts base-to-derived and writes at offset `0x20`. All **11** integration test files drive the codecs that way, as would every Rust consumer; C and C++ callers reach the thunks directly and are unaffected. **F24, owner 5.5, and it blocks 5.2** — `ParseSliceHeaderSyntaxs` (`decoder_core.rs:2000-2576`) takes `&mut` of a NAL unit's `sSliceHeaderExt` *after* `&mut` of its nested `sSliceHeader`, invalidating the inner borrow, which is then used 74 times; 141 uses of three overlapping borrows, T5.B2's fix shape. **Both were found by one test and neither is byte-visible**: `decode_slice_loop_runs_under_the_aliasing_checker` decodes a 711-byte stream under Miri, and it is the first Miri-covered code path that ever entered the decoder's public API or its slice decode — the `exit`-level Miri targets are the three `tests/*differential*.rs`, which call internal functions directly. S22's "an instrument's scope list is part of the instrument", third instance. The test ships `#[cfg_attr(miri, ignore)]` until F24 closes, labelled a debt at the site; **the queue depth past F24 is unknown** — the probe reports one defect per run and has not yet reached the loop it was written to examine. **F21 (FIXED at T4b.3b, PINNED at T5.B1)** is the class to carry forward: the port had translated one C++ function (`expand_pic.cpp:388`) **three times**, and two copies had drifted in the sub-16-chroma case — `manage_dec_ref`'s had no `else` at all, so a frame narrower than 32 pixels kept an unexpanded chroma border. Unreachable by any gate here **by construction**, not by luck: the smallest legal frame is 16 pixels wide and every corpus stream is 176x144 or larger. Fixed by unification, not by patching the missing branch — *a duplicate family is a divergence that has not happened yet.* **Pinned at T5.B1** by three narrow-frame assets, of which — and this is the part to carry — **only the one that conceals covers it**: the two clean narrow rows pass with the fix reverted, because the divergent copy served exactly one call site and it is `WelsInitRefList`'s concealment prefetch. F19 and F20 are T4b.2a's: F19 is a leak of the parameter-set strategy on every encoder teardown (C++ deletes it at `encoder_ext.cpp:1995`; the port did not) that **no gate in this project could catch** — found by the ownership audit an `Option<Box<_>>` conversion forces, not by a test; F20 is three stale comments claiming one ported paraset strategy where there are two, which had propagated into a session brief and nearly into a conversion. **All four of Phase 3's own findings are closed**: **F15 FIXED at T3.3**, **F16 resolved at T3.1b with its second instance and the whole stored-extent class closed at T3.3** (nothing stores an extent any more, so it is unrepresentable rather than repaired), **F17 fixed and proven red** at session C's start (`eae61b94`), and **F18** — an address-comparison test that had been failing the Miri gate since *before* F17 made that gate capable of failing — found and fixed at the exit. **F18's lesson: a repaired gate has a backlog, and it surfaces only at the level and moment the repaired gate first runs**; the `exit` level runs once per phase. **F3's session-start advice, revised again at 4b session C: a start-of-session hit is a tendency, not a rule — three of the last six. Apply S14 to whatever arrives; a single-configuration re-run that *does* reproduce is evidence and is cheap, so run it first, and escalate at sweep level with nothing between the sweeps (S23b).** |

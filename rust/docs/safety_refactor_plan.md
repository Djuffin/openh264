...
| **Ratchet** | *(Phase 5 session T, 2026-08-14)* `unsafe_fn` **1245**, `raw_ptr` **4117**, `unsafe_block` **618**, `mem_zeroed` **30**, `SHIM(` **160**; decoder-only `raw_ptr` **980** (phase5.md's monotone exit metric, 1283 at the re-plan). **Every metric is unchanged from session S's close, and that is the correct outcome rather than a stall to explain away**: T was scoped by D-par-1 as parity work, W6/W7 are deferred, and the instrument counts pointer *types written* (S16) — it has nothing to say about whether the decoder returns the error code the C++ returns. The session's metric is the corpus one in the Gates row. *(Prior: Phase 5 session R — `unsafe_fn` 1248, `raw_ptr` 4117, decoder-only 980; session P″ — 1257 / 4368 / 1229.)*

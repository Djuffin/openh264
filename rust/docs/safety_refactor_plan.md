...
| **Phase 8b in progress — sessions A, B DONE (`925ac828`..`65042ac9`), session C's brief written** | *Correctness and parity before safety (D-prio-1; Phase 10 now after Phase 9).* The tally is a **seed-pinned** ratchet (S49): `gtest_stretch.sh --check` at `SEED=20260822` is gate 6c of `exit`, allowlist `tools/abi_harness/gtest_known_failures.txt` — **173/199 at B's close, 26 rows, every row owned** (8b.C 18, Phase 10 7, D-poc-1 1). **Session A** (155→179→165): the decoder option arms whole, `ForceCodingIDR` (F86), the census and its instruments, F80 reachable (F87), the F77 instrument. **Session B** (164→173 at the pinned seed — A's 165 was an unseeded sample, **F89**): `DecodeParser` ported whole — **four** missing pieces not three (the `sSavedData` EBSP capture the brief missed, the caches, the copy-out), referee `ecref --parse-only` + `decoder_parseonly_parity_test.rs` (6 assets agree, 1 diverging) + a `dlopen` harness part, so the last decoder slot has a referee (S47 discharged); and all five parameter-set strategies with `WriteSavcParaset_Listing` and the validation adjustments, five targeted pairs byte-identical, the 17-hook call map read site by site (`UpdatePpsList` was the one with no port call — F76/F77 again). **F88 reproduced at seed 5/10** and is a *concealment* divergence (`iBufferStatus`, `decoder_ec_test.cpp:302`, not the `rv` assertion A recorded). **F96**: `ERROR_CON_DISABLE` had no referee in any decode entry point — `ecref_rs` (the same program built against the port's cdylib) finds `Error_I_P.264` diverges there, `InitRefPicList` returns `ERR_NONE` where the reference returns `ERR_INFO_REFERENCE_PIC_LOST`. Findings **F89–F96**; two ratchet rebaselines, both `port-raw(Phase 9)` tagged. Close (D-gate-3, fast set): commit gate PASS 528/523/20, `--check` rc 0 173/26, exports 7/7, harness **14/0** (parse-only part ×2), corpus 2902/17·2919/0 unchanged. **C next** ([`prompts/phase8b_session_c.md`](prompts/phase8b_session_c.md)): downsample + denoise (17 rows) — **headline risk: the reference links NEON downsamplers, the port would translate `_c`, and upstream's own dual-hash rows say the two round differently; measure identity first** — the `sp` preset, F80/F87 (`safe::Pool` growth), F96, F88, F93, `ParseOnly_General`. Charter: [`prompts/phase8b.md`](prompts/phase8b.md). |
...
`--filter` passthrough for per-family runs. (2) A diffharness `sp` preset
...
the pinned seed. **C**: downsample + denoise + the `sp` preset (17 rows) — **measure

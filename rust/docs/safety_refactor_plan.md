...
- **S27 — a docs-only tail does not buy a full battery** (2026-08-11, Eugene's
  direction). At session open, when **all** hold — the inherited tail touches only
  `rust/docs/`, the previous session ended `OVERALL: PASS`, and `rust/tools/` and
  the toolchain are unchanged — open with the cheap subset: build both profiles,

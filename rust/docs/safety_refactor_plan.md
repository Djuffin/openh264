...
| **Ratchet** | *(Phase 5 session Y, 2026-08-16)* `unsafe_fn` **1155**, `raw_ptr` **3520**, `unsafe_block` **526**, `mem_zeroed` **30**, `SHIM(` **114**; decoder-only `raw_ptr` **383** (phase5.md's monotone exit metric, 1283 at the re-plan, 456 at Y's open). The movement is one face's: −73 decoder `raw_ptr` across four commits, of which **63 are the slice view's** — 99 functions below the bracket stopped taking a context, and two accessors (`cabac_ctx_base`, `cabac_rbsp_window`) retired with no callers left. **The `unsafe_fn` column barely moved (−1) and that is the session's structural news**: the keyword outlived the pointers it was there for, so `parse_mb_syn_cabac` now carries 34 `unsafe fn` against **10** raw-pointer occurrences and `mv_pred` 21 against 4. Dropping it module by module is what moves deny-clean off 11 of 22. Two per-file budgets were met by *deleting* rather than by re-baselining (S16, sixth session running): the view's constructor is +2 `unsafe fn` in `decoder_context.rs`, paid for by the two retirements in the same file. *(Prior: session X — 1156 / 3593 / 531 / 115, decoder-only 456; sessions U and T — 1245 / 4117 / 980.)* |
| **Gates** | *(Phase 5 session Y, 2026-08-16)* **481 debug / 475 release / 20 ignored** (three null-context tests deleted with the arm they asserted — a `&mut` cannot be null, and the guard moved to the bracket tops), census **59**, decode conformance **60/60** with **zero golden hash literals changed all session**, both benches bit-identical, ratchet clean, malformed corpus unmoved. Close was **`exit` 12 passed / 1 failed / 1 skipped**: Miri `--lib` **336/0** plus the three differential targets (**20 / 7 / 3**), debug sweep **341/341**, and the one failure a single F3 hit in the release sweep at the documented signature (`mt … t=4 sm=3`, short output), acquitted by a 12-round alternation. **Miri earned its keep twice in one session** — once on this session's own UB (the view's first spelling derived the RBSP window from `pCtx->pNalCur`, a second path to a NAL the caller held as `&mut`), and once on the *class* that reverted the flip: every raw pointer a context accessor derives dies at the next call that takes the context. Neither is visible to a byte gate, a golden or a corpus row. *(Prior: session X — 484/478/20, `exit` 13/0/1, Miri 339/0; session W — 483/477/20.)* |

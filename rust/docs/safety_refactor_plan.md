...
| **Phase 9 — after 8b; Phase 10 follows it (D-prio-1 as amended 2026-08-22)** | The hygiene endgame (§4, *2–3 sessions*), and Phase 8 changes what it starts from. **The crate-root `deny(unsafe_code)` is 67 of 80 modules already** — 61 deny, 6 forbid — and **`src/api/` is one of them**, so §2.2.8's "single scoped allow on `src/api/`" is superseded by 44 individually tagged allow items; the remaining 13 modules are `lib.rs`, `api/mod.rs`, `safe/prng.rs` and **the whole of `src/common/`, the one module tree no phase has owned**. The queue is **830 tagged allow items**, greppable by category: `port-raw(Phase 9)` **689** (not 705 — re-grep), `cursor` **61** incl. the 22 dispatch survivors, `C-ABI` **45** which stay, `MT` **21**, `SCREEN_CONTENT(dormant)` **8** which are Phase 10's, `C-ABI(test)` **5**, `send-seam(Phase 9)` **1**. The blockers are unchanged and measured: **F66/S42**'s context split (93 context-derived-pointer sites in 28 callers; session J made the conversion, Miri refused it, and it was reverted) and **F73**'s picture-accessor family (32 + 68 sites), which the `Send` verdict waits on — **14 `E0277`s, 3 decoder / 11 encoder**, every one a `port-raw` pointer. New from Phase 8: **F80** (`IncreasePicBuff`/`DecreasePicBuff` never ported — measure reachability first), the F77 instrument (`MbGrid` accessors want `#[track_caller]` and both structures' dimensions in the message), and **the gtest tally as a to-do list with a number** — **155/199** after F82, the 44 remaining being 21 decoder `GetOption` values (`decode_api_test.cpp`), 8 encoder options, 5 `InitializeExt` rejections, 4 output hashes, 3 LTR, 3 `DecodeParser` (stub), 2 EC, 1 encoder init, 1 the documented POC tiebreak. **Tally by assertion location, not by fixture name** — that mistake is what hid F82. **D4** is now a mechanical extraction rather than a design question, because Phase 8 made the boundary crate's contents precise; **D5** is one question smaller. Perf: D-perf-6's parked recovery, cumulative encode ≈ **+15…+17%** against the **+25%** tripwire, unmoved — and **P13's guard is measurably not one of its targets**. Full block: §4, *Inherited from Phase 8*. |

...
| **Phase 8 next** | `src/api/` — the C ABI boundary and the vtable object model. **Charter [`prompts/phase8.md`](prompts/phase8.md) and session A's brief [`prompts/phase8_session_a.md`](prompts/phase8_session_a.md) written (steward, 2026-08-20)**: the api inventory first (S22), F23 and its twin (12 conveniences, 55 callers), F37/F41 fixed, the twelve api-owned fields owned by the context, `memory_align.rs` retired; B the encoder boundary and the thunks as translators; C the cdylib, the 7 exports, the external-ABI harness and the close. Inheritance re-verified at Phase 7's close: **F23**'s encoder twin (the `ISVCEncoder` thunks cast `this` and reach past the `base`/`pVtbl` pair, so a `&mut self` convenience method hands Miri eight bytes and writes far outside them — which is why `abi_test_driver` calls through the raw vtable); the `api/` back-pointer inventory; **F38**/**F41**; `CWelsDecoderImpl::align`, now one of the two things keeping `common/memory_align.rs` alive (the other is `SWelsDecoderContext::pMemAlign`, surviving as a null sentinel — the allocator is structurally dead on both sides and the file names its own blocker). `wels_encoder_ext.rs` is Phase 8's and was untouched by Phase 7. |

...
2. **The release-build segfault: no longer reproduces, never root-caused.** At `eb463dbd`, a `--release` build crashed on a null `(*pCurMb).pNonZeroCount` in `encoder::deblocking::WelsNonZeroCount_c` while debug ran 200 frames clean. After the restore/fix commits, release `rust_enc` and the release-profile benches run clean and byte-identical — but nothing that landed plausibly touches that pointer wiring, so whether it was config-specific or fixed incidentally was never established. Phase 0 includes a **time-boxed root-cause attempt at `eb463dbd`** (it's a one-command checkout away), because if it was latent T5-class UB it may still be present on some config. Until then: treat any release-only crash there as a possible recurrence, not a novelty, and capture the config immediately.
...
1. **Close out the release-segfault question** (§1.4): the crash measured at `eb463dbd` no longer reproduces after `fa67432f`/`909c368b`, but was never root-caused and nothing that landed plausibly touches the pointer wiring. Time-boxed (one session-half, hard stop): check out `eb463dbd` in a worktree, rebuild the release diffharness, attempt reproduction with the original config. Outcomes: reproduces → root-cause and check whether HEAD still carries the latent bug; doesn't → write down exactly what was tried and close as not-reproducible-with-watch-note. Either way the finding is recorded in the plan's Progress appendix and in memory.

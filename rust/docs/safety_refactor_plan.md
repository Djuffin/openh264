...
- **S56 — a reference argument is a protector for the whole call; a raw parameter never
  ...
  *created* the UB — the C++ has neither a protector nor a conflict, and `*mut SMB` had
  ...
  they read. Instrument: D-gate-4's session-close Miri; `q1c.py` models neither this
  (shape D) nor F114a (shape C) yet — both are cheap scans and the next thing it needs.

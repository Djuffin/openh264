...
| **S11** 🔄 **IN PROGRESS 2026-08-31** — brief: [`prompts/safeplan_s11.md`](prompts/safeplan_s11.md) | **S11.0 (close-out), S11.1a/b (step 1 — the bitstream seam), S11.2a–e / S11.6–S11.24 (step 2's mass), S11.3 (step 3), S11.4 (the slot types), S11.5 (steps 4+5)** — **thirty gated checkpoints**; remaining: the tail of step 2, E2, E3 + the bench debt | **Every structural item in the plan is now done, and steps 3, 4 and 5 with them.** **S11.0** paid S10's debts and **settled the Miri ratio at 2.09 CPU** (**F273** — 73% of the lane is the two fork probes, so **E3's pair must be re-budgeted**). **Step 1**, the last structural item, fell in two: S11.1a threads the bitstream pair from the chain's five tops (both resolvers deleted, ~50 re-derivation sites), S11.1b carves the worker buffers out of the pool **before the fork** so no worker derives from the shared context at all. **Step 2** then produced the session's finding and its rule: **F274**, an in-fork `&mut` to layer state S10.15 had committed *against the comment two lines above it*, which **no gate before E3 could see** — bytes never move, and the lane's encode probes are single-threaded; its probe sees the defect red and the fix green. **F275**: three items the plan had scoped as seams were borrow-*shape* questions, minutes each. **Step 3** made the screen-content downcast **unrepresentable rather than dormant** — the storage type cannot hold an `SVAAFrameInfoExt`, so `vaa_ext_ref()` answers `None` by construction and both raw accessors are deleted (D-scope-6). **S11.4** converted twelve function-pointer slot types and corrected **F276's own numbers in both directions** (**F277** — a grep for one declaration *form* missed ten slot types; and a body with three blockers retires when the *last* goes, so counting shared blockers over-counts payoff every time). **Steps 4 and 5**: the version exports moved to the ABI island with **`abi_exports.sh` 7/7 and every ratchet total flat**, two more files sealed (49 `forbid`), and **D-exit-4's enumerated floor is built** — 46 test allows across 16 files, after six mis-tagged ones were corrected. **The rest of step 2 is conversion mass with no single large lever**, and its families were priced by the compiler rather than by scanning: two C-ABI parameter families that were never on the ABI (**F280** — 82 raw pointers on two signature flips; the test is whether the name is in `abi_exports.sh`), eighteen `unsafe {}` blocks that guarded nothing and had been raising `unused_unsafe` all along (**F281**), all three "context beside a layer inside it" seams — none of which was a seam (**F282**), the BsInfo cursor family's 254 sites, where an inverted walk direction was caught by the test suite and the sweep in the same minute (**F283**), the pre-fork carve of the slice bank, and an `addr_of!` audit in which a third of the sites were ceremony (**F284**). **S11.24** cleared `ref_list_mgr_svc.rs` to a single allow by retiring `ctx_vpp_raw`'s `&mut` half — **F285**, a Miri refusal from S3.B1 whose premise (`SDqLayer::pSrcPool`) was deleted in S10.7, the session's second expired refusal | both bitstream resolvers, `thread_bs_buffer`, `bs_buffer`, `layer_sps`/`layer_pps`/`layer_subset_sps`, `vaa_ext`/`vaa_ext_mut` deleted; `dq_layer_ref`/`_mut`, `rc_and_current_layer_mut`, `MbIndexXY`, `vaa_ext_ref`, `layer_sps_ref` built; **12 slot types safe**; `api/version.rs` created; `with_vpp`/`ctx_vpp_mut` built and `ctx_vpp_raw` deleted; allows 327 → **187** |
...
| 2026-08-31 | **S11** (in progress) | S11.0 (close-out + the ratio settle), S11.1a/b (**step 1 — the bitstream seam, the last structural item**), S11.2a–e (step 2), S11.3 (**step 3**), S11.4 (twelve slot types), S11.5 (**steps 4+5**), S11.6–S11.24 (step 2 continued) — **thirty gated checkpoints**; remaining: the tail of step 2, E2, E3 + the bench debt | **327 → 187** | **Two cadences, by the user's direction.** S11.0/1a/1b ran the full `session` battery (**OVERALL PASS** each; Miri **299 → 300**). From S11.2a the battery is batched to the session close and every checkpoint carries the **differential sweep as its byte referee — byte-identical at all thirty** (583/583 through S11.13, then **847/847** once all eight presets were run together) — plus `cargo test --lib` (405 → 407), `f239_span_scan.py` (**0 spans** throughout), the cascade to convergence, and the ratchet. Targeted Miri wherever an aliasing shape changed: S11.1b's partition probe and S11.2a's F274 probe, **each with its control seen red**. Miri lane cpu **3041 → 3034** across the seam conversion (ratio 1.00 — cost-neutral). **`abi_exports.sh` 7/7** at S11.5, the constraint on the export move. **Ratchet regenerated four times**, each after a field-by-field diff proving a relocation rather than a regression: S11.2a (+1 probe block), S11.2d (+1 `MbIndexXY` block), S11.4 (**+7 audited boundary blocks against −26 `unsafe fn`** — the `unsafe_op_in_unsafe_fn` trade §7.1 asks for), S11.5 (**every total flat**, the two exports moving file). Targeted Miri again at **S11.25**, where `with_vpp` reverses a refusal S3.B1 recorded (**F285**) — the `fork_join_encodes` probe is the referee, because the byte sweep is structurally blind to a retag. **No benches** — E3 owns them, and the debt is now **sixty** checkpoints deep. |

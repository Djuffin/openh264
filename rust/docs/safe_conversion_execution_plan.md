...
| **S11** 🔄 **IN PROGRESS 2026-08-31** — brief: [`prompts/safeplan_s11.md`](prompts/safeplan_s11.md) | **S11.0 (close-out debts), S11.1a/b (step 1 — the bitstream seam), S11.2a–e (step 2's mass), S11.3 (step 3 — screen content), S11.4 (the slot types), S11.5 (steps 4+5), S11.6–S11.7 (more mass)** — **thirteen gated checkpoints**; remaining: the rest of step 2, E2, E3 + the bench debt | **Every structural item in the plan is now done, and steps 3, 4 and 5 with them.** **S11.0** paid S10's debts and **settled the Miri ratio at 2.09 CPU** (**F273** — 73% of the lane is the two fork probes, so **E3's pair must be re-budgeted**). **Step 1**, the last structural item, fell in two: S11.1a threads the bitstream pair from the chain's five tops (both resolvers deleted, ~50 re-derivation sites), S11.1b carves the worker buffers out of the pool **before the fork** so no worker derives from the shared context at all. **Step 2** then produced the session's finding and its rule: **F274**, an in-fork `&mut` to layer state S10.15 had committed *against the comment two lines above it*, which **no gate before E3 could see** — bytes never move, and the lane's encode probes are single-threaded; its probe sees the defect red and the fix green. **F275**: three items the plan had scoped as seams were borrow-*shape* questions, minutes each. **Step 3** made the screen-content downcast **unrepresentable rather than dormant** — the storage type cannot hold an `SVAAFrameInfoExt`, so `vaa_ext_ref()` answers `None` by construction and both raw accessors are deleted (D-scope-6). **S11.4** converted twelve function-pointer slot types and corrected **F276's own numbers in both directions** (**F277** — a grep for one declaration *form* missed ten slot types; and a body with three blockers retires when the *last* goes, so counting shared blockers over-counts payoff every time). **Steps 4 and 5**: the version exports moved to the ABI island with **`abi_exports.sh` 7/7 and every ratchet total flat**, two more files sealed (49 `forbid`), and **D-exit-4's enumerated floor is built** — 46 test allows across 16 files, after six mis-tagged ones were corrected | both bitstream resolvers, `thread_bs_buffer`, `bs_buffer`, `layer_sps`/`layer_pps`/`layer_subset_sps`, `vaa_ext`/`vaa_ext_mut` deleted; `dq_layer_ref`/`_mut`, `rc_and_current_layer_mut`, `MbIndexXY`, `vaa_ext_ref`, `layer_sps_ref` built; **12 slot types safe**; `api/version.rs` created; allows 327 → **237** |

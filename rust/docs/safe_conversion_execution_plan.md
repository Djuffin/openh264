...
| **S10** ✅ **CLOSED 2026-08-30** — brief: S10's brief (the source-plane referee, the fork seam, the rest of the middle) | **S10.1 (step 1 — the source-plane referee), S10.5a + S10.5b (step 5's camera path and parameter family), S10.6a (the harvest) — four gated checkpoints**; **step 2 and the slice-header half of step 3 built, measured and reverted (F259, F263)**; steps 3, 4 and 6 not landed; roll to S11: **step 2 as a kernel-family migration**, **step 3 as TWO seams — the slice bank *and* the bitstream writer (F263)**, step 4a/4b, the rest of 5b, step 6, E2, E3, and the bench debt | **The keystone landed, and the two biggest steps turned out to be mis-priced in the same way — both are type migrations, not call-site edits.** **Step 1** built the instrument F254 said did not exist, in safe code, now in the Miri lane; **its calibration contradicts F234 and that is the finding** (**F258**) — both controls red at a *single* round, so 64 was chosen for cost, not sensitivity. **Step 2 is not 21 call sites** (**F259**): the cost kernels are reached through a function pointer whose second operand position takes enc, scratch **and reference** cursors alike, so the reference plane must follow the source onto the seam, dragging in `common/mc.rs`'s 29 decoder-shared, loop-measured filters. Two objections died by measurement (a `processing/` caller **cannot** produce a `RecCursor`, so `common` must stay generic; cells and by-value rows are **free** — 33.8/33.8/33.9 ms). **Step 5 then paid**: `ref_list_mgr_svc.rs` 27 → 19 (**F260** — the raw roots were holding up a hand-maintained *ordering rule in a comment*, F239's own precondition), and the coding-parameter family took references — **259 raw dereferences retired by six parameter flips**, plus six mis-tagged `SCREEN_CONTENT(dormant)` reads collapsed into one honest claim. `WelsInitSps`'s flip was **reverted after checking S29's premise is still live**. **The harvest then found exactly one** (**F262**), which is the useful result: the remaining 347 are gated on three structural items and nothing mechanical is left | the source-plane race referee; `ltr_family_mut` / `vpp_and_ref_list_mut` / `vaa_ext_screen_frame_complexity` / `ctx_sps_ref` / `ctx_pps_ref` / `layer_pps_ref`; the coding-parameter family by reference; allows 371 → **347** |

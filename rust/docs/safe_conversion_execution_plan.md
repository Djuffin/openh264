...
1. **`#![forbid(unsafe_code)]`** at the top of every file under `src/` **except**:
   ...
   - the screen-content feature family's sites (**D-scope-5**, 2026-08-28: descoped —
     neither converted nor deleted; tagged `SCREEN_CONTENT(dormant)`, enumerated at E2).
2. **Zero raw-pointer types** (`*mut`/`*const`) in fields, parameters, returns, or locals
   outside `src/api/` — except the screen-content family's fields and functions (D-scope-5).
...
6. The unsafe ratchet baseline reaches its floor and is **pinned**: the census allowlist
   reduces to the api files, the two D1 lines, plus the screen-content family's enumerated
   sites (D-scope-5), so any new `unsafe` anywhere else fails CI.
...
- **D-scope-5 (user, 2026-08-28): the screen-content feature family is descoped** — C5
  closes as "stopped, ruled": neither converted (the ownership shape has no owner to name
  in this tree, F229) nor deleted (the search path is public-API-reachable, F233). Full
  entry in `safety_refactor_plan.md`'s decisions ledger.
...
| C5 | ~~Screen-content feature arena → `Vec` + index tables~~ **Descoped by D-scope-5 (2026-08-28)**: neither converted nor deleted; the family stays tagged `SCREEN_CONTENT(dormant)` and its sites are enumerated into the census allowlist at E2 |

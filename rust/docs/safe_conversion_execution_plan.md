...
| **S10** ✅ **CLOSED 2026-08-30** — brief: S10's brief (the source-plane referee, the fork seam, the rest of the middle) | **S10.1 (step 1 — the source-plane race referee) and S10.5a (step 5a's first file) — two gated checkpoints**; **step 2 built to its boundary, measured, and reverted (F259)**; steps 3/4/6 not started; roll to S11: **step 2 re-scoped as a kernel-family migration**, the slice-bank seam (F256), steps 4a/4b, the rest of 5a/5b, step 6, E2, E3, and the bench debt | **The session's keystone landed and its largest step turned out to be mis-priced.** **Step 1** built the instrument F254 said did not exist: two threads over one `SPicture`'s luma plane through the production `RoPicView`, writer running `VaaBackgroundMbDataUpdate`'s luma copy, reader running the mode-decision 16x16 fetch — entirely safe code, and now in the Miri lane. Two controls, both **red**, the first with F254's predicted diagnosis ("retag read of type `[u8]`") where *the reader touches no byte the writer touches*. **Its calibration contradicts F234 and that is the finding** (**F258**): both controls are red at a **single** round, so the count was chosen for cost (64 = 16.8 s vs 200's 35.8 s), not sensitivity — a calibration is a property of the probe, not of the project. **Step 2 is not 21 call sites** (**F259**): the cost kernels are reached through a *function pointer*, whose second operand position receives enc, scratch **and reference** cursors alike, so the reference plane must follow the source onto the seam — and the reference cursor is what feeds `common/mc.rs`'s 29 `&PlaneCursor` filters, decoder-shared and loop-measured. Two objections were killed by measurement (`common` may not depend on `encoder` — solved by `RefSamples` generics, and a `processing/` caller **cannot** produce a `RecCursor`; cells and by-value rows are **free**, 33.8/33.8/33.9 ms on a 16x16 SAD microbenchmark) — but an incomplete type migration cannot be committed, so it was reverted rather than half-landed. **Step 5a** then took `ref_list_mgr_svc.rs` 27 → 19 (**F260**): the raw roots there were holding up a *hand-maintained ordering rule in a comment*, which is the precondition F239 punishes, and two combined accessors on §4.6's existing idiom dissolved it | the source-plane race referee; `ltr_family_mut` / `vpp_and_ref_list_mut` / `ctx_sps_ref`; eight LTR-family bodies safe; allows 371 → **363** |
...
| 2026-08-30 | **S10** | S10.1 (step 1 — the source-plane race referee), S10.5a (step 5a, `ref_list_mgr_svc.rs`) — **two gated checkpoints**; **step 2 built to the `mc.rs` boundary, priced, and reverted (F259)**; roll to S11: step 2 as a kernel-family migration, the slice-bank seam, steps 4a/4b, the rest of 5, step 6, E2, E3, the bench debt | **371 → 363** | `session` with `MIRI_SCOPE=encoder` at both checkpoints — **PASS at both** (9 passed / 0 failed / 2 skipped); sweeps 583/583 ×2 throughout, gtest 4/4, Miri 4 shards **297 → 298** (step 1's probe joins the lane). Miri cpu **1458 s** against S9's 1419 s, **ratio 1.03** — and F257's prepend fix verified by reading the baseline back the way `gates.sh` reads it. **S10.5a failed its first gate run** on `ignored set is 21, must be 20` with no test added: a ```ignore doc fence on a *public* accessor is an ignored doctest (**F261**); ```text is the tree's convention and is not collected. Ratchet regenerated after a field-by-field diff: **one file changed, `unsafe_fn` 25 → 17, every other field identical** — downward only. Instruments after the batch: `f239_span_scan.py` **0 spans** / 115 exclusive field derivations / 2648 bodies / 84 files; `deunsafe_cascade.py` re-run, which is what found the eighth body. **No benches** — E3 owns them, and the debt is now **twenty-eight** checkpoints deep. |

...
def collect():
    ...
    for path in sorted(SRC.rglob("*.rs")):
        ...
        for i, ln in enumerate(lines):
            m = TAG_RE.search(ln)
            if not m or m.group(1) not in PHASE9_TAGS:
                continue
            j = item_after(lines, i)
            sig = signature(lines, j) if j is not None else None
            if sig is None:
                sites.append(Site(rel, i + 1, m.group(1), "stmt", "-", "", []))
            else:
                sites.append(Site(rel, i + 1, m.group(1), "fn", fn_name(sig), sig,
...
def report(sites):
    ...
    add(f"**{len(sites)} Phase-9 tags**: {len(pr)} `port-raw(Phase 9)` + {len(cu)} `cursor`.\n")
